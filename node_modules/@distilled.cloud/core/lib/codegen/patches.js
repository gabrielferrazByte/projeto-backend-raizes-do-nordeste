...
import { applyOperation, isStaleTargetError, } from "../json-patch.js";
import { verbNounSmithyModel } from "./rewrite-operation-ids.js";
/** Smithy-model JSON pointers — OpenAPI has no `/shapes` tree. */
export const isSmithyPatchPath = (pointer) => pointer.startsWith("/shapes") ||
 ...
 */
export const listRfc6902PatchFiles = async (dir) => {
    if (!(await exists(dir)))
        return [];
    return (await fs.readdir(dir))
        .filter((f) => f.endsWith(".json"))
        .sort((a, b) => Number(a.endsWith(".manual.json")) -
        Number(b.endsWith(".manual.json")) || a.localeCompare(b))
        .map((f) => path.join(dir, f));
};
export const applyRfc6902Files = async (target, files, opts = {}) => {
...
export const finalizeConvert = async (o) => {
    ...
    if (!(await exists(specsDir)))
        return;
    const patchesDir = o.patchesDir === false
        ? undefined
    ...
    for (const modelPath of files) {
        ...
        if (o.include && !o.include(resource))
            continue;
        const model = JSON.parse(await fs.readFile(modelPath, "utf8"));
        if (model.metadata?.[FINALIZED_KEY]) {
            throw new Error(`${path.relative(o.root, modelPath)} was already finalized — finalizeConvert is not idempotent; re-run this package's convert from the spec instead`);
        }
        if (patchesDir) {
            ...
            if (applied.errors.length) {
                for (const err of applied.errors)
                    console.error(`❌ bad patch: ${err}`);
                throw new Error(`${applied.errors.length} patch operation(s) failed for ${resource} — fix the pointers or delete the patch`);
            }
            if (applied.applied > 0) {
                console.log(`   patched ${resource}: ${applied.files} file(s), ${applied.applied} op(s)` +
        ...
        if (dangling.length) {
            ...
            for (const d of dangling.slice(0, 5))
                console.error(`   ❌ ${d}`);
    ...
    if (broken.length) {
        throw new Error(`finalizeConvert: ${broken.length} model(s) reference shapes that do not exist:\n  ${broken.join("\n  ")}`);
    }
};
/**
