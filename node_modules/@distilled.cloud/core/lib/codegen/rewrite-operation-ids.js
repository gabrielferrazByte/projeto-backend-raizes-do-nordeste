 ...
 */
const isIndexVerb = (raw) => raw.toLowerCase() === "index";
/**
...
export const singularize = (raw) => {
    ...
    const keepCase = (s) => raw[0] === raw[0]?.toUpperCase()
    ...
    if (raw !== lower && raw.toUpperCase() === raw)
        return raw;
    if (lower.length <= 3)
        return raw;
    if (!lower.endsWith("s"))
    ...
    if (/(?:ss|us|is|os|as|[^aeiou]ys)$/.test(lower))
        return raw;
    ...
    if (/[aeiou]ses$/.test(lower))
        return raw.slice(0, -1);
    if (lower.endsWith("ses"))
        return raw.slice(0, -2);
    return raw.slice(0, -1);
};
const lowerFirst = (s) => s.length === 0 ? s : s.charAt(0).toLowerCase() + s.slice(1);
/**
...
const isStrongVerb = (raw) => STRONG_VERBS.has(raw.toLowerCase());
const isTrailingVerb = (raw) => TRAILING_VERBS.has(raw.toLowerCase());
const isWeakLeadingVerb = (raw) => WEAK_LEADING_VERBS.has(raw.toLowerCase());
const alias = (raw) => VERB_ALIAS[raw.toLowerCase()] ?? raw.toLowerCase();
/** `version`-like tokens (`V1`, `v1beta1`, `V2`) are never nouns to singularise. */
const isVersionToken = (raw) => /^[vV]\d/.test(raw);
 ...
 */
const assemble = (verb, nouns, tail = []) => {
    ...
    const shaped = ordered.map((p, i) => {
        ...
        if (!isLast || isVersionToken(p))
            return pascalToken(p);
        if (verb === "list" && tail.length === 0)
            return pascalToken(p);
 ...
 */
export const toVerbNoun = (operationId) => {
    ...
    if (segments.length >= 2) {
        ...
        if (verbSeg > 0) {
            ...
            if (!compound)
                return assemble(alias(verb), head, object);
        }
    ...
    const strong = (raw) => !isIndexVerb(raw) && isStrongVerb(raw);
    const trailing = (raw) => !isIndexVerb(raw) && isTrailingVerb(raw);
    const weakLeading = (raw) => !isIndexVerb(raw) && isWeakLeadingVerb(raw);
    if (strong(first)) {
        return alias(first) + parts.slice(1).map(pascalToken).join("");
    }
    if (parts.some((p) => /^(or|and)$/i.test(p)))
        return lowerFirst(trimmed);
...
const remapTargets = (node, mapping) => {
 ...
 */
export const isVerbatimRouteId = (operationId, ctx) => {
    if (!operationId)
        return true;
    if (!isMechanicalOperationId(operationId, ctx))
        return false;
    const id = stripTag(operationId);
    const m = HTTP_METHOD_ID.exec(id);
    const norm = (t) => singularize(t.toLowerCase()).toLowerCase();
    ...
    if (!head.every((t) => literal.has(t) || paramTokens.has(t)))
        return false;
...
const stripTag = (operationId) => operationId.includes("/")
    ? operationId.slice(operationId.lastIndexOf("/") + 1)
    : operationId;
export const isMechanicalOperationId = (operationId, ctx) => {
    if (!operationId)
        return true;
    const id = stripTag(operationId);
    const m = HTTP_METHOD_ID.exec(id);
    if (!m || m[1].toLowerCase() !== ctx.method.toLowerCase())
        return false;
    const norm = (t) => singularize(t.toLowerCase()).toLowerCase();
    ...
    if (idTokens.length === 0)
        return !route.some((r) => r.required);
...
export const pathToVerbNoun = (ctx, hints = {}) => {
    const method = ctx.method.toLowerCase();
    const segments = ctx.path.split(/[?#]/)[0].split("/").filter(Boolean);
    const literal = [];
    // `{id}`, `:id`, and composite segments such as `{slug}-{id}`.
    const isParam = (seg) => /\{[^}]*\}/.test(seg) || seg.startsWith(":");
    ...
    if (hints.nouns !== undefined) {
        ...
        for (let i = 0; i < raw.length; i++) {
            ...
            if (isNoiseSegment(t) && routeNoise.has(lower))
                continue;
            let n = 0;
            let skip = 0;
            if (/^(by|for|of|in|with|and|to)$/.test(lower)) {
                ...
                if (n > 0 && !hints.verbatim && i + skip !== raw.length)
                    n = 0;
            }
                ...
                const prevIsResource = prev !== undefined &&
                    raw
                        .slice(i, i + n)
                        .some((x) => singularize(prev) === singularize(x.toLowerCase()) ||
                        prev === x.toLowerCase());
                if (echoes && !prevIsResource)
                    n = 0;
            }
            if (n > 0) {
                if (tokens.length)
                    followedByParam.add(tokens.length - 1);
                i += skip - 1;
                continue;
            }
            tokens.push(t);
        ...
        if (hints.verbatim) {
            for (const i of followedByParam)
    ...
    // Rails `index` action.
    const lastIsVerb = lastTokens.length > 0 &&
    ...
    if (lastIsVerb) {
        ...
        const nouns = parents.map((t, i) => i === parents.length - 1
    ...
    const shaped = tokens.map((t, i) => {
        ...
        return t.map((x, j) => {
            ...
            if (!isResource || !plural)
                return pascalToken(singularize(x));
            return pascalToken(x);
        });
    });
...
export const verbNounSmithyModel = (model) => {
    ...
    for (const [id] of ops) {
        ...
        for (const candidate of Object.keys(shapes)) {
            if (candidate === id || mapping.has(candidate))
                continue;
            ...
            if (cNs !== ns || !cLocal.startsWith(local))
                continue;
...
//# sourceMappingURL=rewrite-operation-ids.js.map