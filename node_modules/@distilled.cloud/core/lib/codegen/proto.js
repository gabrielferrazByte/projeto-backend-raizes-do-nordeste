 ...
 */
import { SENSITIVE_FIELD_PATTERNS } from "./openapi.js";
// ============================================================================
// Prelude
// ============================================================================
...
class ProtoParseError extends Error {
    constructor(filename, line, col, message) {
...
class Parser {
    ...
    stringLit() {
        ...
        if (q !== '"' && q !== "'")
            this.fail("expected string literal");
        this.bump();
        let out = "";
    ...
    parseOptions() {
        ...
        while (true) {
            ...
            else if (this.tryIdent("true"))
                value = true;
            else if (this.tryIdent("false"))
                value = false;
    ...
    parseField() {
        ...
        let repeated = false;
        let optional = false;
        if (this.tryIdent("repeated"))
            repeated = true;
        else if (this.tryIdent("optional"))
            optional = true;
        let mapKey;
        let mapValue;
        let type;
    ...
    parseRpc() {
        ...
        this.expect(")");
        if (!this.tryIdent("returns"))
            this.fail("expected `returns`");
        this.expect("(");
        ...
        if (this.tryChar("{")) {
            while (true) {
                ...
                if (this.tryIdent("option")) {
                    ...
                    if (/deprecated\s*=\s*true/.test(body))
                        deprecated = true;
                    this.expect(";");
                    continue;
                }
...
export const parseProto = (text, filename = "input.proto") => {
...
const resolveName = (name, pkg, scope, index) => {
    const stripped = name.startsWith(".") ? name.slice(1) : name;
    if (SCALAR_TARGETS[stripped])
        return stripped;
    ...
    if (name.startsWith(".")) {
        ...
        if (pkg)
            candidates.push(`${pkg}.${stripped}`);
        candidates.push(stripped);
    }
    for (const c of candidates) {
        if (index.messages.has(c) || index.enums.has(c))
            return c;
    }
...
const isSensitive = (name) => SENSITIVE_FIELD_PATTERNS.some((re) => re.test(name));
const EMPTY_STRUCT_ID = "GoogleProtobufEmpty";
const emitEnum = (ctx, en) => {
    ...
    for (const v of en.values) {
        ...
        while (used.has(mn))
            mn = `${ident(v.name)}_${k++}`;
        used.add(mn);
        const traits = { "smithy.api#enumValue": v.name };
...
const fieldTarget = (ctx, field, scope) => {
...
const memberTraits = (field, tsName) => {
    const traits = {};
    ...
    if (wire !== tsName)
        traits["smithy.api#jsonName"] = wire;
...
const emitMembers = (ctx, fields, scope) => {
    const members = {};
    const used = new Set();
    for (const field of fields) {
        let tsName = ident(camel(field.name));
        let k = 2;
        while (used.has(tsName))
            tsName = `${ident(camel(field.name))}${k++}`;
        used.add(tsName);
...
const copyIo = (ctx, rpcName, side, typeName) => {
    ...
    const msg = ctx.index.messages.get(resolved);
    if (!msg)
        throw new Error(`rpc ${rpcName}: ${side} type ${typeName} is not a message`);
    return addShape(ctx.bag, `${rpcName}${side}`, {
        type: "structure",
...
export const rpcGroupName = (rpcName) => {
    const m = rpcName.match(/^([A-Z][a-z]+|[A-Z]+(?![a-z]))/);
    return m?.[1] ?? rpcName;
};
export const convertProtoToSmithy = (options) => {
    const index = indexFiles(options.files);
    const service = index.services.find((s) => s.fullName === options.protoService);
    if (!service) {
        throw new Error(`proto service ${options.protoService} not found (have ${index.services
            .map((s) => s.fullName)
            .join(", ")})`);
    }
    ...
    for (const rpc of service.rpcs) {
        if (options.rpcNames && !options.rpcNames.has(rpc.name))
            continue;
