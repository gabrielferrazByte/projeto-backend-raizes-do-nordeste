...
export const mirrorRepositoryName = (pkg) => `spec-mirror-${pkg}`;
/** Whether this process reads specs from `specs/.local`. */
export const isLocalSpecs = () => (process.env[LOCAL_ENV] ?? "").trim() !== "";
let announced = false;
/**
 ...
 * cannot interpret.
 */
const tailWithinSpecs = (specPath) => {
 ...
 */
export const resolveSpecPath = (root, specPath) => {
    const production = path.resolve(root, specPath);
    if (!isLocalSpecs() || path.isAbsolute(specPath))
        return production;
    const tail = tailWithinSpecs(specPath);
    if (tail === undefined) {
        throw new Error(`${LOCAL_ENV} is set, but "${specPath}" has no \`specs/\` segment to ` +
    ...
    if (!fs.existsSync(local)) {
        ...
        throw new Error(`${local} does not exist. Materialise this package's mirror first:\n` +
...
//# sourceMappingURL=spec-path.js.map