...
import { readIntrospection } from "./graphql.js";
export const graphqlTypeString = (ref) => {
    ...
    if (!ref.name)
        throw new Error("Unnamed GraphQL type reference");
    return ref.name;
};
/** Retain every schema coordinate, wrapper, default and abstract-type relationship. */
export const convertGraphQLClient = (introspection, options = {}) => {
    ...
    for (const type of [...schema.types].sort((a, b) => a.name.localeCompare(b.name))) {
        ...
        types[type.name] = {
            ...
            ...(type.kind === "SCALAR"
                ? { scalar: scalars[type.name] ?? "unknown" }
                : {}),
            ...(type.fields
                ? {
                    fields: Object.fromEntries([...type.fields]
                        ...
                        field.name,
                        {
                            ...
                            ...(field.isDeprecated
                                ? {
                                    deprecated: field.deprecationReason || true,
                                }
                                : {}),
                        },
                    ])),
            ...
            ...(type.inputFields
                ? {
                    inputFields: Object.fromEntries(type.inputFields.map((field) => [field.name, argument(field)])),
                }
                : {}),
            ...(type.enumValues
            ...
            ...("interfaces" in type && Array.isArray(type.interfaces)
                ? {
                    interfaces: type.interfaces.map((ref) => ref.name),
                }
                : {}),
        };
    }
...
export const validateGraphQLModel = (model) => {
    const checkType = (ref, coordinate) => {
        const name = ref.replace(/[\[\]!]/g, "");
        if (!model.types[name])
            throw new Error(`${coordinate}: unknown GraphQL type ${name}`);
    };
    const checkError = (error, coordinate) => {
...
const union = (values) => values.length ? values.join(" | ") : "never";
const documentation = (description) => description ? `/** ${description.replaceAll("*/", "* /")} */\n` : "";
/** Emit schema-indexed types and the matching runtime graph from one patched model. */
export const generateGraphQLClient = (model, options) => {
    validateGraphQLModel(model);
    const inputType = (ref) => {
        if (ref.endsWith("!"))
            return requiredInput(ref.slice(0, -1));
    ...
    const argsType = (args) => `{ ${Object.entries(args)
        .map(([name, arg]) => `${literal(name)}${arg.type.endsWith("!") && arg.defaultValue === undefined ? "" : "?"}: ${inputType(arg.type)}`)
        .join("; ")} }`;
    ...
    const identifier = (name) => name.replace(/[^A-Za-z0-9_$]/g, "_").replace(/^(?=[0-9])/, "_") ||
        "generated";
    const allocate = (preferred, fallback) => {
    ...
    const errorNames = new Map(Object.keys(model.errors).map((tag) => [
        tag,
    ...
    for (const [name, type] of Object.entries(model.types)) {
        if (type.kind === "SCALAR" || type.kind === "ENUM")
            lines.push(`${literal(name)}: ${type.kind === "SCALAR" ? (type.scalar ?? "unknown") : union(type.enumValues.map(literal))};`);
    }
    lines.push("}", "export type Inputs = {");
    for (const [name, type] of Object.entries(model.types)) {
        if (type.kind === "INPUT_OBJECT")
            lines.push(documentation(type.description), `${literal(name)}: ${argsType(type.inputFields ?? {})};`);
    }
    lines.push("}");
    const categories = {
        auth: "withAuthError",
    ...
    for (const [tag, error] of Object.entries(model.errors)) {
        ...
        lines.push(documentation(error.description), `export class ${name} extends S.TaggedError<${name}>()(${literal(tag)}, G.errorFields)${pipes.length ? `.pipe(${pipes.join(", ")})` : ""} {}`);
    }
    lines.push("export type Errors = {");
    for (const [tag, name] of errorNames)
        lines.push(`${literal(tag)}: ${name};`);
    lines.push("}", "export type Types = {");
    for (const [name, type] of Object.entries(model.types)) {
        if (!["OBJECT", "INTERFACE", "UNION"].includes(type.kind))
            continue;
        lines.push(documentation(type.description), `${literal(name)}: {`);
        for (const [fieldName, field] of Object.entries(type.fields ?? {})) {
            lines.push(documentation(field.description), `${literal(fieldName)}: G.Field<${argsType(field.args)}, ${literal(field.type)}, ${union(field.errors.map(literal))}>;`);
        }
        lines.push("};");
    }
    ...
    lines.push("}", "export type Schema = {", "types: Types; scalars: Scalars; errors: Errors; possibleTypes: PossibleTypes;", `query: ${literal(model.queryType)};`, `mutation: ${model.mutationType ? literal(model.mutationType) : "never"};`, `subscription: ${model.subscriptionType ? literal(model.subscriptionType) : "never"};`, `globalErrors: ${union(model.globalErrors.map(literal))};`, "};");
    lines.push("export type Selection<Name extends keyof Types> = G.Selection<Schema, Name>;", "export type Result<Name extends string, Select> = G.Result<Schema, Name, Select>;", "export type SelectedErrors<Name extends keyof Types, Select> = G.Errors<Schema, Name, Select>;");
    lines.push(`export const schema: G.GraphQLModel = ${JSON.stringify(model)};`);
    lines.push(`export const errorClasses = { ${[...errorNames].map(([tag, name]) => `${literal(tag)}: ${name}`).join(", ")} };`);
    lines.push(`export const client = G.makeClient<Schema, ${options.requirementsType}>(schema, ${options.transportName ?? "transport"}, errorClasses);`, "export const query = client.query;", "export const mutation = client.mutation;", "export const report = client.report;");
    ...
    for (const [kind, name] of [
        ...
        for (const field of Object.keys(model.types[name ?? ""]?.fields ?? {})) {
            ...
            if (!operationNames.has(field))
                operationNames.set(field, exportName);
            lines.push(`export const ${exportName} = client.operation(${literal(kind)}, ${literal(field)});`);
        }
    }
