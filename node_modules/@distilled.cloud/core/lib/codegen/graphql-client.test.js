...
import { compile } from "../graphql.js";
import { fixtureSDL } from "../graphql.fixture.js";
import { applyOperation } from "../json-patch.js";
import { convertGraphQLClient, generateGraphQLClient, graphqlTypeString, validateGraphQLModel, } from "./graphql-client.js";
const named = (name, kind = "OBJECT") => ({ kind, name });
const required = (ofType) => ({ kind: "NON_NULL", ofType });
const list = (ofType) => ({ kind: "LIST", ofType });
...
describe("GraphQL-native compiler", () => {
    ...
    `);
        ...
        expect(output).toContain('export const queryTransport = client.operation("query", "transport");');
        ...
        expect([...output.matchAll(/export const ([A-Za-z_$][\w$]*) =/g)].map((match) => match[1])).toEqual(expect.arrayContaining(["query", "report", "queryCatchTags"]));
        const declarations = [
        ...
        expect(generateGraphQLClient(model, {
            ...options,
            operationAliases: {
                default: "delete",
                deleteField: "delete",
    ...
    test("schema type names and patched error tags cannot shadow generated support symbols", () => {
        const model = convertGraphQLClient(introspectionFromSchema(buildSchema(`
    ...
    `)));
        ...
        expect(output).toContain('export class SchemaError extends S.TaggedError<SchemaError>()("Schema", G.errorFields)');
        expect(output).toContain('export class catchTagsError extends S.TaggedError<catchTagsError>()("catchTags", G.errorFields)');
        expect(output).toContain('export class classError extends S.TaggedError<classError>()("class", G.errorFields)');
    ...
    `);
        ...
        expect(validate(schema, parse(compiled.query))).toEqual([]);
        expect(() => compile(model, "query", { names: { where: { paging: {} } } })).toThrow("owner is required");
    });
    test("provider scalar overrides stay aligned in runtime metadata and generated scalar types", () => {
        const schema = buildSchema(`scalar DateTime scalar JSON type Query { date: DateTime! blob: JSON! }`);
        const model = convertGraphQLClient(introspectionFromSchema(schema), {
        ...
        expect(validate(schema, parse(compile(model, "query", { date: true, blob: true }).query))).toEqual([]);
    });
    ...
    test("field errors patch the one model used by emitted type unions and runtime classification", () => {
        ...
        const output = generateGraphQLClient(model, options);
        expect(output).toContain('"children": G.Field<{ "first"?: Scalars["Int"]; "filter"?: Inputs["Filter"] | null }, "[Project!]!", "ChildrenUnavailable">;');
        expect(output).toContain('export class ChildrenUnavailable extends S.TaggedError<ChildrenUnavailable>()("ChildrenUnavailable", G.errorFields).pipe(Category.withServerError)');
        expect(output).toContain('"errors":["ChildrenUnavailable"]');
        expect(output).toContain('"matchers":[{"code":"CHILDREN_UNAVAILABLE"}]');
        expect(output).toContain('export const projectCreate = client.operation("mutation", "projectCreate");');
    ...
    test("unknown coordinates, incomplete references and stale patches fail immediately", () => {
        ...
        expect(() => validateGraphQLModel(model)).toThrow("Project.children: unknown GraphQL error MissingError");
        expect(() => graphqlTypeString({ kind: "NON_NULL" })).toThrow("Incomplete GraphQL NON_NULL");
        expect(() => applyOperation(model, {
            op: "replace",
...
//# sourceMappingURL=graphql-client.test.js.map