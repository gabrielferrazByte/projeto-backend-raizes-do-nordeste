...
import { FINALIZED_KEY, danglingTargets, finalizeConvert, syncServiceOperations, } from "./patches.js";
const model = () => ({
    smithy: "2.0",
    shapes: {
        "ns#Svc": {
...
const read = (root) => JSON.parse(readFileSync(join(root, ".generated-specs", "svc.json"), "utf8"));
describe("finalizeConvert", () => {
    test("renames, syncs the service list, stamps the marker", async () => {
        const root = scaffold();
        await finalizeConvert({ root });
        const m = read(root);
        expect(Object.keys(m.shapes).sort()).toEqual(["ns#Svc", "ns#ListApps", "ns#ListAppsRequest", "ns#GetApp"].sort());
    ...
    test("applies Smithy patches and repairs the service list after a move", async () => {
        ...
        expect(m.shapes["ns#Svc"].operations.map((o) => o.target)).toEqual([
            "ns#ListApps",
            "ns#FetchApp",
        ]);
    });
...
describe("syncServiceOperations", () => {
    test("drops missing and appends unlisted, keeping order", () => {
        const m = model();
        delete m.shapes["ns#AppsGet"];
        m.shapes["ns#Zed"] = { type: "operation" };
...
//# sourceMappingURL=patches.test.js.map