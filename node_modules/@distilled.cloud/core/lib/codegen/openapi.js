 ...
 */
import { isMechanicalOperationId, isVerbatimRouteId, pathToVerbNoun, resolveOperationName, toVerbNoun, } from "./rewrite-operation-ids.js";
// ============================================================================
 ...
 */
const responseIsCollection = (ctx, responses, order) => {
    ...
    for (const [rawPath, pathItem] of Object.entries(doc.paths ?? {})) {
        ...
        for (const method of httpMethods) {
            ...
            if (skipDeprecated && op.deprecated === true)
                continue;
            const rawOperationId = typeof op.operationId === "string" && op.operationId
                ? op.operationId
                : `${method}_${rawPath}`;
            const idCtx = { path: rawPath, method };
            const naming = options.operationNaming ?? "verbNoun";
            let named = options.operationNames !== undefined
            ...
            let resolved = named ?? rawOperationId;
            ...
            if (naming === "verbNoun" &&
                ...
                isMechanicalOperationId(typeof op.operationId === "string" ? op.operationId : undefined, idCtx)) {
                const lastParam = /\{([^}]+)\}[^/]*$/.exec(rawPath)?.[1];
                if (lastParam)
                    resolved = `${resolved}By${pascal(lastParam)}`;
            }
