...
import * as G from "./graphql.js";
import { adversarialArgument, errorClasses, fixtureModel, multipleErrorEnvelope, partialProjectEnvelope, } from "./graphql.fixture.js";
const harness = (body, status = 200) => {
    const requests = [];
    const client = G.makeClient(fixtureModel, (request) => Effect.sync(() => {
...
describe("GraphQL selection compilation", () => {
    ...
    test("input objects preserve explicit null and omit optional members", () => {
        ...
        expect(JSON.parse(JSON.stringify(Object.values(compiled.variables)[0]))).toEqual({ name: null, metadata: { text: adversarialArgument } });
    ...
    test("rejects invalid selections before transport execution", () => {
        expect(() => G.compile(fixtureModel, "query", {
...
describe("GraphQL query execution", () => {
    test("structurally wider selections fail in the typed channel before transport", async () => {
        ...
        const invalidRoot = { ping: true, invented: true };
        const invalidProjection = { id: true, invented: true };
        const invalidNested = {
        ...
        expect((await failure(client.operation("query", "project")({ id: "p1" }, invalidProjection)))._tag).toBe("GraphQLRequestError");
    ...
    test("__proto__ aliases are own properties without changing result prototypes", async () => {
        ...
        const value = await Effect.runPromise(client.query({
            __alias: {
                ["__proto__"]: {
                    project: { where: { id: "p1" }, select: { id: true } },
                },
    ...
    test("decodes null field and nullable list items without inventing missing fields", async () => {
        ...
        const value = await Effect.runPromise(client.query({ projects: { select: { id: true, description: true } } }));
...
describe("GraphQL typed failures", () => {
    ...
    test("report mode preserves partial data and typed issues as a success", async () => {
        const { client } = harness(partialProjectEnvelope);
        const report = await Effect.runPromise(client.report.query({ ...servicesSelection, ping: true }));
    ...
    test("multiple errors retain aliases, tags, and all unaffected data", async () => {
        ...
        const error = await failure(client.query({
            __alias: {
                production: {
                    project: { where: { id: "p1" }, select: { id: true } },
                },
    ...
    test("null bubbling and list indexes do not prevent schema-coordinate matching", async () => {
        ...
        const report = await Effect.runPromise(client.report.query({
    ...
    test("a mutation with response errors is never automatically retried", async () => {
        ...
        const error = await failure(client.mutation({
            editProject: {
    ...
    test("catchTags recovers only when every issue has an allowed tag", async () => {
        ...
        const value = await Effect.runPromise(client
    ...
    test("catchTags never hides another issue in a mixed aggregate", async () => {
        ...
        let recovered = false;
        const error = await failure(client.query(projectIdentity).pipe(G.catchTags("ProjectNotFound", () => Effect.sync(() => {
            recovered = true;
            return "absent";
        }))));
...
describe("GraphQL connection pagination", () => {
    ...
    test("items advances cursors and stops on hasNextPage=false", async () => {
        ...
        const values = await Effect.runPromise(Stream.runCollect(services.items({ first: 1 }, { id: true })));
    ...
    test("pages omits internally selected pageInfo from the requested projection", async () => {
        ...
        const pages = await Effect.runPromise(Stream.runCollect(services.pages({}, { edges: { node: { id: true } } })));
    ...
    test("a user alias named pageInfo cannot collide with hidden pagination fields", async () => {
        ...
        const pages = await Effect.runPromise(Stream.runCollect(services.pages({}, {
    ...
    test("wrapped pageInfo selections still fetch the internal pagination cursor", async () => {
        ...
        const pages = await Effect.runPromise(Stream.runCollect(services.pages({}, { pageInfo: { select: { hasNextPage: true } } })));
    ...
    test("invalid pagination input fails in the typed channel without making a request", async () => {
        const { services, requests } = paginated([]);
        const stream = services.items({ first: "bad" }, { id: true });
...
describe("GraphQL safe query retries", () => {
    ...
    const retried = (response) => {
        const requests = [];
        const client = G.makeClient(retryableModel, (request) => Effect.sync(() => {
            requests.push(request);
    ...
    test("a safely retryable query reuses the document and variables", async () => {
        const { client, requests } = retried((attempt) => attempt === 1 ? unavailable() : { data: { services: { edges: [] } } });
    ...
    test("retryable query failures stop after five retries", async () => {
        ...
        const error = await failure(client.query({ services: { select: { edges: { node: { id: true } } } } }));
        expect(error._tag).toBe("GraphQLFailure");
