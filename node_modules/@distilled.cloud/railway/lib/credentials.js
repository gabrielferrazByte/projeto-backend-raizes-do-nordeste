...
export class Credentials extends Context.Service()("RailwayCredentials") {
}
/** Build {@link Credentials} from an explicit token. */
export const CredentialsFromToken = (config) => Layer.succeed(Credentials, Effect.succeed(toConfig({
...
const envConfig = EffectConfig.all({
    ...
    apiBaseUrl: EffectConfig.String("RAILWAY_API_URL").pipe(EffectConfig.withDefault(DEFAULT_API_BASE_URL)),
});
const MISSING_TOKEN = "RAILWAY_API_TOKEN (or RAILWAY_TOKEN / RAILWAY_PROJECT_TOKEN) environment variable is required";
/**
...
export const CredentialsFromEnv = Layer.succeed(Credentials, Effect.gen(function* () {
    const config = yield* envConfig.pipe(Effect.mapError(() => new ConfigError({ message: MISSING_TOKEN })));
    const account = Option.getOrUndefined(config.apiToken) ??
