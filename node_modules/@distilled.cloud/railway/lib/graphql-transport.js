...
export const transport = (request) => Effect.gen(function* () {
    ...
    const response = yield* http
        .execute(HttpClientRequest.post(url).pipe(HttpClientRequest.setHeaders({
        ...auth,
        ...
        .pipe(Effect.mapError((cause) => new G.GraphQLTransportError({
        message: "Railway GraphQL HTTP request failed",
        cause,
    })));
    const text = yield* response.text.pipe(Effect.mapError((cause) => new G.GraphQLTransportError({
    ...
    const body = yield* Effect.try({
        try: () => JSON.parse(text),
        catch: (cause) => response.status >= 400
...
//# sourceMappingURL=graphql-transport.js.map