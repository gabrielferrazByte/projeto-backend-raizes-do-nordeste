...
export class Credentials extends Context.Service()("PrismaCredentials") {
}
/** Layer from a plain token + optional base URL. */
export const fromApiToken = (config) => Layer.succeed(Credentials, Effect.succeed({
    apiToken: Redacted.make(config.apiToken),
    apiBaseUrl: config.apiBaseUrl ?? DEFAULT_API_BASE_URL,
}));
const envConfig = EffectConfig.all({
    apiToken: EffectConfig.String("PRISMA_API_TOKEN").pipe(EffectConfig.orElse(() => EffectConfig.String("PRISMA_POSTGRES_API_TOKEN"))),
});
/**
...
export const CredentialsFromEnv = Layer.succeed(Credentials, envConfig.pipe(Effect.mapError(() => new ConfigError({
    message: "PRISMA_API_TOKEN (or PRISMA_POSTGRES_API_TOKEN) environment variable is required",
})), Effect.map(({ apiToken }) => ({
