 ...
 */
export class MachineIdentity extends Context.Service()("FlyIoMachineIdentity") {
}
const FlyIoProtocolRest = makeRestProtocol({
...
export const FlyIoProtocol = Layer.effect(API.Protocol, Effect.gen(function* () {
    const rest = yield* API.Protocol;
    return API.Protocol.of({
        encode: (args) => Effect.gen(function* () {
            const request = yield* rest.encode(args);
            const machine = yield* Effect.serviceOption(MachineIdentity);
            if (Option.isNone(machine))
                return request;
            return request.pipe(HttpClientRequest.removeHeader("authorization"), HttpClientRequest.setHeader("connection", "close"));
        }),
        decode: rest.decode,
    });
...
const discoverOrgSlug = (fly) => Effect.gen(function* () {
    ...
    const currentReq = HttpClientRequest.make("GET")(`${machinesRoot}/v1/tokens/current`).pipe(HttpClientRequest.setHeaders({
...
const mintSpritesToken = (fly) => Effect.uninterruptibleMask((restore) => Effect.suspend(() => {
    ...
    if (pending !== undefined)
        return restore(Deferred.await(pending));
    const result = Deferred.makeUnsafe();
    mintInFlight.set(key, result);
    return restore(mintOnce(fly)).pipe(Effect.onExit((exit) => Effect.gen(function* () {
        if (Exit.isSuccess(exit))
            mintedTokens.set(key, exit.value);
