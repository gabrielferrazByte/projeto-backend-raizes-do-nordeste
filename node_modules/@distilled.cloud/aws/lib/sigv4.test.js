...
import * as Presign from "./presign.js";
...
describe("SigV4.sign", () => {
    test("signs S3 GET with UNSIGNED-PAYLOAD and excludes range (AWS docs example)", async () => {
        const signed = await Effect.runPromise(SigV4.sign({
            ...creds,
            method: "GET",
        ...
        expect(signed.headers.authorization).toBe("AWS4-HMAC-SHA256 Credential=AKIAIOSFODNN7EXAMPLE/20130524/us-east-1/s3/aws4_request, SignedHeaders=host;x-amz-content-sha256;x-amz-date, Signature=5c0d4ff29e72b8f94c5b6720369921e587e39bf7a64e456887dec4b43a2d1b77");
    });
    test("hashes the body for non-S3 services and signs the session token", async () => {
        ...
        expect(signed.headers.authorization).toBe("AWS4-HMAC-SHA256 Credential=AKIAIOSFODNN7EXAMPLE/20130524/us-west-2/dynamodb/aws4_request, SignedHeaders=host;x-amz-date;x-amz-security-token;x-amz-target, Signature=5ed05481b0774da674f77f2add0ae15d315f9f7b095ad026bb7d0e7bd9c6b35c");
    });
    test("honours an explicit x-amz-content-sha256 and normalises header whitespace", async () => {
        const a = await Effect.runPromise(SigV4.sign({
            ...creds,
            method: "PUT",
        ...
        const b = await Effect.runPromise(SigV4.sign({
            ...creds,
            method: "PUT",
            url: "https://s3-control.us-east-1.amazonaws.com/v20180820/accesspoint",
            headers: { "x-amz-content-sha256": "abc", "x-weird": "a b" },
    ...
    test("presigns into the query string with reserved characters encoded", async () => {
        ...
        expect(url.searchParams.get("X-Amz-Signature")).toBe("8623d0f2477209accf78e1b704b10fecd0ffeb2398d9209e9ded51e313878a87");
    ...
    test("signing stays correct after the derived-key cache evicts entries", async () => {
        ...
        for (let i = 0; i < 200; i++) {
            await Effect.runPromise(SigV4.sign({
                ...request,
    ...
    test("fails with InvalidSigningUrl for a relative URL", async () => {
        const error = await Effect.runPromise(SigV4.sign({
            ...creds,
            url: "/not/absolute",
            service: "s3",
            region: "us-east-1",
    ...
    test("a failed key derivation is not retained by the cache", async () => {
        ...
        const flakyCache = Effect.runSync(Cache.makeWith((scope) => Effect.suspend(() => attempts++ === 0
        ...
        expect(second.headers.authorization).toBe("AWS4-HMAC-SHA256 Credential=AKIAIOSFODNN7EXAMPLE/20130524/us-east-1/s3/aws4_request, SignedHeaders=host;x-amz-content-sha256;x-amz-date, Signature=5c0d4ff29e72b8f94c5b6720369921e587e39bf7a64e456887dec4b43a2d1b77");
...
describe("Presign", () => {
    const layer = Layer.mergeAll(Layer.succeed(Credentials.Credentials, Effect.succeed({
        accessKeyId: Redacted.make(creds.accessKeyId),
        secretAccessKey: creds.secretAccessKey,
        sessionToken: undefined,
        region: "us-east-1",
    })), Layer.succeed(Region.Region, Effect.succeed("us-east-1")));
    test("presignS3Url pins content-type into the signed headers", async () => {
        const url = await Effect.runPromise(Presign.presignS3Url({
            method: "PUT",
...
//# sourceMappingURL=sigv4.test.js.map