...
import * as Providers from "./node.js";
import { chain, CredentialSourceError } from "./shared.js";
const ENV_KEYS = [
    "AWS_ACCESS_KEY_ID",
...
const run = (effect) => Effect.runPromise(Effect.provideService(effect, ConfigProvider.ConfigProvider, ConfigProvider.fromEnv()));
/** Run an effect that is expected to fail and return its typed error. */
const runFail = (effect) => run(Effect.flip(effect));
/** The resolved credentials a `Credentials` layer produces. */
const resolveLayer = (layer) => Effect.flatMap(Credentials.Credentials, (creds) => creds).pipe(Effect.provide(layer));
/** A fake HTTP client keyed on `${method} ${url}`. */
const fakeHttp = (handler) => Layer.succeed(HttpClient.HttpClient)(HttpClient.make((request, url) => Effect.sync(() => {
    const response = handler(request.method, url, request.headers);
    if (!response)
        throw new Error(`unexpected request ${url}`);
    return HttpClientResponse.fromWeb(request, response);
})));
const imdsCreds = (accessKeyId) => Response.json({
...
const writeConfig = (config) => writeFileSync(join(home, ".aws", "config"), config);
const writeCredentials = (credentials) => writeFileSync(join(home, ".aws", "credentials"), credentials);
describe("fromEnv", () => {
...
describe("chain", () => {
    const fail = (message, tryNextLink) => Effect.fail(new CredentialSourceError({ message, tryNextLink }));
...
`);
        const calls = [];
        const http = Layer.succeed(HttpClient.HttpClient)(HttpClient.make((request, url) => Effect.gen(function* () {
            const body = request.body._tag === "Uint8Array"
                ? new TextDecoder().decode(request.body.body)
                : "";
            calls.push({ url, headers: request.headers, body });
            return HttpClientResponse.fromWeb(request, new Response(`<AssumeRoleResponse xmlns="https://sts.amazonaws.com/doc/2011-06-15/">
  <AssumeRoleResult>
    <AssumedRoleUser>
...
</AssumeRoleResponse>`, { status: 200, headers: { "content-type": "text/xml" } }));
        })));
...
`);
        ...
        expect(await run(Providers.fromProcess({ profile: "proc" }))).toMatchObject({ accessKeyId: "AKIA-proc" });
    });
...
`);
        expect((await runFail(Providers.fromProcess({ profile: "v2" }))).message).toContain("did not return Version 1");
        expect((await runFail(Providers.fromProcess({ profile: "junk" }))).message).toContain("invalid JSON");
        expect((await runFail(Providers.fromProcess({ profile: "missing" }))).message).toContain("could not be found");
    });
});
describe("fromTokenFile", () => {
    test("posts the token to AssumeRoleWithWebIdentity unsigned", async () => {
        ...
        const calls = [];
        const http = Layer.succeed(HttpClient.HttpClient)(HttpClient.make((request, url) => Effect.sync(() => {
            const body = request.body._tag === "Uint8Array"
                ? new TextDecoder().decode(request.body.body)
                : "";
            calls.push({ url, headers: request.headers, body });
            return HttpClientResponse.fromWeb(request, new Response(`<AssumeRoleWithWebIdentityResponse xmlns="https://sts.amazonaws.com/doc/2011-06-15/">
...
describe("fromHttp / fromContainerMetadata", () => {
    ...
    test("a 4xx surfaces the endpoint's error code", async () => {
        ...
        const http = fakeHttp(() => Response.json({ Code: "AccessDenied", Message: "nope" }, { status: 403 }));
    ...
    test("nothing configured", async () => {
        expect((await runFail(Providers.fromHttp())).message).toContain("No HTTP credential provider host");
...
describe("fromInstanceMetadata", () => {
    const imds = (options = {}) => fakeHttp((method, url, headers) => {
        options.recordTo?.push(`${method} ${url.pathname} ${headers["x-aws-ec2-metadata-token"] ?? "-"}`);
    ...
    test("IMDSv2: token, role name, then credentials", async () => {
        const calls = [];
        const creds = await run(Providers.fromInstanceMetadata().pipe(Effect.provide(imds({ recordTo: calls }))));
    ...
    test("falls back to IMDSv1 when the token endpoint is 404", async () => {
        const calls = [];
        const creds = await run(Providers.fromInstanceMetadata().pipe(Effect.provide(imds({ tokenStatus: 404, recordTo: calls }))));
        expect(creds.accessKeyId).toBe("AKIA-imds");
    ...
    test("IMDSv1 fallback can be blocked", async () => {
        ...
        const error = await runFail(Providers.fromInstanceMetadata().pipe(Effect.provide(imds({ tokenStatus: 404 }))));
...
`);
    ...
    test("falls through to the container endpoint", async () => {
        ...
        expect((await run(Providers.fromNodeProviderChain().pipe(Effect.provide(http))))
            .accessKeyId).toBe("AKIA-ecs");
    });
...
//# sourceMappingURL=credential-providers.test.js.map