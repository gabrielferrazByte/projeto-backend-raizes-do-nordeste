 ...
 */
import { loadSharedConfigFiles, parseKnownFiles, } from "@smithy/shared-ini-file-loader";
...
import { Credentials, fromAwsCredentialIdentity, } from "../credentials.browser.js";
import * as Region from "../region.js";
import { CredentialSourceError, chain, env, ENV_CMDS_FULL_URI, ENV_CMDS_RELATIVE_URI, ENV_KEY, ENV_SECRET, fromContainerMetadata, fromEnv, fromHttp as fromHttpShared, fromInstanceMetadata as fromInstanceMetadataShared, withHttpClient, } from "./shared.js";
export * from "./shared.js";
const ENV_PROFILE = "AWS_PROFILE";
const DEFAULT_PROFILE = "default";
/** `profile`, else `AWS_PROFILE`, else `default` — as the AWS CLI. */
export const getProfileName = (profile) => profile || env(ENV_PROFILE) || DEFAULT_PROFILE;
const loadProfiles = (profile) => Effect.tryPromise({
    try: () => parseKnownFiles({ profile }),
    catch: (cause) => new CredentialSourceError({
 ...
 * IMDS options.
 */
export const loadConfigProfile = (profile) => Effect.promise(() => loadSharedConfigFiles()).pipe(Effect.map((files) => files.configFile?.[getProfileName(profile)]), Effect.orElseSucceed(() => undefined));
// ---------------------------------------------------------------------------
// File system
// ---------------------------------------------------------------------------
const systemError = (method, path) => (cause) => PlatformError.systemError({
    _tag: "Unknown",
    module: "FileSystem",
    method,
...
const readFileString = (path) => nodeFileSystem.readFileString(path);
// ---------------------------------------------------------------------------
// fromHttp / fromInstanceMetadata with file-system access
// ---------------------------------------------------------------------------
/** {@link fromHttpShared} plus `AWS_CONTAINER_AUTHORIZATION_TOKEN_FILE`. */
export const fromHttp = (options = {}) => fromHttpShared({ ...options, readFile: readFileString });
/** {@link fromInstanceMetadataShared} plus the profile's IMDS settings. */
export const fromInstanceMetadata = (options = {}) => fromInstanceMetadataShared({
    ...options,
 ...
 */
const stsRegion = (profileRegion, profile) => profileRegion
    ? Effect.succeed(profileRegion)
    : Region.fromEnvironment.pipe(Effect.catch(() => Effect.map(loadConfigProfile(profile), (config) => (config?.region ?? "us-east-1"))));
const stsCredentials = (roleArn, response) => {
    const credentials = response.Credentials;
    if (!credentials?.AccessKeyId || !credentials.SecretAccessKey) {
        return Effect.fail(new CredentialSourceError({
...
const stsFailure = (cause) => new CredentialSourceError({
    message: typeof cause === "object" && cause !== null && "message" in cause
...
export const assumeRole = (sourceCredentials, params, region) => Effect.gen(function* () {
    const STS = yield* Effect.promise(() => import("../services/sts.js"));
    const response = yield* STS.assumeRole(params).pipe(Effect.provideService(Credentials, Effect.succeed(fromAwsCredentialIdentity(sourceCredentials, region))), withHttpClient, Effect.mapError(stsFailure));
 ...
 */
export const assumeRoleWithWebIdentity = (params, region) => Effect.gen(function* () {
    const STS = yield* Effect.promise(() => import("../services/sts.js"));
    const response = yield* STS.assumeRoleWithWebIdentity(params).pipe(Effect.provideService(Credentials, Effect.succeed(fromAwsCredentialIdentity({ accessKeyId: "", secretAccessKey: "" }, region))), withHttpClient, Effect.mapError(stsFailure));
    return yield* stsCredentials(params.RoleArn, response);
});
// ---------------------------------------------------------------------------
// credential_process
...
const execCommand = (command) => Effect.callback((resume, signal) => {
    exec(command, { signal }, (error, stdout) => {
        resume(error
            ? Effect.fail(new CredentialSourceError({
...
const resolveProcessCredentials = (profileName, profiles) => {
    ...
    const invalid = (reason, cause) => new CredentialSourceError({
        message: `Profile ${profileName} credential_process ${reason}.`,
        cause,
    });
    return execCommand(credentialProcess).pipe(Effect.flatMap((stdout) => Effect.try({
...
export const fromProcess = (options = {}) => Effect.flatMap(loadProfiles(options.profile), (profiles) => resolveProcessCredentials(getProfileName(options.profile), profiles));
// ---------------------------------------------------------------------------
 ...
 */
export const fromTokenFile = (options = {}) => Effect.gen(function* () {
    ...
    const webIdentityToken = yield* readFileString(webIdentityTokenFile).pipe(Effect.mapError((cause) => new CredentialSourceError({
...
const isStaticCredsProfile = (profile) => isString(profile.aws_access_key_id) &&
    isString(profile.aws_secret_access_key) &&
    isOptionalString(profile.aws_session_token) &&
    isOptionalString(profile.aws_account_id);
const isAssumeRoleProfile = (profile) => isString(profile.role_arn) &&
...
const isWebIdentityProfile = (profile) => isString(profile.web_identity_token_file) &&
    isString(profile.role_arn) &&
    isOptionalString(profile.role_session_name);
const isProcessProfile = (profile) => isString(profile.credential_process);
const isSsoProfile = (profile) => isString(profile.sso_start_url) ||
    isString(profile.sso_account_id) ||
    isString(profile.sso_session) ||
    isString(profile.sso_region) ||
    isString(profile.sso_role_name);
const isCredentialSourceWithoutRoleArn = (profile) => !profile.role_arn && !!profile.credential_source;
const staticCredentials = (profile) => ({
...
const credentialSource = (source, profileName) => {
    switch (source) {
        case "EcsContainer":
        ...
        default:
            return Effect.fail(new CredentialSourceError({
                message: `Unsupported credential source in profile ${profileName}. Got ${source}, ` +
                    `expected EcsContainer or Ec2InstanceMetadata or Environment.`,
            }));
    }
};
const provideNodeServices = (effect) => effect.pipe(Effect.provideService(FileSystem.FileSystem, nodeFileSystem), Effect.provide(Path.layer), withHttpClient);
/**
 ...
 */
const ssoCredentials = (profileName) => Auth.loadProfileCredentials(profileName).pipe(Effect.map((resolved) => ({
    ...
    sessionToken: resolved.sessionToken
        ...
        : undefined,
    expiration: resolved.expiration === undefined
        ? undefined
        : new Date(resolved.expiration),
})), Effect.mapError((cause) => new CredentialSourceError({
    message: "message" in cause ? cause.message : String(cause),
    cause,
    // The SDK never falls through past an SSO profile that fails.
    tryNextLink: false,
})), provideNodeServices);
const resolveProfileData = (profileName, profiles, options, visited, isAssumeRoleRecursiveCall = false) => {
    const profile = profiles[profileName];
    if (!profile) {
        return Effect.fail(new CredentialSourceError({
...
const resolveAssumeRoleCredentials = (profileName, profiles, options, visited) => Effect.gen(function* () {
    const profile = profiles[profileName];
    const sourceProfile = profile.source_profile;
    if (sourceProfile && visited.has(sourceProfile)) {
        return yield* new CredentialSourceError({
            message: `Detected a cycle attempting to resolve credentials for profile` +
                ` ${getProfileName(options.profile)}. Profiles visited: ` +
                [...visited].join(", "),
        });
    }
    const source = sourceProfile
        ? resolveProfileData(sourceProfile, profiles, options, new Set([...visited, sourceProfile]), isCredentialSourceWithoutRoleArn(profiles[sourceProfile] ?? {}))
    ...
    if (profile.mfa_serial) {
        ...
        params.TokenCode = yield* options
            ...
            .pipe(Effect.mapError((cause) => new CredentialSourceError({
 ...
 */
export const fromIni = (options = {}) => Effect.flatMap(loadProfiles(options.profile), (profiles) => resolveProfileData(getProfileName(options.profile), profiles, options, new Set()));
// ---------------------------------------------------------------------------
// Default chain
// ---------------------------------------------------------------------------
...
const envUnlessProfile = (profile) => Effect.suspend(() => {
...
const remoteProvider = (profile) => Effect.suspend(() => {
 ...
 */
export const fromNodeProviderChain = (options = {}) => chain([
