...
import { makeAuthService, ssoRoleCredentialsCacheName, ssoTokenCacheName, } from "./auth.js";
...
const harness = (home) => {
    ...
    files.set(join(cacheDir, `${ssoTokenCacheName(SESSION)}.json`), JSON.stringify({
    ...
    const fs = FileSystem.layerNoop({
        readFileString: (path) => files.has(path)
    ...
    const load = (profile) => Effect.runPromise(Effect.flatMap(makeAuthService(), (auth) => auth.loadProfileCredentials(profile)).pipe(Effect.provide(layer)));
    const credsPath = (profile) => join(cacheDir, `${ssoRoleCredentialsCacheName({ sso_session: SESSION, ...profile })}.credentials.json`);
...
describe("SSO role credentials cache (#565)", () => {
    ...
    test("two profiles sharing one sso_session each get their own account's keys", async () => {
        ...
        expect(h.files.has(join(h.cacheDir, `${ssoTokenCacheName(SESSION)}.credentials.json`))).toBe(false);
    ...
    test("expired cache entry is refetched", async () => {
        ...
        expect(JSON.parse(h.files.get(path)).accessKeyId).toBe("AKIA-111111111111-AdministratorAccess");
    });
    ...
    test("token file stays keyed by session only", async () => {
        ...
        const http = Layer.succeed(HttpClient.HttpClient)(HttpClient.make((request, url) => Effect.succeed(HttpClientResponse.fromWeb(request, Response.json({
            roleCredentials: {
        ...
        for (const profile of ["dev", "prod"]) {
            await Effect.runPromise(Effect.flatMap(makeAuthService(), (auth) => auth.loadProfileCredentials(profile)).pipe(Effect.provide(layer)));
        }
        const tokenPath = join(h.cacheDir, `${ssoTokenCacheName(SESSION)}.json`);
        const tokenReads = reads.filter((p) => p.startsWith(h.cacheDir) && !p.endsWith(".credentials.json"));
    ...
    test("role cache key formula covers session, account and role", () => {
        ...
        expect(ssoRoleCredentialsCacheName(base)).toBe(sha1(`${SESSION}\n111111111111\nAdministratorAccess`));
        expect(ssoRoleCredentialsCacheName(base)).not.toBe(ssoRoleCredentialsCacheName({ ...base, sso_account_id: "222222222222" }));
        expect(ssoRoleCredentialsCacheName(base)).not.toBe(ssoRoleCredentialsCacheName({ ...base, sso_role_name: "ReadOnlyAccess" }));
        ...
        expect(ssoRoleCredentialsCacheName({
            ...base,
