import { A as errorRefNotResolvable, j as errorSpaceHeadRefMissing, n as errorBundleNotFoundForGraphNode, p as errorHashNotInGraph, r as errorContractDeserializationFailed, t as MigrationToolsError } from "./errors-DD6iLGXk-B4Q4tIha.mjs";
import { r as readContractSnapshotJson, t as contractSnapshotDir } from "./contract-snapshot-store-1S_6vaGt-mzFobPtq.mjs";
import { s as readMigrationsDir } from "./io-xKnQrxIC-D_C_lwGm.mjs";
import { t as EMPTY_CONTRACT_HASH } from "./constants-CM-w38DQ-Cz_EXbOc.mjs";
import { n as isGraphNode } from "./graph-membership-BQ43lP4A-B8rZC7rC.mjs";
import { i as findPathWithDecision, o as reconstructGraph } from "./migration-graph-BoiiZAaU-DLht12k7.mjs";
import { a as readRefsTolerant, r as readRef, t as HEAD_REF_NAME } from "./refs-qwvw4_dV-Bk9tl756.mjs";
...
async function readContractSnapshotEntry(migrationsDir, hash, deserializeContract, verifySnapshotContent) {
...
async function resolveContractAt(args) {
	const { hash, opts, refsDir, migrationsDir, packages, graph, deserializeContract, verifySnapshotContent } = args;
	...
	if (refName !== void 0) {
		...
		if (refEntry) {
			const { contractJson, contract } = await readContractSnapshotEntry(migrationsDir, refEntry.hash, deserializeContract, verifySnapshotContent);
			return {
				hash: refEntry.hash,
				contractJson,
				contract,
...
async function resolveGraphNodeContractAt(args) {
	const { hash, migrationsDir, packages, deserializeContract, verifySnapshotContent, explicitLabel } = args;
	if (!packages.find((pkg) => pkg.metadata.to === hash)) throw errorBundleNotFoundForGraphNode(hash, explicitLabel);
	const { contractJson, contract } = await readContractSnapshotEntry(migrationsDir, hash, deserializeContract, verifySnapshotContent);
	return {
		hash,
		contractJson,
		contract,
...
function createAggregateContractSpace(args) {
	const { spaceId, packages, refs, headRef, refsDir, migrationsDir, resolveContract, deserializeContract, verifySnapshotContent } = args;
	let graphMemo;
	let contractMemo;
...
async function loadContractSpaceAggregate(input) {
	...
	const appState = await loadAppSpace(migrationsDir, appContract, deserializeContract, verifySnapshotContent);
	const extensionStates = await loadExtensionSpaces(migrationsDir, deserializeContract, verifySnapshotContent);
...
async function loadAppSpace(migrationsDir, appContract, deserializeContract, verifySnapshotContent) {
...
async function loadExtensionSpaces(migrationsDir, deserializeContract, verifySnapshotContent) {
	...
	const states = [];
	for (const spaceId of extensionIds) states.push(await loadExtensionSpace(migrationsDir, spaceId, deserializeContract, verifySnapshotContent));
	return states;
}
async function loadExtensionSpace(migrationsDir, spaceId, deserializeContract, verifySnapshotContent) {
	...
	const rawContract = await readRawContractDeferred(migrationsDir, spaceId, headRef, verifySnapshotContent);
	return {
		space: createAggregateContractSpace({
			spaceId,
			packages,
...
*/
async function readRawContractDeferred(migrationsDir, spaceId, headRef, verifySnapshotContent) {
...
//# sourceMappingURL=aggregate-CZyUi3Gt.mjs.map