import { i as expandContractInputs } from "./exports-BOv2LgXz.mjs";
import { r as getEmittedArtifactPaths, t as emit } from "./exports-DI1kMms6--e9LdT9A.mjs";
import { D as errorNoInvariantPath, M as errorUnknownInvariant, b as errorInvalidRefName, r as errorContractDeserializationFailed, t as MigrationToolsError } from "./errors-DD6iLGXk-B4Q4tIha.mjs";
import { r as verifyMigrationHash, t as computeMigrationHash } from "./hash-BAZVWU4a-BdynTmar.mjs";
import { a as snapshotsImportPathFrom, n as createSnapshotContentVerifier, o as writeContractSnapshot, r as readContractSnapshotJson, t as contractSnapshotDir } from "./contract-snapshot-store-1S_6vaGt-mzFobPtq.mjs";
import { t as deriveProvidedInvariants } from "./invariants-C0lpc60l-MH7CqDAw.mjs";
import { i as materialiseExtensionMigrationPackageIfMissing, r as formatMigrationDirName, u as writeMigrationPackage } from "./io-xKnQrxIC-D_C_lwGm.mjs";
import { t as EMPTY_CONTRACT_HASH } from "./constants-CM-w38DQ-Cz_EXbOc.mjs";
import { n as isGraphNode, t as assertHashIsGraphNode } from "./graph-membership-BQ43lP4A-B8rZC7rC.mjs";
import { i as findPathWithDecision, r as findPath } from "./migration-graph-BoiiZAaU-DLht12k7.mjs";
import { d as writeRef, i as readRefs, l as validateRefName, n as deleteRef, o as refsByContractHash, t as HEAD_REF_NAME, u as validateRefValue } from "./refs-qwvw4_dV-Bk9tl756.mjs";
import { a as isValidSpaceId, c as spaceMigrationDirectory, l as spaceRefsDirectory, o as listContractSpaceDirectories, s as readContractSpaceHeadRef, t as APP_SPACE_ID$1 } from "./verify-contract-spaces-CYjc343k-9_2GpPRg.mjs";
import { d as resolveRecordedPath, f as verifyMigration, l as planMigration, n as buildFabricatedMigrationEdge, r as collectAggregateNamespaces, s as loadContractSpaceAggregate, t as allStorageElementsExternal, u as requireHeadRef } from "./aggregate-CZyUi3Gt.mjs";
import { i as emitContractSpaceArtifacts, o as planAllSpaces } from "./spaces-D1-7OKid.mjs";
import { c as internalImportRoot, o as importRootForDependencies, r as createImportSpecifierResolver, t as ImportRootError } from "./import-roots-CYpOz1XZ.mjs";
import { n as parseContractRef, r as parseMigrationRef } from "./ref-resolution-be3JMf5X.mjs";
...
import { isStructuredErrorCode, structuredError } from "@prisma/orm-framework/utils/structured-error";
...
import { ERROR_CODE_DESTRUCTIVE_CHANGES, errorConsentPlanMismatch, errorDestructiveChanges, errorHashMismatch, errorMarkerMissing, errorRunnerFailed, errorRuntime, errorTargetMismatch } from "@prisma/orm-framework/errors/execution";
...
//#region ../../../1-framework/3-tooling/cli/dist/ref-BK7yr-IT.mjs
/**
...
*/
function errorAdvanceRefArgConflict(options) {
	const fix = "Pass either --advance-ref <name> or --no-advance-ref, not both.";
	return new ActionableCliError("CLI.ADVANCE_REF_ARG_CONFLICT", "Cannot specify both --advance-ref and --no-advance-ref", {
...
function errorRefSetHashNotInGraph(resolvedHash, reachableHashes) {
	const reachableList = reachableHashes.length > 0 ? reachableHashes.join(", ") : "(none — migration graph is empty)";
	const fix = reachableHashes.length > 0 ? "Set the ref to the `to` hash of an on-disk migration; `{bin} migration list` shows them." : "Run `{bin} migration plan` first.";
	const nextActions = reachableHashes.length > 0 ? [runCommandAction("List the migrations and their destination hashes", "{bin} migration list"), chooseAction("Set the ref to the `to` hash of one of them")] : [runCommandAction("Plan the first migration", "{bin} migration plan")];
...
function errorPlanForgotTheFlag(resolvedHash, reachableRefs, options) {
	const reachableList = reachableRefs.length > 0 ? reachableRefs.map((r) => `${r.name} (${r.hash})`).join(", ") : "(none)";
	const refFix = reachableRefs.length > 0 ? `Run migration plan with ${reachableRefs.map((r) => `--from ${r.name}`).join(" or ")}.` : "Pass --from <contract> naming the `to` hash of an on-disk migration (`{bin} migration list` shows them), or point the db ref at one with `{bin} migration ref set db <contract>`.";
	const nextActions = reachableRefs.length > 0 ? reachableRefs.map((ref) => runCommandAction(`Plan from ${ref.name}`, `{bin} migration plan --from ${ref.name}`)) : [runCommandAction("Plan from an explicit origin", "{bin} migration plan --from <contract>"), runCommandAction("Point the db ref at a graph node", "{bin} migration ref set db <contract>")];
...
*/
function errorPlanOriginUnknown(reachableRefs) {
	const fromSuggestion = reachableRefs.length > 0 ? reachableRefs.map((r) => `--from ${r.name}`).join(" or ") : "--from <contract>";
	...
	return new ActionableCliError("MIGRATION.PLAN_ORIGIN_UNKNOWN", "Cannot determine the plan origin: migrations exist but no origin is named", {
		...
		meta: { reachableRefs: reachableRefs.map((r) => r.name) }
	});
}
function errorMarkerMismatch(markerHash, reachableHashes) {
	return new ActionableCliError("MIGRATION.MARKER_MISMATCH", "Database marker is not reachable in the on-disk migration graph", {
		why: `DB marker is ${markerHash}, but the on-disk migration graph reaches: ${reachableHashes.length > 0 ? reachableHashes.join(", ") : "(none — migration graph is empty)"}.`,
		fix: [
		...
		nextActions: [
			runCommandAction("Catch the on-disk graph up to the live marker", "{bin} migration plan --from <contract>"),
...
*/
function snapshotVerifierFor(config) {
...
*/
function resolveMigrationPaths(config, cwd) {
...
async function buildReadAggregate(config, options) {
	...
	try {
		...
		const loaded = await loadContractSpaceAggregateForCli({
			...
			deserializeContract,
			...ifDefined("verifySnapshotContent", options.verifySnapshotContent ?? snapshotVerifierFor(config))
		});
		if (!loaded.ok) return loaded;
		return ok({
...
*/
function projectImportRoot(projectDir) {
	const nearest = nearestManifest(projectDir === void 0 ? resolve(".") : resolve(projectDir));
...
function createProjectSpecifierResolver(projectDir) {
...
function failedToResolveContractSource(why, fix, meta, cause, diagnostics) {
...
function diagnosticLocation(diagnostic) {
	...
	return {
		sourceId,
		line: start && typeof start["line"] === "number" ? start["line"] : void 0,
		character: start && typeof start["column"] === "number" ? start["column"] : void 0
	};
}
function formatLocation({ sourceId, line, character }) {
	if (sourceId === void 0) return void 0;
	return line !== void 0 && character !== void 0 ? `${sourceId}:${line}:${character}` : sourceId;
}
/** One source diagnostic as a line of text: `<sourceId>:<line>:<column> <code> <message>`. */
function formatSourceDiagnostic(raw) {
	if (!isRecord(raw)) return String(raw);
	const code = typeof raw["code"] === "string" ? raw["code"] : "diagnostic";
	const message = typeof raw["message"] === "string" ? raw["message"] : "";
	return [
		formatLocation(diagnosticLocation(raw)),
		code,
		message
...
*/
function sourceDiagnosticToFinding(raw) {
	if (!isRecord(raw)) return void 0;
	const code = typeof raw["code"] === "string" ? raw["code"] : "diagnostic";
	const message = typeof raw["message"] === "string" ? raw["message"] : "";
	const location = diagnosticLocation(raw);
	const formatted = formatLocation(location);
	const locatedSummary = (text) => formatted === void 0 ? text : `${formatted} ${text}`;
	const finding = {
		severity: "error",
		nextActions: [],
		...ifDefined("where", location.sourceId === void 0 ? void 0 : {
			path: location.sourceId,
			...ifDefined("line", location.line)
		})
	};
	return isStructuredErrorCode(code) ? {
		code,
		summary: locatedSummary(message),
		...finding
	} : {
...
function sourceDiagnosticsToFindings(diagnostics) {
	const findings = [];
...
function diagnosticLocationSuffix(diagnostic) {
	...
	return formatted === void 0 ? "" : ` (${formatted})`;
}
...
function sourceFailureError(failure) {
	return failedToResolveContractSource(failure.summary, "Edit the schema where each finding points, then run contract emit again.", {
		diagnostics: failure.diagnostics,
		issues: mapDiagnosticsToIssues(failure.diagnostics),
		...ifDefined("providerMeta", failure.meta)
	}, void 0, sourceDiagnosticsToFindings(failure.diagnostics));
}
function requireContractConfig(config) {
...
function requireSourceProvider(contractConfig) {
...
async function resolveContractSource(contractConfig, stack, signal) {
	const sourceContext = {
		composedExtensions: stack.extensions.map((p) => p.id),
		composedExtensionContracts: stack.extensionContracts,
		authoringContributions: stack.authoringContributions,
	...
	switch (validated.kind) {
		case "malformed": throw validated.error;
		case "failed": return notOk(validated.failure);
		case "ok": return ok(validated.value);
	}
}
/**
...
* error emit raises.
*/
async function loadContractSource(config, options = {}) {
	const contractConfig = requireContractConfig(config);
	requireSourceProvider(contractConfig);
	return resolveContractSource(contractConfig, createControlStack(config), options.signal ?? new AbortController().signal);
}
/**
...
async function executeContractEmit(options, dependencies = defaultContractEmitDependencies) {
	const { config, projectDir, outputPath, signal = new AbortController().signal, onProgress } = options;
	...
	return queueEmitByOutput(outputJsonPath, async () => {
		...
		try {
			...
			const enrichedIR = enrichContract(blindCast(resolved.value), frameworkComponents);
			...
			emitResult = await unlessAborted(dependencies.emit(deserializedContract, stack, config.family.emission, {
				...
				serializeContract,
				deserializeContract: (json) => familyInstance.deserializeContract(json),
				resolveImportSpecifier: createProjectSpecifierResolver(projectDir ?? dirname(outputJsonPath)),
				...ifDefined("shouldPreserveEmpty", contractSerializer.shouldPreserveEmpty),
				...ifDefined("sortStorage", contractSerializer.sortStorage),
...
async function resolveContractRefToSnapshot(options) {
	try {
		...
		if (bundles.find((p) => p.metadata.to === targetHash)) return ok({
			hash: targetHash,
			contractJson: blindCast(await readContractSnapshotJson(options.migrationsDir, targetHash, snapshotVerifierFor(options.config))),
...
async function executeMigrateShowPlan(options) {
	...
	const { configPath, migrationsDir, migrationsRelative, refsDir } = resolveMigrationPaths(config, options.cwd);
...
async function checkSnapshotConsistency(space, pkg, migrationsDir) {
	...
	if (space.verifySnapshotContent !== void 0) {
		...
		try {
			...
			if (!MigrationToolsError.is(error) || error.code !== "MIGRATION.CONTRACT_SNAPSHOT_CONTENT_MISMATCH") throw error;
			return {
				space: spaceId,
...
*/
async function enumerateCheckSpaces(aggregate, projectMigrationsDir, cwd, verifySnapshotContent) {
...
async function loadAggregateIntegrityViolations(config, migrationsDir, verifySnapshotContent) {
	try {
...
function outgoingDestinations(graph, hash) {
...
function assertFromIsGraphNode(fromHash, graph, refs) {
	try {
		assertHashIsGraphNode(fromHash, graph);
	} catch (error) {
		if (MigrationToolsError.is(error) && error.code === "MIGRATION.HASH_NOT_IN_GRAPH") throw errorPlanForgotTheFlag(fromHash, getReachableRefs(refs, graph), { cause: error });
		throw error;
	}
}
/**
...
* the hash.
*/
function resolveDefaultOriginHash(space) {
	const refs = space.refs;
	const dbRef = refs["db"];
	if (graphIsEmpty(space)) return ok(dbRef ? {
	...
	const graph = space.graph();
	if (!dbRef) return notOk(errorPlanOriginUnknown(getReachableRefs(refs, graph)));
	try {
...
async function resolveFromForPlan(input) {
	...
	if (optionsFrom === void 0) {
		...
		if (value.kind === "ref" || value.kind === "graph-node") {
			...
			if (outgoingTo.length > 0) return ok({
				...value,
				defaultOriginForks: {
					refName: "db",
					refHash: value.fromHash,
					outgoingTo
				}
			});
		}
	...
	if (!refResult.ok) {
		if (looksLikeFullHash(optionsFrom)) {
			if (graphIsEmpty(space)) return notOk(errorSnapshotMissing(optionsFrom, { viaRef: false }));
...
async function executeMigrationNewCommand(options) {
	...
	const { migrationsDir, appMigrationsDir, appMigrationsRelative } = resolveMigrationPaths(config, cwd);
	const stack = createControlStack(config);
	const familyInstance = config.family.create(stack);
	...
	if (options.from !== void 0) {
		if (packages.length === 0) return notOk(errorRuntime("MIGRATION.HASH_NOT_IN_GRAPH", "--from has no meaning on an empty graph", {
		...
		const matchedHashes = [...new Set(packages.filter((p) => p.metadata.to.startsWith(options.from ?? "")).map((p) => p.metadata.to))].sort();
		if (matchedHashes.length === 0) return notOk(errorRuntime("MIGRATION.HASH_NOT_IN_GRAPH", "Starting contract not found", {
		...
		if (matchedHashes.length > 1) return notOk(errorRuntime("MIGRATION.REF_AMBIGUOUS", `Ambiguous --from prefix: "${options.from}"`, {
			...
			meta: {
				input: options.from,
				candidates: matchedHashes
			}
		}));
		fromHash = matchedHashes[0] ?? null;
	} else {
		const origin = resolveDefaultOriginHash(aggregate.app);
		if (!origin.ok) return notOk(origin.failure);
		if (origin.value.kind === "ref-needs-baseline") return notOk(errorRuntime("MIGRATION.HASH_NOT_IN_GRAPH", "The db ref is not a graph node yet", {
			...
			meta: {
				refName: origin.value.refName,
				resolvedHash: origin.value.fromHash
			}
		}));
...
async function runPlannerLeg(planner, migrations, frameworkComponents, contract, fromContract, spaceId, ownership, snapshotsImportPath, resolveImportSpecifier) {
	...
	try {
		...
		if (CliStructuredError$1.is(e) && e.code === "MIGRATION.UNFILLED_PLACEHOLDER") {
			hasPlaceholders = true;
			try {
				plannedOps = (await Promise.allSettled(plannerResult.plan.operations)).flatMap((entry) => entry.status === "fulfilled" ? [entry.value] : []);
			} catch {
				plannedOps = [];
			}
		} else throw e;
...
*/
function refuseUnconsentedDestructiveBaseline(leg, baselineToHash, consent) {
	...
	if (consent === void 0) {
		const verdict = {
			destructiveOperations: destructiveOps.map((op) => ({
				id: op.id,
				label: op.label
			})),
			planHash
		};
		return errorDestructiveChanges(`The baseline migration contains ${destructiveOps.length} destructive operation(s) that require confirmation`, {
	...
	if (consent.planHash !== planHash) return errorConsentPlanMismatch({
		consentedPlanHash: consent.planHash,
		planHash
	});
	return null;
}
...
async function executeMigrationPlanCommandInner(options, startTime, callbacks) {
	const config = options.config;
	const cwd = options.cwd;
	const { configPath, migrationsDir, appMigrationsDir, appMigrationsRelative } = resolveMigrationPaths(config, cwd);
	const contractPathAbsolute = resolveContractPath(config);
	...
	const warnForks = (forks) => {
		warnings.push(`The default origin ref '${forks.refName}' points at ${forks.refHash}, which already has a migration leading to ${forks.outgoingTo.join(", ")}. Planning from it forks the migration graph; pass --from to choose the origin explicitly.`);
	};
	switch (resolutionResult.value.kind) {
		...
		case "graph-node":
			...
			if (resolutionResult.value.defaultOriginForks !== void 0) warnForks(resolutionResult.value.defaultOriginForks);
			break;
		case "ref":
			fromHash = resolutionResult.value.fromHash;
			fromContract = resolutionResult.value.fromContract;
	...
	const seededThisRun = seedResult.seeded.flatMap((r) => r.newMigrationDirs.map((dirName) => ({
		spaceId: r.spaceId,
		dirName
	})));
	const carried = options.carryEmittedExtensionDirs ?? [];
	const emittedExtensionDirs = [...carried, ...seededThisRun.filter((entry) => !carried.some((prior) => prior.spaceId === entry.spaceId && prior.dirName === entry.dirName))];
	...
	try {
		...
		if (isAutoBaseline && fromHash !== null && fromContract !== null && fromContractInStore) {
			...
			const consentFailure = refuseUnconsentedDestructiveBaseline(baselineLeg.value, fromHash, options.consent);
			if (consentFailure !== null) return notOk(consentFailure);
			...
			if (baselineLeg.value.hasPlaceholders || deltaLeg.value.hasPlaceholders) return ok({
				ok: true,
				noOp: false,
				from: fromHash,
				to: toStorageHash,
			...
			const preview = hasOperationPreview(familyInstance) ? familyInstance.toOperationPreview([...baselineOps, ...deltaOps]) : void 0;
			return ok({
				ok: true,
				noOp: false,
				from: fromHash,
				...
				...preview !== void 0 ? { preview } : {},
				...warnings.length > 0 ? { warnings } : {},
				summary: buildAutoBaselinePlanSummary(baselineOps.length, deltaOps.length, emittedExtensionDirs.length),
				timings: { total: Date.now() - startTime }
			});
		}
		const timestamp = /* @__PURE__ */ new Date();
...
function buildAutoBaselinePlanSummary(baselineOpsCount, deltaOpsCount, emittedExtensionDirsCount) {
...
async function executeRefSetCommand(name, contractInput, options) {
	...
	try {
		...
		if (!isGraphNode(resolvedHash, graph)) return notOk(errorRefSetHashNotInGraph(resolvedHash, [...graph.nodes].sort()));
...
//#endregion
export { formatSourceDiagnostic as $, errorLegendHumanOnly as A, runContractSpaceSeedPhase as At, errorUnexpected as B, errorContractArgConflict as C, resolveAppTargetPath as Ct, errorDriverRequired as D, resolveMigrationRef as Dt, errorDestructiveChanges as E, resolveFromForPlan as Et, errorPathUnreachable as F, statusForMigrationHash as Ft, executeMigrateShowPlan as G, executeDbInit as H, errorRunnerFailed as I, targetSupportsMigrations as It, executeRefAdvancement as J, executeMigrationNewCommand as K, errorRuntime as L, toDeclaredExtensionsFromRaw as Lt, errorMigrationPackageNotFound as M, runMigrationList as Mt, errorMigrationPlanningFailed as N, sanitizeErrorMessage as Nt, errorFileNotFound as O, resolveToForPlan as Ot, errorNoMigrations as P, snapshotVerifierFor as Pt, findPackageByDirPath as Q, errorTargetMigrationNotSupported as R, errorConsentPlanMismatch as S, requireLiveDatabase as St, errorDatabaseConnectionRequired as T, resolveContractRefToSnapshot as Tt, executeDbUpdate as U, executeContractEmit as V, executeDbVerify as W, executeRefListCommand as X, executeRefDeleteCommand as Y, executeRefSetCommand as Z, disposeEmitQueue as _, refuseDeclaredExtensionTargetMismatch as _t, appContractStandInFromIdentity as a, loadContractSpaceAggregateForCli as at, errorAdvanceRefArgConflict as b, refusePackageCorruptionOnAggregate as bt, buildMigrationSpaceGraphEntries as c, mapContractAtError as ct, checkSingleTarget as d, migrationSpaceListEntriesFromAggregate as dt, hasMigrationPath as et, chooseAction as f, originHashForStatus as ft, deriveStatusEdgeAnnotations as g, refuseContractSpaceIntegrity as gt, createControlClient as h, readMigrationRefs as ht, advanceRefSafely as i, loadContractSource as it, errorMarkerMissing as j, runMigrationCheck as jt, errorHashMismatch as k, runCommandAction as kt, buildReadAggregate as l, mapIntegrityViolations as lt, computeRefAdvancementName as m, readContractEnvelope as mt, ERROR_CODE_DESTRUCTIVE_CHANGES as n, loadAggregateIntegrityViolations as nt, appliedHashesFromLedger as o, looksLikePath as ot, closeQuietly as p, preflightRefAdvancement as pt, executeMigrationPlanCommand as q, NO_REF_ADVANCEMENT as r, loadContractRawSafely as rt, buildContractSpaceAggregate as s, mapCaughtMigrationError as st, CliStructuredError$1 as t, listRefsByContractHash as tt, buildRefAdvancementFields as u, maskConnectionUrl as ut, enrichContract as v, refuseMarkerOutsideGraph as vt, errorContractValidationFailed as w, resolveContractRef$1 as wt, errorConfigValidation$1 as x, refuseUnknownInvariants as xt, enumerateCheckSpaces as y, refuseMissingInvariantPath as yt, errorTargetMismatch as z };

//# sourceMappingURL=ref-BK7yr-IT-Dk0sxEcL.mjs.map