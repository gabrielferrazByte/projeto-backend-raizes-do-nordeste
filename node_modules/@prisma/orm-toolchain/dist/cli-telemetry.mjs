...
//#region ../../../1-framework/3-tooling/cli-telemetry/dist/enrich-CJ5LlTek.mjs
...
async function loadProjectConfig(projectRoot) {
	try {
		...
		const { validateOrmSection } = await import("./exports-BOv2LgXz.mjs").then((n) => n.a);
		const validation = validateOrmSection(config, {
			files: [],
			keys: {}
		});
		if (!validation.ok) return EMPTY_PROJECT_CONFIG;
