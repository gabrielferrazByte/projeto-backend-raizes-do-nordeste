import { i as expandContractInputs, l as loadConfigFiles, m as requireConfigSections, n as buildLoadedConfig, p as ormConfigSection, r as evaluateConfigModule } from "./exports-BOv2LgXz.mjs";
import { t as MigrationToolsError } from "./errors-DD6iLGXk-B4Q4tIha.mjs";
import { t as contractSnapshotDir } from "./contract-snapshot-store-1S_6vaGt-mzFobPtq.mjs";
import { t as EMPTY_CONTRACT_HASH } from "./constants-CM-w38DQ-Cz_EXbOc.mjs";
import { r as readRef } from "./refs-qwvw4_dV-Bk9tl756.mjs";
import { c as spaceMigrationDirectory } from "./verify-contract-spaces-CYjc343k-9_2GpPRg.mjs";
import { i as createScaffoldSpecifierResolver } from "./import-roots-CYpOz1XZ.mjs";
import { $ as formatSourceDiagnostic, A as errorLegendHumanOnly, B as errorUnexpected, C as errorContractArgConflict, Ct as resolveAppTargetPath, D as errorDriverRequired, Dt as resolveMigrationRef, E as errorDestructiveChanges, F as errorPathUnreachable, Ft as statusForMigrationHash, G as executeMigrateShowPlan, I as errorRunnerFailed, It as targetSupportsMigrations, K as executeMigrationNewCommand, L as errorRuntime, Lt as toDeclaredExtensionsFromRaw, M as errorMigrationPackageNotFound, Mt as runMigrationList, N as errorMigrationPlanningFailed, Nt as sanitizeErrorMessage, O as errorFileNotFound, P as errorNoMigrations, Pt as snapshotVerifierFor, Q as findPackageByDirPath, R as errorTargetMigrationNotSupported, S as errorConsentPlanMismatch, St as requireLiveDatabase, T as errorDatabaseConnectionRequired, Tt as resolveContractRefToSnapshot, V as executeContractEmit, X as executeRefListCommand, Y as executeRefDeleteCommand, Z as executeRefSetCommand, at as loadContractSpaceAggregateForCli, b as errorAdvanceRefArgConflict, bt as refusePackageCorruptionOnAggregate, c as buildMigrationSpaceGraphEntries, ct as mapContractAtError, d as checkSingleTarget, dt as migrationSpaceListEntriesFromAggregate, et as hasMigrationPath, f as chooseAction, ft as originHashForStatus, g as deriveStatusEdgeAnnotations, gt as refuseContractSpaceIntegrity, h as createControlClient, ht as readMigrationRefs, i as advanceRefSafely, it as loadContractSource, j as errorMarkerMissing, jt as runMigrationCheck, k as errorHashMismatch, kt as runCommandAction, l as buildReadAggregate, m as computeRefAdvancementName, mt as readContractEnvelope, n as ERROR_CODE_DESTRUCTIVE_CHANGES, nt as loadAggregateIntegrityViolations, o as appliedHashesFromLedger, ot as looksLikePath, p as closeQuietly, pt as preflightRefAdvancement, q as executeMigrationPlanCommand, r as NO_REF_ADVANCEMENT, rt as loadContractRawSafely, st as mapCaughtMigrationError, t as CliStructuredError$1$1, tt as listRefsByContractHash, u as buildRefAdvancementFields, ut as maskConnectionUrl, vt as refuseMarkerOutsideGraph, w as errorContractValidationFailed, wt as resolveContractRef$1, x as errorConfigValidation$1, xt as refuseUnknownInvariants, y as enumerateCheckSpaces, yt as refuseMissingInvariantPath, z as errorTargetMismatch } from "./ref-BK7yr-IT-Dk0sxEcL.mjs";
import { a as PRISMA7_QUICK_REFERENCE_STEP, i as PRISMA7_LOOP_STEP, n as InitOutputSchema, o as buildNextSteps, r as NEXT_STEPS_BEFORE_SCAFFOLD, t as DB_SIGN_STEP } from "./output-BcC8eB0h-C5bRd-aC.mjs";
...
import { defineCommand, defineCommandFamily, defineServerCommand, flag, positional } from "@prisma/cli-engine";
...
//#region ../../../1-framework/3-tooling/cli/dist/load-config-BhBCeXvj.mjs
/**
...
*/
function baseDirFor(config) {
	if (config.baseDir === void 0) throw new InternalError("the orm config section reached a command without baseDir");
	return config.baseDir;
}
/** Where migrations live; the validated config carries it absolute and defaulted. */
function migrationsDirFor(config) {
	return config.migrations?.dir ?? resolve(baseDirFor(config), "migrations");
}
/** The app subspace under {@link migrationsDirFor}. */
function appMigrationsDirFor(config) {
...
function errorConsentOperationsMissing(options) {
...
function destructiveOperationList(operations) {
...
function destructiveConsentQuestion(operations, token) {
	return [`Apply ${operations.length} destructive operation(s) to ${token}? Data they remove cannot be recovered:`, destructiveOperationList(operations)].join("\n");
}
...
async function formatOneFile(inputPath, formatOptions) {
	let contents;
	try {
...
async function executeFormat(options) {
	...
	if (source?.format !== "psl") return ok({
		formatted: false,
		paths: []
	});
	const resolvedInputs = await expandContractInputs(source.inputs);
	if (resolvedInputs.length === 0) return ok({
		formatted: false,
		paths: []
	});
	const formatOptions = {
	...
	for (const inputPath of resolvedInputs) {
		...
		if (outcome.ok) paths.push(outcome.value);
		else failures.push(outcome.failure);
	}
	const [firstFailure] = failures;
	if (firstFailure !== void 0) return notOk(firstFailure);
	return ok({
		formatted: paths.length > 0,
		paths
	});
}
const NOTHING_TO_FORMAT = "Nothing to format (contract source is not PSL).";
function formattedBlock(path, cwd) {
	return {
		kind: "summary",
		status: "ok",
		text: [{ text: "Formatted " }, {
...
function formatPresentations(document, cwd) {
	return {
		stdout: () => [],
		next: () => [],
		human: () => document.formatted ? document.paths.map((path) => formattedBlock(path, cwd)) : [{
			kind: "summary",
			status: "info",
...
function errorInitMissingFlags(options) {
	...
	const prisma7 = options.prisma7SchemaPath === void 0 ? "" : ` This looks like a Prisma 7 project; to use ${options.prisma7SchemaPath} as the contract source instead, pass \`--from-prisma7-schema ${options.prisma7SchemaPath}\`.`;
	return new CliStructuredError$1$1("CLI.INIT_MISSING_FLAGS", "Missing required flags", {
		...
		meta: {
			missingFlags: options.missing,
			prisma7SchemaPath: options.prisma7SchemaPath ?? null
		}
	});
}
/**
...
function errorInitWriteFailed(options) {
	...
	const renamedNote = renamed.length === 0 ? "" : ` Before the failure this run renamed ${renamed.join(", ")}; that rename stays, and a re-run writes the missing files beside it.`;
...
* read or written.
*/
function errorInitFlagConflict(options) {
	const [first, second] = options.flags;
	return new CliStructuredError$1$1("CLI.INIT_FLAG_CONFLICT", "Conflicting flags", {
...
const PRISMA7_SUPPORTED_PROVIDERS = ["postgresql", "mongodb"];
/**
...
*/
function errorInitPrisma7ProviderUnsupported(options) {
	const declared = options.provider === void 0 ? "no string provider" : `\`provider = "${options.provider}"\``;
	const fix = options.provider === void 0 ? "Pass `--target` to name the database, or run `prisma orm init` without `--from-prisma7-schema` to start a fresh Prisma 8 contract." : "Run `prisma orm init` without `--from-prisma7-schema` to start a fresh Prisma 8 contract.";
	return new CliStructuredError$1$1("CLI.INIT_PRISMA7_PROVIDER_UNSUPPORTED", "Unsupported Prisma 7 datasource provider", {
...
function errorInitPrisma7TargetMismatch(options) {
	return new CliStructuredError$1$1("CLI.INIT_PRISMA7_TARGET_MISMATCH", "--target does not match the Prisma 7 schema", {
...
function errorInitPrisma7SchemaInvalid(options) {
	return new CliStructuredError$1$1("CLI.INIT_PRISMA7_SCHEMA_INVALID", "Not a Prisma 7 schema", {
		why: options.reason === "absent" ? `\`${options.schemaPath}\` does not exist.` : `\`${options.schemaPath}\` has no \`datasource\` block, so it is not a Prisma 7 schema.`,
...
* other.
*/
function errorInitPrisma7ConfigCollision(options) {
	return new CliStructuredError$1$1("CLI.INIT_PRISMA7_CONFIG_COLLISION", "Two Prisma 7 config files", {
...
*/
function errorInitPrisma7ConfigUnreadable(options) {
	const extension = options.path.slice(options.path.lastIndexOf(".") + 1);
	const versioned = options.prisma7ConfigPath;
	const why = versioned === void 0 ? `\`${options.path}\` failed to evaluate, so init cannot tell whether it is a Prisma 7 config to rename or a Prisma 8 config to replace: ${options.why}` : `\`${options.path}\` failed to evaluate: ${options.why}. \`${versioned}\` already exists, so init leaves \`${options.path}\` alone.`;
	const fix = versioned === void 0 ? `Install the project's dependencies so \`${options.path}\` can be evaluated (a Prisma 7 config imports \`prisma/config\`), or rename it to \`prisma7.config.${extension}\` by hand, then re-run \`prisma orm init\`.` : `Install the project's dependencies and fix the error in \`${options.path}\` (a missing export usually means the package it imports from needs updating), then re-run \`prisma orm init\`.`;
	return new CliStructuredError$1$1("CLI.INIT_PRISMA7_CONFIG_UNREADABLE", `Could not evaluate ${options.path}`, {
		why,
		fix,
...
function packagesAddedNote(added) {
...
*/
function errorInitPrisma7SourceUnavailable(options) {
	const { packageName, schemaPath } = options;
	const wording = options.reason === "not-resolvable" ? {
	...
	return new CliStructuredError$1$1("CLI.INIT_PRISMA7_SOURCE_UNAVAILABLE", wording.summary, {
		why: wording.why,
...
* happened.
*/
function errorInitPrisma7SchemaRefused(options) {
	const findings = options.diagnostics.map((diagnostic) => `\n  ${formatSourceDiagnostic(diagnostic)}`);
	return new CliStructuredError$1$1("CLI.INIT_PRISMA7_SCHEMA_REFUSED", `Prisma 8 cannot read ${options.schemaPath}`, {
...
const PRISMA7_DEFAULT_SCHEMA_PATH = "prisma/schema.prisma";
...
function findConfigFile(cwd, stem) {
	return CONFIG_EXTENSIONS.map((extension) => `${stem}.${extension}`).find((name) => existsSync(join(cwd, name)));
}
async function evaluateConfig(cwd, path) {
	const evaluated = await evaluateConfigModule(path, { cwd });
	if (!evaluated.ok) return {
		kind: "unreadable",
		why: evaluated.failure.why ?? evaluated.failure.message
	};
	const exported = evaluated.value;
	if (!isRecord$2(exported) || Object.hasOwn(exported, "__esModule")) return {
		kind: "unreadable",
		why: `${path} has no object default export`
	};
	if (exported["$prismaConfig"] !== void 0) return { kind: "prisma8" };
	const schema = exported["schema"];
	return {
		kind: "prisma7",
		schema: typeof schema === "string" ? schema : void 0
	};
}
async function schemaDeclaredBy(cwd, path) {
	if (path === void 0) return;
	const evaluated = await evaluateConfig(cwd, path);
	return evaluated.kind === "prisma7" ? evaluated.schema : void 0;
}
async function detectConfig(cwd) {
	const prismaConfigPath = findConfigFile(cwd, "prisma.config");
	const prisma7ConfigPath = findConfigFile(cwd, "prisma7.config");
	if (prismaConfigPath !== void 0) {
		const evaluated = await evaluateConfig(cwd, prismaConfigPath);
		if (evaluated.kind === "prisma7" && prisma7ConfigPath !== void 0) return {
			kind: "collision",
			prismaConfigPath,
			prisma7ConfigPath
		};
		if (evaluated.kind === "unreadable") return {
			...evaluated,
			path: prismaConfigPath,
			prisma7ConfigPath,
			schema: await schemaDeclaredBy(cwd, prisma7ConfigPath)
		};
		if (evaluated.kind === "prisma7") return {
			...evaluated,
			path: prismaConfigPath
		};
		if (prisma7ConfigPath === void 0) return {
			kind: "prisma8",
			path: prismaConfigPath
		};
	}
	if (prisma7ConfigPath === void 0) return { kind: "none" };
	const evaluated = await evaluateConfig(cwd, prisma7ConfigPath);
	if (evaluated.kind === "prisma8") return { kind: "none" };
	if (evaluated.kind === "unreadable") return {
		...evaluated,
		path: prisma7ConfigPath,
		prisma7ConfigPath: void 0,
		schema: void 0
	};
	return {
		...evaluated,
...
function findProvider(text) {
	...
	return { provider: /\bprovider\s*=\s*"([^"]*)"/.exec(block[1] ?? "")?.[1] };
}
function prismaFilesUnder(directory) {
	return readdirSync(directory, {
		withFileTypes: true,
		recursive: true
	}).filter((entry) => entry.isFile() && entry.name.endsWith(".prisma")).map((entry) => join(entry.parentPath, entry.name));
}
function detectSchema(cwd, path) {
...
function declaredRange(manifest, name) {
	for (const field of ["devDependencies", "dependencies"]) {
		...
		const range = isRecord$2(section) ? section[name] : void 0;
...
function versionMajor(range) {
	const match = /^\s*[\^~>=<v]*\s*(\d+)(?:\.|$|\s|-)/.exec(range);
	return match?.[1] === void 0 ? void 0 : Number(match[1]);
}
function detectCli(cwd) {
	const manifest = readJsonRecord(join(cwd, "package.json"));
	if (manifest === void 0) return { kind: "none" };
	if (declaredRange(manifest, PRISMA7_CLI_PACKAGE) !== void 0) return { kind: "side-by-side" };
	const declared = declaredRange(manifest, "prisma");
	if (declared === void 0) return { kind: "none" };
	const installed = installedVersion(cwd, "prisma");
	const version = installed ?? declared;
	const major = versionMajor(version);
	if (major === void 0 || major >= 8) return { kind: "none" };
	const clientVersion = installedVersion(cwd, "@prisma/client") ?? declaredRange(manifest, "@prisma/client");
	return {
		kind: "earlier",
		version,
		major,
		source: installed === void 0 ? "declared" : "installed",
		clientVersion
	};
}
function warningsFor(config) {
	switch (config.kind) {
		case "unreadable": return [config.schema === void 0 ? `${config.path} could not be evaluated, so the default schema path is assumed: ${config.why}` : `${config.path} could not be evaluated, so the schema path is read from ${config.prisma7ConfigPath}: ${config.why}`];
		case "collision": return [`${config.prismaConfigPath} is a Prisma 7 config, but ${config.prisma7ConfigPath} already exists; init cannot rename one onto the other.`];
		default: return [];
	}
}
/**
...
* package is imported.
*/
async function detectPrisma7Project(ctx) {
	const config = await detectConfig(ctx.cwd);
	const configSchema = config.kind === "prisma7" || config.kind === "unreadable" ? config.schema : void 0;
	const [path, schemaPathSource] = ctx.schemaPath !== void 0 ? [ctx.schemaPath, "flag"] : configSchema !== void 0 ? [configSchema, "config"] : [PRISMA7_DEFAULT_SCHEMA_PATH, "default"];
	return {
		config,
...
*/
function prisma7ConfigFile(target, schemaPath, outputDir, resolveImportSpecifier = keepInternalSpecifiers) {
	return `import 'dotenv/config';
import { definePrismaConfig } from '@prisma/cli-engine';
import { defineConfig as ormConfig, prisma7Schema } from '${targetEntrypoint(target, "config", resolveImportSpecifier)}';
...
function installFailedFinding(failure, filesWritten) {
	return initFinding("CLI.INIT_INSTALL_FAILED", "Failed to install dependencies", {
		...
		nextActions: [...failure.nextActions, filesWritten.length === 0 ? chooseAction(RERUN_AFTER_INSTALL) : runCommandAction("Emit the contract once the dependencies are installed", EMIT_COMMAND)],
		meta: {
			filesWritten,
...
const RERUN_AFTER_INSTALL = "Run `prisma orm init` again once the dependencies install";
...
function emitFailedFinding(cause, filesWritten) {
	return initFinding("CLI.INIT_EMIT_FAILED", "Failed to emit contract", {
		...
		nextActions: [chooseAction("Fix what the cause names — when the config imports something its package does not export, update that package to a version that does — then emit again"), runCommandAction("Emit the contract", EMIT_COMMAND)],
		meta: {
			filesWritten,
			cause
		}
...
function scaffoldTree(document) {
	...
	if (document.filesRenamed.length > 0) roots.push({
		label: "renamed",
		tone: "heading",
		children: document.filesRenamed.map((entry) => ({
	...
	if (installed.status === "installed") {
		...
		roots.push({
			label: "installed",
			tone: "heading",
			children: [...installed.deps.map((dep) => ({
				label: suffix(dep, false),
				tone: "identifier"
			})), ...installed.devDeps.map((dep) => ({
				label: suffix(dep, true),
				tone: "identifier"
			}))]
		});
	}
...
function buildInitNextActions(inputs) {
	...
	if (inputs.prisma7 !== null) {
		...
		if (inputs.prisma7.clientMoved) actions.push(runCommandAction("Regenerate the Prisma 7 client to match its CLI", "prisma7 generate"));
...
async function emitScaffoldedContract(ctx, overrides = {}) {
	...
	if (result.exitCode !== 0) {
		...
		const detail = errorFromEnvelope(result.stdout) ?? tail(firstNonEmpty(result.stderr, result.stdout));
...
* unrelated notice.
*/
function errorFromEnvelope(stdout) {
	for (const line of stdout.split("\n").reverse()) {
		...
		if (!isRecord$1(event) || event["kind"] !== "result" || !isRecord$1(event["envelope"])) continue;
		const error = event["envelope"]["error"];
		if (!isRecord$1(error)) continue;
		const code = typeof error["code"] === "string" ? error["code"] : void 0;
		const summary = typeof error["summary"] === "string" ? error["summary"] : void 0;
		const why = typeof error["why"] === "string" ? ` — ${error["why"]}` : "";
		if (code === void 0 && summary === void 0) continue;
		return `${[code, summary].filter((part) => part !== void 0).join(": ")}${why}`;
	}
}
...
function targetFromProviderName(provider) {
...
function formatRemoveCommand(pm, packages) {
	return [
		pm,
		pm === "npm" ? "uninstall" : "remove",
		...packages
	].join(" ");
}
/**
...
async function installProjectDependencies(ctx) {
	const pair = async (manager) => {
		for (const [packages, dev] of [[ctx.deps, false], [ctx.devDeps, true]]) {
			...
			if (!installed.ok) return installed.failure;
		}
	};
...
const importFromProject = async (cwd, specifier) => {
	let resolved;
	try {
...
var Prisma7CheckInstallFailed = class extends Error {
	failure;
	target;
	schemaPath;
	warnings;
	constructor(failure, target, schemaPath, warnings) {
...
*/
function withPackagesAddedAction(error, added) {
	const engineError = normalizeError(error);
	return new CliStructuredError$1(engineError.code, engineError.message, {
...
* can answer.
*/
function createPrisma7SourceCheck(ctx) {
	return async ({ target, schemaPath }) => {
		...
		if (ctx.install) {
			...
			if (outcome.failure !== void 0) throw new Prisma7CheckInstallFailed(outcome.failure, target, schemaPath, outcome.warnings);
			warnings.push(...outcome.warnings);
			installed = deps;
			const newlyDeclared = deps.filter((dep) => !declaredBefore.has(dep));
			added = newlyDeclared.length === 0 ? void 0 : {
				packages: newlyDeclared,
				removeCommand: formatRemoveCommand(outcome.manager ?? ctx.packageManager, newlyDeclared)
			};
		}
		const undoable = async (step) => {
			try {
				return await step();
			} catch (error) {
				throw added === void 0 ? error : withPackagesAddedAction(error, added);
			}
		};
		const module = await undoable(() => ctx.importFromProject(ctx.cwd, targetEntrypoint(target, "config", resolveImportSpecifier)));
		if (module === void 0) {
			if (!ctx.install) {
				warnings.push(`Could not check that Prisma 8 can read ${schemaPath}: ${packageName} is not installed. Install the dependencies and run \`prisma contract emit\` to check it.`);
				return {
					outcome: "unchecked",
					packageName,
					installed,
		...
		if (typeof defineConfig !== "function" || typeof prisma7Schema !== "function") return {
			outcome: "no-source",
			packageName,
			installed,
			added,
		...
		const config = await undoable(() => {
			...
			const required = requireConfigSections(buildLoadedConfig(isRecord(section) ? section : {}, ctx.cwd), [
				"family",
				"target",
				"adapter",
				"contract"
			]);
			if (!required.ok) throw required.failure;
			return required.value;
		});
		const result = await undoable(() => loadContractSource(config));
		if (!result.ok) throw errorInitPrisma7SchemaRefused({
			schemaPath,
			packageName,
...
function declaredPackages(cwd) {
	...
	if (!isRecord(manifest)) return /* @__PURE__ */ new Set();
	const names = [
		"dependencies",
		"devDependencies",
...
*/
function rewritePrisma7ConfigImport(content) {
	const rewritten = content.replace(/(['"])prisma\/config\1/g, (_match, quote) => `${quote}${PRISMA7_PACKAGE_CONFIG_SPECIFIER}${quote}`);
	return {
		content: rewritten,
		found: rewritten !== content
	};
}
/**
...
*/
const SHELL_TOKEN = /"(?:\\[\s\S]|[^"\\])*(?:"|$)|'[^']*(?:'|$)|\\[\s\S]?|&&|\|\||[;|]|\s+|[^\s"'\\;|&]+|&/g;
...
function commandsOf(script) {
	...
	for (const token of script.matchAll(SHELL_TOKEN)) {
		...
		if (SHELL_OPERATORS.has(text)) {
			...
			commands.push(words);
			inWord = false;
		} else if (/^\s/.test(text)) inWord = false;
		else {
...
function startsWithWords(words, at, expected) {
...
*/
function prismaCommandWord(words, at) {
	let index = at;
	while (ENV_ASSIGNMENT.test(words[index]?.text ?? "")) index += 1;
	const word = words[index];
	if (word?.text === "prisma") return word;
	const prefix = PREFIX_WRAPPERS.find((wrapper) => startsWithWords(words, index, wrapper.words));
	if (prefix !== void 0) {
		let next = index + prefix.words.length;
		while (prefix.flags.includes(words[next]?.text ?? "")) next += 1;
		return prismaCommandWord(words, next);
	}
	const separated = SEPARATOR_WRAPPERS.find((wrapper) => startsWithWords(words, index, wrapper));
	if (separated === void 0) return;
	const separator = words.findIndex((candidate, position) => position >= index + separated.length && candidate.text === "--");
	return separator === -1 ? void 0 : prismaCommandWord(words, separator + 1);
}
/**
...
*/
function rewritePrismaBinary(script) {
	return commandsOf(script).map((words) => prismaCommandWord(words, 0)?.end).filter((end) => end !== void 0).reduceRight((rewritten, end) => `${rewritten.slice(0, end)}7${rewritten.slice(end)}`, script);
}
/**
...
*/
function rewritePrismaScripts(manifest, keepNames) {
	const parsed = blindCast(JSON.parse(manifest));
	...
	for (const [name, command] of Object.entries(scripts)) {
		const rewritten = typeof command === "string" && !kept.has(name) ? rewritePrismaBinary(command) : command;
		if (rewritten !== command) names.push(name);
		next[name] = rewritten;
	}
...
function prisma7QuickReferenceMd(target, schemaPath, outputDir, pkgRun, prisma7Config, resolveImportSpecifier = keepInternalSpecifiers) {
	const vars = {
		schemaPath,
		outputDir,
		...
		configEntrypoint: targetEntrypoint(target, "config", resolveImportSpecifier),
		requirements: requirementsBlock(target, pkgRun),
		prisma7ConfigSentence: prisma7Config === void 0 ? "" : ` Prisma 7 reads its own config from \`${prisma7Config}\`.`,
		prisma7ConfigRow: prisma7Config === void 0 ? "" : `\n| [\`${prisma7Config}\`](${prisma7Config}) | Prisma 7 CLI configuration |`
	};
...
*/
function generatedFilesPrisma7PathReplaces() {
	return [
		CONFIG_FILE,
		join(DEFAULT_CONTRACT_SOURCE_DIR, "db.ts"),
		QUICK_REFERENCE_FILE
	];
}
...
function planScaffold(ctx) {
	...
	const outputDir = source.kind === "prisma7-schema" ? DEFAULT_CONTRACT_SOURCE_DIR : dirname(inputs.schemaPath);
	...
	const files = source.kind === "starter" ? [
		{
		...
		},
		{
			...
			content: quickReferenceMd(inputs.target, source.authoring, inputs.schemaPath, runPrefix, resolveImportSpecifier)
		}
	] : [
		{
			path: CONFIG_FILE,
			content: prisma7ConfigFile(inputs.target, source.schemaPath, outputDir, resolveImportSpecifier)
		},
		{
			path: join(outputDir, "db.ts"),
			content: dbFile(inputs.target, resolveImportSpecifier)
		...
		{
			path: QUICK_REFERENCE_FILE,
			content: prisma7QuickReferenceMd(inputs.target, source.schemaPath, outputDir, runPrefix, source.prisma7Config, resolveImportSpecifier)
		}
	];
	files.push({
	...
	const nextGitattributes = mergeGitattributes(existsSync(gitattributesPath) ? readFileSync(gitattributesPath, "utf-8") : void 0, requiredGitattributesLines(outputDir, inputs.target));
	...
	if (manifestExisted || synthesiseManifest) {
		...
		if (inputs.sideBySide !== null) {
			const next = rewritePrismaScripts(working, REQUIRED_SCRIPTS.map((script) => script.name));
			if (next !== null) {
	...
	if (source.kind === "starter" && existsSync(join(cwd, "src/index.ts"))) if (existsSync(join(cwd, "README.md"))) warnings.push("README.md already exists; leaving it untouched.");
	else {
...
* lands on disk.
*/
function planConfigRename(cwd, inputs, warnings) {
	const rename = inputs.sideBySide?.renameConfig ?? null;
	if (rename === null) return [];
	const to = `prisma7.config.${rename.extension}`;
	if (existsSync(join(cwd, to))) throw errorInitPrisma7ConfigCollision({
		prismaConfigPath: rename.from,
		prisma7ConfigPath: to
	});
	const rewritten = rewritePrisma7ConfigImport(readFileSync(join(cwd, rename.from), "utf-8"));
	if (!rewritten.found) warnings.push(`${rename.from} does not import 'prisma/config', so it was renamed to ${to} with its imports unchanged. If it imports the Prisma 7 config helper another way, point that import at '@prisma/prisma7/config'.`);
	return [{
		from: rename.from,
		to,
...
const SIDE_BY_SIDE_QUESTION = "Prisma 7 is installed as `prisma`. Keep it as @prisma/prisma7 (binary prisma7) and move `prisma` to Prisma 8?";
/**
...
*/
function rejectFlagConflict(flags) {
	if (flags.fromPrisma7Schema === void 0) return;
	if (flags.schemaPath !== void 0) throw errorInitFlagConflict({ flags: [PRISMA7_SCHEMA_FLAG, "schema-path"] });
	if (flags.authoring !== void 0) throw errorInitFlagConflict({ flags: [PRISMA7_SCHEMA_FLAG, "authoring"] });
}
/**
...
*/
function looksLikePrisma7(detection) {
	const { config, schema } = detection;
	return config.kind === "prisma7" || config.kind === "collision" || schema.kind === "datasource";
}
function prisma7Question(detection) {
	const { config, schema } = detection;
	if (schema.kind === "datasource" || config.kind !== "prisma7") return `${schema.path} is a Prisma 7 schema. Use it as the Prisma 8 contract source?`;
...
async function choosePrisma7Path(ctx) {
	if (ctx.flags.fromPrisma7Schema !== void 0) return true;
	if (!looksLikePrisma7(ctx.detection)) return false;
	const { schema } = ctx.detection;
	if (schema.kind === "datasource" && !prisma7Target(schema, ctx.flagTarget, ctx.flags.target).ok) return false;
	try {
		return await ctx.prompt.confirm(prisma7Question(ctx.detection));
	} catch (error) {
		if (isPromptRequired(error)) return false;
		throw error;
	}
}
/**
...
*/
function prisma7Target(schema, flagTarget, rawFlagTarget) {
	const providerTarget = schema.provider === void 0 ? void 0 : targetFromProviderName(schema.provider);
	if (schema.provider !== void 0 && providerTarget === void 0) return notOk(errorInitPrisma7ProviderUnsupported({
		schemaPath: schema.path,
		provider: schema.provider
	}));
	if (schema.provider !== void 0 && flagTarget !== void 0 && flagTarget !== providerTarget) return notOk(errorInitPrisma7TargetMismatch({
		schemaPath: schema.path,
		provider: schema.provider,
		target: rawFlagTarget ?? flagTarget
	}));
	const target = flagTarget ?? providerTarget;
	return target === void 0 ? notOk(errorInitPrisma7ProviderUnsupported({
		schemaPath: schema.path,
		provider: void 0
	})) : ok(target);
}
...
*/
function freshInitWarning(packageName, detection) {
	const { config, schema, cli } = detection;
	const effects = [...config.kind === "prisma7" && config.path === "prisma.config.ts" ? [`asks before replacing the Prisma 7 ${CONFIG_FILE}`] : [], ...cli.kind === "earlier" ? ["installs prisma@latest, which replaces the Prisma 7 CLI"] : []];
...
function sideBySidePlan(detection) {
	const { config, cli } = detection;
	const renameConfig = config.kind === "prisma7" && config.path.startsWith(PRISMA7_CONFIG_STEM) ? {
		from: config.path,
		extension: extname(config.path).slice(1)
	} : null;
	const movePackages = cli.kind === "earlier" ? {
		cliVersion: cli.version,
		clientVersion: cli.clientVersion
	} : null;
	if (renameConfig === null && movePackages === null) return null;
	return {
		renameConfig,
		movePackages
	};
}
function sideBySideQuestion(plan) {
	const { renameConfig, movePackages } = plan;
	if (movePackages === null && renameConfig !== null) return `${renameConfig.from} is a Prisma 7 config. Rename it to prisma7.config.${renameConfig.extension} so Prisma 8 can write its own?`;
	if (renameConfig === null) return SIDE_BY_SIDE_QUESTION;
...
async function requireReinitConsent(ctx) {
	if (ctx.replaced.length === 0) return false;
	if (!await ctx.prompt.consent(consentQuestion(ctx.replaced), { token: consentToken(ctx.cwd) })) throw errorInitUserAborted();
	return true;
}
async function askWriteEnv(flags, prompt) {
	return flags.writeEnv || await prompt.confirm("Also write a .env file from .env.example? (gitignored)", { default: false });
}
/**
...
async function resolvePrisma7Inputs(ctx) {
	...
	const { config, schema } = detection;
	if (config.kind === "collision") throw errorInitPrisma7ConfigCollision(config);
	if (config.kind === "unreadable" && config.path.startsWith(PRISMA7_CONFIG_STEM)) throw errorInitPrisma7ConfigUnreadable(config);
	if (schema.kind !== "datasource") throw errorInitPrisma7SchemaInvalid({
		schemaPath: schema.path,
		reason: schema.kind
	});
	const targetResult = prisma7Target(schema, flagTarget, flags.target);
	if (!targetResult.ok) throw targetResult.failure;
	const target = targetResult.value;
	const check = await ctx.checkPrisma7Source({
		target,
		schemaPath: schema.path
	});
	if (check.outcome === "no-source" && flags.fromPrisma7Schema !== void 0) throw errorInitPrisma7SourceUnavailable({
		schemaPath: schema.path,
	...
	try {
		if (check.outcome === "no-source") {
			...
			return await resolveStarterInputs({
				...
				prompt,
				target,
				flagAuthoring: void 0,
				prisma7SchemaPath: void 0,
				warnings: [...detection.warnings, ...check.warnings],
				installed: check.installed
			});
		}
		return await confirmPrisma7Inputs({
			cwd,
			flags,
			prompt,
			detection,
		...
		throw check.added === void 0 ? error : withPackagesAddedAction(error, check.added);
	}
}
function prisma7ConfigAfterInit(detection, sideBySide) {
	const rename = sideBySide?.renameConfig;
	if (rename !== void 0 && rename !== null) return `prisma7.config.${rename.extension}`;
	const { config } = detection;
	return config.kind === "prisma7" || config.kind === "unreadable" ? config.path : void 0;
}
...
async function confirmPrisma7Inputs(ctx) {
	...
	const { config } = detection;
	const prisma7ConfigOccupiesConfigFile = config.kind === "prisma7" && config.path === "prisma.config.ts";
	const reinit = await requireReinitConsent({
		cwd,
		prompt,
		replaced: generatedFilesPrisma7PathReplaces().filter((relative) => (relative !== "prisma.config.ts" || !prisma7ConfigOccupiesConfigFile) && existsSync(join(cwd, relative)))
	});
	const sideBySide = sideBySidePlan(detection);
	if (sideBySide !== null) {
		if (!await prompt.consent(sideBySideQuestion(sideBySide), { token: consentToken(cwd) })) throw errorInitUserAborted();
	}
...
async function resolveStarterInputs(ctx) {
	...
	try {
		...
		throw errorInitMissingFlags({
			missing: [...ctx.target === void 0 ? ["target"] : [], ...flagAuthoring === void 0 ? ["authoring"] : []].filter((flag) => REQUIRED_FLAG_PROMPTS.has(flag)),
	...
	const reinit = await requireReinitConsent({
		cwd,
		prompt,
		replaced: generatedFilesInitReplaces(schemaPath).filter((relative) => existsSync(join(cwd, relative)))
	});
	const writeEnv = await askWriteEnv(flags, prompt);
	const removePreviousFacade = await resolveRemovePreviousFacade({
		cwd,
...
async function resolveInitInputs(ctx) {
	...
	const mayAdoptPrisma7 = flags.fromPrisma7Schema !== void 0 || flagAuthoring === void 0 && flags.schemaPath === void 0;
	let prisma7SchemaPath;
	if (mayAdoptPrisma7) {
		const detection = await detectPrisma7Project({
			cwd,
			schemaPath: flags.fromPrisma7Schema
		});
		if (await choosePrisma7Path({
			flags,
			prompt,
			detection,
			flagTarget
		...
		prisma7SchemaPath = looksLikePrisma7(detection) ? detection.schema.path : void 0;
	}
...
const createInitCommand = (injected) => defineOrmCommand({
	...
	handler: async (args, ctx) => {
		...
		try {
			...
			if (!(error instanceof Prisma7CheckInstallFailed)) throw error;
			for (const warning of error.warnings) warn(warning);
			const document = {
				ok: true,
		...
		for (const warning of inputs.warnings) warn(warning);
		...
		const clientMajor = movePackages?.clientVersion === void 0 ? void 0 : versionMajor(movePackages.clientVersion);
		const moveClient = movePackages !== null && clientMajor !== void 0 && clientMajor < 7;
		const deps = [
		...
		const devDeps = [...scaffold.hasTypesNode ? cliDevDeps : [...cliDevDeps, "@types/node"], ...movePackages !== null ? ["@prisma/prisma7@7"] : []];
		const packagesMoved = [...moveClient ? ["@prisma/client@7"] : [], ...movePackages !== null ? ["@prisma/prisma7@7"] : []];
		const adoptsPrisma7 = inputs.contractSource.kind === "prisma7-schema";
		const prisma7Steps = adoptsPrisma7 ? {
			packagesMoved,
			clientMoved: moveClient
		} : null;
		const findings = [];
		const extraActions = [];
		...
		if (inputs.install) {
			const outcome = await installProjectDependencies({
				...
				deps: depsToInstall,
				devDeps,
				catalogWarnings: packageManager === "pnpm" ? buildCatalogWarnings(ctx.cwd, [...depsToInstall, ...devDeps]) : []
			});
...
const lspCommand = defineServerCommand({
	...
	handler: async (args, io) => {
		const exitCode = await withoutClientProcessIdInArgv(async () => {
			const { startServer } = await import("./exports-DpFO5X4j.mjs");
			return startServer({
				stdin: io.stdin,
				stdout: io.stdout,
				stderr: io.stderr,
...
const migrationCheckCommand = defineOrmCommand({
	...
	handler: async (args, ctx) => {
		...
		if (!loaded.ok) return notOk$1(normalizeError(loaded.failure));
		const spaces = await enumerateCheckSpaces(loaded.value.aggregate, migrationsDir, ctx.cwd, verifySnapshotContent);
		let document;
		let resolvedSpaceId;
		if (target !== void 0) {
			...
			const violations = await loadAggregateIntegrityViolations(ctx.config, migrationsDir, verifySnapshotContent);
...
function createMigrationNewCommand(createClient) {
	return defineOrmCommand({
		help: {
...
const migrationNewCommand = createMigrationNewCommand(createControlClient);
...
function operationNodes$1(operations) {
	return operations.map((operation) => operation.operationClass === "destructive" ? {
		label: operation.label,
		status: "warn"
	} : { label: operation.label });
}
...
*/
function operationRoots(result) {
	const baselineOperations = result.baselineOperations ?? [];
	return [...baselineOperations.length > 0 && result.baselineDir !== void 0 ? [{
		label: result.baselineDir,
		children: operationNodes$1(baselineOperations)
	}] : [], ...result.operations.length > 0 ? [{
		label: result.dir ?? result.baselineDir ?? "operations",
		children: operationNodes$1(result.operations)
	}] : []];
}
...
function warningBlocks(result) {
	return (result.warnings ?? []).map((text) => ({
		kind: "summary",
		status: "warn",
		text
	}));
...
function planNextActions(result, migrationsRelative) {
	if (result.pendingPlaceholders === true) {
		const stubFiles = [.../* @__PURE__ */ new Set([...result.baselineDir === void 0 ? [] : [join(result.baselineDir, "migration.ts")], ...result.dir === void 0 ? [] : [join(result.dir, "migration.ts")]])];
...
function destructiveBaselineQuestion(operations) {
	return [`Write a baseline migration containing ${operations.length} destructive operation(s)? Applying it would remove data that cannot be recovered:`, destructiveOperationList(operations)].join("\n");
}
function createMigrationPlanCommand(createClient) {
	return defineOrmCommand({
		help: {
		...
		handler: async (args, ctx) => {
			...
			const seeded = (record) => {
				...
				for (const dirName of record.newMigrationDirs) seededDirs.push({
			...
			const plan = (consent) => executeMigrationPlanCommand({
				config: ctx.config,
				cwd: ctx.cwd,
				projectDir: baseDirFor(ctx.config),
				...ifDefined("name", args.flags.name),
				...
				...ifDefined("carryEmittedExtensionDirs", consent !== void 0 && seededDirs.length > 0 ? [...seededDirs] : void 0),
				client: createClient({
					family: ctx.config.family,
					target: ctx.config.target,
					adapter: ctx.config.adapter,
			...
			if (!planned.ok && planned.failure.code === ERROR_CODE_DESTRUCTIVE_CHANGES) {
				const verdict = blindCast(planned.failure.meta ?? {});
				if (verdict.destructiveOperations === void 0 || verdict.destructiveOperations.length === 0 || verdict.planHash === void 0) return notOk$1(normalizeError(errorConsentOperationsMissing({ previewCommand: "{bin} migration plan" })));
				const token = consentToken(ctx.cwd);
				if (!await ctx.prompt.consent(destructiveBaselineQuestion(verdict.destructiveOperations), { token })) return notOk$1(normalizeError(planned.failure));
...
const migrationPlanCommand = createMigrationPlanCommand(createControlClient);
...
const migrationStatusCommand = defineOrmCommand({
	...
	handler: async (args, ctx) => {
		...
		const contractConfig = { contract: ifDefined("output", contractPathFor(ctx.config)) };
		try {
