import { a as InternalError, i as CliStructuredError, o as blindCast } from "./result-sR7PBi61.mjs";
import { t as effectResolutionDiagnostic } from "./shared-BTnATsqm-BH4axbJg.mjs";
...
//#endregion
//#region ../../0-framework/3-tooling/cli/dist/pipeline-C7gNjMzJ.mjs
/**
...
function configSource(request) {
	...
	const effectConflict = effectResolutionDiagnostic(request.cwd ?? path.dirname(resolvedEntry));
	...
	if (!path.isAbsolute(configPath)) return {
		ok: false,
		path: configPath,
		diagnostic: new CliStructuredError("CONFIG.PATH_NOT_ABSOLUTE", `The \`composer\` config section's configPath must arrive absolute, and "${configPath}" is relative.`, {
...
//# sourceMappingURL=pipeline-C7gNjMzJ-CixaMvnR.mjs.map