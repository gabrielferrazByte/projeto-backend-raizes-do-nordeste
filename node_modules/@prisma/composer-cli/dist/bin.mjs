#!/usr/bin/env node
import { a as executionDiagnostics, o as executorLoadFailure, t as notOk$1 } from "./result-B874OKwb.mjs";
import { t as alchemyCommandLine } from "./run-alchemy-pANK8XNX-BH1wC0r6.mjs";
import * as fs from "node:fs";
import * as path from "node:path";
import spawn from "cross-spawn";
import { EnvironmentCredentialManager, createCli, defineCommand, defineCommandFamily, defineConfigSection, defineSessionCommand, exitWithChildStatus, flag, loadConfig, positional, resolveSectionPath } from "@prisma/cli-engine";
import { CliStructuredError, notOk, ok } from "@prisma/cli-engine/protocol";
import { fileURLToPath } from "node:url";
//#region ../../0-framework/3-tooling/cli/dist/log-DHFQEBKS.mjs
...
async function deployWithDeps(input, deps) {
	...
	let executor;
	try {
		executor = await import("./execute-deploy-destroy-CvKCwVkD-DCufHnkW.mjs");
	} catch (error) {
...
async function devWithDeps(input, deps) {
	...
	let executor;
	try {
		executor = await import("./execute-dev-BLCK5wF7-B-fG_E31.mjs");
	} catch (error) {
...
async function logWithDeps(input, deps) {
	...
	let executor;
	try {
		executor = await import("./execute-log-B_zCJ17b-BrLUt7vo.mjs");
	} catch (error) {
...
//#endregion
//#region ../../0-framework/3-tooling/cli/dist/engine-cli-BGthfw1R.mjs
/**
...
function validate(raw, provenance) {
	...
	if (configPath !== void 0) try {
		...
		return {
			...
			diagnostics: [diagnostic({
				code: "CONFIG.FIELD_INVALID",
				severity: "error",
				summary: "`composer.configPath` could not be resolved against the file that declared it.",
	...
	return {
		ok: true,
		value: resolvedConfigPath === void 0 ? {} : { configPath: resolvedConfigPath },
...
const createDestroyCommand = (operations) => defineCommand({
	help: {
		...
		examples: ["{bin} destroy module.ts --stage feat-auth", "{bin} destroy module.ts --production"]
	},
	args: {
		positionals: { entry: positional.string({
			brief: "The file whose default export is the application root module, built with module(...).",
			placeholder: "entry"
		}) },
		flags: {
			name: flag.string({
...
//#endregion
//#region ../../0-framework/3-tooling/cli/dist/cli-D0p6Du2I.mjs
/**
