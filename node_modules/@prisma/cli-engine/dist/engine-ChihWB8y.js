...
function reservedConfigSectionName(name) {
	return name === "extends" || name === PARENT_KEY || name === "__proto__" || name.startsWith("$");
}
/**
...
function failedChain(files, diagnostic) {
	return {
		files,
		diagnostics: [{
			section: null,
...
function missingNamedFileDiagnostic(path) {
	return {
		...
		nextActions: [{
			kind: "user-choice",
			label: "Correct the path passed to --config, or drop the flag to let the CLI discover prisma.config.ts from the current directory."
		}],
		where: { path }
	};
}
...
*  can). */
function showParentValue(value) {
	try {
...
function invalidParentDiagnostic(path, value) {
	return {
		...
		nextActions: [{
			kind: "user-choice",
			label: "Set parent to false or to the path of the parent config file, or remove it to let discovery continue upward."
		}],
		where: { path }
	};
}
function missingParentDiagnostic(declaring, target) {
	return {
		...
		nextActions: [{
			kind: "user-choice",
			label: "Correct the parent path, set parent to false, or remove it to let discovery continue upward."
		}],
		where: { path: declaring }
	};
}
function parentCycleDiagnostic(declaring, target) {
	return {
...
function sectionsOf(exported) {
	return Object.fromEntries(Object.entries(exported).filter(([key]) => key !== MARKER_KEY && key !== PARENT_KEY));
}
...
function repositoryBoundary(dir) {
	let current = dir;
	while (true) {
...
function nextDiscoveredFile(dirs, collected) {
	for (const dir of dirs) {
		const candidate = join(dir, CONFIG_FILE_NAME);
		if (isFileAt(candidate) && !collected.has(realpathOr(candidate))) return candidate;
	}
	return null;
}
/** One file's evaluation and marker/version classification, every
*  outcome carrying the diagnostic that names this file. */
async function evaluateChainFile(path, cliVersion) {
	let exported;
	try {
		...
		return {
			ok: false,
			diagnostic: importsMissingPrismaPackage(cause) ? prismaConfigUnresolvedDiagnostic(path, cliVersion) : unreadableDiagnostic(path, cause)
		};
	}
	if (!hasVersionMarker(exported)) return {
		ok: false,
		diagnostic: missingMarkerDiagnostic(path)
	};
...
function followParent(path, parent, collected) {
	if (parent === false) return { next: null };
	if (typeof parent === "string") {
		const target = resolve(dirname(path), parent);
		if (!isFileAt(target)) return { diagnostic: missingParentDiagnostic(path, target) };
		const real = realpathOr(target);
		if (collected.has(real)) return { diagnostic: parentCycleDiagnostic(path, target) };
		return { next: {
			kind: "file",
			path: real
		} };
	}
	if (parent !== void 0) return { diagnostic: invalidParentDiagnostic(path, parent) };
	return { next: {
		kind: "scan",
		dirs: dirsAbove(realpathOr(dirname(path)))
	} };
}
async function collectChain(first, cliVersion) {
	const files = [];
	const collected = /* @__PURE__ */ new Set();
	let next = first;
	while (next !== null) {
		const path = next.kind === "file" ? next.path : nextDiscoveredFile(next.dirs, collected);
		if (path === null) break;
		const evaluated = await evaluateChainFile(path, cliVersion);
		if (!evaluated.ok) return failedChain(files, evaluated.diagnostic);
		files.push({
			path,
			sections: sectionsOf(evaluated.exported)
		});
		collected.add(realpathOr(path));
		const link = followParent(path, evaluated.exported[PARENT_KEY], collected);
		if ("diagnostic" in link) return failedChain(files, link.diagnostic);
		next = link.next;
	}
	return {
		files,
...
async function loadConfig(cwd, configPath, cliVersion) {
	...
	if (configPath === void 0) return collectChain({
		kind: "scan",
		dirs: discoveryDirs(realpathOr(root))
	}, cliVersion);
	const named = resolve(root, configPath);
	if (!existsSync(named)) return failedChain([], missingNamedFileDiagnostic(named));
	return collectChain({
		kind: "file",
		path: realpathOr(named)
	}, cliVersion);
}
//#endregion
...
function withoutFormatFlags(argv) {
	const terminator = argv.indexOf("--");
	...
	for (let index = 0; index < tokens.length; index += 1) {
		...
		if (token.startsWith("--format=") && isFormat(token.slice(9))) continue;
...
function formatFlagGiven(argv) {
	...
	return tokens.includes("--json") ? "json" : void 0;
}
//#endregion
...
function applySharedFlags(state, shared, runtime) {
	...
	state.colorEnabled = state.format === "markdown" ? false : resolveColorEnabled(shared, runtime);
...
function rejectReservedSectionName(owner, section) {
	...
	throw constructionError(`${owner} declares config section '${section.name}', a name the config file reserves ('parent' is the engine's chain directive, 'extends' is a config loader's merge directive, and '$'-prefixed keys are metadata)`);
}
...
function snapshotContribution(file, name) {
	if (!Object.hasOwn(file.sections, name)) return null;
	...
	const entries = Object.entries(raw).filter(([, value]) => value !== void 0);
	return {
		file,
...
function mergePerKey(parent, child) {
...
function provenanceOf(value, contributions) {
	const files = contributions.map((contribution) => contribution.file.path);
	return {
		files,
		keys: isPlainObject(value) ? Object.fromEntries(Object.keys(value).map((key) => [key, contributions.find((contribution) => contribution.keys?.has(key))?.file.path ?? files[0]])) : {}
	};
}
/**
...
* consumes.
*/
function resolveSectionOverChain(section, files) {
	const contributions = [];
	...
	let merged;
	try {
		merged = contributions.map((contribution) => contribution.value).reduceRight((parent, child) => merge(parent, child));
	} catch (cause) {
		return {
			ok: false,
			file: contributions[0].file.path,
			cause
		};
	}
...
*/
function resolveSectionPath(provenance, key, path) {
	if (isAbsolute(path)) return path;
	if (!Object.hasOwn(provenance.keys, key)) throw new Error(`@prisma/cli-engine: resolveSectionPath resolves only top-level section keys, and '${key}' is not a top-level key of the resolved section`);
...
*  every text format. */
function commentaryLine(event) {
	switch (event.kind) {
		case "endpoint": return `${event.name}: ${event.url}`;
		case "status": return `${event.subject}: ${event.from === void 0 ? "" : `${event.from} → `}${event.status}`;
		case "artifact": return `${event.path}${event.description === void 0 ? "" : ` — ${event.description}`}`;
	}
}
/**
...
function writeFields(rows, rail, paint, write) {
	const cells = rows.map((row) => ({
		...
		value: row.sensitive === true ? MASK : orPlaceholder$1(row.value)
	}));
...
function writeTable(columns, rows, paint, write) {
	const all = [columns.map((column) => toned(sentenceCase(column), "heading")), ...rows.map((row) => row.map(orPlaceholder$1))];
	const widths = [];
...
//#endregion
//#region src/execution/markdown.ts
const BACKSLASH = /\\/g;
const PIPE = /\|/g;
const NEWLINE = /\n/g;
...
function codeSpan(text) {
	...
	return `${delimiter}${text.startsWith("`") || text.endsWith("`") ? ` ${text} ` : text}${delimiter}`;
}
/** A fenced block whose fence is one backtick longer than any run in
*  its lines, and at least three. */
function fenced(lines, language = "") {
...
function renderBlockMarkdown(block) {
	switch (block.kind) {
		case "summary": return [`[${block.status}] ${plainText(block.text)}`];
		case "fields": return block.rows.map((row) => `${plainText(row.label)}: ${row.sensitive === true ? MASK : orPlaceholder(row.value)}`);
		case "table": return renderTable(block.columns, block.rows);
		case "list": return block.items.map((item) => `- ${plainText(item)}`);
		case "tree": return block.roots.flatMap((root) => renderTreeNode(root, 0));
		case "drawing": return fenced(block.lines.map(plainText));
	}
}
function escapeCell(text) {
	return text.replace(BACKSLASH, "\\\\").replace(PIPE, "\\|").replace(NEWLINE, " ");
}
function cell(text) {
...
function renderTreeNode(node, depth) {
	...
	return [node.status === void 0 ? `${indent}- ${label}` : `${indent}- [${node.status}] ${label}`, ...(node.children ?? []).flatMap((child) => renderTreeNode(child, depth + 1))];
}
function renderNextActionMarkdown(action) {
	const target = action.command ?? action.url;
	if (target === void 0 && action.commands !== void 0) return [`- ${action.label}`, ...action.commands.map((command) => `  - ${codeSpan(command)}`)];
	if (target === void 0 || target === action.label) return [action.command !== void 0 ? `- ${codeSpan(action.label)}` : `- ${action.label}`];
	return [action.command !== void 0 ? `- ${action.label}: ${codeSpan(target)}` : `- ${action.label}: ${target}`];
}
function whereLine(where) {
	if (where === void 0) return;
	if (where.path !== void 0 && where.line !== void 0) return `where: ${where.path}:${where.line}`;
	if (where.path !== void 0) return `where: ${where.path}`;
	if (where.line !== void 0) return `where: line ${where.line}`;
}
function renderDiagnosticMarkdown(diagnostic) {
	const lines = [`[${diagnostic.severity}] ${diagnostic.code}: ${diagnostic.summary}`];
	if (diagnostic.why !== void 0) lines.push(`why: ${diagnostic.why}`);
	const where = whereLine(diagnostic.where);
	if (where !== void 0) lines.push(where);
	for (const action of diagnostic.nextActions) lines.push(...renderNextActionMarkdown(action));
	if (diagnostic.docsUrl !== void 0) lines.push(`docs: ${diagnostic.docsUrl}`);
	return lines;
}
...
function diagnosticsSection(diagnostics) {
	...
	return ["### Diagnostics", ...diagnostics.flatMap((diagnostic, index) => [...index === 0 ? [] : [""], ...renderDiagnosticMarkdown(diagnostic)])];
}
...
function renderCompletedMarkdown(invocation, presented) {
	const { runtime, state } = invocation;
	const sections = presented.presentation.human.map(renderBlockMarkdown);
	if (presented.presentation.next.length > 0) sections.push(["### Next", ...presented.presentation.next.flatMap(renderNextActionMarkdown)]);
	sections.push(diagnosticsSection(presented.diagnostics.map((diagnostic) => withDocsUrl(state, diagnostic))));
...
function renderErroredMarkdown(invocation, envelope) {
	invocation.runtime.stdout.write(joinSections([renderDiagnosticMarkdown(envelope.error), diagnosticsSection(envelope.diagnostics)]));
}
/** Config-section warnings of an OK run, ahead of whatever the run
*  prints next, with one blank line between. */
function renderWarningsMarkdown(invocation, diagnostics) {
	const section = joinSections(diagnostics.map(renderDiagnosticMarkdown));
	if (section !== "") invocation.runtime.stdout.write(`${section}\n`);
}
function renderChildNextActionsMarkdown(invocation, actions) {
	invocation.runtime.stdout.write(joinSections([actions.flatMap(renderNextActionMarkdown)]));
}
/** One line per event on stdout as it happens. A step starting, its
*  progress, and a remediation are not printed. */
function renderEventMarkdown(invocation, event) {
...
function helpTable(headers, rows) {
	return [
		pipeRow(headers),
		pipeRow(["---", "---"]),
		...rows.map((row) => pipeRow([escapeCell(codeSpan(row.name.trimStart())), escapeCell(row.suffix === void 0 || row.suffix === "" ? row.brief : `${row.brief} ${parenthesized(row.suffix)}`)]))
	];
}
function helpSection(heading, headers, rows) {
...
function renderHelpMarkdown(card) {
	const sections = [[`# ${card.name}`]];
	if (card.tagline !== void 0 && card.tagline !== "") sections.push([card.tagline]);
	if (card.usage !== void 0) sections.push(["## Usage"], fenced([card.usage], "bash"));
	if (card.description !== void 0) sections.push([card.description]);
	sections.push(...helpSection("Commands", ["Command", "Description"], card.commands), ...helpSection("Workflow", ["Run", "Purpose"], card.workflow.map((step) => ({
		name: step.run,
		brief: step.brief
	}))), ...helpSection("Arguments", ["Argument", "Description"], card.arguments), ...helpSection("Options", ["Flag", "Description"], card.options), ...helpSection("Global options", ["Flag", "Description"], card.globalOptions));
	if (card.note !== void 0) sections.push([card.note]);
	if (card.examples.length > 0) sections.push(["## Examples"], fenced(card.examples, "bash"));
	if (card.docsUrl !== void 0) sections.push([`Docs: ${card.docsUrl}`]);
...
function unknownSections(loaded, configSections) {
	...
	return loaded.files.flatMap((file) => Object.keys(file.sections).filter((key) => !declared.has(key)).map((key) => unknownSectionDiagnostic(file.path, key, configSections)));
}
...
function validateConfigSection(section, loaded, invocation) {
	const resolved = resolveSectionOverChain(section, loaded.files);
	if (!resolved.ok) return needsErrored(sectionUnreadableError(section.name, resolved.file, resolved.cause));
	let validation;
	try {
	...
	if (!validation.ok) return needsErrored(sectionInvalidError(section.name, resolved.contributors, loaded.files), validation.diagnostics);
...
function sectionUnreadableError(name, file, cause) {
	return new CliStructuredError("CLI.CONFIG_SECTION_INVALID", `The '${name}' section of ${file} is invalid: resolving its value threw '${(cause instanceof Error ? cause.message : String(cause)).split("\n", 1)[0].trim()}'.`, { nextActions: [{
		kind: "user-choice",
...
function sectionInvalidError(name, contributors, files) {
	const fix = {
		kind: "user-choice",
		label: "Fix the reported problems in that section, then run the command again."
	};
	if (contributors.length === 1) return new CliStructuredError("CLI.CONFIG_SECTION_INVALID", `The '${name}' section of ${contributors[0].path} is invalid.`, { nextActions: [fix] });
	if (contributors.length > 1) {
		const paths = contributors.map((file) => file.path);
		const parents = paths.length === 2 ? "its parent config file" : "its parent config files";
		return new CliStructuredError("CLI.CONFIG_SECTION_INVALID", `The '${name}' section, merged from ${paths[0]} and ${parents}, is invalid.`, {
			why: `The resolved section combines these files, nearest first: ${paths.join(", ")}.`,
			nextActions: [fix]
		});
	}
	const target = files[0]?.path ?? "prisma.config.ts";
	return new CliStructuredError("CLI.CONFIG_SECTION_INVALID", `The '${name}' section is missing: ${files.length === 0 ? "no config file was found" : "no loaded config file declares it"}.`, {
		why: files.length === 0 ? void 0 : `Config files loaded, nearest first: ${files.map((file) => file.path).join(", ")}.`,
		nextActions: [{
			kind: "user-choice",
...
function writeSectionWarnings(invocation, diagnostics) {
	const state = invocation.state;
	const shown = diagnostics.filter((diagnostic) => SEVERITY_RANK[diagnostic.severity] <= SEVERITY_RANK[state.logLevel]).map((diagnostic) => withDocsUrl(state, diagnostic));
	if (state.format === "markdown") {
		renderWarningsMarkdown(invocation, shown);
		return;
	}
	for (const diagnostic of shown) writeDiagnostic(invocation.runtime.stderr, diagnostic, makePaint(state.colorEnabled));
}
	...
	*  a child owns the terminal is a construction error. */
	...
	const surface = {
		...
		consent: async (question, opts) => {
			...
			if (state.yes || !state.interactive) throw consentUnavailable(question, state, token);
...
*  width. */
function unboundedUi() {
	return uiWith(false, () => Number.POSITIVE_INFINITY);
}
function uiWith(colorEnabled, width) {
...
function makeContext(invocation, def, config, configFiles, capabilities) {
	const state = invocation.state;
	const ui = state.format === "markdown" ? unboundedUi() : makeUi(state.colorEnabled, invocation.runtime.stderr);
...
function preParseColorEnabled(argv, runtime, stream) {
	if (formatFlagGiven(argv) === "markdown") return false;
...
*  Markdown. */
function bareGroupInvocation(root, rawArgv) {
...
function renderHelp(spec, root, argv, options, out) {
...
function helpCard(spec, root, argv) {
	const { target, path } = resolveTarget(root, helpPath(argv));
	return target.kind === "leaf" ? leafCard(spec, target.entry, path) : nodeCard(spec, target.node, path);
}
function nodeCard(spec, node, path) {
	...
	return {
		...
		workflow: resolvedSteps((atRoot ? spec.help?.workflow : group?.workflow) ?? [], spec.name),
		arguments: [],
		options: [],
		globalOptions: atRoot ? sharedFlagRows() : [],
		note: atRoot ? void 0 : `Run '${spec.name} ${groupPath} <command> --help' for details on a command.`,
		examples: resolvedExamples(atRoot ? spec.help?.examples : [], spec.name),
		docsUrl: atRoot ? spec.help?.docsUrl : void 0
	};
}
function leafCard(spec, entry, path) {
	const def = entry.def;
	const usage = [
		spec.name,
		...path,
	...
	return {
		...
		commands: [],
		workflow: [],
		arguments: Object.values(def.args.positionals).map((spec) => positionalRuntime(spec)).map((runtime) => ({
		...
		globalOptions: [],
		note: def.kind === "server-command" ? void 0 : `Global options also apply: ${sharedNames}. Run '${spec.name} --help' for details.`,
		examples: resolvedExamples(def.help.examples, spec.name),
		docsUrl: entry.docsBaseUrl
	};
}
function resolvedSteps(steps, cliName) {
	return steps.map((step) => ({
		run: resolveExample(step.run, cliName),
		brief: step.brief
	}));
}
function resolvedExamples(examples, cliName) {
	return (examples ?? []).map((example) => resolveExample(example, cliName));
}
function renderHelpTerminal(card, paint) {
	const lines = [];
	lines.push(header(card, paint));
	lines.push("");
	if (card.kind === "leaf") {
		lines.push(sectionLabel(paint, "Usage"));
		lines.push(rail(paint, `${GAP}${paint("muted", "$")} ${paint("emphasis", card.usage ?? "")}`));
	} else railRows(card.commands, paint, lines);
	if (card.description !== void 0) {
	...
	if (card.note !== void 0) {
		lines.push(rail(paint));
		if (card.kind === "leaf") proseLines(card.note, paint, lines, "muted");
		else lines.push(rail(paint, paint("muted", card.note)));
	}
...
function header(card, paint) {
	const name = paint("emphasis", card.name);
	if (card.tagline === void 0 || card.tagline === "") return name;
...
function rowSection(label, rows, paint, lines) {
...
function exampleLines(examples, paint, lines) {
	if (examples.length === 0) return;
	lines.push(rail(paint));
	lines.push(sectionLabel(paint, "Examples"));
	for (const example of examples) lines.push(rail(paint, `${GAP}${paint("muted", "$")} ${example}`));
}
...
function workflowLines(steps, paint, lines) {
	...
	for (const step of steps) {
		...
		lines.push(rail(paint, `${GAP}${paint("muted", "$")} ${step.run}${pad}${GAP}${paint("muted", step.brief)}`));
	}
}
...
function settleChildStatus(invocation, def, settlement) {
	...
	if (child.signal === null) renderChildNextActions(invocation, settlement.nextActions);
	settleVerbatimExitCode(invocation, exitCode);
}
function renderChildNextActions(invocation, actions) {
	...
	for (const action of actions) invocation.runtime.stderr.write(`${renderNextAction(action, paint)}\n`);
}
...
function releaseStdin(state) {
	const iterator = state.stdinIterator;
	if (iterator === void 0) return;
	state.stdinIterator = void 0;
...
var EngineImpl = class {
		...
		*  so the engine always outlives the child. */
		...
		if (helpFlagGiven(argv) || bareGroupInvocation(this.tree, argv)) {
			...
			*  stream. Never fires telemetry, like --version. */
			...
			renderHelp(this.spec, this.tree, argv, {
				format,
				colorEnabled: preParseColorEnabled(argv, runtime, format === "json" ? "stderr" : "stdout")
			}, stream);
			return 0;
		}
		const stricliProcess = {
			...
			*  exactly the frame stream, so help prose goes to stderr instead. */
			stdout: { write: (text) => (state.format === "json" ? runtime.stderr : runtime.stdout).write(text) },
			stderr: { write: (text) => {
	...
	*  never mount a command never reach here and so never report. */
	...
	async executeMounted(invocation, entry, rawFlags, values) {
		...
		const ctx = makeContext(invocation, entry.def, needsOutcome.config, needsOutcome.configFiles, declaredCapabilities(entry.def));
...
//#endregion
export { credentialWorkspaceId as C, credentialsRequiredError as D, credentialWorkspaceMismatchError as E, emptyServiceTokenError as O, claimedIdentity as S, credentialRejectedError as T, detectCI as _, definePrismaConfig as a, readActiveAccessToken as b, PRISMA_CONFIG_VERSION as c, PRESENTED as d, readUserConfig as f, exitWithChildStatus as g, resolveGating as h, CONFIG_FILE_NAME as i, noSessionForWorkspaceError as k, SERVICE_TOKEN_ENV_VAR as l, writeUserConfig as m, resolveSectionOverChain as n, loadConfig as o, userConfigPath as p, resolveSectionPath as r, realpathOr as s, buildEngine as t, WORKSPACE_ID_ENV_VAR as u, flag as v, authServiceError as w, claimedExpiresAt as x, positional as y };
