import { $ as ResolvedSection, A as Format, At as CommandArgs, B as Ui, Bt as CredentialOrigin, C as defineSessionCommand, Ct as PackageManagerRunRequest, D as OpenUrlRequest, Dt as Args, E as OpenUrlOutcome, Et as PackageOperations, F as Span, Ft as ActiveAccessTokenOptions, G as ConfigSection, Gt as CredentialRefresher, H as Severity, Ht as Session, I as Status, It as ActiveCredential, J as ConfigSchema, Jt as TokenStorage, K as SectionValidation, Kt as ManagementApiClient, L as Text, Lt as Credential, M as PRESENTED, Mt as PositionalSpec, N as Presentations, Nt as flag, O as PromptSurface, Ot as ArgsSpec, P as PresentedResult, Pt as positional, Q as validateSectionWithSchema, R as Tone, Rt as CredentialIdentity, S as defineServerCommand, St as PackageManagerId, T as CommandContext, Tt as PackageManagerRunner, U as StreamEvent, Ut as StoredSessions, V as EngineEvent, Vt as SERVICE_TOKEN_ENV_VAR, W as StreamMeta, Wt as CredentialRefreshResult, X as configSchema, Y as ConfigSchemaValue, Z as reference, _ as ServerCommandDefinition, _t as SpawnChild, a as RedirectSpec, at as InputStream, b as WorkflowStep, bt as SpawnedChild, c as CommandDefinition, ct as OutputStream, d as CommandNeeds, dt as TelemetryPayload, et as SectionProvenance, f as CompletedEnvelope, ft as EngineCommandSnapshot, g as NeedsSpec, gt as ExitWithChildStatusOptions, h as HelpSpec, ht as ChildStatusSettlement, i as MountedTree, it as HostProcess, j as Outcome, jt as FlagSpec, k as Block, kt as Char, l as CommandHandler, lt as PRISMA_CONFIG_VERSION, m as Handler, mt as ChildResult, n as CommandFamily, nt as resolveSectionPath, o as defineCommandFamily, ot as LoadedConfig, p as ErroredEnvelope, pt as RunSummary, q as defineConfigSection, qt as ManagementApiClientConfig, r as CommandRedirect, rt as Host, s as AnyCommand, st as LoadedConfigFile, t as TelemetryDeclaration, tt as resolveSectionOverChain, u as CommandHelp, ut as Runtime, v as SessionCommandDefinition, vt as SpawnOptions, w as BrowserWaitRequest, wt as PackageManagerRunResult, x as defineCommand, xt as exitWithChildStatus, y as SpawnDeclarations, yt as SpawnRequest, z as TreeNode, zt as CredentialManager } from "./report-B_8n69OY.js";
import { n as CliStructuredError } from "./protocol-YjqPCCOG.js";

//#region src/active-access-token.d.ts
/** Shared implementation used by every CredentialManager. */
declare function readActiveAccessToken(storage: TokenStorage, refreshCredential: CredentialRefresher | undefined, options: ActiveAccessTokenOptions): Promise<string | null>;
//#endregion
//#region src/ci.d.ts
 ...
 */
declare function detectCI(env: Env): boolean;
//#endregion
//#region src/cli.d.ts
/**
...
declare function createCli(spec: {
  ...
  readonly groups: Readonly<Record<string, {
    ...
    readonly description?: string; /** The group's common path, rendered as a `Workflow` section. */
    readonly workflow?: readonly WorkflowStep[];
  }>>;
  readonly commands: MountedTree; /** Words for the root help card; the engine formats. */
  readonly help?: {
    readonly tagline?: string;
    readonly description?: string; /** The CLI's common path, rendered as a `Workflow` section. */
    readonly workflow?: readonly WorkflowStep[];
 ...
 */
declare function definePrismaConfig<T extends Record<string, unknown>>(config: T): T & {
...
declare function realpathOr(path: string): string;
/**
 ...
 */
declare function loadConfig(cwd: string, configPath?: string, cliVersion?: string): Promise<LoadedConfig>;
//#endregion
//#region src/credential-errors.d.ts
type CredentialsRequiredReason = "unauthenticated" | "expired" | "expiring-soon" | "session-ended" | "sessions-held-none-selected";
/**
 ...
 */
declare function credentialsRequiredError(reason?: CredentialsRequiredReason): CliStructuredError;
/**
 ...
 * fix.
 */
declare function authServiceError(): CliStructuredError;
/**
 ...
 */
declare function credentialRejectedError(origin: CredentialOrigin, envVar: string): CliStructuredError;
/**
 ...
 * raises.
 */
declare function credentialWorkspaceMismatchError(workspaceId: string): CliStructuredError;
/**
 ...
 */
declare function emptyServiceTokenError(spec: {
 ...
 * you have.
 */
declare function noSessionForWorkspaceError(workspaceRef: string): CliStructuredError;
//#endregion
//#region src/environment-credential-manager.d.ts
declare class EnvironmentCredentialManager implements CredentialManager {
  #private;
  constructor(spec: {
  ...
  createSession(_credential: Credential, _workspaceId: string): Promise<Session>;
  selectSession(_workspaceId: string): Promise<Session>;
  endSession(_workspaceId: string): Promise<void>;
...
//#endregion
//#region src/telemetry/gating.d.ts
/**
 ...
 */
type TelemetryDisabledReason = "ci" | "env-opt-out" | "stored-opt-out";
type TelemetryEnabledReason = "stored-opt-in" | "default-on";
type TelemetryStatusReason = TelemetryDisabledReason | TelemetryEnabledReason;
//#endregion
//#region src/telemetry/commands.d.ts
 ...
 * ```
 */
declare function telemetryCommandGroup(options: {
  readonly docsUrl: string;
}): {
 ...
 * enforced.
 */
declare function credentialWorkspaceId(token: string): string | undefined;
declare function claimedExpiresAt(token: string): Date | undefined;
/**
 ...
 */
declare function claimedIdentity(token: string): CredentialIdentity | undefined;
//#endregion
export { type ActiveAccessTokenOptions, type ActiveCredential, type AnyCommand, type Args, type ArgsSpec, type Block, type BrowserWaitRequest, type Char, type ChildResult, type ChildStatusSettlement, type Cli, type CliRunHooks, type CommandArgs, type CommandContext, type CommandDefinition, type CommandFamily, type CommandHandler, type CommandHelp, type CommandNeeds, type CommandRedirect, type CompletedEnvelope, type ConfigSchema, type ConfigSchemaValue, type ConfigSection, type Credential, type CredentialIdentity, type CredentialManager, type CredentialOrigin, type CredentialRefreshResult, type CredentialRefresher, type CredentialsRequiredReason, type EngineCommandSnapshot, type EngineEvent, EnvironmentCredentialManager, type ErroredEnvelope, type ExitWithChildStatusOptions, type FlagSpec, type Format, type Handler, type HelpSpec, type Host, type HostProcess, type InputStream, type LoadedConfig, type LoadedConfigFile, type ManagementApiClient, type ManagementApiClientConfig, type MountedTree, type NeedsSpec, type OpenUrlOutcome, type OpenUrlRequest, type Outcome, type OutputStream, PRESENTED, PRISMA_CONFIG_VERSION, type PackageManagerId, type PackageManagerRunRequest, type PackageManagerRunResult, type PackageManagerRunner, type PackageOperations, type PositionalSpec, type Presentations, type PresentedResult, type PromptSurface, type RedirectSpec, type ResolvedSection, type RunSummary, type Runtime, SERVICE_TOKEN_ENV_VAR, type SectionProvenance, type SectionValidation, type ServerCommandDefinition, type Session, type SessionCommandDefinition, type Severity, type Span, type SpawnChild, type SpawnDeclarations, type SpawnOptions, type SpawnRequest, type SpawnedChild, type Status, type StoredSessions, type StreamEvent, type StreamMeta, type TelemetryDeclaration, type TelemetryPayload, type TelemetryStatus, type TelemetryStatusReason, type Text, type TokenStorage, type Tone, type TreeNode, type Ui, type WorkflowStep, authServiceError, claimedExpiresAt, claimedIdentity, configSchema, createCli, credentialRejectedError, credentialWorkspaceId, credentialWorkspaceMismatchError, credentialsRequiredError, defineCommand, defineCommandFamily, defineConfigSection, definePrismaConfig, defineServerCommand, defineSessionCommand, detectCI, emptyServiceTokenError, exitWithChildStatus, flag, loadConfig, noSessionForWorkspaceError, positional, readActiveAccessToken, realpathOr, reference, resolveSectionOverChain, resolveSectionPath, telemetryCommandGroup, validateSectionWithSchema };