import { G as PlanMeta } from "./domain-envelope-CENjHN4U-BIIt9bl4.mjs";
import { a as CodecCallContext } from "./codec-types-C4Bx6jXr-cKPQHksN.mjs";
...
//#endregion
//#region src/execution/shape.d.ts
/**
 ...
 */
declare const RelationKeys: unique symbol;
/**
 * The relation names an emitted model type declares.
 */
type RelationNamesOf<M> = Rel<M>;
/**
 ...
 * `string` there.
 */
type Rel<M> = M extends {
  readonly [RelationKeys]?: infer R;
 ...
 */
type Scalars<M> = M extends {
  readonly [RelationKeys]?: unknown;
} ? Omit<M, Rel<M> | typeof RelationKeys> : M;
/**
 ...
 */
type Shape<M, Spec extends ShapeSpec<M, Spec> = Record<never, never>> = ShapeOf<M, Spec>;
/**
 ...
 */
type ShapeSpec<M, Spec> = { readonly [K in keyof Spec]: K extends '+' ? Exclude<PlusNames<M, Spec>, keyof Spec> : K extends '-' ? ScalarNamesOf<M> : K extends RelationNamesOf<M> ? ShapeSpec<RelatedModel<M, K>, Spec[K]> : UnknownSpecKey<M, K>; };
type PlusNames<M, Spec> = '-' extends keyof Spec ? RelationNamesOf<M> : ScalarNamesOf<M> | RelationNamesOf<M>;
type UnknownSpecKey<M, K> = [RelationNamesOf<M>] extends [never] ? `'${K & string}' is not a relation of the model, which has none; try '+' or '-'` : `'${K & string}' is not a relation of the model; try '+', '-', or '${RelationNamesOf<M>}'`;
type ScalarNamesOf<M> = M extends unknown ? keyof Scalars<M> & string : never;
type RelatedModel<M, K> = M extends unknown ? K extends keyof M ? UnwrapRelation<M[K]> : never : never;
...
type PlusNamesAScalar<M, Spec> = Spec extends {
  readonly '+': infer Keep;
} ? [Extract<Keep, ScalarNamesOf<M>>] extends [never] ? false : true : false;
type ShapeOfEach<M, Spec, Narrow extends boolean> = M extends unknown ? Flatten<KeptScalars<M, Spec, Narrow> & { [K in IncludedRelations<M, Spec>]: WrapLike<M[K], ShapeOf<RelatedModel<M, K>, NestedSpec<Spec, K>>>; }> : never;
type KeptScalars<M, Spec, Narrow extends boolean> = Narrow extends true ? Spec extends {
  readonly '+': infer Keep;
} ? Pick<Scalars<M>, Extract<Keep, keyof Scalars<M>>> : never : Spec extends {
  readonly '-': infer Drop extends PropertyKey;
} ? Omit<Scalars<M>, Drop> : Scalars<M>;
type IncludedRelations<M, Spec> = Extract<keyof Spec | (Spec extends {
  readonly '+': infer Keep;
} ? Keep : never), RelationNamesOf<M> & keyof M>;
...
//#endregion
export { Scalars as A, raceAgainstAbort as B, RuntimeErrorEnvelope as C, RuntimeMiddleware as D, RuntimeLog as E, checkAborted as F, runtimeAborted as G, runBeforeQueryChain as H, checkMiddlewareCompatibility as I, runtimeError as K, createMetaBuilder as L, ShapeSpec as M, ValidAnnotations as N, RuntimeMiddlewareContext as O, assertAnnotationsApplicable as P, defineAnnotation as R, RuntimeCoreOptions as S, RuntimeExecutor as T, runExecuteWithMiddleware as U, runBeforeExecuteChain as V, runQueryWithMiddleware as W, RelationKeys as _, AsyncIterableResult as a, RuntimeAbortedPhase as b, ExecuteInterceptResult as c, MetaBuilder as d, OperationKind as f, RUNTIME_ABORTED as g, QueryPlan as h, AnnotationValue as i, Shape as j, RuntimeStatementStats as k, ExecutionPlan as l, QueryInterceptResult as m, AfterQueryResult as n, CrossFamilyMiddleware as o, ParamRefMutator as p, AnnotationHandle as r, DefineAnnotationOptions as s, AfterExecuteResult as t, LaneMetaBuilder as u, RelationNamesOf as v, RuntimeExecuteOptions as w, RuntimeCore as x, ResultType as y, isRuntimeError as z };
//# sourceMappingURL=runtime-BLPkDw5z.d.mts.map