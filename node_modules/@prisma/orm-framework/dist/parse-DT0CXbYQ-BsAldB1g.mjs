...
import { n as isTerminatedStringLiteral, t as Tokenizer } from "./tokenizer-CFsbtfFw-BnfUBepb.mjs";
import { A as createSyntaxTree, o as DocumentAst } from "./declarations-SwNXosDu-D2vp62Qo.mjs";
//#region ../../../1-framework/2-authoring/psl-parser/dist/parse-DT0CXbYQ.mjs
...
var PslSources = class PslSources {
	#sourcesByRoot = /* @__PURE__ */ new Map();
	constructor(entries) {
		for (const [root, sourceFile] of entries) this.#sourcesByRoot.set(root, sourceFile);
	}
	/**
	...
	*/
	merge(...others) {
		return new PslSources([...this.#sourcesByRoot, ...others.flatMap((other) => [...other.#sourcesByRoot])]);
	}
	sourceFileNamed(filename) {
		let match;
		for (const sourceFile of this.#sourcesByRoot.values()) {
			if (sourceFile.filename !== filename) continue;
			if (match !== void 0 && match.text !== sourceFile.text) throw new InternalError(`Ambiguous PSL diagnostic filename "${filename}": registered sources have different text`);
			match = sourceFile;
		}
		if (match === void 0) throw new InternalError(`No SourceFile registered for PSL diagnostic filename "${filename}"`);
		return match;
	}
	sourceFileFor(node) {
		const root = node.root();
		const sourceFile = this.#sourcesByRoot.get(root);
		if (sourceFile === void 0) throw new InternalError("No SourceFile registered for PSL syntax root");
		return sourceFile;
	}
};
...
//#endregion
export { greenToken as a, greenNode as i, PslSources as n, parse as o, SourceFile as r, GreenNodeBuilder as t };

//# sourceMappingURL=parse-DT0CXbYQ-BsAldB1g.mjs.map