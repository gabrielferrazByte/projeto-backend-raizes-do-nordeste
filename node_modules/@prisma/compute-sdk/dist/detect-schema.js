...
function selectPrismaNextConfig(appPath, candidates, mode) {
    ...
    return (sortByPreferredRelativePath(appPath, matches.map((candidate) => candidate.path), 
    // prisma.config.ts is the canonical name for Prisma 8+; prefer it when
    // both it and the legacy prisma-next.config.ts are present in one app.
    "prisma.config.ts")
...
const PRISMA_NEXT_CONFIG_EXTENSIONS = [
    ".cjs",
    ".cts",
    ".js",
    ".mjs",
...
/**
 * Returns true when the file name is a Prisma Next config file.
 *
 * Two canonical names are accepted:
 * - `prisma.config.*` — the canonical name used by the `prisma` CLI (Prisma 8+).
 * - `prisma-next.config.*` — the legacy name used before the prisma CLI adopted
 *   the canonical name. Kept for the transition period.
 */
function isPrismaNextConfigFile(fileName) {
    const hasRecognizedExtension = PRISMA_NEXT_CONFIG_EXTENSIONS.some((extension) => fileName.endsWith(extension));
    if (!hasRecognizedExtension) {
        return false;
    }
    return (fileName.startsWith("prisma.config.") ||
        fileName.startsWith("prisma-next.config."));
}
