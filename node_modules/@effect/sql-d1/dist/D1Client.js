...
export const make = options => Effect.gen(function* () {
  ...
  const makeConnection = Effect.gen(function* () {
    ...
    const runStatementRaw = (statement, params = []) => Effect.tryPromise({
      try: async () => {
    ...
    const runStatement = (statement, params = []) => Effect.map(runStatementRaw(statement, params), response => response.results || []);
    const runRaw = (sql, params = []) => runStatementRaw(db.prepare(sql), params);
    const runCached = (sql, params = []) => Effect.flatMap(Cache.get(prepareCache, sql), s => runStatement(s, params));
    const runUncached = (sql, params = []) => runStatement(db.prepare(sql), params);
