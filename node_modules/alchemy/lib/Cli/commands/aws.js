...
import * as Aws from "../../Alchemist/routes/aws.js";
import { confirmOrDecline } from "./confirm.js";
import { envFile, yes } from "./flags.js";
import { instrumentCommand } from "./instrument.js";
const awsProfile = Flag.String("aws-profile").pipe(Flag.withDescription("AWS CLI/SSO profile to use for bootstrap credentials"), Flag.optional, Flag.map(Option.getOrElse(() => "default")));
...
const runBootstrap = Effect.fn(function* (args) {
    ...
    if (destroy) {
        ...
        yield* result.destroyed.length === 0
            ...
            : prompt.output.success(`Destroyed ${result.destroyed.length} bootstrap bucket(s): ${result.destroyed.join(", ")}`);
        return;
    }
...
const teardownCommand = Command.make("teardown", { envFile, profile: awsProfile, region: awsRegion, yes }, instrumentCommand("provider.aws.teardown", (a) => ({
...
const bootstrapCommand = Command.make("bootstrap", { envFile, profile: awsProfile, region: awsRegion }, instrumentCommand("provider.aws.bootstrap", (a) => ({
    "aws.profile": a.profile,
    "alchemy.region": a.region ?? "",
}))(Effect.fn(function* (args) {
    yield* runBootstrap({ ...args, destroy: false });
}))).pipe(Command.withDescription("Provision the AWS deployment assets bucket"));
export const awsCommand = Command.make("aws", {}).pipe(Command.withDescription("Manage AWS provider prerequisites"), Command.withSubcommands([bootstrapCommand, teardownCommand]));
