...
import * as Cloudflare from "../../Alchemist/routes/cloudflare.js";
import * as CloudflareToken from "../../Alchemist/routes/cloudflareToken.js";
...
import { envFile, parseSince, profile, yes } from "./flags.js";
...
export const formatCreatedCloudflareToken = (result, token) => [
    "",
    `Created Cloudflare API token "${result.name}" (${result.id}).`,
    `Granted ${result.grantedPermissionGroups} permission group(s) across ${result.policies.length} policy(ies)${result.verificationStatus
    ...
    ...result.diagnostics.map((diagnostic) => `${diagnostic.severity.toUpperCase()}: ${diagnostic.message}`),
].join("\n");
...
}, instrumentCommand("provider.cloudflare.bootstrap", (a) => ({
...
const teardownCommand = Command.make("teardown", {
    ...
    workerName: cloudflareWorkerName,
    yes,
}, instrumentCommand("provider.cloudflare.teardown", (a) => ({
    "alchemy.profile": a.profile ?? "",
    "alchemy.worker_name": a.workerName ?? "",
 ...
 */
const createTokenCommand = Command.make("token", {
    envFile,
    ...
    const read = (config) => config.pipe(Effect.provide(ConfigProvider.layer(provider)), Effect.map(Option.getOrUndefined));
    const apiKey = (yield* read(Config.String("CLOUDFLARE_API_KEY").pipe(Config.option))) ??
    ...
    const email = (yield* read(Config.String("CLOUDFLARE_EMAIL").pipe(Config.option))) ??
    ...
    const resolvedAccountIds = accountId ??
        (catalog.accounts.length === 1
            ? [catalog.accounts[0].id]
            : yield* prompt.prompt.multiSelect({
    ...
    if (plan.grantsFullAccess) {
        yield* prompt.output.warning("This token will have FULL access to your Cloudflare account. Keep it secret.");
...
const tailFlag = Flag.Boolean("tail").pipe(Flag.withAlias("t"), Flag.withDescription("Stream logs in real time via the Cloudflare tail websocket instead of fetching past entries."), Flag.withDefault(false));
const limitFlag = Flag.Int("limit").pipe(Flag.withDescription("Number of log entries to fetch (ignored with --tail)"), Flag.withDefault(100));
...
const stateLogsCommand = Command.make("logs", {
    ...
    if (tail) {
        ...
        yield* Cloudflare.tailStateLogs(target).pipe(Stream.runForEach((line) => Console.log(formatLine(line))));
        return;
    }
...
const stateCommand = Command.make("state", {}).pipe(Command.withDescription("Manage the Cloudflare-hosted state store"), Command.withSubcommands([stateLogsCommand]));
export const cloudflareCommand = Command.make("cloudflare", {}).pipe(Command.withDescription("Manage Cloudflare provider prerequisites"), Command.withSubcommands([
    bootstrapCommand,
    teardownCommand,
    createTokenCommand,
    stateCommand,
