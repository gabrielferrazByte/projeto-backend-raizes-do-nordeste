...
import { importStack } from "../../../Alchemist/Session.js";
import { DEFAULT_PROFILE_NAME } from "../../../Auth/Profile.js";
import { CliKit } from "../../CliKit/index.js";
...
export const profileHub = Effect.fn(function* (options) {
    ...
    const computeEntries = Profiles.list().pipe(Effect.map((profiles) => profiles.map((profile) => ({
        name: profile.name,
    ...
    const refreshEntries = computeEntries.pipe(Effect.tap((entries) => Ref.set(lastEntries, entries)));
    const { runProfileDashboardSession } = yield* Effect.promise(() => import("../../components/view/ProfileDashboard.js"));
    ...
    yield* runProfileDashboardSession({
        ...
        loadDetails: (name) => Effect.gen(function* () {
        ...
        execute: (action) => Effect.gen(function* () {
            let selected;
            let message;
        ...
        }).pipe(Effect.catch((error) => Effect.map(Ref.get(lastEntries), (entries) => ({
            ok: false,
            message: error.message,
            entries,
        })))),
        runFlow: (action, events) => Effect.gen(function* () {
            if (action.kind === "refresh") {
                yield* Profiles.refresh({
                    ...
                    entrypoint: options.main,
                    envFile,
                }).pipe(Effect.provideService(Progress, (event) => event._tag === "provider.refresh.started"
                    ? events.onProviderStart(event.provider)
                    : Effect.void));
                return {
                    ok: true,
                    message: action.provider === undefined
        ...
        }).pipe(Effect.catch((error) => Effect.succeed(isPromptCancellation(error)
