...
import { exitDeclined, failWithHelp, UserInputError } from "../errors.js";
import { config, envFile, profile, yes } from "../flags.js";
import { instrumentCommand } from "../instrument.js";
import { deleteProfileFlow, editProfileFlow, profileTui, renameProfileFlow, showProfileFlow, } from "./flows.js";
import { profileHub } from "./hub.js";
const profileName = Argument.String("name").pipe(Argument.withDescription("Profile name"));
const newProfileName = Argument.String("new-name").pipe(Argument.withDescription("New profile name"), Argument.optional);
const refreshProviders = Flag.String("provider").pipe(Flag.withDescription("Refresh only this connected provider (repeatable; defaults to all)"), Flag.atLeast(0));
const showCommand = Command.make("show", { profile, envFile, main: config }, instrumentCommand("profile.show", (a) => ({
...
}))).pipe(Command.withDescription("Show connected providers, authentication status, and account details"));
const listCommand = Command.make("list", { envFile }, instrumentCommand("profile.list")(Effect.fn(function* () {
...
const createCommand = Command.make("create", { name: profileName }, instrumentCommand("profile.create", (a) => ({
    "alchemy.profile": a.name,
}))(Effect.fn(function* ({ name }) {
    yield* Profiles.create({ name });
    yield* CliKit.accessors.output.success(`Created profile '${name}'. Run \`alchemy profile edit --profile ${name}\` to connect accounts.`);
}))).pipe(Command.withDescription("Create an empty authentication profile"));
const renameCommand = Command.make("rename", { name: profileName, newName: newProfileName }, instrumentCommand("profile.rename", (a) => ({
    "alchemy.profile": a.name,
    ...
    if (Option.isSome(newName)) {
        ...
        yield* CliKit.accessors.output.success(`Renamed profile '${name}' to '${newName.value.trim()}'.`);
        return;
    }
...
const addProviders = Flag.String("add").pipe(Flag.withDescription("Connect a provider to the profile (repeatable)"), Flag.atLeast(0));
const reconfigureProviders = Flag.String("reconfigure").pipe(Flag.withDescription("Re-run a connected provider's configuration (repeatable)"), Flag.atLeast(0));
const removeProviders = Flag.String("remove").pipe(Flag.withDescription("Log out a connected provider and disconnect it (repeatable)"), Flag.atLeast(0));
const methodFlag = Flag.String("method").pipe(Flag.withDescription("Configure non-interactively using this method (each provider documents its methods and fields in `alchemy profile edit --help`)"), Flag.optional, Flag.map(Option.getOrUndefined));
const setFlag = Flag.String("set").pipe(Flag.withDescription("Field for non-interactive configure: name=value, name=env:VAR, or name=- to read the value from stdin (repeatable)"), Flag.atLeast(0));
/**
 ...
 */
const resolveSetValues = Effect.fn(function* (sets, envFile) {
    const values = {};
    let stdinUsed = false;
    ...
    for (const entry of sets) {
        ...
        if (raw === "-") {
            ...
            const value = yield* getEnv(variable).pipe(Effect.provide(ConfigProvider.layer(configProvider)), Effect.mapError((e) => new UserInputError({ message: e.message })));
...
const editCommand = Command.make("edit", {
    ...
    envFile,
    main: config,
}, instrumentCommand("profile.edit", (a) => ({
    "alchemy.profile": a.profile ?? "",
    ...
    if (method !== undefined || set.length > 0) {
        if (add.length + reconfigure.length !== 1 || remove.length > 0) {
            return yield* Effect.fail(new UserInputError({
                message: "--method/--set configure exactly one provider: pass a single --add or --reconfigure (and no --remove).",
            }));
        }
        configureInput = {
            method,
    ...
    if (configureInput !== undefined) {
        ...
        yield* Profiles.configure({
            ...
            values: Object.fromEntries(Object.entries(configureInput.values).map(([key, value]) => [
                key,
                Redacted.make(value),
            ])),
        });
        yield* CliKit.accessors.output.success(`${add.length === 1 ? "Added" : "Updated"} '${provider}' in profile '${selectedProfile}'.`);
        return;
    }
    if (add.length === 0 && reconfigure.length === 0 && remove.length > 0) {
        for (const provider of remove) {
            ...
            yield* CliKit.accessors.output.success(`Removed '${provider}' from profile '${selectedProfile}'.`);
        }
        return;
    }
...
const refreshCommand = Command.make("refresh", {
    profile,
    providers: refreshProviders,
    envFile,
    main: config,
}, instrumentCommand("profile.refresh", (a) => ({
    "alchemy.profile": a.profile ?? "",
    "alchemy.providers": a.providers.join(","),
}))(Effect.fn(function* ({ profile, providers, envFile, main }) {
    const selectedProfile = yield* resolveProfileName(envFile, profile);
    const refreshStarted = new Map();
    yield* Profiles.refresh({
        ...
        ? Clock.currentTimeMillis.pipe(Effect.tap((now) => Effect.sync(() => refreshStarted.set(event.provider, now))), Effect.andThen(CliKit.accessors.output.info(`Refreshing ${event.provider}`)))
        : event._tag === "provider.refresh.completed"
            ? Clock.currentTimeMillis.pipe(Effect.flatMap((now) => {
                ...
                return CliKit.accessors.output.success(`Refreshed ${event.provider}${from === undefined ? "" : ` (${formatElapsed(now - from)})`}`);
            }))
            : Effect.void));
    yield* CliKit.accessors.output.success(`Refreshed profile '${selectedProfile}'.`);
}))).pipe(Command.withDescription("Refresh credentials for connected providers without reconfiguring them"));
const currentCommand = Command.make("current", { envFile }, instrumentCommand("profile.current")(Effect.fn(function* () {
    const selected = yield* Profiles.current();
    const source = selected.source === "configuration"
        ? "ALCHEMY_PROFILE"
...
const deleteCommand = Command.make("delete", { name: profileName, envFile, main: config, yes }, instrumentCommand("profile.delete", (a) => ({
    "alchemy.profile": a.name,
    "alchemy.yes": a.yes,
...
export const profileCommand = Command.make("profile", { envFile, main: config }, instrumentCommand("profile")(Effect.fn(function* ({ envFile, main }) {
...
}))).pipe(Command.withDescription("Manage authentication profiles and accounts"), Command.withSubcommands([
    createCommand,
    renameCommand,
    editCommand,
    refreshCommand,
