...
import * as Drift from "../../Alchemist/routes/drift.js";
import * as Stacks from "../../Alchemist/routes/stack.js";
import { Cli } from "../../Report.js";
import * as CliKit from "../CliKit/index.js";
import { planDecisionScreen } from "../components/view/PlanDecision.js";
import { stackOutputsView } from "../components/view/StackOutputs.js";
import { exitDeclined } from "./errors.js";
import { configPath, dryRun as dryRunFlag, envFile, force, optionalConfig, profile, resolveStackArgs, stage, yes, } from "./flags.js";
import { instrumentCommand } from "./instrument.js";
import { renderApply, renderPlanning } from "./render.js";
...
const detailed = Flag.Boolean("detailed").pipe(Flag.withDescription("Show declared resource properties as YAML"), Flag.withDefault(false));
const detectDrift = Flag.Boolean("detect-drift").pipe(Flag.withDescription("Detect infrastructure drift and offer to repair it before deploying"), Flag.withDefault(false));
const detectAndMaybeRepairDrift = Effect.fn(function* (target, options) {
    const cli = yield* Cli;
    const snapshot = yield* Drift.inspect(target).pipe(renderPlanning({
        operation: "Drift",
        stage: target.stage,
    ...
    if (!options.yes) {
        ...
        if (terminal.terminal.input) {
            decision = yield* terminal.prompt
                ...
                .pipe(Effect.catchTag("TerminalCancelled", () => Effect.succeed("cancel")));
        }
        else {
            yield* cli.displayPlan(snapshot.repairPlan.native, {
                detailed: options.detailed,
            ...
            yield* CliKit.accessors.output.warning("Drift detected in a non-interactive terminal. Re-run with --yes to repair and deploy.");
        }
    }
    if (decision === "repair") {
...
const runStack = Effect.fn(function* (options) {
    const cli = yield* Cli;
    ...
    if (!options.yes && Stacks.hasChanges(snapshot.summary)) {
        const approved = yield* cli.approvePlan(snapshot.native, display).pipe(Effect.tap((approved) => approved
            ? Effect.void
            : CliKit.accessors.output.info(`${operation} aborted: plan declined.`)), Effect.catchTag("NonInteractiveTerminal", () => CliKit.accessors.output
            .warning(`Cannot prompt for approval in a non-interactive terminal. Nothing was changed — re-run with --yes to ${operation.toLowerCase()}.`)
...
export const deployCommand = Command.make("deploy", {
    ...
    detailed,
    detectDrift,
}, (args) => resolveStackArgs("live")(args).pipe(Effect.flatMap(instrumentCommand("deploy", stackSpanAttrs)(runStack))));
export const destroyCommand = Command.make("destroy", {
    ...
    yes,
    profile,
}, (args) => resolveStackArgs("live")(args).pipe(Effect.flatMap(instrumentCommand("destroy", stackSpanAttrs)((options) => runStack({
    ...options,
    destroy: true,
})))));
export const planCommand = Command.make("plan", {
    ...
    profile,
    detailed,
}, (args) => resolveStackArgs("live")(args).pipe(Effect.flatMap(instrumentCommand("plan", stackSpanAttrs)((options) => runStack({
    ...options,
