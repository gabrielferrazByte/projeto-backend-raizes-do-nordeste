...
import { checkLatestVersion } from "./checkVersion.js";
import { GlobalLogLive, logRunHeader } from "./GlobalLog.js";
import { handleCliErrors } from "./commands/errors.js";
import { compatibilityCommand, compatibilityCommands, } from "./commands/compat.js";
...
const placeholderCommand = (name, description) => Command.make(name, {}, () => Effect.void).pipe(Command.withDescription(description), name === "unsafe" ? Command.unlisted : (command) => command);
const loadCommand = async (name) => {
    switch (name) {
        case "provider":
            return (await import("./commands/provider.js")).providerCommand;
        case "deploy":
            return (await import("./commands/deploy.js")).deployCommand;
        case "dev":
            return (await import("./commands/dev.js")).devCommand;
        case "destroy":
            return (await import("./commands/deploy.js")).destroyCommand;
        case "plan":
            return (await import("./commands/deploy.js")).planCommand;
        case "logs":
            return (await import("./commands/logs.js")).logsCommand;
        case "profile":
            return (await import("./commands/profile/index.js")).profileCommand;
        case "state":
            return (await import("./commands/state.js")).stateCommand;
        case "drift":
            return (await import("./commands/drift.js")).driftCommand;
        case "unsafe":
            return (await import("./commands/nuke.js")).unsafeCommand;
    }
};
...
const commandNames = new Set(commandMetadata.map(([name]) => name));
const requestedCommand = argv.find((value) => commandNames.has(value));
const loadEveryCommand = argv.includes("--completions");
const commands = await Promise.all(commandMetadata.map(([name, description]) => loadEveryCommand || name === requestedCommand
    ? loadCommand(name)
...
const NoInput = GlobalFlag.Setting("no-input")({
    flag: Flag.Boolean("no-input").pipe(Flag.withDescription("Disable prompts and the interactive TUI (plain output; commands needing input fail)"), Flag.withDefault(false)),
});
const root = Command.make("alchemy", {}, () => Effect.fail(new CliError.ShowHelp({ commandPath: ["alchemy"], errors: [] }))).pipe(Command.withDescription("Define, deploy, and operate cloud infrastructure with type-safe Effect programs."), Command.withExamples([
    ...
    ...commands,
    ...Object.keys(compatibilityCommands).map((name) => compatibilityCommand(name)),
...
const devRunMode = import.meta.url.includes("/node_modules/")
    ...
    : `${typeof globalThis.Bun !== "undefined"
        ...
        : `node ${process.versions.node}`}, ${moduleExtension(import.meta.url) === ".ts" ? "src" : "lib"}`;
const cli = Command.run(root, {
    version: devRunMode === undefined
        ? packageJson.version
        : `${packageJson.version} (${devRunMode})`,
});
const services = Layer.mergeAll(CliConfig.layer({
    builtIns: [
        GlobalFlag.Help,
        GlobalFlag.Version,
...
}), Layer.provideMerge(AlchemyContextLive, PlatformServices), Layer.provide(ProfileStoreLive, PlatformServices), Layer.provide(CredentialsStoreLive, PlatformServices), 
...
// wins over this one.
Layer.succeed(ArtifactStore, createArtifactStore()), FetchHttpClient.layer, ConfigProvider.layer(ConfigProvider.fromEnv()), Layer.provide(Layer.provideMerge(Layer.mergeAll(selectCliServices(), CliKit.CliKitInteraction), CliKit.layer()), PlatformServices), 
...
Effect.provide(services), Effect.scoped, handleCliErrors);
/** Fully wired CLI program. */
export const main = mainEffect;
