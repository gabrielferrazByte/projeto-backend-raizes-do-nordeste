...
const toFlyStatics = (statics) => {
    if (statics === undefined || statics.length === 0)
        return undefined;
...
const buildConfig = (input) => ({
    ...
    statics: input.statics !== undefined && input.statics.length > 0
        ? input.statics
        : undefined,
});
...
const sameStatics = (observed, desired) => deepEqual(observed ?? [], desired ?? [], { stripNullish: true });
    ...
    return Service.Provider.of({
        ...
        diff: Effect.fn(function* ({ id, news, output }) {
            ...
            if (isResolved({
                ...
                const hash = yield* hosted.hash(statics);
