...
import { copyExtraFiles, contextRootOf, extraFileDestination, hashExtraFiles, isContextRootDest, posixRelUnder, resolveExtraSource, } from "../Util/extraFiles.js";
...
const generateDockerfile = (props, hasChunks, install, entryRel) => {
    ...
    if (install !== undefined && Object.keys(install).length > 0) {
        lines.push(`COPY package.json /app/package.json`, `RUN npm install --omit=dev --no-fund --no-audit`);
    }
    if (props.isExternal === true) {
        lines.push(`COPY . /app`);
        const entry = entryRel !== undefined && entryRel.length > 0
            ? entryRel
            : "serve-node.mjs";
        lines.push(`ENV PORT=${String(port)}`, `ENV HOST=0.0.0.0`, `EXPOSE ${String(port)}`, `ENTRYPOINT ["node", ${JSON.stringify(`/app/${entry}`)}]`);
        return `${lines.join("\n")}\n`;
    }
    ...
    for (const extra of props.extraFiles ?? []) {
        ...
        if (isContextRootDest(dest) || seen.has(dest))
            continue;
        seen.add(dest);
        lines.push(`COPY ${dest} /app/${dest}`);
    }
    lines.push(`ENV PORT=${String(port)}`, `ENV HOST=0.0.0.0`, `EXPOSE ${String(port)}`, `ENTRYPOINT ["node", "/app/index.mjs"]`);
    return `${lines.join("\n")}\n`;
};
...
export const createFlyHostedSupport = ({ stackName, stage, virtualEntryPlugin, docker, dotAlchemy, }) => {
    ...
    const computeCodeHash = Effect.fn(function* (props) {
        ...
        const root = contextRootOf(realMain, extras, path, (source) => resolveExtraSource(source, path));
        const entryRel = props.isExternal === true
    ...
    const resolveImage = Effect.fn(function* (input) {
        ...
        if (!(yield* imageExists(imageRef))) {
            ...
            const fs = yield* FileSystem.FileSystem;
            const path = yield* Path.Path;
            const files = input.props.isExternal === true
