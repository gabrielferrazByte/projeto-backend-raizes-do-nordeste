...
import { cacheControlOf, contentTypeOf } from "../../Website/assets.js";
 ...
 * @resource
 */
export const AssetDeployment = Resource("Fly.Website.AssetDeployment");
const normalizePrefix = (prefix) => prefix ? prefix.replace(/^\/+|\/+$/g, "") : "";
const scopeOf = (bucket) => Effect.gen(function* () {
    ...
    const region = (asPlain(bucket.region) ?? "auto");
...
const withTigris = (scope, operation) => operation.pipe(Effect.provide(Layer.mergeAll(fromCredentials({
    accessKeyId: scope.accessKeyId,
    secretAccessKey: scope.secretAccessKey,
}, scope.region), AwsEndpoint.of(scope.endpoint), FetchHttpClient.layer)));
...
const listObserved = (scope, prefix) => withTigris(scope, s3.listObjectsV2
    .pages({
    Bucket: scope.bucketName,
    Prefix: prefix.length > 0 ? `${prefix}/` : undefined,
})
    .pipe(Stream.flatMap((page) => Stream.fromIterable(page.Contents ?? [])), Stream.runFold(() => new Map(), (observed, object) => {
    if (object.Key !== undefined) {
        observed.set(object.Key, object.ETag);
    }
    return observed;
})));
const deleteObject = (scope, key) => withTigris(scope, s3.deleteObject({
    Bucket: scope.bucketName,
    Key: key,
})).pipe(Effect.catchTag("NoSuchKey", () => Effect.void));
export const AssetDeploymentProvider = () => Provider.effect(AssetDeployment, Effect.succeed({
    list: () => Effect.succeed([]),
    ...
    reconcile: Effect.fn(function* ({ news, session }) {
        ...
        const prepared = yield* Effect.all(files.map((relative) => Effect.gen(function* () {
        ...
        if (news.purge ?? true) {
            yield* Effect.all([...observed.keys()]
                .filter((key) => !desired.has(key))
                .map((key) => deleteObject(scope, key)), { concurrency: s3Concurrency });
        }
        const output = {
        ...
        yield* session.note(`Uploaded ${output.fileCount} file(s) to tigris://${output.bucketName}/${output.prefix}`);
        return output;
    }),
    delete: Effect.fn(function* ({ olds, output }) {
        ...
        yield* Effect.all([...observed.keys()].map((key) => deleteObject(scope, key)), { concurrency: s3Concurrency });
