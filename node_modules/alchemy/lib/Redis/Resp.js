 ...
 */
import { ProtocolError, ReplyError } from "./Errors.js";
...
const argBytes = (arg) => {
    ...
    if (typeof arg === "number")
        return bytesOf(String(arg));
    return arg;
};
export const encodeSimpleString = (value) => concat([bytesOf(`+${value}`), CRLF]);
export const encodeError = (value) => concat([bytesOf(`-${value}`), CRLF]);
export const encodeInteger = (value) => concat([bytesOf(`:${value}`), CRLF]);
export const encodeBoolean = (value) => bytesOf(value ? "#t\r\n" : "#f\r\n");
...
export const encodeBigNumber = (value) => concat([bytesOf(`(${value}`), CRLF]);
export const encodeNull = () => bytesOf("$-1\r\n");
export const encodeNullArray = () => bytesOf("*-1\r\n");
export const encodeBulk = (value) => {
    const payload = typeof value === "string" ? bytesOf(value) : value;
    return concat([bytesOf(`$${payload.length}\r\n`), payload, CRLF]);
};
export const encodeArray = (items) => concat([bytesOf(`*${items.length}\r\n`), ...items]);
/** Encode a client command as a RESP array of bulk strings. */
export const encodeCommand = (command, args = []) => {
    const parts = [bytesOf(command), ...args.map(argBytes)];
    const chunks = [bytesOf(`*${parts.length}\r\n`)];
    for (const part of parts) {
        chunks.push(bytesOf(`$${part.length}\r\n`), part, CRLF);
    }
    return concat(chunks);
};
export const encodeReply = (value) => {
    if (value === null)
        return encodeNull();
    ...
    if (value instanceof ReplyError)
        return encodeError(value.message);
    if (Array.isArray(value)) {
        return encodeArray(value.map(encodeReply));
    }
    const entries = Object.entries(value);
    const encoded = [bytesOf(`%${entries.length}\r\n`)];
    for (const [key, item] of entries) {
        encoded.push(encodeSimpleString(key), encodeReply(item));
    }
    return concat(encoded);
};
const INCOMPLETE = { ok: false, incomplete: true };
const fail = (message) => ({
    ok: false,
    error: new ProtocolError({ message }),
});
const ok = (frame, offset) => ({ ok: true, frame, offset });
...
const parseCount = (buf, offset) => {
    const line = parseLine(buf, offset);
    if (line === undefined)
        return { kind: "incomplete" };
    if (line instanceof ProtocolError)
        return { kind: "error", error: line };
    if (line.text === "?")
        return { kind: "stream", next: line.next };
    if (line.text === "-1")
        return { kind: "null", next: line.next };
...
const parseBulkBody = (buf, offset, n) => {
...
const parseStreamedString = (buf, offset) => {
    const chunks = [];
    let cursor = offset;
    while (true) {
        ...
        if (count.kind === "incomplete")
            return INCOMPLETE;
        if (count.kind === "error")
            return { ok: false, error: count.error };
        if (count.kind !== "n")
            return fail("invalid streamed string chunk length");
        if (count.n === 0) {
...
const parseVerbatim = (buf, offset) => {
    ...
    if (count.kind === "error")
        return { ok: false, error: count.error };
    if (count.kind !== "n")
        return fail("invalid verbatim string length");
    const body = parseBulkBody(buf, count.next, count.n);
    if (!body.ok)
        return body;
    if (body.frame._tag !== "Reply" || typeof body.frame.value !== "string") {
...
const parseBlobError = (buf, offset) => {
    const count = parseCount(buf, offset);
    if (count.kind === "incomplete")
        return INCOMPLETE;
    if (count.kind === "error")
        return { ok: false, error: count.error };
    if (count.kind !== "n")
        return fail("invalid blob error length");
    const body = parseBulkBody(buf, count.next, count.n);
    if (!body.ok)
        return body;
    if (body.frame._tag !== "Reply" || typeof body.frame.value !== "string") {
...
const parseElements = (buf, offset, n) => {
    if (n > MAX_AGGREGATE) {
...
const parseStreamedAggregate = (buf, offset) => {
    const values = [];
    let cursor = offset;
    while (true) {
        if (cursor >= buf.length)
        ...
        if (buf[cursor] === DOT) {
            ...
            if (line.text.length !== 0)
                return fail("invalid aggregate terminator");
...
const parseAggregate = (buf, offset, kind) => {
    const count = parseCount(buf, offset);
    if (count.kind === "incomplete")
        return INCOMPLETE;
    if (count.kind === "error")
    ...
    const parsed = count.kind === "stream"
...
const parseBulk = (buf, offset) => {
    ...
    if (count.kind === "stream")
        return parseStreamedString(buf, count.next);
...
function parseFrame(buf, offset) {
    ...
    switch (type) {
        ...
        default:
            return fail(`unexpected type byte ${JSON.stringify(String.fromCharCode(type))} (0x${type.toString(16)})`);
    }
}
/**
