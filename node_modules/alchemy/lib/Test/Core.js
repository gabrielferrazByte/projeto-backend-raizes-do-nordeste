...
import { userStage } from "../Cli/commands/flags.js";
 ...
 */
export const defaultStage = () => process.env.ALCHEMY_TEST_STAGE || Effect.runSync(userStage("test"));
/** File-level stage: `options.stage` if set, otherwise {@link defaultStage}. */
export const resolveStage = (options) => options.stage ?? defaultStage();
...
export const makeSidecarHandle = (options) => {
    ...
    if (singleton === undefined) {
        ...
        const lazy = Layer.effect(RpcProviderProxy.RpcProviderProxy, Effect.gen(function* () {
            ...
            const ambient = Context.omit(Scope.Scope)(yield* Effect.context());
            const realProxy = Layer.buildWithMemoMap(real, memoMap, scope).pipe(Effect.map((built) => Context.get(built, RpcProviderProxy.RpcProviderProxy)), Effect.provideContext(ambient), Scope.provide(scope), Effect.orDie);
            return RpcProviderProxy.RpcProviderProxy.of({
                get: (providersUrl, providerName) => Effect.flatMap(realProxy, (proxy) => proxy.get(providersUrl, providerName)),
            });
        }));
...
const platformLayer = () => Layer.mergeAll(PlatformServices, Option.getOrElse(alchemyTestDevOverride(), () => false)
    ? flociWebsiteHttp
    : FetchHttpClient.layer, Layer.provide(ProfileStoreLive, PlatformServices), Layer.provide(CredentialsStoreLive, PlatformServices));
...
export const scratchStack = (options, name, file) => {
    ...
    const pinToFloci = (effect) => Option.getOrElse(alchemyTestDevOverride(), () => false)
        ? Effect.provide(effect, flociServices())
        : effect;
    const buildAndApply = (effect) => pinToFloci(effect).pipe(makeStack({
        name: stackName,
        providers: options.providers,
        state: stateLayer,
    }), Effect.flatMap((compiled) => Plan.make(compiled).pipe(Effect.flatMap(apply), pinToFloci, Effect.provide(compiled.services))), Effect.provide(Layer.succeed(Stage, stage)), provideFreshArtifactStore);
    const buildPlan = (effect) => pinToFloci(effect).pipe(makeStack({
        name: stackName,
        providers: options.providers,
        state: stateLayer,
    }), Effect.flatMap((compiled) => pinToFloci(Plan.make(compiled)).pipe(Effect.provide(compiled.services))), Effect.provide(Layer.succeed(Stage, stage)), provideFreshArtifactStore);
    return {
        name: stackName,
        stage,
        state: stateLayer,
