 ...
 */
const cleanDockerIgnorePath = (value) => {
    const parts = [];
...
const compileDockerIgnoreRule = (raw) => {
    if (raw.startsWith("#")) {
        return undefined;
    }
    ...
    pattern = cleanDockerIgnorePath(pattern
        .replace(/^\.\/+/, "")
        .replace(/^\/+/, "")
        .replace(/\/+$/, ""));
    if (pattern.length === 0 || pattern === ".") {
...
const isDockerIgnored = (relativePath, rules) => {
 ...
 */
export const resolveDockerBuildPaths = Effect.fn(function* (source) {
...
const resolveDockerIgnore = Effect.fn(function* ({ context, dockerfile, }) {
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
 ...
 */
export const selectDockerBuildContext = Effect.fn(function* (source) {
    const path = yield* Path.Path;
    ...
    const dockerfilePath = relativeDockerfile === ".." ||
 ...
 * participate.
 */
export const hashDockerBuildInputs = Effect.fn(function* (source, mode) {
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    const { context, dockerfile } = yield* resolveDockerBuildPaths(source);
    const dockerignore = mode === "effective"
        ? yield* resolveDockerIgnore({ context, dockerfile })
        : undefined;
    const hasher = yield* Effect.sync(() => crypto.createHash("sha256"));
    yield* Effect.sync(() => hasher.update(JSON.stringify({
        platform: source.platform,
        buildArgs: Object.entries(source.buildArgs ?? {}).sort(([a], [b]) => a < b ? -1 : a > b ? 1 : 0),
    ...
    for (const entry of entries.sort()) {
        ...
        if (info.type === "File") {
            yield* fs.stream(fullPath).pipe(Stream.runForEach((chunk) => Effect.sync(() => {
                hasher.update(chunk);
            })));
        }
    }
...
//# sourceMappingURL=BuildHash.js.map