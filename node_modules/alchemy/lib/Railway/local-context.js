...
import { hashDockerBuildInputs, selectDockerBuildContext, } from "../Docker/BuildHash.js";
import { initialCwd } from "../Util/Node.js";
import { sha256Object } from "../Util/sha256.js";
import { tarGzipDirectory } from "../Util/tarGzip.js";
...
export class ServiceImageOrMainRequired extends Data.TaggedError("Railway.ServiceImageOrMainRequired") {
}
export class ServiceSourceInvalid extends Data.TaggedError("Railway.ServiceSourceInvalid") {
}
export class ServiceDockerfilePathInvalid extends Data.TaggedError("Railway.ServiceDockerfilePathInvalid") {
}
export class ServiceContextPathInvalid extends Data.TaggedError("Railway.ServiceContextPathInvalid") {
    get message() {
        return `Railway.Service expected ${this.path} to be an existing ${this.expected}`;
    }
}
export class ServiceDockerfileOutsideContext extends Data.TaggedError("Railway.ServiceDockerfileOutsideContext") {
}
export class ServiceContextSymlinkUnsupported extends Data.TaggedError("Railway.ServiceContextSymlinkUnsupported") {
}
export class ServiceContextPathUnsupported extends Data.TaggedError("Railway.ServiceContextPathUnsupported") {
}
...
export const resolveRailwayServiceSource = Effect.fn(function* (props) {
    ...
    if (modes.length !== 1) {
        return yield* new ServiceSourceInvalid({
            message: "Railway.Service requires exactly one source: `main`, `context`, `image`, or `repo` (`image` may configure the base image for `main`).",
        });
    }
    switch (modes[0]) {
        case "main":
            return { mode: "main", main: main };
        case "context":
            return {
                mode: "context",
                context: context,
        ...
        case "image":
            return { mode: "image", image: image };
        case "repo":
            return { mode: "repo", repo: repo };
        default:
            return yield* new ServiceSourceInvalid({
...
const resolveBuild = Effect.fn(function* (source) {
...
const paddedTarBytes = (size) => Math.ceil(size / TAR_BLOCK_BYTES) * TAR_BLOCK_BYTES;
const fitsUstarPath = (relativePath) => {
...
const tarEntryBytes = (relativePath, type, size) => {
    ...
    return (longNameBytes +
        TAR_BLOCK_BYTES +
        (type === "File" ? paddedTarBytes(size) : 0));
};
const checkContextLimit = (size, entries) => size > MAX_CONTEXT_BYTES || entries > MAX_CONTEXT_ENTRIES
    ? Effect.fail(new ServiceContextTooLarge({
...
export const hashRailwayLocalContext = Effect.fn(function* (source) {
...
export const prepareRailwayLocalContext = Effect.fn(function* (source) {
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
