import { waitUntilDeleted, projectServices as fetchProjectServices, projectBuckets as fetchProjectBuckets, projectGroups as fetchProjectGroups, environmentVolumes, } from "./GraphQL.js";
...
const resolveGroupProps = (props) => Effect.gen(function* () {
    ...
    const resources = yield* Effect.forEach(resolved.resources, (item) => Effect.isEffect(item)
        ? item
        : Effect.succeed(item));
    return { ...resolved, project, environment, resources };
});
const GroupResource = Resource("Railway.Group");
/**
 ...
 * @resource
 */
export const Group = Object.assign((id, props) => GroupResource(id, resolveGroupProps(props)), GroupResource);
...
const uniqueSorted = (ids) => Array.from(new Set(ids.filter((id) => id.length > 0))).sort();
const sameIds = (left, right) => uniqueSorted(left).join("\0") === uniqueSorted(right).join("\0");
...
const membersOf = (resources) => {
    ...
    for (const resource of resources ?? []) {
        ...
        if (classified.kind === "service")
            serviceIds.push(classified.id);
        else if (classified.kind === "volume")
            volumeIds.push(classified.id);
        else
            bucketIds.push(classified.id);
    }
    return {
...
const parseRecord = (value) => value !== null && typeof value === "object" && !Array.isArray(value)
    ? value
    : undefined;
const parseEnvironmentConfig = (value) => {
    if (value === null || typeof value !== "object" || Array.isArray(value)) {
        return {};
    }
    const rec = value;
    return {
        groups: parseRecord(rec.groups),
        services: parseRecord(rec.services),
        buckets: parseRecord(rec.buckets),
    };
};
/**
 ...
 */
const parseCanvasGroupRefs = (value) => {
    if (value === null || typeof value !== "object" || Array.isArray(value)) {
        return {};
    }
    const out = {};
...
const idsForGroup = (refs, groupId) => {
    const ids = [];
...
const servicesForGroup = (config, groupId) => {
    const services = config.services;
    if (services === undefined || services === null)
        return [];
    const ids = [];
    for (const [serviceId, row] of Object.entries(services)) {
        if (row === null || row.isDeleted === true)
            continue;
...
const bucketsForGroup = (config, groupId) => {
...
const liveServicesForGroup = (services, groupId) => uniqueSorted(services
    .filter((service) => service.deletedAt == null && service.groupId === groupId)
    .map((service) => service.id));
const liveBucketsForGroup = (buckets, groupId) => uniqueSorted(buckets
    .filter((bucket) => bucket.groupId === groupId)
    .map((bucket) => bucket.id));
const optionalString = (value) => value !== null && value !== undefined && value.length > 0 ? value : undefined;
const volumeIdsOf = (env) => {
    if (env === undefined)
        return [];
    const ids = [];
    for (const edge of env.volumeInstances.edges) {
        const node = edge.node;
        if (node.deletedAt != null || node.state === "DELETED")
            continue;
        ids.push(node.volumeId, node.volume.id, node.id);
    }
    return uniqueSorted(ids);
};
const toAttrs = (group, extra) => ({
...
const getProject = (projectId) => Effect.gen(function* () {
...
}).pipe(Effect.map((project) => (project.deletedAt != null ? undefined : project)), railway.catchTags(["RailwayNotFound"], () => Effect.succeed(undefined)));
const getEnvironment = (environmentId, projectId) => Effect.gen(function* () {
    const environment = yield* railway.environment({ id: environmentId, projectId }, { deletedAt: true, config: true, canvasGroupRefs: true });
...
const getEnvironmentConfig = (environmentId, projectId) => getEnvironment(environmentId, projectId).pipe(Effect.map((env) => env === undefined
    ? {}
    : parseEnvironmentConfig(env.config)));
const listEnvironmentIds = (project) => railway.environments
    .items({ projectId: project.projectId, first: 50 }, { id: true, deletedAt: true })
    .pipe(Stream.filter((env) => env.deletedAt == null), Stream.map((env) => env.id), Stream.runCollect, Effect.map((ids) => {
    const set = new Set(Array.from(ids));
    if (project.environmentId.length > 0) {
        set.add(project.environmentId);
    }
    return Array.from(set);
}), railway.catchTags(["RailwayNotFound"], () => Effect.succeed(project.environmentId.length > 0 ? [project.environmentId] : [])));
const commitPatch = (input) => withEnvironmentConfigLock(input.environmentId, railway.environmentPatchCommit({
    environmentId: input.environmentId,
    commitMessage: input.commitMessage,
    patch: input.patch,
})).pipe(railway.catchTags(["RailwayValidationError", "RailwayInternalError"], () => Effect.succeed("")));
const previewCanvas = (environmentId) => railway
    .previewCanvasViewMerge({
    sourceEnvironmentId: environmentId,
    targetEnvironmentId: environmentId,
}, { mutations: true })
    .pipe(railway.catchTags(["RailwayNotFound", "RailwayValidationError", "RailwayInternalError"], () => Effect.succeed(undefined)));
const mergeCanvas = (sourceEnvironmentId, targetEnvironmentId) => railway
    .mergeCanvasView({
    sourceEnvironmentId,
    targetEnvironmentId,
})
    .pipe(railway.catchTags(["RailwayNotFound", "RailwayValidationError", "RailwayInternalError"], () => Effect.succeed(false)));
const projectGroups = (project) => project?.groups.edges.map((edge) => edge.node) ?? [];
const projectServices = (project) => project?.services.edges.map((edge) => edge.node) ?? [];
const projectBuckets = (project) => project?.buckets.edges.map((edge) => edge.node) ?? [];
const matchProjectGroup = (groups, match) => {
...
const matchConfigGroup = (config, match) => {
    ...
    if (match.groupId !== undefined && match.groupId.length > 0) {
        ...
        if (row !== undefined)
            return { groupId: match.groupId, row };
    }
    if (match.name !== undefined && match.name.length > 0) {
...
const observe = Effect.fn(function* (input) {
    ...
    if (env === undefined || env.deletedAt != null)
        return undefined;
    ...
    const volumeIds = uniqueSorted([
        ...
        ...fromCanvas.filter((id) => knownVolumeIds.has(id) ||
...
const waitUntilPresent = (input) => observe({
    projectId: input.projectId,
    environmentId: input.environmentId,
    groupId: input.groupId,
    name: input.name,
    volumeIds: input.volumeIds,
}).pipe(Effect.flatMap((group) => {
    if (group === undefined) {
...
}), Effect.catchTag("Railway.GroupPending", () => observe({
    projectId: input.projectId,
    environmentId: input.environmentId,
    groupId: input.groupId,
    name: input.name,
...
const waitUntilGone = (input) => waitUntilDeleted("Group", input.groupId, observe({
    projectId: input.projectId,
    environmentId: input.environmentId,
    groupId: input.groupId,
    name: input.name,
}).pipe(Effect.map((group) => group === undefined)), 4);
...
const servicePatch = (assignments) => {
    const out = {};
...
export const GroupProvider = () => Provider.succeed(Group, {
    ...
    read: Effect.fn(function* ({ id, olds, output }) {
        ...
        const desired = membersOf(olds?.resources ?? []);
    ...
    list: Effect.fn(function* () {
        ...
        const rows = yield* Effect.forEach(projects, (project) => fetchProjectGroups(project.projectId, groupSelection).pipe(Effect.map((live) => live.flatMap((group) => {
    ...
    reconcile: Effect.fn(function* ({ id, news, output }) {
        ...
        if (environmentId === undefined) {
            return yield* new GroupEnvironmentRequired({
                message: "Group requires a Railway environment (pass environment or a Project with environmentId)",
            });
        }
        const name = yield* resolveName(id, props.name, output?.name);
        const desired = membersOf(props.resources);
        ...
        if (current === undefined) {
            ...
            if (desired.bucketIds.length > 0) {
                yield* commitPatch({
                    ...
                    patch: {
                        buckets: bucketPatch(desired.bucketIds.map((bucketId) => ({
                            bucketId,
                            groupId,
                        }))),
                    },
        ...
        return toAttrs(fallbackGroup({
            ...
            serviceIds: resolved !== undefined && resolved.serviceIds.length > 0
                ? resolved.serviceIds
                : desired.serviceIds,
    ...
    delete: Effect.fn(function* ({ output }) {
        ...
        const environmentId = output.environmentId;
        const projectId = output.projectId;
        if (groupId.length === 0 || environmentId.length === 0)
            return;
