 ...
 */
export const createRailwayName = Effect.fn(function* (id) {
...
export const createRailwayEnvironmentName = Effect.fn(function* (id) {
    const raw = yield* createPhysicalName({
        id,
        lowercase: true,
 ...
 */
const sanitizeTo = (name, maxLength) => {
    const lowered = name
        .toLowerCase()
        .replace(/[^a-z0-9-]+/g, "-")
        .replace(/-+/g, "-")
...
export const sanitizeRailwayName = (name) => sanitizeTo(name, RAILWAY_NAME_MAX_LENGTH);
export const sanitizeRailwayEnvironmentName = (name) => sanitizeTo(name, RAILWAY_ENVIRONMENT_NAME_MAX_LENGTH);
