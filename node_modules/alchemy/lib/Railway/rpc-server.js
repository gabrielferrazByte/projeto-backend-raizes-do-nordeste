...
import { PRIVATE_HOST_SUFFIX, RPC_ERROR_TAG, RPC_PATH_PREFIX, RPC_TOKEN_ENV, RPC_TOKEN_HEADER, } from "./rpc-token.js";
const header = (headers, name) => headers[name] ?? headers[name.toLowerCase()] ?? "";
...
const isPrivateMesh = (host) => hostnameOf(host).endsWith(PRIVATE_HOST_SUFFIX);
 ...
 */
export const serveRailwayRpc = (shape, fallback) => Effect.gen(function* () {
    ...
    const host = hostHeader.length > 0
        ? hostHeader
        : yield* Effect.sync(() => {
            try {
...
export const enableRailwayRpc = () => {
    globalThis.__R = serveRailwayRpc;
};
