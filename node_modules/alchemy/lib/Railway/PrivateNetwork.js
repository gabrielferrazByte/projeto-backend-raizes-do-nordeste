import { projectServices as fetchProjectServices } from "./GraphQL.js";
...
import { sanitizeRailwayName } from "./Metadata.js";
...
const NetworkConfig = Schema.Struct({
    ...
    services: Schema.optional(Schema.NullOr(Schema.Record(Schema.String, Schema.NullOr(Schema.Struct({
        networking: Schema.optional(Schema.NullOr(Schema.Struct({
            privateNetworkEndpoint: Schema.optional(Schema.NullOr(Schema.String)),
        }))),
    }))))),
});
const readNetworkConfig = Effect.fn(function* (environmentId) {
    const environment = yield* railway.environment({ id: environmentId }, { config: { where: { decryptVariables: false } } });
    return yield* Schema.decodeUnknownEffect(NetworkConfig)(environment.config);
});
export class PrivateNetworkConfigPending extends Data.TaggedError("Railway.PrivateNetworkConfigPending") {
}
const waitForNetworkConfig = (environmentId, ready) => readNetworkConfig(environmentId).pipe(Effect.flatMap((config) => ready(config)
    ? Effect.void
    : Effect.fail(new PrivateNetworkConfigPending({
        environmentId,
...
export class PrivateNetworkNameUnsupported extends Data.TaggedError("Railway.PrivateNetworkNameUnsupported") {
    get message() {
...
const resolveNetworkName = Effect.fn(function* (name) {
...
const listNetworks = (environmentId) => railway.privateNetworks({ environmentId }, selection).pipe(Effect.map((items) => items.filter((network) => !isGoneNetwork(network))), railway.catchTags(["RailwayNotFound"], () => Effect.succeed([])));
...
const ensureNetwork = Effect.fn(function* (environmentId) {
    const previousPrivateNetworkDisabled = yield* withEnvironmentConfigLock(environmentId, Effect.gen(function* () {
        ...
        if (config.privateNetworkDisabled === true || network === undefined) {
            ...
            yield* waitForNetworkConfig(environmentId, (observed) => observed.privateNetworkDisabled === false);
        }
    ...
    const network = yield* observeNetwork({
        environmentId,
        name: PLATFORM_NETWORK_NAME,
    }).pipe(Effect.flatMap((network) => network === undefined
        ? Effect.fail(new PrivateNetworkNotCreated({
            name: PLATFORM_NETWORK_NAME,
            environmentId,
        }))
        : Effect.succeed(network)), Effect.retry({
...
export const PrivateNetworkProvider = () => Provider.succeed(PrivateNetwork, {
    ...
    read: Effect.fn(function* ({ olds, output }) {
        ...
        if (environmentId === undefined)
            return undefined;
        const config = yield* readNetworkConfig(environmentId).pipe(railway.catchTags("RailwayNotFound", () => Effect.succeed(undefined)));
    ...
    delete: Effect.fn(function* ({ output }) {
        ...
        if (output.previousPrivateNetworkDisabled !== true)
            return;
        yield* withEnvironmentConfigLock(output.environmentId, Effect.gen(function* () {
            ...
            yield* waitForNetworkConfig(output.environmentId, (observed) => observed.privateNetworkDisabled === true);
...
export class PrivateNetworkEndpointRestoreUnavailable extends Data.TaggedError("Railway.PrivateNetworkEndpointRestoreUnavailable") {
    get message() {
...
const getEndpoint = (input) => railway
    .privateNetworkEndpoint({
    environmentId: input.environmentId,
    privateNetworkId: input.privateNetworkId,
    serviceId: input.serviceId,
}, endpointSelection)
    .pipe(Effect.map((endpoint) => endpoint == null || isGoneEndpoint(endpoint) ? undefined : endpoint), railway.catchTags(["RailwayNotFound"], () => Effect.succeed(undefined)));
const resolveServiceName = (serviceId, hint) => hint !== undefined && hint.length > 0
    ? Effect.succeed(hint)
    : railway.service({ id: serviceId }, { name: true }).pipe(Effect.map((service) => service.name), railway.catchTags(["RailwayNotFound"], () => Effect.succeed(sanitizeRailwayName(serviceId))));
export class PrivateNetworkEndpointNameUnavailable extends Data.TaggedError("Railway.PrivateNetworkEndpointNameUnavailable") {
}
...
export const PrivateNetworkEndpointProvider = () => Provider.succeed(PrivateNetworkEndpoint, {
    ...
    reconcile: Effect.fn(function* ({ news, output }) {
        ...
        if (network?.name !== PLATFORM_NETWORK_NAME) {
            return yield* new PrivateNetworkEndpointTargetMissing({
                message: "PrivateNetworkEndpoint requires the platform-managed Railway network; legacy named networks are no longer supported",
            });
        }
        const previousDnsPrefix = yield* withEnvironmentConfigLock(environmentId, Effect.gen(function* () {
            const config = yield* readNetworkConfig(environmentId);
            if (config.privateNetworkDisabled === true) {
                return yield* new PrivateNetworkEndpointTargetMissing({
                    message: "Enable private networking with PrivateNetwork before configuring an endpoint",
                });
            }
            ...
            const hasDesiredName = current !== undefined &&
        ...
        return {
            ...
            previousDnsPrefix: output?.previousDnsPrefix !== undefined
    ...
    delete: Effect.fn(function* ({ olds, output }) {
        ...
        yield* withEnvironmentConfigLock(output.environmentId, Effect.gen(function* () {
            const config = yield* readNetworkConfig(output.environmentId);
            const configuredPrefix = config.services?.[output.serviceId]?.networking
                ?.privateNetworkEndpoint;
            if ((configuredPrefix ?? null) !== managedPrefix ||
            ...
            yield* waitForNetworkConfig(output.environmentId, (observed) => (observed.services?.[output.serviceId]?.networking
