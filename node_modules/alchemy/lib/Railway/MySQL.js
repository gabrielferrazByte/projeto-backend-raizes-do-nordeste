...
export const DEFAULT_MYSQL_START_COMMAND = "docker-entrypoint.sh mysqld --innodb-use-native-aio=0 --disable-log-bin --performance_schema=0";
const resolveMySQLProps = (props) => Effect.gen(function* () {
...
const MySQLResource = Resource("Railway.MySQL");
/**
 ...
 * @resource
 */
export const MySQL = Object.assign((id, props) => MySQLResource(id, resolveMySQLProps(props)), MySQLResource);
/**
...
const isMysqlImage = (image) => image != null && /(^|\/)mysql([:-]|$)/i.test(image);
const mysqlStartCommand = (image) => isMysqlImage(image) ? DEFAULT_MYSQL_START_COMMAND : undefined;
...
const isTemplateValue = (value) => value !== undefined && value.includes("${{");
const privateConnectionUri = (input) => `mysql://${encodePart(input.user)}:${encodePart(input.password)}@${input.name}.railway.internal:${MYSQL_PORT}/${encodePart(input.database)}`;
const publicConnectionUri = (input) => `mysql://${encodePart(input.user)}:${encodePart(input.password)}@${input.domain}:${input.port}/${encodePart(input.database)}`;
const desiredVariables = (input) => {
    const vars = {
        ...
        [MYSQL_URL_SECRET]: "mysql://${{MYSQLUSER}}:${{MYSQLPASSWORD}}@${{RAILWAY_PRIVATE_DOMAIN}}:3306/${{MYSQLDATABASE}}",
        [MYSQL_PUBLIC_URL_SECRET]: "mysql://${{MYSQLUSER}}:${{MYSQLPASSWORD}}@${{RAILWAY_TCP_PROXY_DOMAIN}}:${{RAILWAY_TCP_PROXY_PORT}}/${{MYSQLDATABASE}}",
    };
    if (input.user !== "root") {
...
const waitForDeployment = (environmentId, serviceId) => getInstance(environmentId, serviceId).pipe(Effect.flatMap((instance) => {
    ...
    return Effect.fail(new MySQLDeployPending({
        serviceId,
...
const passwordFromVars = (vars, fallback) => {
...
export const MySQLProvider = () => Provider.succeed(MySQL, {
    ...
    reconcile: Effect.fn(function* ({ id, news, output }) {
        ...
        if (environmentId === undefined) {
            return yield* new MySQLProjectRequired({
                message: "MySQL requires a Railway environment (pass environment or a Project with environmentId)",
            });
        }
        ...
        const existingVars = current !== undefined
            ...
            : {};
        const user = existingVars.MYSQLUSER ??
            existingVars.MYSQL_USER ??
            props.user ??
            DEFAULT_MYSQL_USER;
        const database = existingVars.MYSQLDATABASE ??
            existingVars.MYSQL_DATABASE ??
            props.database ??
            DEFAULT_MYSQL_DATABASE;
        const password = passwordFromVars(existingVars) ??
