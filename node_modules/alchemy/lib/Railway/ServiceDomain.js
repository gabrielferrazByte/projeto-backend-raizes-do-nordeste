...
export const listServiceDomains = (projectId, environmentId, serviceId) => railway
    .domains({ environmentId, projectId, serviceId }, { serviceDomains: selection })
    .pipe(Effect.map((result) => result.serviceDomains.filter((domain) => !isGone(domain))), railway.catchTags(["RailwayNotFound"], () => Effect.succeed([])));
const findCloudDomainById = (input) => railway
    .domains({
    projectId: input.projectId,
    environmentId: input.environmentId,
    serviceId: input.serviceId,
}, { serviceDomains: selection })
    .pipe(Effect.map((result) => result.serviceDomains.find((candidate) => candidate.id === input.domainId)), railway.catchTags(["RailwayNotFound"], () => Effect.succeed(undefined)));
...
export const deleteOwnedServiceDomain = Effect.fn(function* (input) {
    if (input.domainId === undefined && input.domain === undefined)
        return;
    const live = yield* listServiceDomains(input.projectId, input.environmentId, input.serviceId);
    const owned = live.filter((row) => (input.domainId !== undefined && row.id === input.domainId) ||
    ...
    if (keys.size === 0)
        return;
    yield* withEnvironmentConfigLock(input.environmentId, railway.environmentPatchCommit({
        environmentId: input.environmentId,
    ...
    for (const row of owned) {
        if (row.syncStatus === "DELETING")
            continue;
        yield* withEnvironmentConfigLock(input.environmentId, railway.deleteServiceDomain({ id: row.id })).pipe(railway.catchTags(["RailwayNotFound"], () => Effect.void), Effect.asVoid);
    }
    yield* waitUntilDeleted("ServiceDomain", [...keys].join(","), listServiceDomains(input.projectId, input.environmentId, input.serviceId).pipe(Effect.map((rows) => !rows.some((row) => keys.has(row.id) ||
        (input.domain !== undefined && row.domain === input.domain)))), 10);
});
const listedOrUndefined = (input) => listServiceDomains(input.projectId, input.environmentId, input.serviceId).pipe(Effect.map((rows) => input.domainId === undefined
    ? rows[0]
    : input.domainId === null
        ? undefined
 ...
 */
const createViaEnvironmentPatch = (input) => Effect.gen(function* () {
 ...
 */
const waitForServiceDomainById = (input) => findCloudDomainById(input).pipe(Effect.repeat({
 ...
 * a second hostname.
 */
const waitForNewServiceDomain = (input) => listServiceDomains(input.projectId, input.environmentId, input.serviceId).pipe(Effect.repeat({
    schedule: Schedule.spaced("1 second"),
    until: (rows) => rows.some((domain) => domain.id === input.preferId ||
        !input.preexistingIds.has(domain.id)),
    times: 10,
}), Effect.map((rows) => rows.find((domain) => domain.id === input.preferId) ??
    rows.find((domain) => !input.preexistingIds.has(domain.id))));
const createViaMutation = (input) => {
    const listed = input.domainId === undefined
        ? listedOrUndefined(input)
        : waitForServiceDomainById({ ...input, domainId: input.domainId });
    const missing = () => new ServiceDomainNotCreated({
        serviceId: input.serviceId,
        environmentId: input.environmentId,
    });
    const listedOrFail = (error) => listed.pipe(Effect.flatMap((row) => row !== undefined ? Effect.succeed(row) : Effect.fail(error)));
    const create = railway
        ...
        .pipe(Effect.flatMap((created) => input.domainId === undefined
        ? listedOrFail(missing())
        : Effect.succeed(created)), railway.catchTags("RailwayServiceDomainCreateFailed", (_issue, error) => listedOrFail(error)), railway.catchTags("RailwayServiceInstanceNotFound", (_issue, error) => input.domainId !== undefined ? listedOrFail(error) : Effect.fail(error)), railway.catchTags("RailwayValidationError", (_issue, error) => listedOrFail(error)));
    return withEnvironmentConfigLock(input.environmentId, create).pipe(Effect.retry({
        while: (error) => (input.domainId === undefined &&
...
const syncDomain = (input) => {
    ...
    return withEnvironmentConfigLock(input.current.environmentId, railway.updateServiceDomain({
        input: {
...
export const ensureServiceDomain = Effect.fn(function* (input) {
    let current = yield* listedOrUndefined(input);
    let createdDomainId;
    if (current === undefined) {
        const preexisting = yield* listServiceDomains(input.projectId, input.environmentId, input.serviceId);
