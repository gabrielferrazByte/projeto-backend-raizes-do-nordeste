...
import { OwnedBySomeoneElse, Unowned } from "../AdoptPolicy.js";
 ...
 * @product Railway
 */
export const SandboxCheckpoint = Resource("Railway.SandboxCheckpoint");
const listCheckpoints = (environmentId) => railway
    .sandboxCheckpoints({ environmentId }, selection)
    .pipe(railway.catchTags("RailwayNotFound", () => Effect.succeed([])));
const toAttrs = (checkpoint, sandboxId) => ({
...
const sameCapture = (checkpoint, output) => checkpoint.id === output.sandboxCheckpointId &&
    checkpoint.createdAt === output.createdAt;
const findRecordedCheckpoint = Effect.fn(function* (id, props, output, items) {
    const byId = items.find((item) => item.id === output.sandboxCheckpointId);
    if (byId !== undefined)
        return byId;
    if (props?.sandbox?.environmentId !== output.environmentId ||
...
const conflict = (id, name) => new OwnedBySomeoneElse({
...
export const SandboxCheckpointProvider = () => Provider.succeed(SandboxCheckpoint, {
    ...
    diff: Effect.fn(function* ({ news, output }) {
        if (!isResolved(news) || output === undefined)
            return;
        if (news.sandbox.sandboxId !== output.sandboxId ||
            news.sandbox.environmentId !== output.environmentId) {
            return {
                action: "replace",
                deleteFirst: news.sandbox.environmentId === output.environmentId &&
    ...
    list: Effect.fn(function* () {
        ...
        const rows = yield* Effect.forEach(projects, (project) => Effect.gen(function* () {
            ...
            return (yield* Effect.forEach(environments, (environmentId) => listCheckpoints(environmentId).pipe(Effect.map((items) => items
                .filter((item) => matchesAlchemyPhysicalName(item.key))
                .map((item) => toAttrs(item)))))).flat();
        }));
        return rows.flat();
    }),
    reconcile: Effect.fn(function* ({ id, news, output }) {
        ...
        if (current === undefined) {
            current = yield* railway.createSandboxCheckpoint({ environmentId, sandboxId, name }, selection);
        }
        if (current.key !== name) {
            current = yield* railway.renameSandboxCheckpoint({ environmentId, id: current.id, name }, selection);
        }
    ...
    delete: Effect.fn(function* ({ id, olds, output }) {
        ...
        yield* waitUntilDeleted("SandboxCheckpoint", current.id, listCheckpoints(output.environmentId).pipe(Effect.map((rows) => !rows.some((item) => item.id === current.id &&
            item.createdAt === current.createdAt))), 10);
    }),
});
//# sourceMappingURL=SandboxCheckpoint.js.map