...
import { Credentials, DEFAULT_API_BASE_URL, } from "@distilled.cloud/planetscale/Credentials";
...
import { PLANETSCALE_AUTH_PROVIDER_NAME, } from "./AuthProvider.js";
 ...
 * ```
 */
export const fromToken = (input) => Layer.succeed(Credentials, Effect.succeed({
    tokenId: typeof input.tokenId === "string"
        ? Redacted.make(input.tokenId)
        : input.tokenId,
    token: typeof input.token === "string"
...
export const fromAuthProvider = () => Layer.effect(Credentials, Effect.gen(function* () {
    const apiBaseUrl = yield* Config.String("PLANETSCALE_API_BASE_URL").pipe(Config.withDefault(DEFAULT_API_BASE_URL));
    const { profileName, resolve } = yield* resolveProviderConfig(PLANETSCALE_AUTH_PROVIDER_NAME);
    ...
    const cached = yield* CredentialsCache.cacheUntilExpiry(resolve, (creds) => (creds.type === "oauth" ? creds.expires : undefined));
    return cached.pipe(Effect.map((creds) => creds.type === "oauth"
        ? {
            type: "oauth",
        ...
        message: `Failed to resolve Planetscale credentials from ${profileName === undefined ? "the CI environment" : `profile '${profileName}'`}: ${e.message ?? String(e)}`,
    })), Effect.orDie);
}));
