...
import { selectableScopes, tokenPolicies, } from "../../Cloudflare/Auth/TokenPolicy.js";
...
const withGlobalKey = (credentials) => Effect.provideService(CloudflareCredentials.Credentials, Effect.succeed(apiKeyCredentials({
...
function* (_credentials) {
    ...
    return {
        ...
        permissionGroups: permissionGroupResponse.result.flatMap((group) => group.id && group.name && group.scopes?.length
            ? [
                {
                    id: group.id,
                    name: group.name,
...
export const plan = Effect.fn("Alchemist.cloudflare.token.plan")(function* (input) {
    const tokenCatalog = yield* catalog(input.credentials);
    const selected = input.permissionGroupIds === "all"
        ...
        : yield* Effect.forEach(input.permissionGroupIds, (id) => {
            ...
            if (group === undefined) {
                return Effect.fail(new AlchemistInvalidInput({
...
export const create = Effect.fn("Alchemist.cloudflare.token.create")(function* (input) {
    const result = yield* user.createToken({
        name: input.plan.name,
        policies: input.plan.policies,
    });
    if (!result.value) {
        return yield* Effect.fail(new CloudflareTokenError({
            message: "Cloudflare did not return a token value.",
        }));
    }
    const granted = (result.policies ?? []).reduce((count, policy) => count + (policy.permissionGroups?.length ?? 0), 0);
    const verificationStatus = yield* user.verifyToken({}).pipe(Effect.provideService(CloudflareCredentials.Credentials, Effect.succeed(apiTokenCredentials({ apiToken: result.value }))), Effect.map(({ status }) => status), Effect.catch(() => Effect.succeed(undefined)));
    return {
...
//# sourceMappingURL=cloudflareToken.js.map