...
import { silentConsole } from "../../AWS/AuthProvider.js";
import { bootstrap as bootstrapAws, destroyBootstrap as destroyBootstrapAws, } from "../../AWS/Bootstrap.js";
import * as AWSCredentials from "../../AWS/Credentials.js";
import { AWSEnvironment } from "../../AWS/Environment.js";
import * as AWSRegion from "../../AWS/Region.js";
import { loadConfigProvider } from "../../Util/ConfigProvider.js";
const environment = Effect.fn(function* (target) {
    ...
    const credentialServices = yield* Effect.context();
    const aws = Layer.provideMerge(Layer.mergeAll(AWSRegion.fromEnvironment, AWSCredentials.fromEnvironment), Layer.succeed(AWSEnvironment, Effect.succeed({
        accountId: ssoProfile.sso_account_id,
        region,
        credentials: Auth.loadProfileCredentials(target.profile).pipe(Effect.provideService(EffectConsole.Console, silentConsole), Effect.provide(credentialServices)),
        profile: target.profile,
    })));
    return {
        accountId: ssoProfile.sso_account_id,
        region,
        layer: Layer.provide(aws, ConfigProvider.layer(yield* loadConfigProvider(Option.fromNullishOr(target.envFile)))),
    };
});
/** Provision the AWS deployment assets bucket. */
export const bootstrap = Effect.fn("Alchemist.provider.aws.bootstrap")(function* (target) {
...
export const teardown = Effect.fn("Alchemist.provider.aws.teardown")(function* (target) {
    const env = yield* environment(target);
