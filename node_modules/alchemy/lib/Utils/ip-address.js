 ...
 */
export const canonicalCidr = (cidr) => {
    if (cidr === undefined)
        return undefined;
    if (/\s/.test(cidr))
        return cidr;
    const [address, prefixText, extra] = cidr.split("/");
    if (!address || prefixText === undefined || extra !== undefined)
        return cidr;
...
const formatIpv4 = (address) => [24n, 16n, 8n, 0n]
    .map((shift) => Number((address >> shift) & 255n))
    .join(".");
const formatIpv6 = (address) => {
    const words = Array.from({ length: 8 }, (_, index) => ((address >> BigInt((7 - index) * 16)) & 0xffffn).toString(16));
    let longestStart = -1;
    let longestLength = 1;
