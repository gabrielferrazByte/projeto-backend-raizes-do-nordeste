...
import { decodeFunctionImageSource, makeFunctionImage, } from "./FunctionImage.js";
...
const accessPointArnOf = (config) => typeof config.accessPoint === "string"
    ...
    : typeof config.accessPoint
        ...
        : config.arn;
const isFunctionImageProps = (props) => props.image !== undefined;
/**
 * Validate package-mode exclusivity before any Lambda or ECR mutation.
 * @internal
 */
export const validateFunctionPackageProps = Effect.fn("AWS.Lambda.validateFunctionPackageProps")(function* (id, props) {
    if (props.image === undefined) {
        if (props.main === undefined) {
            return yield* Effect.fail(new Error(`Function(${id}): declare exactly one of main or image`));
        }
        return;
    }
    const incompatible = [
    ...
    if (incompatible.length > 0) {
        return yield* Effect.fail(new Error(`Function(${id}): image functions cannot use ZIP/runtime options: ${incompatible.join(", ")}`));
    }
});
/**
 ...
 */
const toLambdaImageConfig = (config) => config === undefined ||
 ...
 */
const imageConfigForUpdate = (props) => isFunctionImageProps(props)
...
export const FunctionProvider = () => Provider.effect(Function, Effect.gen(function* () {
    ...
    const waitForFunctionUpdate = Effect.fn(function* (functionName, session, vpc) {
        return yield* Effect.gen(function* () {
            ...
            if (configuration?.State === "Failed" ||
                ...
                return yield* new FunctionUpdateFailed({
                    functionName,
                    reason: configuration.LastUpdateStatusReason ??
            ...
            schedule: Schedule.spaced("2 seconds").pipe(Schedule.tap(({ attempt }) => session.note(`Waiting for Lambda function update: ${functionName} (${attempt * 2}s)`))),
    ...
    const prepareImageFunctionCode = Effect.fn(function* ({ id, props, previousImage, session, }) {
    ...
    const preparePrecreatedFunctionCode = Effect.fn(function* ({ id, props, session, }) {
    ...
    const prepareFunctionCode = Effect.fn(function* ({ id, props, previousImage, session, }) {
        if (isFunctionImageProps(props)) {
    ...
    const createOrUpdateFunction = Effect.fn(function* ({ id, news, roleArn, code, env, functionName, preferUpdate, fileSystemConfigs, vpc, session, }) {
        ...
        const isSecurityGroupPropagationError = (e) => e._tag === "InvalidParameterValueException" &&
            e.message?.includes("InvalidGroup.NotFound");
        const noteCreateDependencyWait = () => session.note(`Waiting for Lambda creation dependencies to propagate: ${functionName} (${Math.ceil((Date.now() - waitStartedAt) / 1000)}s)`);
        const tags = yield* createInternalTags(id);
        const codeLocation = yield* Effect.gen(function* () {
            if (code.packageType === "Image") {
                return { ImageUri: code.imageUri };
            }
    ...
    const summary = (code) => code.packageType === "Image"
        ? code.imageUri
    ...
    const refreshPersistedFunctionCode = ({ output, packageType, observed, }) => {
    ...
    return {
        ...
        reconcile: Effect.fn(function* ({ id, news, olds, bindings, output, session, }) {
            ...
            const nextImage = "image" in prepared.attributes
                ? prepared.attributes.image
                : undefined;
            if (previousImage?.ownsRepository === true &&
