...
import { Docker, DockerLive } from "../../Docker/Docker.js";
...
import { flociProvidersUrl, makeDevWatchProvider, } from "../Local/DevWatchProvider.js";
import { imageSourceTrigger } from "../Local/ImageSourceTrigger.js";
import { buildMicrovmDockerfile, bundleMicrovmProgram, DEFAULT_MICROVM_PORT, MICROVM_BASE_DOCKER_IMAGE, } from "./MicrovmBundle.js";
import { MicrovmImage } from "./MicrovmImage.js";
import { MicrovmImageProvider } from "./MicrovmProvider.js";
/**
 ...
 */
const LOCAL_MICROVM_BASE_IMAGE = process.env.FLOCI_MICROVM_BASE_IMAGE ||
    "public.ecr.aws/amazonlinux/amazonlinux:2023";
const rewriteBaseImage = (dockerfile) => dockerfile
    .split("\n")
    .map((line) => {
 ...
 */
const buildLocalImage = Effect.fn(function* (id, news) {
    ...
    if (news.main) {
        ...
        const userDockerfile = news.dockerfile !== undefined && isInlineDockerfile(news.dockerfile)
            ? news.dockerfile.content
            : news.dockerfile;
    ...
    if (news.context) {
        ...
        const dockerfilePath = news.dockerfile !== undefined && !isInlineDockerfile(news.dockerfile)
...
export const FlociMicrovmImageProvider = () => makeDevWatchProvider(MicrovmImage, flociProvidersUrl(), {
    ...
    replaceOn: ({ id: _id, olds, news }) => Effect.sync(() => (olds.name ?? null) !== (news.name ?? null)
        ? { action: "replace" }
        : undefined),
    ...
    transformReconcileNews: ({ id, news }) => Effect.gen(function* () {
    ...
    startWatch: (ctx) => Effect.gen(function* () {
        ...
        Stream.runForEach(() => Effect.gen(function* () {
            ...
            yield* Effect.logInfo(`[alchemy dev] ${ctx.id}: microvm source changed — rebuilding`);
            ...
            if (attrs.codeArtifact?.hash !== previous.codeArtifact?.hash) {
                yield* Effect.logInfo(`[alchemy dev] ${attrs.name}: microvm image rebuilt (v${attrs.latestActiveImageVersion}) in ${Date.now() - startedAt}ms`);
            }
        }).pipe(Effect.catchCause((cause) => Effect.logWarning(`[alchemy dev] ${ctx.id}: microvm image rebuild failed`, cause)))));
    }),
});
//# sourceMappingURL=FlociMicrovmImageProvider.js.map