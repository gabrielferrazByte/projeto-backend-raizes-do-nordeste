...
const toAttrs = ({ instance, tags, skipFinalSnapshot, finalDBSnapshotIdentifier, masterUserPasswordFingerprint, }) => ({
    ...
    dbParameterGroupApplyStatuses: Object.fromEntries((instance.DBParameterGroups ?? []).flatMap((group) => group.DBParameterGroupName
        ? [[group.DBParameterGroupName, group.ParameterApplyStatus]]
        : [])),
    vpcSecurityGroupIds: (instance.VpcSecurityGroups ?? []).flatMap((group) => group.VpcSecurityGroupId ? [group.VpcSecurityGroupId] : []),
...
const sameMembers = (desired, observed) => {
    const want = new Set(desired);
    const have = new Set(observed);
...
const resolveAssociations = Effect.fn(function* (props, observed) {
    ...
    if (props.engine.startsWith("db2-") &&
    ...
    if (!custom && dbParameterGroupName === undefined) {
        ...
        const families = new Set(pages.flatMap((page) => (page.DBEngineVersions ?? []).map((version) => version.DBParameterGroupFamily)));
        const family = [...families][0];
        if (families.size !== 1 || !family) {
            return yield* new InvalidDBInstanceAssociations({
                message: "No unambiguous DB parameter group family; declare engineVersion or dbParameterGroupName",
            });
        }
    ...
    if (vpcSecurityGroupIds === undefined) {
        const vpcId = props.dbSubnetGroupName !== undefined
        ...
        if (!vpcId) {
            return yield* new InvalidDBInstanceAssociations({
                message: "No VPC found for the DB instance; declare dbSubnetGroupName or provide a default VPC",
            });
        }
        ...
        vpcSecurityGroupIds = (groups.SecurityGroups ?? []).flatMap((group) => group.GroupId ? [group.GroupId] : []);
    ...
    if (vpcSecurityGroupIds.length === 0) {
        return yield* new InvalidDBInstanceAssociations({
            message: "At least one VPC security group is required; omit vpcSecurityGroupIds to select the VPC default group",
        });
    }
    return {
...
const parameterGroupMatches = (desired, instance) => desired.dbParameterGroupName === undefined ||
    sameMembers([desired.dbParameterGroupName], (instance.DBParameterGroups ?? []).map((group) => group.DBParameterGroupName));
const securityGroupsMatch = (desired, instance) => desired.vpcSecurityGroupIds === undefined ||
    sameMembers(desired.vpcSecurityGroupIds, (instance.VpcSecurityGroups ?? []).map((group) => group.VpcSecurityGroupId));
const associationsConverged = (desired, instance) => parameterGroupMatches(desired, instance) &&
    (desired.dbParameterGroupName === undefined ||
        (instance.DBParameterGroups ?? []).every((group) => group.ParameterApplyStatus === "in-sync" ||
...
const desiredInstancePort = Effect.fn(function* (props, observed) {
    ...
    if (props.port !== undefined) {
        if (!Number.isInteger(props.port) ||
            props.port < 1150 ||
            props.port > 65535) {
    ...
    if (props.engine === "postgres")
        return 5432;
    if (props.engine === "mysql" || props.engine === "mariadb")
        return 3306;
    if (props.engine.startsWith("oracle-") ||
        props.engine.startsWith("custom-oracle-"))
        return 1521;
    if (props.engine.startsWith("sqlserver-") ||
...
const portConverged = (instance, port) => port === undefined ||
...
const resolveStorage = Effect.fn(function* (props, allocatedFloor = 0) {
    ...
    if (!["standard", "gp2", "gp3", "io1", "io2"].includes(storageType) ||
    ...
    const allocation = requested > allocatedFloor && allocatedFloor > 0
    ...
    if (storageType !== "gp3") {
        if (props.storageThroughput !== undefined ||
    ...
    const minimumIops = Math.max(baselineIops, sqlServer ? Math.ceil(ratioCapacity * 0.5) : 0, (props.storageThroughput ?? baselineThroughput) * 4);
    const iops = props.iops ?? minimumIops;
    const minimumThroughput = Math.max(baselineThroughput, ["mysql", "mariadb"].includes(props.engine) ? Math.ceil(iops / 64) : 0);
...
const hasPendingStorage = (instance) => instance.PendingModifiedValues?.AllocatedStorage !== undefined ||
...
const storageConverged = (observed, desired) => desired === undefined ||
...
const toStorageConfiguration = Effect.fn(function* (props) {
    const supportsAutoscaling = props.dbClusterIdentifier === undefined &&
        !props.engine.startsWith("aurora") &&
    ...
    const maxAllocatedStorage = supportsAutoscaling
        ...
        : undefined;
    if (maxAllocatedStorage !== undefined &&
    ...
    return {
        allocatedStorage: props.dbClusterIdentifier === undefined
            ? props.allocatedStorage
            : undefined,
        maxAllocatedStorage,
    };
});
const allocationConverged = (desired, observed) => desired.allocatedStorage === undefined ||
    (observed.AllocatedStorage !== undefined &&
        observed.AllocatedStorage >= desired.allocatedStorage);
const autoscalingConverged = (desired, observed) => {
    ...
    if (ceiling === 0 || ceiling === observed.AllocatedStorage) {
        return ((observed.MaxAllocatedStorage ?? 0) === 0 ||
...
export const DBInstanceProvider = () => Provider.effect(DBInstance, Effect.gen(function* () {
    ...
    const waitForAssociations = Effect.fn(function* (instanceId, desired) {
        const instance = yield* readInstance(instanceId).pipe(Effect.flatMap((instance) => instance?.DBParameterGroups?.some((group) => group.ParameterApplyStatus === "failed-to-apply")
            ? Effect.fail(new InvalidDBInstanceAssociations({
                message: `DB instance '${instanceId}' parameter group failed to apply`,
            }))
            : Effect.succeed(instance)), Effect.repeat({
            schedule: Schedule.min([
                Schedule.exponential("5 seconds"),
                Schedule.spaced("1 minute"),
            ]),
            times: 10,
            until: (instance) => instance !== undefined &&
    ...
    const waitForPort = Effect.fn(function* (instanceId, port) {
        const instance = yield* readInstance(instanceId).pipe(Effect.repeat({
            schedule: Schedule.min([
                Schedule.exponential("5 seconds"),
                Schedule.spaced("1 minute"),
            ]),
    ...
    const waitForStorage = Effect.fn(function* (instanceId, converged) {
    ...
    return {
        ...
        diff: Effect.fn(function* ({ id, olds, news, output }) {
            ...
            if (output !== undefined) {
                ...
                if (!storageConverged(instance, desiredStorage) ||
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            if (!observed?.DBInstanceArn) {
                created = yield* rds
                    ...
                    .pipe(Effect.as(true), Effect.catchTag("DBInstanceAlreadyExistsFault", () => Effect.succeed(false)));
            ...
            if (!created) {
                ...
                if (hasPendingStorage(observed)) {
                    observed = yield* waitForStorage(identifier, (instance) => !hasPendingStorage(instance));
                }
                ...
                if (!securityGroupsMatch(associations, observed)) {
                    if (news.engine.startsWith("custom-")) {
                        return yield* new InvalidDBInstanceAssociations({
                            message: "Changing existing RDS Custom security-group associations through ModifyDBInstance is not supported",
                        });
                    }
                ...
                if (coreDirty) {
                    ...
                    observed = yield* waitForInstance(identifier);
                    if (core.DBParameterGroupName !== undefined ||
                    ...
                    if (core.AllocatedStorage !== undefined) {
                        observed = yield* waitForStorage(identifier, (instance) => storageConverged(instance, desiredStorage));
                    }
                }
            ...
            if (desiredStorage && !storageConverged(observed, desiredStorage)) {
                yield* rds.modifyDBInstance({
                    DBInstanceIdentifier: identifier,
                    ...desiredStorage.request,
                    MaxAllocatedStorage: storage.maxAllocatedStorage === 0
                        ? desiredStorage.request.AllocatedStorage
                        : storage.maxAllocatedStorage,
                    ApplyImmediately: true,
                });
            ...
            if (!autoscalingConverged(storage, observed)) {
                const ceiling = storage.maxAllocatedStorage === 0
                ...
                observed = yield* waitForStorage(identifier, (instance) => allocationConverged(storage, instance) &&
                    autoscalingConverged(storage, instance));
            }
            ...
            if (port !== undefined && !portConverged(observed, port)) {
                if (observed.PendingModifiedValues?.Port !== port) {
                    if (news.engine.startsWith("custom-")) {
                        return yield* new InvalidDBInstancePort({
                            message: "Changing an existing RDS Custom listener through ModifyDBInstance is not supported",
                        });
                    }
        ...
        delete: Effect.fn(function* ({ output }) {
            ...
            const finalDBSnapshotIdentifier = skipFinalSnapshot
                ...
                : (output.finalDBSnapshotIdentifier ??
                    (yield* Effect.sync(() => `${output.dbInstanceIdentifier}-final-${new Date()
                        .toISOString()
                        .replaceAll(/[:.]/g, "-")
                        .toLowerCase()}`)));
            yield* rds
