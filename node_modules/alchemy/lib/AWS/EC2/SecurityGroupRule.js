...
import { canonicalCidr } from "../../Utils/ip-address.js";
...
export const SecurityGroupRuleProvider = () => Provider.effect(SecurityGroupRule, Effect.gen(function* () {
    ...
    const tagsMatch = (rule, tags) => {
        const { removed, upsert } = diffTags(Object.fromEntries((rule.Tags ?? []).map((tag) => [tag.Key, tag.Value])), tags);
    ...
    return {
        ...
        read: Effect.fn(function* ({ output }) {
            if (!output)
                return undefined;
            return yield* describeRule(output.securityGroupRuleId).pipe(Effect.map(toAttrs), Effect.catchTag("InvalidSecurityGroupRuleId.NotFound", () => Effect.succeed(undefined)));
        }),
        ...
        diff: Effect.fn(function* ({ id, news, olds, output }) {
            ...
            if (output) {
                const observed = yield* describeRule(output.securityGroupRuleId).pipe(Effect.catchTag("InvalidSecurityGroupRuleId.NotFound", () => Effect.succeed(undefined)));
                if (!observed) {
                    return { action: "update", stables: ["groupOwnerId"] };
                }
                if (observedSecurityGroupRuleKey(observed) !==
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            const matches = (rule) => observedSecurityGroupRuleKey(rule) === desiredKey &&
                tagsMatch(rule, desiredTags);
            const final = yield* describeRule(ruleId).pipe(Effect.repeat({
                until: matches,
        ...
        delete: Effect.fn(function* ({ olds, output, session }) {
            ...
            if (isEgress) {
                ...
                ], () => Effect.void), Effect.catchTag("InvalidGroup.NotFound", () => Effect.void));
            }
            else {
                yield* ec2
...
export const securityGroupRuleKey = (rule) => {
...
export const observedSecurityGroupRuleKey = (rule) => securityGroupRuleKey({
    ipProtocol: rule.IpProtocol,
    fromPort: rule.FromPort,
    toPort: rule.ToPort,
    cidrIpv4: rule.CidrIpv4,
...
export const expandSecurityGroupRules = (rules) => rules.flatMap(({ cidrIpv4, cidrIpv6, referencedGroupId, prefixListId, ...rule }) => [
