...
import { State, isActionState } from "../../State/State.js";
...
import { expandSecurityGroupRules, observedSecurityGroupRuleKey, securityGroupRuleKey, } from "./SecurityGroupRule.js";
...
export const SecurityGroupProvider = () => Provider.effect(SecurityGroup, Effect.gen(function* () {
    ...
    const findOwnedGroup = Effect.fn(function* (id, groupName, vpcId) {
    ...
    const declaredRuleIds = Effect.fn(function* (groupId) {
    ...
    const desiredEgress = (props) => props.egress ?? [{ ipProtocol: "-1", cidrIpv4: "0.0.0.0/0" }];
    const rulesMatch = (observed, desired) => JSON.stringify(observed.map(observedSecurityGroupRuleKey).sort()) ===
    ...
    const syncRules = Effect.fn(function* (groupId, isEgress, desired, observed, session) {
        if (desired.some((rule) => [
            rule.cidrIpv4,
            rule.cidrIpv6,
        ...
        if (rulesMatch(observed, desired))
            return;
        const current = [];
    ...
    return {
        ...
        read: Effect.fn(function* ({ id, olds, output }) {
            const sg = output
                ? yield* describeSecurityGroup(output.groupId).pipe(Effect.catchTag("InvalidGroup.NotFound", () => Effect.succeed(undefined)))
        ...
        diff: Effect.fn(function* ({ id, news, olds, output }) {
            ...
            if (output) {
                const group = yield* describeSecurityGroup(output.groupId).pipe(Effect.catchTag("InvalidGroup.NotFound", () => Effect.succeed(undefined)));
                if (group === undefined)
                    return { action: "update", stables: [] };
                const owned = yield* declaredRuleIds(output.groupId);
                const observed = (yield* describeSecurityGroupRules(output.groupId)).filter((rule) => !owned.has(rule.SecurityGroupRuleId));
                if (!rulesMatch(observed.filter((rule) => !rule.IsEgress), news.ingress ?? []) ||
                    !rulesMatch(observed.filter((rule) => rule.IsEgress), desiredEgress(news))) {
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            const currentIngress = currentRules.filter((rule) => !rule.IsEgress && !owned.has(rule.SecurityGroupRuleId));
            const currentEgress = currentRules.filter((rule) => rule.IsEgress && !owned.has(rule.SecurityGroupRuleId));
            ...
            const matches = (rules) => {
                ...
                return (rulesMatch(inline.filter((rule) => !rule.IsEgress), news.ingress ?? []) &&
                    rulesMatch(inline.filter((rule) => rule.IsEgress), desiredEgress(news)));
            };
            const finalRules = yield* describeSecurityGroupRules(groupId).pipe(Effect.repeat({
                until: matches,
