 ...
 */
export const findPublicHostedZoneId = Effect.fn("findPublicHostedZoneId")(function* (domainName) {
 ...
 */
export const resolveHostedZoneId = Effect.fn("resolveHostedZoneId")(function* (hostedZoneId, domainName) {
    if (hostedZoneId !== undefined) {
        return hostedZoneId;
    }
    const inferred = yield* findPublicHostedZoneId(domainName);
    if (inferred === undefined) {
        return yield* Effect.fail(new Error(`No public Route 53 hosted zone in this account contains "${domainName}" — ` +
            `create the zone first or pass "hostedZoneId" explicitly.`));
    }
    return inferred;
});
//# sourceMappingURL=HostedZoneLookup.js.map