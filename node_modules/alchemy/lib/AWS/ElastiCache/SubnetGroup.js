 ...
 * @resource
 */
export const SubnetGroup = Resource("AWS.ElastiCache.SubnetGroup");
export const SubnetGroupProvider = () => Provider.effect(SubnetGroup, Effect.gen(function* () {
    ...
    const attrs = Effect.fn(function* (group) {
        if (!group.CacheSubnetGroupName || !group.ARN) {
            return yield* Effect.fail(new Error("ElastiCache subnet group is missing its name or ARN"));
        }
        return {
    ...
    return {
        ...
        diff: Effect.fn(function* ({ id, olds, news }) {
            if (!isResolved(news))
                return undefined;
            if ((yield* toName(id, olds ?? { description: "", subnetIds: [] })) !==
                (yield* toName(id, news ?? { description: "", subnetIds: [] })))
                return { action: "replace" };
        }),
        read: Effect.fn(function* ({ id, olds, output }) {
            const group = yield* readGroup(output?.subnetGroupName ??
        ...
        reconcile: Effect.fn(function* ({ id, news, output, session }) {
            ...
            const observedSubnets = (group.Subnets ?? [])
                .map((subnet) => subnet.SubnetIdentifier)
                .filter((id) => id !== undefined);
            if (group.CacheSubnetGroupDescription !== props.description ||
            ...
            if (group.ARN) {
                const { removed, upsert } = diffTags(yield* readElastiCacheTags(group.ARN), desiredTags);
                if (upsert.length)
        ...
        delete: Effect.fn(function* ({ output }) {
            yield* elasticache
                ...
                .pipe(Effect.catchTag("CacheSubnetGroupNotFoundFault", () => Effect.void), Effect.retry({
        ...
        list: () => elasticache.describeCacheSubnetGroups.pages({}).pipe(Stream.runCollect, Effect.map((pages) => Array.from(pages).flatMap((page) => page.CacheSubnetGroups ?? [])), Effect.flatMap((groups) => Effect.forEach(groups.filter((group) => group.ARN), attrs))),
    };
}));
