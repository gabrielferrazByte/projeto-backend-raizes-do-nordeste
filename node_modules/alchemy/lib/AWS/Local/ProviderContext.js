 ...
 */
export const withProviderContext = (provider, services) => new Proxy(provider, {
    get: (target, prop) => {
        ...
        if (prop === "modes" && value !== undefined) {
            ...
            return Object.fromEntries(Object.entries(value).map(([mode, build]) => [
                mode,
                Effect.map(build, (resolved) => withProviderContext(resolved, services)),
            ]));
        }
 ...
 */
export const captureAwsEnvironment = Effect.gen(function* () {
    let ctx = Context.empty();
    const endpoint = yield* Effect.serviceOption(Endpoint.Endpoint);
    ctx = Context.add(ctx, Endpoint.Endpoint, Option.getOrElse(endpoint, () => Effect.succeed(undefined)));
    const region = yield* Effect.serviceOption(Region);
    if (Option.isSome(region))
        ctx = Context.add(ctx, Region, region.value);
 ...
 * provided closer.
 */
export const pinCollectionEnvironment = (collection, environment) => {
    const wrap = (provider) => {
    ...
    return {
        ...collection,
        get: (service) => wrapped[service] ?? wrap(collection.get(service)),
        providers: wrapped,
    };
};
const isProviderService = (value) => Predicate.hasProperty(value, "reconcile") &&
 ...
 */
export const provideProviderContext = (providerLayer, services) => Layer.fromBuildMemo((memoMap, scope) => Effect.gen(function* () {
    ...
    const built = yield* Layer.buildWithMemoMap(providerLayer.pipe(
    // Closest wins: overrides first, then the ambient build context.
    Layer.provide(servicesLayer), Layer.provide(Layer.succeedContext(ambient))), memoMap, scope);
    const wrapped = new Map();
    for (const [key, value] of built.mapUnsafe) {
        wrapped.set(key, isProviderService(value)
            ? withProviderContext(value, servicesLayer)
            : value);
    }
    return Context.makeUnsafe(wrapped);
}));
