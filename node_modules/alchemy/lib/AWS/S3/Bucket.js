...
export const BucketProvider = () => Provider.effect(Bucket, Effect.gen(function* () {
    ...
    const ensureBucketExists = Effect.fn(function* ({ id, news = {}, }) {
        ...
        if (region === "us-east-1") {
            const exists = yield* s3
                ...
                .pipe(Effect.map(() => true), Effect.catchTag("NoSuchBucket", () => Effect.succeed(false)));
    ...
    const fetchBucketTags = (bucketName) => s3.getBucketTagging({ Bucket: bucketName }).pipe(Effect.map((r) => Object.fromEntries((r.TagSet ?? []).map((t) => [t.Key, t.Value]))), Effect.catchTag("NoSuchTagSet", () => Effect.succeed({})));
    ...
    return {
        ...
        diff: Effect.fn(function* ({ id, news = {}, olds = {}, output }) {
            ...
            if (output) {
                const observed = yield* readBucketEncryption(output.bucketName).pipe(Effect.catchTag("NoSuchBucket", () => Effect.succeed(undefined)));
                if (encryptionFingerprint(observed) !==
...
const desiredEncryptionRule = (encryption) => {
...
const encryptionFingerprint = (rule) => {
    ...
    return JSON.stringify({
        ...
        blocked: [
            ...new Set(rule?.BlockedEncryptionTypes?.EncryptionType?.filter((type) => type !== "NONE") ?? []),
        ].sort(),
    });
};
const readBucketEncryption = (bucket) => s3
    .getBucketEncryption({ Bucket: bucket })
    .pipe(Effect.map((result) => result.ServerSideEncryptionConfiguration?.Rules?.[0]));
export const syncBucketEncryption = Effect.fn(function* (bucket, encryption) {
    const desired = desiredEncryptionRule(encryption);
    const matches = (rule) => encryptionFingerprint(rule) === encryptionFingerprint(desired);
    if (matches(yield* readBucketEncryption(bucket)))
        return false;
    yield* s3.putBucketEncryption({
        Bucket: bucket,
        ServerSideEncryptionConfiguration: { Rules: [desired] },
    });
    const observed = yield* readBucketEncryption(bucket).pipe(Effect.repeat({
        until: matches,
