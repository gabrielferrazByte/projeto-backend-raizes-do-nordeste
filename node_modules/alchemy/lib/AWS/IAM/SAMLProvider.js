...
const isEmptyUpdateError = (error) => error._tag === "ValidationError" &&
    (error.message?.includes("No updates are defined") ?? false);
const isTransientWriteError = (error) => error._tag === "ConcurrentModificationException" ||
        ...
        if (!observed) {
            const created = yield* iam
                ...
                if (!hasTags(internalTags, existingTags.Tags)) {
                    return yield* Effect.fail(new Error(`SAML provider '${name}' already exists and is not managed by alchemy`));
                }
            ...
            // reconcile).
            const metadataChanged = (observed.SAMLMetadataDocument ?? undefined) !==
                news.samlMetadataDocument;
            const encryptionChanged = news.assertionEncryptionMode !== undefined &&
