    ...
    return {
        ...
        delete: withKvsRegionFn(Effect.fn(function* ({ output }) {
            yield* retryForKvsReadiness(deleteOp({
                ...
                key: output.key,
                entry: output.entry,
            }).pipe(Effect.catchTag("ConflictException", (error) => 
            // The data plane reports ConflictException for deleted stores.
            // Confirm absence through the control plane before ignoring it.
            cloudfront.listKeyValueStores.pages({}).pipe(Stream.flatMap((page) => Stream.fromIterable(page.KeyValueStoreList?.Items ?? [])), Stream.filter((store) => store.ARN === output.store), Stream.runHead, Effect.flatMap((store) => Option.isNone(store) ? Effect.void : Effect.fail(error)))))).pipe(Effect.catchTag("ResourceNotFoundException", () => Effect.void));
        })),
    };
}));
