...
export const FlociTaskProvider = () => makeDevWatchProvider(Task, flociProvidersUrl(), {
    ...
    onReconciled: ({ id, previous, attrs }) => Effect.gen(function* () {
        if (previous?.taskDefinitionArn === attrs.taskDefinitionArn)
            return;
        ...
        yield* Effect.logInfo(`[alchemy dev] ${attrs.taskFamily}: task definition swapped (${restarted} task(s) restarted) in ${Date.now() - startedAt}ms`);
    }).pipe(Effect.catchCause((cause) => Effect.logWarning(`[alchemy dev] ${id}: task restart failed`, cause))),
