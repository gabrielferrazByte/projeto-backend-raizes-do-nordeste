...
const clusterArnOfProps = (cluster) => typeof cluster === "string"
    ? cluster
    : typeof cluster?.clusterArn ===
        "string"
        ? cluster.clusterArn
        : undefined;
/** The truly-immutable post-create fields (mirrors the live diff). */
const immutableFieldsOf = (props) => ({
...
const taskRefOf = (props) => "task" in props ? props.task : undefined;
export const FlociServiceProvider = () => makeDevWatchProvider(Service, flociProvidersUrl(), {
    ...
    replaceOn: ({ olds, news }) => Effect.sync(() => {
    ...
    onReconciled: ({ id, previous, attrs }) => Effect.gen(function* () {
        if (attrs.taskFamily === undefined ||
        ...
        yield* Effect.logInfo(`[alchemy dev] ${attrs.serviceName}: task definition swapped (${restarted} service task(s) rolling) in ${Date.now() - startedAt}ms`);
    }).pipe(Effect.catchCause((cause) => Effect.logWarning(`[alchemy dev] ${id}: service roll failed`, cause))),
    startWatch: (ctx) => Effect.gen(function* () {
        ...
        Stream.runForEach(() => ctx.rerunReconcile.pipe(Effect.catchCause((cause) => Effect.logWarning(`[alchemy dev] ${ctx.id}: image swap failed`, cause)))));
    }),
});
//# sourceMappingURL=FlociServiceProvider.js.map