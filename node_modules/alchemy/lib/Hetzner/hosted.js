...
import { contextRootOf, extraFileDestination, hashExtraFiles, isContextRootDest, posixRelUnder, resolveExtraSource, } from "../Util/extraFiles.js";
...
const skipExtraPath = (relative) => relative
    .replaceAll("\\", "/")
    .split("/")
    .some((segment) => segment === "node_modules" ||
    segment === ".git" ||
    segment.startsWith(".alchemy-hetzner"));
const toSharedExtraFiles = (extraFiles) => extraFiles?.map((file) => ({
    source: file.source,
    dest: file.destination,
}));
const collectExtraFiles = Effect.fn(function* (extraFiles) {
...
export const createHetznerHostedSupport = ({ stackName, stage, virtualEntryPlugin, }) => {
    ...
    const bundleProgram = Effect.fn(function* (_id, props) {
        ...
        const install = identity !== undefined && Object.keys(identity.resolved).length > 0
            ? identity.resolved
            : undefined;
        const packageJson = install === undefined
            ? undefined
            : `${JSON.stringify({ private: true, type: "module", dependencies: install }, null, 2)}\n`;
        const installFiles = packageJson !== undefined
            ? [
                {
                    path: "package.json",
        ...
        if (props.isExternal) {
            ...
            const root = contextRootOf(realMain, extras, pathMod, (source) => resolveExtraSource(source, pathMod));
            const entryRel = posixRelUnder(root, realMain, pathMod) ?? pathMod.basename(realMain);
            const entryBytes = yield* fs.readFile(realMain);
            const zipEntries = [
                ...extraFiles.filter((file) => !(file.path === "package.json" && packageJson !== undefined)),
                ...installFiles,
            ];
        ...
        const bundleOutput = yield* buildBundle(realMain, virtualEntryPlugin(makeBunBootstrap(handler)));
    ...
    const renderUnit = (unitName, appDir, entryRel) => `[Unit]
...
`;
    ...
    const hetznerTransient = (e) => e._tag === "TooManyRequests" ||
    ...
    const attachAndMount = Effect.fn(function* (input) {
        for (const { volumeId, path } of input.volumes) {
            let volume = yield* Services.volumes.getVolume({ id: volumeId }).pipe(Effect.map(({ volume }) => volume), Effect.retry({
            ...
            if (volume.server !== input.serverId) {
                ...
                }), Effect.tap(({ action }) => waitForAction(action).pipe(Effect.catchTag("ActionTimeout", () => Effect.void))), Effect.catchTag(["UnprocessableEntity", "Locked", "Conflict"], () => Effect.void));
    ...
    const deployUnit = Effect.fn(function* (input) {
        ...
        yield* input.ssh.scp(new TextEncoder().encode(renderUnit(input.unitName, appDir, input.entryRel ?? "index.mjs")), `/etc/systemd/system/${input.unitName}.service`);
        yield* input.ssh.exec([
            `set -euo pipefail`,
            `export HOME=/root`,
