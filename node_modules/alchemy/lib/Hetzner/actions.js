...
const retryable = (e) => e._tag === "ActionPending" ||
 ...
 */
export const waitForAction = (ref) => Effect.gen(function* () {
    ...
    return yield* Services.actions.getAction({ id }).pipe(Effect.flatMap(({ action }) => Effect.gen(function* () {
        ...
        times: 24,
        schedule: backoff,
    }), Effect.catchTag("ActionPending", (e) => new ActionTimeout({
        actionId: e.actionId,
        status: e.status,
    })));
});
 ...
 */
export const waitForActions = (refs) => Effect.forEach(refs, waitForAction, { concurrency: 1 });
/**
 ...
 * immediately.
 */
export const waitForZoneAction = (ref) => Effect.gen(function* () {
    if (typeof ref !== "number" && "status" in ref) {
        yield* failIfError(ref);
        if (isSuccessStatus(ref.status)) {
            return ref;
    ...
    const id = actionIdOf(ref);
    return yield* Services.zoneActions.getZonesAction({ id }).pipe(Effect.flatMap(({ action }) => Effect.gen(function* () {
        yield* failIfError(action);
        if (!isSuccessStatus(action.status)) {
            return yield* new ActionPending({
                actionId: action.id,
