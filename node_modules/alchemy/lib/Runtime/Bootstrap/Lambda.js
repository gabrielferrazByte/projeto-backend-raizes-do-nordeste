...
import { registerLambdaExtension } from "../../AWS/Lambda/RuntimeExtension.js";
import { reifyBoundConfigProvider } from "../../Runtime.js";
import { entrypointLayer, entrypointTag, stackFromEnv } from "./Process.js";
/**
 ...
 * dispatch.
 */
export const bootstrap = async (entrypoint) => {
    await registerLambdaExtension();
    const instanceScope = Scope.makeUnsafe();
    const platform = Layer.mergeAll(nodeServicesLayer, fetchHttpClientLayer, 
    // TODO(sam): wire this up to telemetry more directly
    Logger.layer([Logger.consolePretty()]));
    const entryLayer = entrypointLayer(entrypoint).pipe(Layer.provideMerge(stackFromEnv), Layer.provideMerge(Credentials.fromEnv()), Layer.provideMerge(Region.fromEnv()), 
    ...
    Layer.provideMerge(Endpoint.fromEnv()), Layer.provideMerge(platform), Layer.provideMerge(Layer.succeed(ConfigProvider.ConfigProvider, 
    ...
    reifyBoundConfigProvider(ConfigProvider.fromEnv(), process.env))), Layer.provideMerge(Layer.succeed(MinimumLogLevel, process.env.DEBUG ? "Debug" : "Info")));
    const handlerEffect = Layer.buildWithScope(entryLayer, instanceScope).pipe(Effect.flatMap((context) => entrypointTag.pipe(Effect.flatMap((func) => func.RuntimeContext.exports), Effect.flatMap((exports) => exports.handler), Effect.provideContext(context))), Scope.provide(instanceScope));
