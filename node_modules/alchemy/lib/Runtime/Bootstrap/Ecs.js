...
import { entrypointLayer, resolveProgram, runProcess, stackFromEnv, } from "./Process.js";
/**
 ...
 */
export const bootstrap = (entrypoint) => {
    const platform = Layer.mergeAll(BunServices.layer, FetchHttpClient.layer, Logger.layer([Logger.consolePretty()]));
    const program = resolveProgram("program", { telemetry: true }).pipe(Effect.provide(entrypointLayer(entrypoint).pipe(Layer.provideMerge(stackFromEnv), 
    ...
    Layer.provideMerge(Credentials.fromChain()), Layer.provideMerge(Region.fromEnv()), 
    ...
    // unaffected.
    Layer.provideMerge(Endpoint.fromEnv()), Layer.provideMerge(BunHttpServer()), Layer.provideMerge(platform), Layer.provideMerge(Layer.succeed(ConfigProvider.ConfigProvider, reifyBoundConfigProvider(ConfigProvider.fromEnv(), process.env))))), Effect.scoped);
