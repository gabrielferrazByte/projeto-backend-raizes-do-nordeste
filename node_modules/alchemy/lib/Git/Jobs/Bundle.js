 ...
 */
import { BlobStoreError } from "../BlobStore.js";
import { RuntimeContext } from "../../RuntimeContext.js";
import * as Effect from "effect/Effect";
import * as Stream from "effect/Stream";
import { encodeTypeSize, makeSha1, } from "../Protocol/ObjectCodec.js";
import { StoreError } from "../Protocol/Store.js";
import { bundleKey, bundleSidebandKey } from "../Store/Keys.js";
import { sidebandFramedLength, sidebandRechunk } from "../Protocol/Sideband.js";
/**
 ...
 * buffering.
 */
export const packByteLength = (entries) => {
    let total = 12 + 20;
    for (const entry of entries) {
        total +=
            encodeTypeSize(entry.type, entry.size).length +
                entry.zsize;
    }
    return total;
};
 ...
 */
export const runBundleJob = (options) => Effect.gen(function* () {
    ...
    yield* options.blobs
        ...
        .pipe(Stream.mapError((error) => new BlobStoreError({ reason: error.reason }))), { contentLength: size })
        .pipe(Effect.mapError((error) => new StoreError({ reason: `blob put ${key}: ${error.reason}` })), Effect.provide(RuntimeContext.phantom));
    ...
    yield* options.blobs
        .put(sidebandKey, options.packStream(options.entries).pipe(Stream.mapError((error) => new BlobStoreError({ reason: error.reason })), sidebandRechunk(1)), { contentLength: sidebandFramedLength(size) })
 ...
 */
export const bundleCovers = (bundle, request) => {
 ...
 */
export const bundleUncovered = (bundle, request) => {
