...
import { DEFAULT_LIST_LIMIT, RegistryStore, RESERVED_OWNERS, decodeCursor, encodeCursor, toEntry, ulid, } from "./RegistryObject.js";
import { REGISTRY_DDL } from "./Store/Sql.js";
/**
 ...
 */
const d1 = (what, effect) => Effect.catchCause(effect, (cause) => Effect.fail(new StoreError({ reason: `registry ${what}: ${cause}` })));
/**
 ...
 */
export const RegistryD1 = (database) => Layer.effect(RegistryStore, Effect.gen(function* () {
    const db = yield* Cloudflare.D1.QueryDatabase(database);
    // The DDL is shared with the DO (Store/Sql.ts) — one definition, so
    // the backends can never drift apart.
    const ready = Effect.suspend(() => Effect.forEach(REGISTRY_DDL, (statement) => d1("schema", db.prepare(statement).run()))).pipe(Effect.asVoid, Effect.cached);
    ...
    const shape = {
        createRepo: Effect.fn(function* (input) {
            ...
            const inserted = yield* d1("createRepo", db
                .prepare(`INSERT INTO repos
                ...
                .bind(owner, name, repoId, input.description ?? null, input.public === true ? 1 : 0, input.forkOf ?? null, createdAt, owner, name)
                .run());
        ...
        list: Effect.fn(function* (input) {
            ...
            const page = yield* d1("list", db
                .prepare(`SELECT * FROM repos WHERE ${conditions.join(" AND ")}
        ...
        bumpForkCount: Effect.fn(function* (repoId, delta) {
            ...
            const row = yield* d1("forkCount", db
        ...
        updateSummary: Effect.fn(function* (repoId, summary) {
            yield* schema;
            yield* d1("updateSummary", db
                .prepare(`UPDATE repos SET default_branch = ?, read_only = ?, is_public = ?, status = ?
                  WHERE repo_id = ?`)
                .bind(summary.defaultBranch, summary.readOnly ? 1 : 0, summary.public ? 1 : 0, summary.status, repoId)
                .run());
        }),
        removeRow: Effect.fn(function* (repoId) {
            yield* schema;
            const row = yield* d1("removeRow read", db
            ...
            const statements = [
                ...
                ...(row.fork_of !== null
                    ? [
                        db
                            .prepare(`UPDATE repos SET fork_count = MAX(0, fork_count - 1) WHERE repo_id = ?`)
                            .bind(row.fork_of),
                    ]
                    : []),
            ];
...
//# sourceMappingURL=RegistryD1.js.map