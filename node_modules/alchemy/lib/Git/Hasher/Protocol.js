...
export class HashError extends Schema.TaggedError()("HashError", {
    reason: Schema.String,
}) {
}
export const encodeBoundsRequest = (payload, bounds) => {
...
export const decodeBoundsRequest = (body) => {
    const jsonLength = new DataView(body.buffer, body.byteOffset).getUint32(0);
    const bounds = JSON.parse(new TextDecoder().decode(body.subarray(4, 4 + jsonLength)));
...
export const encodeScanResult = (result) => {
    const blobs = [];
    let at = 0;
    const put = (bytes) => {
        blobs.push(bytes);
        const ref = [at, bytes.length];
        at += bytes.length;
        return ref;
    };
    const wire = {
...
export const decodeScanResult = (bytes) => {
    const jsonLength = new DataView(bytes.buffer, bytes.byteOffset).getUint32(0);
    const wire = JSON.parse(new TextDecoder().decode(bytes.subarray(4, 4 + jsonLength)));
    const blobs = bytes.subarray(4 + jsonLength);
    const slice = (ref) => ref === undefined ? undefined : blobs.subarray(ref[0], ref[0] + ref[1]);
    return {
...
export const makeFrameReader = (body) => {
...
export const encodeDeltaBatch = (bases, jobs) => {
    const blobs = [];
    let at = 0;
    const put = (bytes) => {
        blobs.push(bytes);
...
export const decodeDeltaBatch = (bytes) => {
    const jsonLength = new DataView(bytes.buffer, bytes.byteOffset).getUint32(0);
    const wire = JSON.parse(new TextDecoder().decode(bytes.subarray(4, 4 + jsonLength)));
    const blobs = bytes.subarray(4 + jsonLength);
    const slice = (ref) => blobs.subarray(ref[0], ref[0] + ref[1]);
    return {
...
export const encodeDeltaResults = (results) => {
    const blobs = [];
    let at = 0;
    const put = (bytes) => {
        blobs.push(bytes);
...
export const decodeDeltaResults = (bytes) => {
    const jsonLength = new DataView(bytes.buffer, bytes.byteOffset).getUint32(0);
    const wire = JSON.parse(new TextDecoder().decode(bytes.subarray(4, 4 + jsonLength)));
    const blobs = bytes.subarray(4 + jsonLength);
    const slice = (ref) => blobs.subarray(ref[0], ref[0] + ref[1]);
