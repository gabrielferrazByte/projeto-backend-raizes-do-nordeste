...
import { InvokeFunction } from "../../AWS/Lambda/InvokeFunction.js";
import { hashBounds, resolveDeltas, scanPart, } from "../Protocol/PartialScan.js";
import { Hasher, HashError } from "./Hasher.js";
import { decodeHashResponse, encodeHashEvent, LAMBDA_CHUNK_BYTES, } from "./LambdaEvent.js";
export const HasherLambda = (fn) => Layer.effect(Hasher, Effect.gen(function* () {
    ...
    const remote = (payload, options) => Effect.gen(function* () {
        ...
        const chunks = response.Payload
            ? Array.from(yield* Stream.runCollect(response.Payload).pipe(Effect.mapError((error) => new HashError({
    ...
    return {
        ...
        (payload.length > LAMBDA_CHUNK_BYTES
            ? Effect.fail(new HashError({
                reason: `payload of ${payload.length} bytes exceeds the invoke limit`,
            }))
            : remote(payload, options)).pipe(Effect.catchTag("HashError", (error) => {
