...
import { decodeDeltaBatch, encodeDeltaResults, encodeScanResult, frame, } from "./Protocol.js";
import { resolveDeltas, scanPart } from "../Protocol/PartialScan.js";
export default {
    async fetch(request) {
        ...
        if (query.get("mode") === "deltas") {
            ...
            const resolved = await Effect.runPromise(Effect.result(resolveDeltas(bases, jobs, { maxObjectSize })));
            if (resolved._tag === "Failure") {
                const failure = resolved.failure;
                return new Response(`${failure._tag}: ${"reason" in failure ? failure.reason : ""}`, { status: 422 });
            }
            return new Response(frame(encodeDeltaResults(resolved.success)), { headers: { "content-type": "application/octet-stream" } });
        }
        ...
        const result = await Effect.runPromise(Effect.result(scanPart(skip > 0 ? body.subarray(skip) : body, {
            base,
            remaining,
            maxObjectSize,
        ...
        return new Response(frame(encodeScanResult(result.success)), {
            headers: { "content-type": "application/octet-stream" },
        });
    },
};
//# sourceMappingURL=WorkerLoaderModule.js.map