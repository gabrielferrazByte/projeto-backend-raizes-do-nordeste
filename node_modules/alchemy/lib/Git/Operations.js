...
import { CommitDiff, CommitInfo, Comparison, DiffEntry, ImportFailed, MergeResult, ObjectStats, ObjectTooLarge, Pull, PullDetail, PushStats, Ref, Repo, RepoAlreadyExists, RepoCreated, RepoNotFound, RepoNotReady, TreeEntry, } from "./Api.js";
import { parseCommit, parseTree, treeEntryKind, } from "./Protocol/ObjectCodec.js";
import { RegistryStore } from "./RegistryObject.js";
import { RepoStore, } from "./RepoObject.js";
...
const toRef = (ref) => ref.peeled === undefined
...
const toDiffEntry = (entry) => new DiffEntry({
    path: entry.path,
    status: entry.status,
...
const toPullDetail = (pull) => new PullDetail({
    number: pull.number,
    title: pull.title,
    body: pull.body,
    baseRef: pull.baseRef,
...
const toCommitInfo = (commit) => new CommitInfo({
 ...
 */
const registryFallbackRepo = (entry) => new Repo({
    owner: entry.owner,
    name: entry.name,
    repoId: entry.repoId,
    ...
    public: entry.public,
    forkOf: entry.forkOf,
    status: entry.deletedAt !== null ? "deleting" : entry.status,
...
export const makeOperations = Effect.gen(function* () {
    ...
    const cacheKey = (owner, repo) => `${owner.toLowerCase()}/${repo.toLowerCase()}`;
    const dropCached = (owner, repo) => Effect.sync(() => {
        resolveCache.delete(cacheKey(owner, repo));
    });
    const resolveCached = (owner, repo) => Effect.gen(function* () {
     ...
     */
    const resolveIncludingDeleting = (owner, repo) => resolveCached(owner, repo).pipe(Effect.catchTag("StoreError", (error) => Effect.die(error)), Effect.flatMap((entry) => entry === undefined
     ...
     */
    const resolveOrNotFound = (owner, repo) => resolveIncludingDeleting(owner, repo).pipe(Effect.filterOrFail((entry) => entry.deletedAt === null, () => new RepoNotFound({ owner, repo })));
    const remoteUrl = (owner, name) => Effect.succeed(`/${owner}/${name}.git`);
     ...
     */
    const insertRepoRow = (input) => registryStub()
        .createRepo(input)
        .pipe(Effect.catchTag("StoreError", (error) => Effect.die(error)), Effect.catchTag("RepoAlreadyExists", (conflict) => Effect.gen(function* () {
        const existing = yield* resolveCached(input.owner, input.name).pipe(Effect.catchTag("StoreError", (error) => Effect.die(error)));
        ...
        const orphaned = yield* repos
            ...
            .readMeta()
            .pipe(Effect.as(false), Effect.catchTag("RepoNotFound", () => Effect.succeed(true)), Effect.catchCause(() => Effect.succeed(false)));
        if (!orphaned) {
            return yield* Effect.fail(conflict);
        }
        yield* registryStub()
    ...
    const reposRoutes = {
        create: ({ payload, }) => Effect.gen(function* () {
            ...
            Effect.onError(() => registryStub()
                .removeRow(entry.repoId)
                .pipe(Effect.ignore, Effect.andThen(dropCached(entry.owner, entry.name)))), Effect.catchTag("StoreError", (error) => Effect.die(error)));
        ...
        get: ({ params, }) => Effect.gen(function* () {
            ...
            const meta = yield* repos
                .getByName(entry.repoId)
                .getRepoMeta()
                .pipe(Effect.catchTag("StoreError", (error) => Effect.die(error)), Effect.catchTag("RepoNotFound", () => Effect.fail(new RepoNotFound({
                owner: params.owner,
                repo: params.repo,
            }))));
            return toRepo(meta);
        ...
        delete: ({ params, }) => Effect.gen(function* () {
            const entry = yield* resolveIncludingDeleting(params.owner, params.repo);
            ...
            Effect.catchTag("RepoNotFound", () => registryStub()
                .removeRow(entry.repoId)
                .pipe(Effect.catchTag("StoreError", (error) => Effect.die(error)))));
            yield* registryStub()
        ...
        fork: ({ params, payload, }) => Effect.gen(function* () {
            ...
            Effect.catchTag("ValidationError", () => Effect.fail(new RepoAlreadyExists({
                owner: payload.targetOwner,
        ...
        compact: ({ params, }) => Effect.gen(function* () {
            const entry = yield* resolveOrNotFound(params.owner, params.repo);
            yield* repos
                .getByName(entry.repoId)
                .startCompact()
        ...
        import: ({ payload, }) => Effect.gen(function* () {
            const entry = yield* registryStub()
                .createRepo({
                owner: payload.owner,
                name: payload.name,
            ...
            Effect.catchTag("ValidationError", (error) => Effect.fail(new ImportFailed({ reason: error.message }))));
            const init = yield* repos
                .getByName(entry.repoId)
                .startImport({
                repoId: entry.repoId,
    ...
    const refsRoutes = {
        list: ({ params, query, }) => Effect.gen(function* () {
            const entry = yield* resolveOrNotFound(params.owner, params.repo);
        ...
        get: ({ params, query, }) => Effect.gen(function* () {
            const entry = yield* resolveOrNotFound(params.owner, params.repo);
        ...
        update: ({ params, query, payload, }) => Effect.gen(function* () {
            const entry = yield* resolveOrNotFound(params.owner, params.repo);
        ...
        remove: ({ params, query, payload, }) => Effect.gen(function* () {
            const entry = yield* resolveOrNotFound(params.owner, params.repo);
            const stub = repos.getByName(entry.repoId);
            yield* stub
                .removeRef({
    ...
    const objectsRoutes = {
        commit: ({ params, }) => Effect.gen(function* () {
            const entry = yield* resolveOrNotFound(params.owner, params.repo);
        ...
        log: ({ params, query, }) => Effect.gen(function* () {
            const entry = yield* resolveOrNotFound(params.owner, params.repo);
            const page = yield* repos
                .getByName(entry.repoId)
                .readCommitLog({
        ...
        tree: ({ params, }) => Effect.gen(function* () {
            const entry = yield* resolveOrNotFound(params.owner, params.repo);
            const data = yield* repos
                .getByName(entry.repoId)
            ...
            return {
                oid: params.oid,
                entries: entries.map((item) => new TreeEntry({
                    mode: item.mode,
                    name: item.name,
                    oid: asOid(item.oid),
        ...
        blob: ({ params, }) => Effect.gen(function* () {
            const entry = yield* resolveOrNotFound(params.owner, params.repo);
            const data = yield* repos
                .getByName(entry.repoId)
        ...
        diff: ({ params, }) => Effect.gen(function* () {
            const entry = yield* resolveOrNotFound(params.owner, params.repo);
            const data = yield* repos
                .getByName(entry.repoId)
        ...
        compare: ({ params, query, }) => Effect.gen(function* () {
            const entry = yield* resolveOrNotFound(params.owner, params.repo);
            const data = yield* repos
                .getByName(entry.repoId)
    ...
    const pullsRoutes = {
        create: ({ params, payload, }) => Effect.gen(function* () {
            const entry = yield* resolveOrNotFound(params.owner, params.repo);
        ...
        merge: ({ params, payload, }) => Effect.gen(function* () {
            const entry = yield* resolveOrNotFound(params.owner, params.repo);
            const stub = repos.getByName(entry.repoId);
...
export class Operations extends Context.Service()("alchemy/Git/Operations") {
}
