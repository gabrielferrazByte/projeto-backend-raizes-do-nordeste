 ...
 * `^[0-9a-f]{4}#`).
 */
import { concatBytes, utf8Encode, ZERO_OID } from "./ObjectCodec.js";
 ...
 */
export const uploadPackCapabilities = (defaultBranch) => `multi_ack_detailed no-done side-band-64k shallow ofs-delta agent=${AGENT} symref=HEAD:refs/heads/${defaultBranch} object-format=sha1`;
/**
 ...
 */
export const receivePackCapabilities = () => `report-status report-status-v2 delete-refs side-band-64k atomic ofs-delta object-format=sha1 agent=${AGENT}`;
/**
 * The `Content-Type` of a `GET info/refs?service=` advertisement response.
 */
export const advertisementContentType = (service) => `application/x-${service}-advertisement`;
/**
 * The `Content-Type` of a `POST /git-{upload,receive}-pack` result response.
 */
export const resultContentType = (service) => `application/x-${service}-result`;
/**
 * Builds the `# service=` prelude: one pkt-line plus a flush. Sent before the
 * ref list on `info/refs` for both services.
 */
export const servicePrelude = (service) => concatBytes([pktLine(utf8Encode(`# service=${service}\n`)), flushPkt]);
 ...
 */
const refListSection = (records, capabilities) => {
    const lines = [];
    if (records.length === 0) {
 ...
 */
export const receivePackAdvertisement = (snapshot) => concatBytes([
 ...
 */
export const advertisement = (service, snapshot) => service === "git-upload-pack"
