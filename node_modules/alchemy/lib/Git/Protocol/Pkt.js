...
import { concatBytes, utf8Decode, utf8Encode } from "./ObjectCodec.js";
/**
 ...
 */
export class PktLineError extends Schema.TaggedError()("PktLineError", { reason: Schema.String }) {
}
/**
 ...
 */
export class ProtocolError extends Schema.TaggedError()("ProtocolError", { reason: Schema.String }) {
}
/** The flush packet singleton. */
export const flush = { _tag: "flush" };
/** The delim packet singleton. */
export const delim = { _tag: "delim" };
/** The 4 raw bytes of a flush-pkt (`0000`). */
export const flushPkt = utf8Encode("0000");
/** The 4 raw bytes of a delim-pkt (`0001`). */
export const delimPkt = utf8Encode("0001");
 ...
 */
export const pktText = (text) => pktLine(utf8Encode(text.endsWith("\n") ? text : `${text}\n`));
/**
 ...
 * and aborts.
 */
export const errPkt = (message) => pktText(`ERR ${message}`);
/**
 * Decodes a data pkt-line payload as UTF-8 text with the optional trailing
 * LF stripped (readers MUST strip it).
 */
export const pktPayloadText = (payload) => {
    const trimmed = payload.length > 0 && payload[payload.length - 1] === 0x0a
 ...
 */
export const readPktLineAt = (buf, offset) => {
    ...
    if (n === 0)
        return { _tag: "pkt", pkt: flush, next: offset + 4 };
    if (n === 1)
        return { _tag: "pkt", pkt: delim, next: offset + 4 };
    if (n === 2) {
 ...
 * malformed framing.
 */
export const decodePktLines = (buf, offset = 0) => Effect.suspend(() => {
    const out = [];
    let pos = offset;
 ...
 */
export const decodePktStream = (input) => Stream.transformPull(input, (pull) => Effect.sync(() => {
    let buffer = EMPTY;
    ...
    const loop = Effect.suspend(() => {
        const drained = drain();
        if (drained instanceof PktLineError)
            return Effect.fail(drained);
        if (Arr.isReadonlyArrayNonEmpty(drained)) {
            return Effect.succeed(drained);
        }
        if (upstreamDone) {
            if (buffer.length > 0) {
                return Effect.fail(new PktLineError({
                    reason: "byte stream ended inside a pkt-line",
                }));
            }
            return Cause.done();
        }
        return pull.pipe(Effect.map((chunks) => {
            buffer = concatBytes([buffer, ...chunks]);
        }), Pull.catchDone(() => Effect.sync(() => {
            upstreamDone = true;
        })), Effect.flatMap(() => loop));
    });
    return loop;
