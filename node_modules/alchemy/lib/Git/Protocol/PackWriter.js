...
import { encodeTypeSize, makeSha1, utf8Encode } from "./ObjectCodec.js";
/**
 ...
 */
export const writePack = (entries, objects, options) => Stream.unwrap(Effect.sync(() => {
    const sha = makeSha1();
    ...
    const header = Stream.fromEffect(Effect.sync(() => data(packHeader(total))));
    const body = Stream.flatMap(Stream.fromArray(entries), (entry) => {
        const head = Stream.fromEffect(Effect.sync(() => data(encodeTypeSize(entry.type, entry.size))));
        const zdata = Stream.map(objects.readZData(entry.oid), data);
        const progress = Stream.unwrap(Effect.sync(() => {
            written += 1;
    ...
    const trailer = Stream.fromEffect(Effect.sync(() => ({ _tag: "data", bytes: sha.digest() })));
 ...
 * response shape).
 */
export const packDataBytes = (events) => Stream.flatMap(events, (event) => event._tag === "data" ? Stream.succeed(event.bytes) : Stream.empty);
/**
 * Convenience: the raw pack byte stream for a manifest, with no progress
 * events.
 */
export const writePackBytes = (entries, objects) => packDataBytes(writePack(entries, objects));
//# sourceMappingURL=PackWriter.js.map