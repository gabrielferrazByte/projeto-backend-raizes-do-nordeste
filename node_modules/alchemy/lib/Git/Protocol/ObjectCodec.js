...
export const isOid = (s) => OID_REGEX.test(s);
/**
 ...
 */
export class ObjectParseError extends Schema.TaggedError()("ObjectParseError", { reason: Schema.String }) {
}
// ─────────────────────────────────────────────────────────────────────────────
// Object types
// ─────────────────────────────────────────────────────────────────────────────
 ...
 */
export const isDeltaType = (type) => type === 6 || type === 7;
// ─────────────────────────────────────────────────────────────────────────────
// Bytes / hex helpers
// ─────────────────────────────────────────────────────────────────────────────
 ...
 */
export const concatBytes = (chunks, totalLength) => {
 ...
 */
export const looseHeader = (type, size) => utf8Encode(`${objectTypeName(type)} ${size}\0`);
/**
...
export const hashObjectSync = (type, content) => {
    const sha = makeSha1();
    sha.update(looseHeader(type, content.length));
    sha.update(content);
    return sha.digestHex();
};
export const hashObject = (type, content) => Effect.sync(() => {
    const sha = makeSha1();
    sha.update(looseHeader(type, content.length));
    sha.update(content);
    return sha.digestHex();
 ...
 */
export const parseIdentity = (value) => Effect.suspend(() => {
 ...
 */
const parseHeaderBlock = (content) => {
 ...
 */
export const parseCommit = Effect.fn(function* (content) {
 ...
 */
export const formatIdentity = (id) => `${id.name} <${id.email}> ${id.when} ${id.tz}`;
const TZ_REGEX = /^[+-]\d{4}$/;
const validateIdentity = (role, id) => {
    if (/[<>\n]/.test(id.name) || /[<>\n]/.test(id.email)) {
 ...
 */
export const encodeCommit = (fields) => Effect.suspend(() => {
    if (!isOid(fields.tree)) {
        return Effect.fail(new ObjectParseError({
 ...
 */
export const parseTag = Effect.fn(function* (content) {
    const { headers, message } = parseHeaderBlock(content);
    let object;
    let targetType;
    let tag;
 ...
 */
export const treeEntryKind = (mode) => mode === TREE_MODE ? "tree" : mode === GITLINK_MODE ? "commit" : "blob";
/**
 * Parses a tree object's content into its entries:
 * `<mode ascii octal> SP <name> NUL <20 raw sha1 bytes>` concatenated with no
 * separators.
 */
export const parseTree = (content) => Effect.suspend(() => {
    const entries = [];
    let pos = 0;
 ...
 */
export const treeEntryCompare = (a, b) => {
 ...
 */
export const encodeTree = (entries) => Effect.suspend(() => {
    try {
 ...
 */
export const decodeTypeSize = (buf, offset) => {
 ...
 * {@link decodeTypeSize}).
 */
export const encodeTypeSize = (type, size) => {
    const out = [];
 ...
 */
export const decodeSizeVarint = (buf, offset) => {
    let value = 0;
    let shift = 0;
    let i = offset;
    for (;;) {
 ...
 */
export const encodeSizeVarint = (value) => {
    const out = [];
    let v = value;
    for (;;) {
        let b = v & 0x7f;
 ...
 */
export const decodeOfsDeltaOffset = (buf, offset) => {
    if (offset >= buf.length) {
 ...
 */
export const encodeOfsDeltaOffset = (value) => {
...
//# sourceMappingURL=ObjectCodec.js.map