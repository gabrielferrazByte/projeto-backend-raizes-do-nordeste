 ...
 */
export class ZlibError extends Schema.TaggedError()("ZlibError", {
 ...
 */
const inflateEntryInfoSync = (buf, offset, maxOutput, expectedSize) => {
    try {
...
const inflateEntryUnsafeSync = (buf, offset, maxOutput, expectedSize) => {
 ...
 */
export const inflateEntrySync = (buf, offset, options) => offset >= 0 && offset < buf.length
    ? inflateEntryUnsafeSync(buf, offset, options.maxOutput, options.expectedSize)
    : undefined;
export const inflateEntry = (buf, offset, options) => Effect.suspend(() => {
    const fast = offset >= 0 && offset < buf.length && options?.expectedSize !== undefined
        ? inflateEntryUnsafeSync(buf, offset, options.maxOutput, options.expectedSize)
        : undefined;
...
const inflateEntryStreaming = (buf, offset, options) => Effect.callback((resume) => {
    if (offset < 0 || offset >= buf.length) {
        resume(Effect.fail(new ZlibError({
 ...
 */
export const inflate = (data) => Effect.try({
    try: () => new Uint8Array(zlib.inflateSync(data)),
    catch: (error) => new ZlibError({
 ...
 */
export const deflate = (data, level = 6) => Effect.try({
 ...
 */
export const inflateExactSpan = (span, expectedSize) => Effect.try({
    try: () => {
