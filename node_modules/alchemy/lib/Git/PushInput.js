...
import { Oid, RefName } from "./Api/Schema.js";
import { StoreError } from "./Protocol/Store.js";
import { incomingStates } from "./Push.js";
import { makeStreamingSource } from "./Store/StreamingSource.js";
 ...
 */
export const fromStream = (updates, pack, options) => Effect.gen(function* () {
    ...
    let total = 0;
    const receiving = yield* Effect.forkScoped(pack.pipe(Stream.runForEach((bytes) => Effect.andThen(feeder.push(bytes), Effect.sync(() => {
        total += bytes.length;
    }))), Effect.map(() => {
        feeder.end();
        return { total };
    }), Effect.mapError((error) => new StoreError({ reason: `pack stream failed: ${String(error)}` })), Effect.tapError((error) => Effect.sync(() => feeder.fail(error))), Effect.result));
    const state = {
        feeder,
        receiving,
        packStart: 0,
...
//# sourceMappingURL=PushInput.js.map