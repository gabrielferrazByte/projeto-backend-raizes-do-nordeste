...
export const makeStreamingSource = (options) => {
     ...
     * releases the feeder.
     */
    const pushMayProceed = () => received - consumed <= backpressureBytes ||
    ...
    const waitFor = (need) => Effect.suspend(() => {
        if (failure !== undefined)
            return Effect.fail(failure);
        if (received >= need || ended)
            return Effect.void;
    ...
    const waitForFallback = Effect.suspend(() => {
        if (failure !== undefined)
            return Effect.fail(failure);
        if (fallback !== undefined && ended)
            return Effect.succeed(fallback);
        return Effect.callback((resume) => {
            fallbackWaiters.push(() => {
                resume(failure !== undefined
                    ? Effect.fail(failure)
                    : fallback !== undefined
    ...
    const source = {
        ...
        readSync: (offset, length) => {
            ...
            if (end <= offset)
                return ended ? new Uint8Array(0) : undefined;
            if (end < offset + length && !ended)
                return undefined;
        ...
        read: (offset, length) => Effect.gen(function* () {
            if (offset < retainFrom) {
    ...
    return {
        source,
        pushSync: (chunk) => {
            if (failure !== undefined || ended)
                return false;
            append(chunk);
            wake();
            return true;
        },
        push: (chunk) => Effect.gen(function* () {
            if (failure !== undefined)
...
//# sourceMappingURL=StreamingSource.js.map