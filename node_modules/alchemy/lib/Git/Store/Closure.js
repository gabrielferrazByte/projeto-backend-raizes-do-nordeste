...
import { parseTag, parseTree, treeEntryKind, } from "../Protocol/ObjectCodec.js";
 ...
 */
export class ManifestTooLarge extends Schema.TaggedError()("ManifestTooLarge", { count: Schema.Number, cap: Schema.Number }) {
}
/** Loads `commits` rows for the given oids; missing oids are absent. */
const loadCommitNodes = (sql, oids) => Effect.gen(function* () {
    const map = new Map();
    if (oids.length === 0) {
        return map;
    }
    const rows = yield* sql.inChunks((ph) => `SELECT oid, tree, gen FROM commits WHERE oid IN (${ph})`, oids);
...
const loadParents = (sql, oids) => Effect.gen(function* () {
    const map = new Map();
    if (oids.length === 0) {
        return map;
    }
    const rows = yield* sql.inChunks((ph) => `SELECT oid, parent, ord FROM commit_parents WHERE oid IN (${ph})`, oids);
    ...
    for (const [oid, list] of grouped) {
        ...
        map.set(oid, list.map((row) => row.parent));
    }
    return map;
});
// ─────────────────────────────────────────────────────────────────────────────
// Closure computation
// ─────────────────────────────────────────────────────────────────────────────
const dedup = (oids) => Array.from(new Set(oids));
 ...
 */
export const computeClosure = (options) => Effect.fn(function* (request) {
            ...
            switch (meta.type) {
                ...
                case 4: {
                    ...
                    if (hops > MAX_TAG_CHAIN) {
                        return yield* Effect.fail(new StoreError({ reason: `tag chain too deep at ${meta.oid}` }));
                    }
                    if (!tagSet.has(meta.oid)) {
                        tagSet.add(meta.oid);
                        tagOrder.push(meta.oid);
                    }
                    const content = yield* objects.readContent(meta.oid);
                    const parsed = yield* parseTag(content).pipe(Effect.mapError((error) => new StoreError({
                        reason: `bad tag ${meta.oid}: ${error.reason}`,
                    })));
                    const target = yield* objects.getMeta(parsed.object);
                    if (target === undefined) {
    ...
    const repoShallowSet = new Set(request.repoShallow ?? []);
    const haveNodes = yield* loadCommitNodes(sql, dedup(request.haves));
    const haveSet = new Set(haveNodes.keys());
    const clientShallowSet = new Set(request.clientShallow);
    ...
    // the common set.
    const walkThroughHave = (oid) => depth !== undefined && clientShallowSet.has(oid);
    ...
    while (frontier.length > 0) {
        ...
        for (const oid of frontier) {
            ...
            if (node === undefined) {
                return yield* Effect.fail(new StoreError({ reason: `commit graph missing commit ${oid}` }));
            }
    ...
    const included = new Set(newCommits.map((c) => c.oid));
    const shallow = [];
    ...
    if (depth !== undefined ||
        ...
        for (const commit of newCommits) {
            ...
            const cut = ps.some((parent) => !included.has(parent) &&
                !common.has(parent) &&
    ...
    // boundary.
    const unshallow = Array.from(clientShallowSet).filter((oid) => (included.has(oid) || candidateSet.has(oid)) && !shallowSet.has(oid));
    // ── 6. Tree/blob closure (shared visited set — the v1 fat) ──────────────
    const treeOrder = [];
    const blobOrder = [];
    const visitedObjects = new Set();
    let total = newCommits.length + tagOrder.length;
    const failIfOverCap = (count) => count > cap
    ...
    while (level.length > 0) {
        ...
        for (const tree of batch) {
            ...
            if (content === undefined) {
                return yield* Effect.fail(new StoreError({ reason: `tree ${tree} vanished during closure` }));
            }
            const entries = yield* parseTree(content).pipe(Effect.mapError((error) => new StoreError({ reason: `bad tree ${tree}: ${error.reason}` })));
    ...
    for (const oid of orderedOids) {
        ...
        if (meta === undefined) {
            return yield* Effect.fail(new StoreError({ reason: `object ${oid} vanished during closure` }));
        }
        entries.push(meta);
    }
    const result = { entries, shallow, unshallow };
    return result;
});
/**
 ...
 */
export const makeClosureSource = (options) => {
    const compute = computeClosure(options);
    return {
        commitClosure: (request) => compute(request).pipe(Effect.catchTag("ManifestTooLarge", (error) => Effect.fail(new StoreError({
