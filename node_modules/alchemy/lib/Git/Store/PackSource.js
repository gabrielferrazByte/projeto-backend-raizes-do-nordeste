...
export const blobRandomAccess = (options) => {
    ...
    const fetchWindow = (index) => Effect.gen(function* () {
        ...
        if (body === null) {
            return yield* Effect.fail(new StoreError({ reason: `incoming pack missing: ${options.key}` }));
        }
        const bytes = yield* body.bytes.pipe(Effect.mapError((error) => new StoreError({
            reason: `incoming pack read ${options.key}: ${error.reason}`,
        })));
    ...
    return {
        ...
        readSync: (offset, length) => {
            ...
            if (first !== Math.floor((end - 1) / windowBytes))
                return undefined;
...
//# sourceMappingURL=PackSource.js.map