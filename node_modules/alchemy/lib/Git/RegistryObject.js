...
import { RepoAlreadyExists, ValidationError } from "./Api.js";
import { StoreError } from "./Protocol/Store.js";
import { initRegistrySchema, makeSqlClient, } from "./Store/Sql.js";
// ─────────────────────────────────────────────────────────────────────────────
// ULID
// ─────────────────────────────────────────────────────────────────────────────
...
export const encodeCursor = (owner, name) => Buffer.from(`${owner}\0${name}`, "utf8").toString("base64url");
 ...
 */
export class Registry extends Cloudflare.DurableObject()("GitRegistry", {
 ...
 * @binding
 */
export class RegistryStore extends Context.Service()("alchemy/Git/RegistryStore") {
}
/**
 * The Registry DO implementation Layer. Provide on the hosting Worker's
 * init effect (`Effect.provide(RegistryLive)`).
 */
export const RegistryLive = Registry.make(Effect.gen(function* () {
    ...
    return Effect.gen(function* () {
        ...
        const shape = {
            createRepo: Effect.fn(function* (input) {
                ...
                return yield* sql.transactionSync((raw, rollback) => {
                    const existing = raw
                        .exec(`SELECT * FROM repos WHERE owner = ? AND name = ?`, owner, name)
                        .toArray();
                    if (existing.length > 0) {
                        // Includes soft-deleted rows: the name frees only after purge.
                        rollback(new RepoAlreadyExists({ owner, repo: name }));
                    }
                    raw.exec(`INSERT INTO repos (owner, name, repo_id, description, is_public, fork_of, fork_count, created_at, deleted_at)
               VALUES (?, ?, ?, ?, ?, ?, 0, ?, NULL)`, owner, name, repoId, input.description ?? null, input.public === true ? 1 : 0, input.forkOf ?? null, createdAt);
                    if (input.forkOf !== undefined) {
                        raw.exec(`UPDATE repos SET fork_count = fork_count + 1 WHERE repo_id = ?`, input.forkOf);
                    }
                    return {
                        owner,
                        name,
            ...
            resolve: Effect.fn(function* (owner, name) {
                ...
                const row = yield* sql.first(`SELECT * FROM repos WHERE owner = ? AND name = ?`, owner.toLowerCase(), name.toLowerCase());
            ...
            list: Effect.fn(function* (input) {
                ...
                const rows = yield* sql.all(`SELECT * FROM repos WHERE ${conditions.join(" AND ")}
             ORDER BY owner, name LIMIT ?`, ...bindings, limit + 1);
            ...
            bumpForkCount: Effect.fn(function* (repoId, delta) {
                if (delta !== 0) {
                    yield* sql.run(`UPDATE repos SET fork_count = MAX(0, fork_count + ?) WHERE repo_id = ?`, delta, repoId);
                    const row = yield* sql.first(`SELECT fork_count FROM repos WHERE repo_id = ?`, repoId);
                ...
                const row = yield* sql.first(`SELECT COUNT(*) AS n FROM repos WHERE fork_of = ?`, repoId);
                return row?.n ?? 0;
            }),
            markDeleted: Effect.fn(function* (repoId) {
                yield* sql.run(`UPDATE repos SET deleted_at = ? WHERE repo_id = ? AND deleted_at IS NULL`, Date.now(), repoId);
            }),
            updateSummary: Effect.fn(function* (repoId, summary) {
                yield* sql.run(`UPDATE repos SET default_branch = ?, read_only = ?, is_public = ?, status = ?
              WHERE repo_id = ?`, summary.defaultBranch, summary.readOnly ? 1 : 0, summary.public ? 1 : 0, summary.status, repoId);
            }),
            removeRow: Effect.fn(function* (repoId) {
                const row = yield* sql.first(`SELECT * FROM repos WHERE repo_id = ?`, repoId);
                if (row === undefined) {
                    return; // idempotent — purge re-runs are expected
                }
                yield* sql.run(`DELETE FROM repos WHERE repo_id = ?`, repoId);
                if (row.fork_of !== null) {
                    yield* sql.run(`UPDATE repos SET fork_count = MAX(0, fork_count - 1) WHERE repo_id = ?`, row.fork_of);
                }
            }),
        };
        return shape;
 ...
 */
export const RegistryDurableObject = Layer.effect(RegistryStore, Effect.gen(function* () {
...
//# sourceMappingURL=RegistryObject.js.map