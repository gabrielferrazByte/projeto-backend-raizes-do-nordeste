...
const isoSeconds = (sig) => new Date(sig.date * 1000).toISOString();
...
const ghJson = (value, options) => HttpServerResponse.json(value, {
    status: options?.status ?? 200,
    headers: options?.headers,
}).pipe(Effect.orDie);
const ghError = (status, message) => ghJson({ message }, { status });
 ...
 */
const ghCatch = (effect) => effect.pipe(Effect.catchIf((error) => typeof error?._tag === "string", (error) => {
    switch (error._tag) {
        case "RepoNotFound":
        case "RefNotFound":
        case "ObjectNotFound":
        ...
        case "PullExists":
            return ghError(422, "Validation Failed: a pull request already exists for this head/base");
...
const ghCommit = (commit, origin, repoUrl) => ({
    sha: commit.oid,
    node_id: commit.oid,
...
const ghPullState = (state) => state === "open" ? "open" : "closed";
const ghPull = (pull, meta, origin) => {
    ...
    return {
        ...
        mergeable_state: pull.mergeableReason === "conflict"
            ? "dirty"
            : pull.mergeable === true
                ? "clean"
                : "unknown",
...
export const gitHubCompatRoutes = (options) => {
    ...
    const withRepo = (handler) => Effect.gen(function* () {
        ...
        const result = yield* ghCatch(handler({
    ...
    const linkHeaders = (base, query, nextCursor) => {
        if (nextCursor === null)
            return {};
    ...
    const resolveSha = (repo, revision) => OID_RE.test(revision)
    ...
    return {
        ...
        repo: () => withRepo(({ repo, origin }) => repo
            .getRepoMeta()
            .pipe(Effect.flatMap((meta) => ghJson(ghRepo(meta, origin))))),
        /** `GET /api/v3/repos/:owner/:repo/branches` */
        branches: () => withRepo(({ repo }) => repo.listRefs("refs/heads/").pipe(Effect.flatMap((page) => ghJson(page.refs.map((ref) => ({
            name: shortRef(ref.name),
            commit: { sha: ref.oid },
            protected: false,
        })))))),
        /** `GET /api/v3/repos/:owner/:repo/commits` */
        commits: () => withRepo(({ repo, origin, repoUrl, url }) => repo
            .readCommitLog({
            ref: url.searchParams.get("sha") ?? undefined,
            cursor: url.searchParams.get("cursor") ?? undefined,
            limit: perPage(url),
        })
            .pipe(Effect.flatMap((page) => ghJson(page.items.map((commit) => ghCommit(commit, origin, repoUrl)), {
            headers: linkHeaders(`${repoUrl}/commits`, url.searchParams, page.hasMore ? page.nextCursor : null),
        })))),
        /** `GET /api/v3/repos/:owner/:repo/commits/:sha` */
        commit: () => withRepo(({ repo, origin, repoUrl, params }) => Effect.gen(function* () {
            const oid = yield* resolveSha(repo, params.sha ?? "");
            const log = yield* repo.readCommitLog({ ref: oid, limit: 1 });
            const commit = log.items[0];
            if (commit === undefined)
                return yield* ghNotFound;
        ...
        contents: () => withRepo(({ repo, url, repoUrl }) => Effect.gen(function* () {
        ...
        pulls: () => withRepo(({ repo, entry, origin, repoUrl, url }) => Effect.gen(function* () {
            ...
            const items = page.items.filter((pull) => requested === "all"
                ? true
                : requested === "open"
                    ? pull.state === "open"
                    : pull.state !== "open");
            return yield* ghJson(items.map((pull) => ghPull(pull, meta, origin)), {
                headers: linkHeaders(`${repoUrl}/pulls`, url.searchParams, page.hasMore ? page.nextCursor : null),
            });
        })),
        /** `POST /api/v3/repos/:owner/:repo/pulls` */
        createPull: () => withRepo(({ repo, origin, request }) => Effect.gen(function* () {
            const body = (yield* request.json.pipe(Effect.mapError(() => ({ _tag: "ValidationError" }))));
        ...
        pull: () => withRepo(({ repo, origin, params }) => Effect.gen(function* () {
        ...
        updatePull: () => withRepo(({ repo, origin, params, request }) => Effect.gen(function* () {
            const number = Number.parseInt(params.number ?? "", 10);
            if (!Number.isFinite(number))
                return yield* ghNotFound;
            const body = (yield* request.json.pipe(Effect.mapError(() => ({ _tag: "ValidationError" }))));
        ...
        mergePull: () => withRepo(({ repo, params, request }) => Effect.gen(function* () {
            const number = Number.parseInt(params.number ?? "", 10);
            if (!Number.isFinite(number))
                return yield* ghNotFound;
            const raw = yield* request.json.pipe(Effect.catch(() => Effect.succeed({})));
            const body = raw;
            if (body.merge_method !== undefined &&
        ...
        pullFiles: () => withRepo(({ repo, params }) => Effect.gen(function* () {
            const number = Number.parseInt(params.number ?? "", 10);
            if (!Number.isFinite(number))
                return yield* ghNotFound;
            const pull = yield* repo.getPull(number);
...
//# sourceMappingURL=GitHubCompat.js.map