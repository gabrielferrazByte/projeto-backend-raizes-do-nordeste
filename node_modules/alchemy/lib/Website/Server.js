...
import { initialCwd, moduleExtension } from "../Util/Node.js";
...
const definedEnv = (env) => env === undefined
    ? undefined
 ...
 * @resource
 */
export const Server = Resource("Website.Server", {
...
export const ServerProviderLive = () => Provider.effect(Server, Effect.gen(function* () {
    ...
    const runBuild = Effect.fn(function* (props) {
        ...
        return yield* Effect.mapError(service.build({ root, env: definedEnv(props.env) }), (cause) => new FrameworkServerError({
            framework: props.framework,
    ...
    return {
        ...
        delete: Effect.fn(function* ({ olds, output }) {
            ...
            const distDir = path.resolve(initialCwd, output.distDir);
            if (!(yield* fs.exists(root)) || !(yield* fs.exists(distDir)))
                return;
 ...
 */
const normalizeAdvertisedUrl = (url) => url.replace(/^(https?:\/\/)(?:0\.0\.0\.0|\[::\]|\[0+(?::0+){7}\])(?=[:/]|$)/, "$1localhost");
/**
 ...
 */
export const ServerProviderLocal = () => LocalProvider.make(Server, import.meta.resolve(`./ServerLocal${moduleExtension(import.meta.url)}`, import.meta.url), Effect.gen(function* () {
    const path = yield* Path.Path;
    const fs = yield* FileSystem.FileSystem;
    return {
