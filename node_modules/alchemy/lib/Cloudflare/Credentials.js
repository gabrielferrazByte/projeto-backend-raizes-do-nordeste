...
import * as CredentialsCache from "../Auth/CredentialsCache.js";
import { resolveProviderConfig } from "../Auth/Resolve.js";
import { CLOUDFLARE_AUTH_PROVIDER_NAME, } from "./Auth/AuthConfig.js";
 ...
 * and cache forever.
 */
export const cacheUntilExpiry = (resolve) => CredentialsCache.cacheUntilExpiry(resolve, (credentials) => credentials.type === "oauth" ? credentials.expiresAt : undefined);
/**
...
export const fromAuthProvider = () => Layer.effect(Credentials, Effect.gen(function* () {
    const { profileName, resolve: resolveAuth } = yield* resolveProviderConfig(CLOUDFLARE_AUTH_PROVIDER_NAME);
    const resolve = resolveAuth.pipe(Effect.map((creds) => Match.value(creds).pipe(Match.when({ type: "apiToken" }, (c) => apiTokenCredentials({
        ...
        message: `Failed to resolve Cloudflare credentials from ${profileName === undefined ? "the CI environment" : `profile '${profileName}'`}: ${e.message ?? String(e)}`,
    })));
