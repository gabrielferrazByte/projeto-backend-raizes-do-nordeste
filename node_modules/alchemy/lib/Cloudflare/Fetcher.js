 ...
 */
const sameHeaders = (web, eff) => {
    const keys = Object.keys(eff);
    let webCount = 0;
    for (const _ of web.keys()) {
        webCount++;
        if (webCount > keys.length)
            return false;
    }
    if (webCount !== keys.length)
        return false;
    for (const key of keys) {
        if (web.get(key) !== eff[key])
            return false;
    }
    return true;
};
...
export const fromCloudflareFetcher = (fetcher) => {
    ...
    return {
        ...
        fetch: (request) => HttpClientRequest.isHttpClientRequest(request)
            ...
            : pipe(HttpServerRequest.toWeb(request), 
            ...
            Effect.map((webRequest) => sameHeaders(webRequest.headers, request.headers)
                ? webRequest
                : new Request(webRequest, {
                    headers: request.headers,
                })), Effect.flatMap(fetch), Effect.map((response) => {
...
Effect.runSync(Socket.fromTransformStream(Effect.tryPromise({
    try: () => Promise.resolve(cfSocket.opened).then(() => ({
