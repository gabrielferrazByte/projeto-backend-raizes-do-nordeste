...
const TypeId = "Cloudflare.R2.DataCatalog";
/**
 ...
 */
export const DataCatalog = Resource(TypeId);
/**
 * Returns true if the given value is an R2DataCatalog resource.
 */
export const isDataCatalog = (value) => Predicate.hasProperty(value, "Type") && value.Type === TypeId;
export const DataCatalogProvider = () => Provider.succeed(DataCatalog, {
    ...
    diff: Effect.fn(function* ({ olds, news, output }) {
        ...
        const oldBucket = output?.bucketName ??
    ...
    list: Effect.fn(function* () {
        ...
        return yield* rdc.listR2DataCatalogs({ accountId }).pipe(Effect.map(({ warehouses }) => warehouses
    ...
    reconcile: Effect.fn(function* ({ news, olds, output }) {
        ...
        if (!observed || observed.status !== "active") {
            yield* rdc.enableR2DataCatalog({ accountId: acct, bucketName }).pipe(Effect.retry({
        ...
        let credentialStatus = (observed.credentialStatus ?? "absent");
        if (news.token !== undefined) {
            const rotated = olds?.token === undefined ||
        ...
        const compactionDirty = news.compaction !== undefined &&
        ...
        const expirationDirty = news.snapshotExpiration !== undefined &&
        ...
        return toAttributes({ ...observed, status: "active", maintenanceConfig: maintenance }, acct, credentialStatus);
    }),
...
const toAttributes = (catalog, accountId, credentialStatus) => ({
    ...
    compaction: catalog.maintenanceConfig?.compaction
        ? {
            state: catalog.maintenanceConfig.compaction.state,
