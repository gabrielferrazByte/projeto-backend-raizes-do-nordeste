...
export const ProviderLive = () => Provider.effect(Bucket, Effect.gen(function* () {
    ...
    const listManagedDomain = Effect.fn(function* (bucketName, jurisdiction, options) {
        const { accountId } = yield* yield* CloudflareEnvironment;
        ...
        Effect.map((response) => response.enabled ? response.domain : undefined), Effect.catchTag("NoSuchBucket", () => Effect.succeed(undefined)));
    });
    const reconcileManagedDomain = (bucketName, jurisdiction, desired) => Effect.gen(function* () {
        const { accountId } = yield* yield* CloudflareEnvironment;
    ...
    return {
        ...
        reconcile: Effect.fn(function* ({ id, news = {}, output }) {
            ...
            const publicDomain = yield* reconcileManagedDomain(attrs.bucketName, attrs.jurisdiction, news.publicAccess ?? false);
            return {
                ...attrs,
                domains,
                lifecycleRules,
        ...
        delete: Effect.fn(function* ({ olds = {}, output, force }) {
            ...
            yield* deleteBucket.pipe(Effect.catchTag("BucketNotEmpty", (error) => Effect.sleep("2 seconds").pipe(Effect.andThen(emptyBucket(output.bucketName, output.jurisdiction)), Effect.andThen(Effect.fail(error)))), Effect.retry({
