...
export const HostnameTlsSettingProvider = () => Provider.succeed(HostnameTlsSetting, {
    ...
    list: Effect.fn(function* () {
        ...
        const rows = yield* Effect.forEach(zones, (zone) => Effect.forEach(settingIds, (settingId) => hostnames
            .listSettingsTls({ zoneId: zone.id, settingId })
            .pipe(Effect.map((settings) => settings.flatMap((setting) => setting.hostname == null
            ? []
            : [
                toAttributes(zone.id, settingId, setting.hostname, setting),
            ]))), { concurrency: "unbounded" }).pipe(Effect.map((perSetting) => perSetting.flat())), { concurrency: 10 });
        return rows.flat();
    }),
