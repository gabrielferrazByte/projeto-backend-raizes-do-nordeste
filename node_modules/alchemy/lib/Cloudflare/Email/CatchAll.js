...
import { retryWorkerScriptNotFound } from "./retry.js";
const CatchAllTypeId = "Cloudflare.Email.CatchAll";
/**
 ...
 * @category Email
 */
export const CatchAll = Resource(CatchAllTypeId, {
    aliases: ["Cloudflare.EmailCatchAll"],
});
/**
 * Returns true if the given value is an CatchAll resource.
 */
export const isCatchAll = (value) => Predicate.hasProperty(value, "Type") && value.Type === CatchAllTypeId;
export const CatchAllProvider = () => Provider.succeed(CatchAll, {
    nuke: { singleton: true },
    stables: [
        "ruleId",
        "zoneId",
    ...
    list: Effect.fn(function* () {
        ...
        const rows = yield* Effect.forEach(allZones.map((zone) => zone.id), (zoneId) => emailRouting.getRuleCatchAll({ zoneId }).pipe(Effect.map((observed) => toAttributes(zoneId, observed, observedInitial(observed))), 
    ...
    reconcile: Effect.fn(function* ({ news, output }) {
        ...
        if ((observed.enabled ?? false) === desiredEnabled &&
        ...
        const result = yield* emailRouting
            ...
            zoneId,
            matchers: [{ type: "all" }],
            actions: news.actions.map((a) => a.type === "drop"
                ? { type: a.type }
    ...
    delete: Effect.fn(function* ({ output }) {
        ...
        yield* emailRouting
            ...
            zoneId,
            matchers: [{ type: "all" }],
            actions: initialActions.map((a) => a.type === "drop"
                ? { type: a.type }
                : { type: a.type, value: a.value }),
        ...
        Effect.catchTag(["DestinationNotVerified", "WorkerScriptNotFound"], () => emailRouting
            .putRuleCatchAll({
            zoneId,
            matchers: [{ type: "all" }],
            actions: [{ type: "drop" }],
...
const normalizeActions = (actions) => (actions ?? []).map((a) => a.type === "drop"
    ? { type: "drop" }
    : a.type === "forward"
        ? { type: "forward", value: [...(a.value ?? [])] }
        : { type: "worker", value: [...(a.value ?? [])] });
const actionsEqual = (a, b) => a.length === b.length &&
    a.every((x, i) => {
        const y = b[i];
        if (x.type !== y.type)
            return false;
