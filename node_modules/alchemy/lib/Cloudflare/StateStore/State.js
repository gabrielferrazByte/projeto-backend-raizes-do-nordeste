...
import { currentProfileName } from "../../Auth/Profile.js";
...
import { CREDENTIALS_FILE, StoredStateStoreCredentials, isStateStoreCredentialsStale, } from "./CredentialsFile.js";
...
export const state = () => Layer.effect(State, Effect.gen(function* () {
    ...
    const init = Effect.gen(function* () {
        ...
        const credentials = yield* credStore.read(profileName, CREDENTIALS_FILE, StoredStateStoreCredentials);
        if (credentials) {
            ...
            if (isStateStoreCredentialsStale(credentials, accountId)) {
                yield* Interaction.accessors.output.info(`Cloudflare State Store credentials were minted for a different ` +
 ...
 */
const withStateBootstrapEvents = (store) => (effect) => Effect.gen(function* () {
...
export const bootstrap = (options = {}) => withStateBootstrapEvents(options.workerName ?? STATE_STORE_SCRIPT_NAME)(Effect.gen(function* () {
    const interaction = yield* Interaction.Interaction;
    const isCI = yield* CI;
    ...
    if (yield* isStateStoreServing(accountId)) {
        ...
        if (!force) {
            yield* Interaction.accessors.output.info(`Worker '${scriptName}' already exists; adopting and refreshing credentials. ` +
        ...
        if (!isCI) {
            ...
            yield* store.write(profileName, CREDENTIALS_FILE, StoredStateStoreCredentials, credentials);
        }
        const { matches, expected, observed } = yield* checkStateStoreVersion(url);
        ...
        return yield* interaction.task({ label: `Deploying Cloudflare State Store '${scriptName}'` }, deployWithLocalState({
            scriptName,
            profileName,
            isCI,
            force,
...
export const teardownStateStore = (options = {}) => Effect.gen(function* () {
    ...
    yield* workers.deleteScript({ accountId, scriptName, force: true }).pipe(Effect.asVoid, Effect.catchTag("WorkerNotFound", () => interaction.output.info(`  Worker '${scriptName}' not found (already gone).`)));
    ...
    for (const store of stores) {
        ...
        if (deleteEmptyStore && remaining === 0) {
            ...
            yield* Interaction.accessors.output.info(`Secrets store '${store.id}' still has ${remaining} other ` +
    ...
    yield* Interaction.accessors.output.success(`Cloudflare State Store '${scriptName}' torn down.`);
...
export const loginWithCloudflare = (profileName, force) => Effect.gen(function* () {
    ...
    const interaction = yield* Interaction.Interaction;
    const credentials = yield* interaction.task({ label: "Refreshing Cloudflare State Store credentials" }, Effect.gen(function* () {
    ...
    if (!isCI)
        yield* interaction.output.info(`  url:     ${credentials.url}`);
    return credentials;
