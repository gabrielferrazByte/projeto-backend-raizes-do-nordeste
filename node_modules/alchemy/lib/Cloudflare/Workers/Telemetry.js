...
import * as AlchemyTelemetry from "../../Telemetry.js";
import { getCompatibility } from "./Compatibility.js";
import { layer as cloudflareTracerLayer } from "./CloudflareTracer.js";
 ...
 */
export class CloudflareTelemetryCompatibilityError extends Data.TaggedError("CloudflareTelemetryCompatibilityError") {
    get message() {
        return (`Cloudflare.Telemetry() requires compatibility date >= ${this.minimumDate} ` +
            `(tracing.startActiveSpan). This Worker is pinned to ${this.date}. ` +
            `Set compatibility: { date: "${this.minimumDate}" } (or later) on the Worker.`);
    }
}
const tracesFromProps = (props) => ({
 ...
 * @internal
 */
export const assertCloudflareTelemetryCompatibility = (news, bindings) => {
    const hasBind = bindings.some((b) => b.data.observability?.traces != null);
    if (!hasBind)
        return Effect.void;
    const { date } = getCompatibility(news);
    if (date >= MIN_CLOUDFLARE_TRACING_DATE)
        return Effect.void;
    return Effect.fail(new CloudflareTelemetryCompatibilityError({
        date,
...
export const Telemetry = (props = {}) => Layer.unwrap(Effect.gen(function* () {
    ...
    if (!globalThis.__ALCHEMY_RUNTIME__) {
        yield* Namespace.push(host.LogicalId, host.bind("Cloudflare.Telemetry", {
