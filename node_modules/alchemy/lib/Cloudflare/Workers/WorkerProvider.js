...
import { assertCloudflareTelemetryCompatibility } from "./Telemetry.js";
import { isSelfUrl, Worker, } from "./Worker.js";
import { getCacheBinding, getCronBindings, isContainerDecl, resolveObservability, } from "./WorkerAsyncBindings.js";
 ...
 * @internal
 */
export const resolvePreviewParentName = (preview) => {
...
const toPreviewWireKey = (key) => key.replace(/[A-Z]/g, (c) => `_${c.toLowerCase()}`);
const toPreviewWire = (value) => {
    if (Array.isArray(value))
        return value.map(toPreviewWire);
...
const bindingsToPreviewEnv = (bindings) => {
    const env = {};
...
export const LiveWorkerProvider = () => Provider.effect(Worker, Effect.gen(function* () {
    ...
    const reconcileCrons = (scriptName, desired, previous, session) => Effect.gen(function* () {
        ...
        if (desired.length > 0 || previous.length > 0 || live.length > 0) {
            yield* session.note(`Reconciling Cron Triggers (${desired.length}) ...`, { kind: "status" });
        }
    ...
    const reconcileDomains = (scriptName, desired, zone, previewsEnabled) => Effect.gen(function* () {
        const { accountId } = yield* yield* CloudflareEnvironment;
    ...
    const prepareBundle = (id, fqn, props) => (isPythonMain(props.main)
    ...
    const viteBuild = Effect.fn(function* (id, fqn, props, selfUrl) {
    ...
    const prepareAssetsAndBundle = (id, fqn, workerName, props, opts = {}) => Effect.gen(function* () {
     ...
     */
    const secretTextFromEnvValue = (value) => {
    ...
    const newsWithBoundEnv = (news, bindings) => {
        const bound = {};
        for (const binding of bindings) {
            if (binding.action === "delete")
                continue;
            if (binding.data?.env)
                Object.assign(bound, binding.data.env);
        }
        const boundKeys = Object.keys(bound);
        if (boundKeys.length === 0)
            return { news, boundKeys };
     ...
     */
    const putWorkerVersion = Effect.fn(function* (id, fqn, news, bindings, session, output) {
        ...
        const { assets, bundle, hash: preparedHash, } = yield* prepareAssetsAndBundle(id, fqn, parentName, news, {
        ...
        let deploymentId;
        if (traffic > 0) {
            yield* session.note(`Deploying version at ${traffic}% of ${parentName}'s traffic ...`, { kind: "status" });
     ...
     */
    const putWorkerPreview = Effect.fn(function* (id, fqn, news, bindings, session, output) {
        const { accountId } = yield* yield* CloudflareEnvironment;
        const preview = news.preview;
        const parentName = resolvePreviewParentName(preview);
        if (parentName === undefined) {
            return yield* Effect.fail(new WorkerPreviewConfigError({
        ...
        if (forbidden.length > 0) {
            return yield* Effect.fail(new WorkerPreviewConfigError({
                message: `preview.of creates a Preview of '${parentName}' — script-level settings belong to the parent Worker and cannot be set here: ${forbidden.join(", ")}. ` +
                    `Remove ${forbidden.length === 1 ? "this prop" : "these props"} or configure ${forbidden.length === 1 ? "it" : "them"} on the parent.`,
            }));
        }
        const cronBindings = getCronBindings(bindings);
        ...
        yield* Effect.logInfo(`Cloudflare Worker Preview: ${previewName} of ${parentName} (from ${id})`);
        const existingPreview = yield* workers
            ...
            .pipe(Effect.catchTag("PreviewNotFound", () => Effect.succeed(undefined)), Effect.catchTag("WorkerNotFound", () => Effect.fail(new WorkerPreviewConfigError({
            message: `preview.of script '${parentName}' does not exist. Deploy the parent Worker first (or check the referenced stage/stack).`,
        }))));
        const previewResource = existingPreview ??
            (yield* workers
                .createPreview({
                accountId,
                workerId: parentName,
                ...
                .pipe(Effect.catchTag("WorkerNotFound", () => Effect.fail(new WorkerPreviewConfigError({
                message: `preview.of script '${parentName}' does not exist. Deploy the parent Worker first (or check the referenced stage/stack).`,
            })))));
        const accountSubdomain = yield* getAccountSubdomain(accountId);
        const parentAttrs = typeof preview.of === "object" && preview.of !== null
            ? preview.of
            : undefined;
        ...
        yield* Effect.logInfo(`Cloudflare Worker Preview: preparing bundle for ${parentName} (preview ${previewName})`);
        const { assets, bundle, hash: preparedHash, } = yield* prepareAssetsAndBundle(id, fqn, parentName, news, {
            skipAssetsRead: false,
        });
        const metadataHash = yield* resolveWorkerMetadataHash({
        ...
        if (classNames.length > 0) {
            const latest = yield* workers
                ...
                .pipe(Effect.catchTag(["PreviewNotFound", "PreviewDeploymentNotFound"], () => Effect.succeed(undefined)));
        ...
        const compatibility = getCompatibility(news);
        const annotations = preview.message !== undefined || preview.tag !== undefined
            ? {
    ...
    const putWorker = Effect.fn(function* (id, fqn, news, bindings, olds, output, session, existingSettings) {
        ...
        const { assets, bundle, hash: preparedHash, } = yield* prepareAssetsAndBundle(id, fqn, name, news, {
        ...
        if (prebuiltAssets?.skip) {
            ...
            if (assets.hash === prebuiltAssets?.hash) {
                ...
                const { jwt } = yield* uploadAssets(accountId, name, assets, session, dispatchNamespace);
                metadataAssets = {
                    jwt,
        ...
        if (rolloutTraffic !== undefined &&
            ...
            yield* session.note(`Uploading version of ${name} (${bundleSize}) ...`, { kind: "status" });
            const created = yield* workers
                .createScriptVersion({
                accountId,
                scriptName: name,
            ...
            if (rolloutTraffic > 0) {
                yield* session.note(`Deploying version at ${rolloutTraffic}% of traffic ...`, { kind: "status" });
                deploymentId = yield* deployVersionTraffic({
                    accountId,
                    scriptName: name,
                    versionId,
        ...
        if (workersDev.enabled !== observedSubdomain.enabled ||
            ...
            yield* session.note(`${workersDev.enabled || workersDev.previewsEnabled ? "Enabling" : "Disabling"} workers.dev subdomain...`, { kind: "status" });
        ...
        if (manageCustomDomains) {
            const desiredHostnames = domainConfig
                ...
                : [];
            yield* session.note(`Reconciling custom domains (${desiredHostnames.length}) ...`, { kind: "status" });
            ...
            const liveBeforeReconcile = yield* workers
                ...
                ? [[d.hostname, d.zoneId]]
                : [])), Effect.catch(() => Effect.succeed([])));
            const reconciled = yield* reconcileDomains(name, desiredHostnames, domainConfig?.zone, domainConfig?.previews);
        ...
        if (desiredRoutes.length > 0 || previousRoutes.length > 0) {
            yield* session.note(`Reconciling worker routes (${desiredRoutes.length}) ...`, { kind: "status" });
        }
    ...
    const hasChanged = Effect.fn(function* (id, fqn, props, output, bindings, accountId) {
        ...
        if (bindings) {
            ...
            const versionParent = props.version?.parent != null
                ...
                : undefined;
            const previewParent = props.preview?.of != null
    ...
    return Worker.Provider.of({
        ...
        diff: Effect.fn(function* ({ id, fqn, news: desired, olds, output, newBindings, }) {
            ...
            if (legacyWorkerId ||
                ...
                routesChanged ||
                cronsChanged ||
                (yield* hasChanged(id, fqn, news, output, Array.isArray(newBindings)
                    ? newBindings
                    : undefined, accountId))) {
        ...
        precreate: Effect.fn(function* ({ id, news, session, bindings }) {
            ...
            if (news.version?.parent != null || news.preview?.of != null) {
                yield* Effect.logInfo(`Cloudflare Worker precreate: skipping stub for ${news.preview?.of != null ? "Preview" : "version"} worker ${id}`);
                return {
             ...
             * undefined.
             */
            const workersDevUrl = resolveWorkersDev(news.workersDev).enabled
        ...
        read: Effect.fn(function* ({ id, output, olds }) {
            ...
            if (output?.previewOf !== undefined || olds?.preview?.of != null) {
                ...
                return yield* workers
                    ...
                    .pipe(Effect.map(() => output), Effect.catchTag(["WorkerNotFound", "PreviewNotFound"], () => Effect.succeed(undefined)));
            }
        ...
        delete: Effect.fn(function* ({ output }) {
            ...
            if (output.previewOf !== undefined) {
                ...
                if (!previewId)
                    return;
                yield* Effect.logInfo(`Cloudflare Worker delete: deleting Preview ${previewId} of ${output.previewOf}`);
                yield* workers
                    .deletePreview({
                    accountId: output.accountId,
                    workerId: output.previewOf,
