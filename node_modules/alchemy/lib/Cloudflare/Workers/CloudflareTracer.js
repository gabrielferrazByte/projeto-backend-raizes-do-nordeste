...
class Span extends Tracer.NativeSpan {
    runInContext;
    cloudflareSpan;
    constructor(options, runInContext, cloudflareSpan) {
    ...
    attribute(key, value) {
        ...
        this.cloudflareSpan?.setAttribute("effect.exit", Exit.isSuccess(exit)
            ? "success"
 ...
 */
export const layer = Layer.effect(Tracer.Tracer, Effect.gen(function* () {
    ...
    const contextFor = (span) => {
        while (span?._tag === "Span") {
            if (span instanceof Span)
                return span.runInContext;
    ...
    return Tracer.make({
        span(options) {
            ...
            return parentContext(() => tracing.startActiveSpan(options.name, (span) => new Span(options, AsyncLocalStorage.snapshot(), span)));
        },
...
//# sourceMappingURL=CloudflareTracer.js.map