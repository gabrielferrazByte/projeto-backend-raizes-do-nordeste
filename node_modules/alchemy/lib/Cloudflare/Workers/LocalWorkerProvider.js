...
import { ANSI_RESET, ansiFg, colorsEnabled } from "../../Util/Terminal.js";
import { theme } from "../../Util/Theme.js";
import { formatResourceTag, makeResourceLogger, makeResourceOutput, } from "../../Util/ResourceOutput.js";
import { makeDevLogDirectory, makeDevLogOpener } from "../../Local/DevLog.js";
import { FQN_SEPARATOR } from "../../FQN.js";
...
import { isLiveId, LOCAL_PROVIDERS_URL, LocalRuntimeState, localStorageDirectory, } from "../LocalRuntime.js";
...
const workerStartedMessage = (fqn, elapsed, url, logDir) => {
...
const workerUpdatedMessage = (fqn, elapsed) => {
    const duration = `${Math.round(elapsed)}ms`;
...
export const LocalWorkerProvider = () => LocalProvider.make(Worker, LOCAL_PROVIDERS_URL, Effect.gen(function* () {
    ...
    // directory.
    const workerLogSegments = (worker) => [
        stack.stage,
        ...worker.fqn.split(FQN_SEPARATOR),
    ];
    const workerLogDir = (worker) => devLogDir(...workerLogSegments(worker));
     ...
     */
    const resolveConfig = Effect.fn(function* ({ id, fqn, news, bindings, }) {
        const props = news;
        ...
        const boundEnv = { ...props.env };
        for (const { data } of bindings) {
            if (data.env)
                Object.assign(boundEnv, data.env);
    ...
    const stopContainerWatcher = (id) => Effect.suspend(() => {
     ...
     */
    const ensureContainerWatcher = (worker, restart) => Effect.gen(function* () {
        ...
        for (const namespace of worker.durableObjectNamespaces) {
            ...
            if (image === undefined || !("dockerfile" in image))
                continue;
            const context = path.resolve(image.context ?? ".");
            if (context.split(path.sep).includes(".alchemy"))
                continue;
            watched.set(context, {
                dockerfile: image.dockerfile !== undefined
        ...
        if (existing !== undefined) {
            if (existing.key === key)
                return; // same watcher keeps running
        ...
        const fingerprint = Effect.gen(function* () {
            const hashes = [];
            const walk = (dir) => Effect.gen(function* () {
            ...
            for (const [context, { dockerfile }] of watched) {
                ...
                if (dockerfile !== undefined && !dockerfile.startsWith(context)) {
                    hashes.push(`${dockerfile}:${yield* fs.readFile(dockerfile).pipe(Effect.flatMap(sha256))}`);
                }
            }
        ...
        const watchLoop = Effect.gen(function* () {
            ...
            }).pipe(Stream.debounce("500 millis"), Stream.runForEach(() => Effect.gen(function* () {
                const next = yield* fingerprint;
                if (next === undefined || next === last)
                    return;
                last = next;
                yield* Effect.logInfo(`[${worker.fqn}] Container build context changed, restarting instance`);
                yield* restart;
            })), Effect.catchCause((cause) => Effect.logWarning(`[${worker.fqn}] Container context watch failed`, Cause.squash(cause))));
        });
        const fiber = yield* watchLoop.pipe(Effect.ensuring(Effect.sync(() => {
    ...
    const serveWith = (worker, bundle, proxy) => Semaphore.withPermits(serveLock(worker.fqn), 1)(
    ...
    Effect.uninterruptibleMask((restore) => Effect.gen(function* () {
        ...
        while (true) {
            ...
            const devLog = yield* openDevLog(...workerLogSegments(worker)).pipe(Scope.provide(scope));
            ...
            MutableHashMap.set(localRuntimeState.workerRestarts, worker.name, restartWorker(worker.fqn));
            // Idempotent: the registry keeps ONE watcher per worker FQN
            // across restarts (a new one only when the watched path set
            // changed).
            yield* ensureContainerWatcher(worker, Effect.suspend(() => restartWorker(worker.fqn)));
        ...
        for (const replaced of previous
            ? [...superseded, previous]
            : superseded) {
            yield* Scope.close(replaced, Exit.void).pipe(Effect.catchCause((cause) => Effect.logWarning(`[${worker.fqn}] Failed to stop previous local worker instance`, Cause.squash(cause))));
        }
        return url;
    })));
    /**
     ...
     * updated state.
     */
    const logRestartFailure = (fqn) => Effect.catchCause((cause) => Effect.logWarning(`[${fqn}] Failed to restart local worker`, Cause.squash(cause)));
    const restartWorker = (id) => Effect.suspend(() => {
        const latest = latestServes.get(id);
        if (latest) {
            return serveWith(latest.worker, latest.bundle, latest.proxy).pipe(Effect.asVoid, logRestartFailure(latest.worker.fqn));
        }
        ...
        if (vite) {
            ...
            // on the serve lock).
            return serveVite(vite, { onlyIfConsumersChanged: true }).pipe(Effect.asVoid, logRestartFailure(vite.worker.fqn));
        }
        return Effect.void;
    });
    ...
    const serveBundleStream = Effect.fn(function* (worker, proxy, bundles) {
        ...
        yield* bundles.pipe(Stream.tap((event) => {
            ...
            if (exit._tag === "Success") {
                ...
                // lines stay compact.
                const message = Effect.log(status === "update"
    ...
    const runWorker = Effect.fn(function* (worker) {
        ...
        if (worker.script !== undefined) {
            ...
            yield* Effect.log(workerStartedMessage(worker.fqn, Date.now() - start, proxy.url, workerLogDir(worker)));
            return proxy.url;
        }
    ...
    const serveVite = (args, options) => Semaphore.withPermits(serveLock(args.worker.fqn), 1)(
    ...
    Effect.uninterruptibleMask((restore) => Effect.gen(function* () {
        ...
        if (options?.onlyIfConsumersChanged) {
            ...
            if (workerdScopes.has(worker.fqn) &&
        ...
        while (true) {
            ...
            }, (channel, line) => logResourceOutput(channel, line).pipe(Effect.andThen(Effect.sync(() => devLog.writeLine(line))))).pipe(Scope.provide(scope))).pipe(
            // The scope hangs off `rootScope`, so a failed or
            // interrupted start must close it here — nothing else owns
            // it yet.
            Effect.onExit((exit) => exit._tag === "Failure"
            ...
            yield* child.exitCode.pipe(Effect.flatMap((exitCode) => {
                const message = `[${worker.fqn}] Dev server child exited unexpectedly with code ${exitCode}`;
                return Effect.all([
                    Effect.logWarning(message),
                    proxy.fail(message),
                ]);
            }), Effect.andThen(invalidate), Effect.forkIn(scope));
    ...
    const runSource = Effect.fn(function* (worker) {
        ...
        const source = yield* loadSource(worker.source).pipe(Effect.provideService(AlchemyArtifacts, makeScopedArtifacts(createArtifactStore(), worker.fqn)));
        const devCtx = {
            id: worker.id,
            fqn: worker.fqn,
