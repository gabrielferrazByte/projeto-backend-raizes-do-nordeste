...
const resolveRunner = (basename) => fileURLToPath(import.meta.resolve(`./${basename}${moduleExtension(import.meta.url)}`));
export const startViteChild = (config, onOutput) => Effect.gen(function* () {
    ...
    yield* child.stderr.pipe(Stream.decodeText, Stream.splitLines, Stream.runForEach((line) => onOutput("stderr", line)), Effect.forkScoped);
...
export const runViteBuildChild = (config, onOutput) => Effect.scoped(Effect.gen(function* () {
    ...
    const child = yield* spawner.spawn(ChildProcess.make(process.execPath, isBun ? ["run", runner] : [...nodeLoaderArgs(runner), runner], {
        cwd: config.rootDir,
        stdin: Stream.succeed(serializedConfig),
        stdout: "pipe",
        stderr: "pipe",
