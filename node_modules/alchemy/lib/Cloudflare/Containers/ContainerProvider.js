...
import { repositoryFromImageRef } from "../../Docker/Registry.js";
...
import { retryContainerPublication } from "./ContainerPublication.js";
/**
...
const RegistryDigest = Schema.String.pipe(Schema.check(Schema.isPattern(/^[a-z0-9]+:[a-f0-9]{64}$/i)));
...
const digestFromImageRef = (imageRef) => {
...
export const LiveContainerProvider = () => Provider.effect(ContainerPlatform, Effect.gen(function* () {
    ...
    const applicationConfigurationHash = Effect.fn("applicationConfigurationHash")(function* (scaling, affinities, configuration) {
    ...
    const registryCredentials = Effect.fn("registryCredentials")(function* (props, permissions) {
        const { accountId } = yield* yield* CloudflareEnvironment;
    ...
    const resolveRegistryDigest = Effect.fn("resolveRegistryDigest")(function* (imageRef, credentials) {
        ...
        const request = HttpClientRequest.head(manifestUrl).pipe(HttpClientRequest.basicAuth(credentials.username, credentials.password), HttpClientRequest.setHeader("Accept", "application/vnd.oci.image.index.v1+json, application/vnd.oci.image.manifest.v1+json, application/vnd.docker.distribution.manifest.list.v2+json, application/vnd.docker.distribution.manifest.v2+json"));
        const response = yield* http.execute(request).pipe(Effect.flatMap(HttpClientResponse.filterStatusOk), Effect.mapError((cause) => new ContainerRegistryError({
            reason: "ManifestRequestFailed",
            message: "Failed to resolve the container registry digest",
            imageRef,
            cause,
        })));
        return yield* Schema.decodeUnknownEffect(RegistryDigest)(response.headers["docker-content-digest"]).pipe(Effect.mapError((cause) => new ContainerRegistryError({
    ...
    const resolvePublishedImageRef = Effect.fn("resolvePublishedImageRef")(function* (props, imageRef) {
    ...
    const publishImage = Effect.fn("publishImage")(function* (id, props, build, imageRef, session) {
    ...
    const buildAndPushImage = Effect.fn("buildAndPushImage")(function* (id, props, build, imageRef, imageHash, previousImageRef, session) {
        const { accountId } = yield* yield* CloudflareEnvironment;
        ...
        const candidate = yield* Effect.cached(publishImage(id, props, build, imageRef, session).pipe(Effect.onExit((exit) => Exit.isFailure(exit)
            ? Effect.sync(() => {
        ...
        return {
            ...
            previousDigest: previousImageRef === undefined
                ? undefined
    ...
    const upsertApplication = Effect.fn(function* ({ id, news, bindings, existing, durableObjects, session, }) {
        ...
        if (imageHash !== existing.hash?.image) {
            const published = yield* buildAndPushImage(id, news, build, imageRef, imageHash, existing.hash?.digest === undefined
        ...
        const configurationHash = yield* applicationConfigurationHash(scaling, news.affinities, configuration);
        if (existing.hash?.configuration === configurationHash) {
            yield* Effect.logInfo(`Cloudflare Container update: ${existing.applicationName} has no effective changes`);
    ...
    const getDurableObjects = (bindings) => {
        ...
        const dos = bindings.flatMap((b) => b.data.durableObjects?.namespaceId ? [b.data.durableObjects] : []);
    ...
    return ContainerPlatform.Provider.of({
        ...
        diff: Effect.fn(function* ({ id, olds = {}, news = {}, output, newBindings, oldBindings, }) {
            ...
            const hasUnresolvedAttachment = !hasDurableObjects &&
        ...
        precreate: Effect.fn(function* ({ id, news = {}, session }) {
            ...
            const { build, imageRef, imageHash, dev } = yield* computeImage(id, news, env);
            const published = yield* buildAndPushImage(id, news, build, imageRef, imageHash, undefined, session);
            const configuration = desiredConfiguration(news, env, published.imageRef);
            const configurationHash = yield* applicationConfigurationHash(scalingDefaults(news), news.affinities, configuration);
        ...
        reconcile: Effect.fn(function* ({ id, news = {}, bindings, output, session, }) {
            ...
            const hasUnresolvedAttachment = durableObjects === undefined &&
            ...
            const recordedDurableObjects = existing
                ...
                : output?.accountId === accountId && isLiveId(output.applicationId)
                    ...
                    : undefined;
            const durableObjectsForRecovery = durableObjects ??
                (hasUnresolvedAttachment && recordedDurableObjects?.namespaceId
                    ? recordedDurableObjects
                    : undefined);
            if (hasUnresolvedAttachment &&
                durableObjectsForRecovery === undefined) {
                return yield* Effect.fail(new Error(`Container application "${name}" has an unresolved Durable Object namespace and no recorded attachment. Reconcile its Worker first.`));
            }
            const { build, imageRef, imageHash, dev } = yield* computeImage(id, news, env);
