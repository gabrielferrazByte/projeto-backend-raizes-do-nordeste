...
export const DatasetProvider = () => Provider.effect(Dataset, Effect.gen(function* () {
    ...
    const toAttrs = Effect.fn(function* (dataset) {
        if (!dataset.edgeDeployment || !dataset.edgeDeploymentUrl) {
            return yield* Effect.fail(new Error(`Axiom dataset "${dataset.name}" is missing its edge deployment metadata`));
        }
