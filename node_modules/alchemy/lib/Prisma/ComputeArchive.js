...
const createComputeArchiveFile = Effect.fn(function* (options) {
    const { directory, entrypoint, ignore = [], ignorePrefix, requiredFiles = [], maxUncompressedBytes = MAX_UNCOMPRESSED_BYTES, maxFileBytes = MAX_FILE_BYTES, maxEntries = MAX_ENTRIES, } = options;
    ...
    for (const file of requiredFiles) {
        ...
        if (!isArchivedRegularFile(entries, `bundle/${normalized}`)) {
            return yield* Effect.fail(new Error(`Required file not found in compute artifact: ${normalized}`));
        }
    }
...
const compileIgnorePattern = (input, prefix) => {
