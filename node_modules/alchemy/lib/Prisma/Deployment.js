...
import { getServiceDeployments, createServiceDeployment, } from "@distilled.cloud/prisma/management";
...
const listAppDeployments = (appId) => Effect.gen(function* () {
    const deployments = [];
    let cursor;
    while (true) {
        const page = yield* getServiceDeployments(cursor === undefined
        ...
        if (nextCursor === null) {
            return yield* Effect.fail(new PrismaPaginationError({
                message: "Invalid Prisma Management API pagination response from getServiceDeployments: hasMore was true without a non-empty nextCursor",
            }));
        }
        cursor = nextCursor;
    }
...
const findDeployment = (appId, foundryVersionId) => foundryVersionId === undefined
    ? Effect.succeed(undefined)
    : listAppDeployments(appId).pipe(Effect.flatMap((deployments) => {
    ...
    return {
        ...
        reconcile: Effect.fn(function* ({ news, output }) {
            ...
            let deployment = deploymentId
                ? yield* observeDeployment(deploymentId).pipe(Effect.catchTag("NotFound", () => Effect.succeed(undefined)))
                : undefined;
            if (deployment) {
            ...
            const cleanupCreatedDeploymentOnFailure = (failedDeploymentId, error) => createdDeploymentId === failedDeploymentId
                ? destroyDeployment(failedDeploymentId).pipe(Effect.catch((cleanupError) => Effect.fail(aggregateCleanupFailure("deployment", failedDeploymentId, `/v1/deployments/${failedDeploymentId}`, error, cleanupError))), Effect.andThen(() => Effect.fail(error)))
                : Effect.fail(error);
            if (!deployment) {
            ...
            if (shouldStart) {
                ...
                deployment = yield* Effect.gen(function* () {
                    if (currentDeployment.status !== "running" &&
                        ...
                        if (started) {
                            return yield* waitForDeploymentStatus(currentDeployment.id, "running").pipe(Effect.map((running) => ({
                                ...running,
        ...
        delete: Effect.fn(function* ({ output }) {
            if (isPrismaDevId(output.deploymentId))
                return;
            const deployment = yield* observeDeployment(output.deploymentId).pipe(Effect.catchTag("NotFound", () => Effect.succeed(undefined)));
            if (!deployment)
                return;
