...
import { deleteEnvironmentVariable, getEnvironmentVariables, getEnvironmentVariable, updateEnvironmentVariable, createEnvironmentVariable, } from "@distilled.cloud/prisma/management";
...
const listVariables = (query = {}) => Effect.gen(function* () {
    const variables = [];
    let cursor;
    while (true) {
        const page = yield* getEnvironmentVariables(cursor === undefined ? query : { ...query, cursor });
...
const findVariable = (projectId, cls, key, branchId) => listVariables({
    projectId,
    class: cls,
    key,
    ...(branchId ? { branchId } : {}),
...
const ProviderLive = () => Provider.effect(EnvironmentVariable, Effect.gen(function* () {
    return {
        ...
        list: () => listVariables().pipe(Effect.map((variables) => variables
        ...
        reconcile: Effect.fn(function* ({ news, output }) {
            ...
            if (!variable) {
                ...
                Retry.none, Effect.map((response) => ({
                    ...
                    created: true,
                })), Effect.catchTag("Conflict", () => Effect.fail(new Error(`Prisma environment variable '${news.key}' appeared after the adoption check. Refusing to overwrite its secret; rerun with adoption enabled if it is the intended variable.`))));
