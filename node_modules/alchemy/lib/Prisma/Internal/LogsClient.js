...
import { PrismaApiError } from "../Client.js";
...
const isValidApiPath = (baseUrl, path) => {
...
const buildWebSocketUrl = (baseUrl, path, query) => buildUrl(baseUrl, path, query).pipe(Effect.map((value) => {
...
export const getDeploymentLogsRequest = (deploymentId, query) => Effect.gen(function* () {
    const credentials = yield* Credentials;
    const { apiToken, apiBaseUrl } = yield* credentials;
    const url = yield* buildWebSocketUrl(apiBaseUrl, `/v1/deployments/${pathSegment(deploymentId)}/logs`, logsQuery(query));
    return {
        url,
        headers: {
...
export const getBuildLogsRequest = (buildId, query) => Effect.gen(function* () {
    const credentials = yield* Credentials;
    const { apiToken, apiBaseUrl } = yield* credentials;
...
//# sourceMappingURL=LogsClient.js.map