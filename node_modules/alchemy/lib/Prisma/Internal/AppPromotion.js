...
import { getService, createServicePromote, createServiceRollback, } from "@distilled.cloud/prisma/management";
const DEFAULT_TIMEOUT_SECONDS = 60;
const DEFAULT_POLL_INTERVAL_MS = 1_000;
export const waitForAppDeploymentTarget = Effect.fn(function* (appId, deploymentId, options = {}) {
    ...
    while (true) {
        ...
        if (remainingBeforeObservation <= 0)
            break;
        const observationOption = yield* getService({ serviceId: appId }).pipe(Effect.map((response) => response.data), Effect.result, Effect.timeoutOption(Duration.millis(remainingBeforeObservation)));
 ...
 * target deletion.
 */
export const promoteAppObserved = Effect.fn(function* (appId, deploymentId, options = {}) {
    ...
    if (Result.isSuccess(promoted)) {
        const observation = yield* waitForAppDeploymentTarget(appId, deploymentId, options).pipe(Effect.result);
    ...
    const observation = yield* getService({ serviceId: appId }).pipe(Effect.map((response) => response.data), Effect.result);
    if (Result.isSuccess(observation) &&
    ...
    const repairedObservation = Result.isSuccess(repaired)
        ...
        : yield* getService({ serviceId: appId }).pipe(Effect.map((response) => response.data), Effect.result);
