import { getProjects, getProject, getProjectDatabases, updateProject, createProject, createProjectDatabase, } from "@distilled.cloud/prisma/management";
...
import { extractConnectionSecrets } from "./Client.js";
    ...
    while (true) {
        ...
        if (nextCursor === null) {
            return yield* Effect.fail(new PrismaPaginationError({
                message: "Invalid Prisma Management API pagination response from getProjects: hasMore was true without a non-empty nextCursor",
            }));
        }
        cursor = nextCursor;
    }
...
const findProjectByName = (name) => listProjects().pipe(Effect.flatMap((projects) => {
...
const recoverGeneratedProjectAfterConflict = (name) => findProjectByName(name).pipe(Effect.flatMap((project) => project
...
const listProjectDatabases = (projectId) => Effect.gen(function* () {
    const databases = [];
    let cursor;
    while (true) {
        const page = yield* getProjectDatabases(cursor === undefined
            ? { projectId, limit: 100 }
        ...
        if (nextCursor === null) {
            return yield* Effect.fail(new PrismaPaginationError({
                message: "Invalid Prisma Management API pagination response from getProjectDatabases: hasMore was true without a non-empty nextCursor",
            }));
        }
        cursor = nextCursor;
    }
...
const defaultDatabase = (projectId) => listProjectDatabases(projectId).pipe(Effect.flatMap((databases) => {
...
const observeDesiredDefaultDatabase = (projectName, projectId, expectedDatabaseId, desiredRegion) => Effect.gen(function* () {
    const project = (yield* getProject({ id: projectId }).pipe(Effect.catchTag("NotFound", () => Effect.fail(new DefaultDatabaseConsistencyError(`Prisma project '${projectName}' (${projectId}) is not visible yet while verifying its new default database.`))))).data;
    const database = yield* requireDefaultDatabaseInRegion(yield* defaultDatabase(projectId).pipe(Effect.catchTag("NotFound", () => Effect.fail(new DefaultDatabaseConsistencyError(`Prisma project '${projectName}' default database list is not visible yet.`)))), projectName, desiredRegion);
    ...
    return {
        ...
        list: Effect.fn(function* () {
            ...
            return yield* Effect.forEach(projects, Effect.fn(function* (project) {
                const database = yield* defaultDatabase(project.id).pipe(Effect.catchTag("NotFound", () => Effect.succeed(undefined)));
        ...
        read: Effect.fn(function* ({ id, output, olds = {} }) {
            ...
            const project = projectId
                ? yield* getProject({ id: projectId }).pipe(Effect.map((response) => response.data), Effect.catchTag("NotFound", () => Effect.succeed(undefined)))
        ...
        reconcile: Effect.fn(function* ({ id, news = {}, olds, output }) {
            ...
            let project = outputProjectId
                ? yield* getProject({ id: outputProjectId }).pipe(Effect.map((response) => response.data), Effect.catchTag("NotFound", () => Effect.succeed(undefined)))
                : news.name === undefined
            ...
            if (!project) {
                ...
                Retry.none, Effect.map((response) => {
                ...
                }), Effect.catchTag("Conflict", () => news.name === undefined
                    ? recoverGeneratedProjectAfterConflict(name).pipe(Effect.map((project) => ({
                        project,
                        database: undefined,
                        secrets: {},
            ...
            if (news.createDatabase === false && !createdProject) {
                const existingDefault = yield* defaultDatabase(project.id).pipe(Effect.catchTag("NotFound", () => Effect.succeed(undefined)));
                if (existingDefault) {
            ...
            if (news.createDatabase !== false && !database) {
                database = yield* defaultDatabase(projectId).pipe(Effect.catchTag("NotFound", () => Effect.succeed(undefined)));
            }
            if (news.createDatabase !== false) {
                ...
                if (!database) {
                    ...
                    const created = yield* createProjectDatabase({
                        projectId,
                        region: desiredRegion,
                        isDefault: true,
                    }).pipe(Retry.none, Effect.map((response) => response.data), Effect.catchTag("Conflict", () => defaultDatabase(projectId).pipe(Effect.flatMap((database) => requireDefaultDatabaseInRegion(database, name, desiredRegion)))));
            ...
            if (defaultDatabaseChanged) {
                ...
                const observed = yield* observeDesiredDefaultDatabase(name, project.id, changedDatabaseId, news.region ?? "us-east-1");
