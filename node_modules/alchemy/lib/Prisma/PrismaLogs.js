...
import { getDeploymentLogsRequest } from "./Internal/LogsClient.js";
...
export const tailDeploymentLogs = (deploymentId, query) => Stream.unwrap(Effect.gen(function* () {
...
}).pipe(Effect.mapError((cause) => new PrismaLogStreamError({
    message: "Failed to resolve Prisma credentials for log tailing",
    cause,
}))));
const tailDeploymentLogsWith = (credentials, deploymentId, query) => Stream.callback((queue) => Effect.gen(function* () {
    ...
    const connect = (cursor) => Effect.gen(function* () {
        if (stopped)
            return;
        const request = yield* getDeploymentLogsRequest(deploymentId, cursor === undefined ? query : { ...query, cursor }).pipe(Effect.provideService(Credentials, Effect.succeed(credentials)), Effect.mapError(() => new PrismaLogStreamError({
