...
import { deleteBucket, getBuckets, getBucket, createBucket, } from "@distilled.cloud/prisma/management";
...
const listBuckets = () => Effect.gen(function* () {
    const buckets = [];
    let cursor;
    while (true) {
        ...
        if (nextCursor === null) {
            return yield* Effect.fail(new PrismaPaginationError({
                message: "Invalid Prisma Management API pagination response from getBuckets: hasMore was true without a non-empty nextCursor",
            }));
        }
        cursor = nextCursor;
    }
...
const ProviderLive = () => Provider.effect(Bucket, Effect.gen(function* () {
    return {
        ...
        read: Effect.fn(function* ({ output }) {
            ...
            if (!bucketId)
                return undefined;
            const bucket = yield* getBucket({ bucketId }).pipe(Effect.map((response) => response.data), Effect.catchTag("NotFound", () => Effect.succeed(undefined)));
        ...
        reconcile: Effect.fn(function* ({ news, output }) {
            ...
            const observed = bucketId
                ? yield* getBucket({ bucketId }).pipe(Effect.map((response) => response.data), Effect.catchTag("NotFound", () => Effect.succeed(undefined)))
                : undefined;
            if (observed) {
