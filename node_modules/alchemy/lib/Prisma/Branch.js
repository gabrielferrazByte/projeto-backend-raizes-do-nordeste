...
import { deleteBranch, getBranch, getProjects, getProjectBranches, updateBranch, createProjectBranch, } from "@distilled.cloud/prisma/management";
...
const listBranches = (projectId, query) => Effect.gen(function* () {
    const branches = [];
    let cursor;
    while (true) {
...
const findBranch = (projectId, gitName) => listBranches(projectId, { gitName }).pipe(Effect.flatMap((branches) => {
...
const ProviderLive = () => Provider.effect(Branch, Effect.gen(function* () {
    return {
        ...
        list: Effect.fn(function* () {
            ...
            const branches = yield* Effect.forEach(projects, (project) => listBranches(project.id).pipe(Effect.catchTag("NotFound", () => Effect.succeed([]))), { concurrency: 8 });
        ...
        read: Effect.fn(function* ({ id, output, olds }) {
            ...
            const branch = branchId
                ? yield* getBranch({ branchId }).pipe(Effect.map((response) => response.data), Effect.catchTag("NotFound", () => Effect.succeed(undefined)))
        ...
        reconcile: Effect.fn(function* ({ id, news, output }) {
            ...
            if (!branch) {
                ...
                Retry.none, Effect.map((response) => response.data), Effect.catchTag("Conflict", () => Effect.fail(new Error(`Prisma branch '${gitName}' appeared after the adoption check. Refusing to take it over; rerun with adoption enabled if it is the intended branch.`))));
            }
        ...
        delete: Effect.fn(function* ({ output }) {
            ...
            if (branch.isDefault) {
                ...
                }).pipe(Effect.map((response) => response.data), Effect.catchTag("NotFound", () => Effect.fail(new Error(`Cannot restore previous default Prisma branch '${previousDefaultBranchId}' because it no longer exists. Promote another branch explicitly before deleting '${branch.gitName}'.`))));
