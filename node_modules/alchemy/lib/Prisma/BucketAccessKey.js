...
import { deleteBucketKey, getBucketKeys, createBucketKey, } from "@distilled.cloud/prisma/management";
...
import { requiredSecretValue, } from "./Internal/Observed.js";
...
const listKeys = (bucketId) => Effect.gen(function* () {
    const keys = [];
    let cursor;
    while (true) {
        const page = yield* getBucketKeys(cursor === undefined
            ? { bucketId, limit: 100 }
        ...
        if (nextCursor === null) {
            return yield* Effect.fail(new PrismaPaginationError({
                message: "Invalid Prisma Management API pagination response from getBucketKeys: hasMore was true without a non-empty nextCursor",
            }));
        }
        cursor = nextCursor;
    }
...
const uniqueKeyNamed = (bucketId, expectedName) => listKeys(bucketId).pipe(Effect.flatMap((keys) => {
