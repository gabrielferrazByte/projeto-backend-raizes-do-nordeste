...
const strategies = [
    ...
    },
    {
        ...
        execute: (options) => Effect.gen(function* () {
            const command = yield* packageCliCommand(options.appPath, "vite", ["build"], "Could not find an installed Vite CLI. Install `vite` in the application or workspace before deploying.");
...
export const runComputeStaticBuild = Effect.fn(function* (options) {
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    const cwd = options.cwd ? path.resolve(options.cwd) : options.appPath;
    const outdir = normalizeRelativePath(options.outdir);
    if (outdir === undefined) {
        return yield* Effect.fail(new Error("Static site outdir must be a relative path without parent segments."));
    }
    const indexPage = normalizeRelativePath(options.indexPage ?? "index.html");
    if (indexPage === undefined || indexPage === ".") {
        return yield* Effect.fail(new Error("Static site indexPage must be a relative file path without parent segments."));
    }
    ...
    if (!(yield* directoryExists(sourceDir))) {
        return yield* Effect.fail(new Error(`Static site build did not produce an output directory at ${sourceDir}.`));
    }
    const indexStat = yield* fs
        .stat(path.join(sourceDir, indexPage))
        .pipe(Effect.catch(() => Effect.succeed(undefined)));
    if (indexStat?.type !== "File") {
        return yield* Effect.fail(new Error(`Static site build did not produce ${indexPage} inside ${sourceDir}.`));
    }
    ...
    const build = Effect.gen(function* () {
        return yield* withStagingDeadline(Effect.gen(function* () {
            ...
            yield* accountStagingEntry(budget, serverPath, yield* Effect.sync(() => new TextEncoder().encode(source).byteLength));
...
const staticSiteServerSource = (options) => [
