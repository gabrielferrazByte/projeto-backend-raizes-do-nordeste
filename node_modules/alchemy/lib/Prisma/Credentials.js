...
import { PrismaEnvironment, fromProfile } from "./PrismaEnvironment.js";
export { Credentials } from "@distilled.cloud/prisma";
const toConfigError = (cause) => new ConfigError({
    message: `Failed to resolve Prisma credentials: ${cause?.message ?? String(cause)}`,
});
/**
...
export const fromAuthProvider = () => Layer.effect(Credentials, Effect.gen(function* () {
    ...
    const environment = Layer.buildWithScope(fromProfile().pipe(Layer.provide(Layer.mergeAll(Layer.succeed(AuthProviders, authProviders), Layer.succeed(ProfileStore, profile)))), scope).pipe(Effect.map((context) => Context.get(context, PrismaEnvironment)));
    return yield* Effect.cached(environment.pipe(Effect.map((env) => ({
        apiToken: env.serviceToken,
        apiBaseUrl: env.baseUrl,
