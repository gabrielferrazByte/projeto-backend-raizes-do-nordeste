...
import { DeleteProductFeature, GetProducts, GetProductFeatures, GetProductFeature, CreateProductFeature, } from "@distilled.cloud/stripe/stripe";
import * as Effect from "effect/Effect";
import { isResolved } from "../Diff.js";
import * as Provider from "../Provider.js";
import { Resource } from "../Resource.js";
import { tagRecord } from "../Tags.js";
import { alchemyMetadataKeys } from "./Metadata.js";
 ...
 * @resource
 */
export const ProductFeature = Resource("Stripe.ProductFeature");
const toAttrs = (product, feature) => ({
    id: feature.id,
    product,
...
const getById = (product, id) => GetProductFeature({ product, id }).pipe(Effect.catchIf(isMissing, () => Effect.succeed(undefined)));
const listFeatures = Effect.fn(function* (product) {
    const features = [];
    let startingAfter;
...
const listProductsByActive = Effect.fn(function* (active) {
    const products = [];
    let startingAfter;
    for (let page = 0; page < LIST_MAX_PAGES; page++) {
...
const listAlchemyProducts = Effect.fn(function* () {
    const [active, inactive] = yield* Effect.all([listProductsByActive(true), listProductsByActive(false)], { concurrency: 2 });
    const seen = new Set();
    const products = [];
...
const shouldReplace = (news, output) => {
    if (output === undefined)
        return false;
    if (news.product !== output.product)
        return true;
    if (news.entitlementFeature !== output.entitlementFeature)
        return true;
    return false;
};
export const ProductFeatureProvider = () => Provider.succeed(ProductFeature, {
    ...
    read: Effect.fn(function* ({ output, olds }) {
        ...
        const entitlementFeature = output?.entitlementFeature ??
        ...
        if (existing === undefined || product === undefined)
            return undefined;
    ...
    list: Effect.fn(function* () {
        ...
        const rows = yield* Effect.forEach(products, (product) => listFeatures(product.id).pipe(Effect.map((features) => features.map((feature) => toAttrs(product.id, feature)))), { concurrency: LIST_CONCURRENCY });
        return rows.flat();
    }),
    reconcile: Effect.fn(function* ({ news, output, instanceId }) {
        ...
        if (current === undefined) {
            ...
            }), Effect.catchIf((e) => e._tag === "InvalidRequestError" || e._tag === "Conflict", (e) => observe({
                product: news.product,
                entitlementFeature: news.entitlementFeature,
            }).pipe(Effect.flatMap((found) => found !== undefined ? Effect.succeed(found) : Effect.fail(e)))));
        }
