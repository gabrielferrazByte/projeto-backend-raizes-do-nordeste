...
export const toMetadataKey = (tagKey) => tagKey.startsWith(TAG_PREFIX)
    ...
    : tagKey;
export const toTagKey = (metadataKey) => metadataKey.startsWith(ALCHEMY_METADATA_PREFIX)
    ? `${TAG_PREFIX}${metadataKey.slice(ALCHEMY_METADATA_PREFIX.length)}`
    : metadataKey;
/** Stripe metadata values are capped at 500 characters. */
export const sanitizeMetadataValue = (value) => value.slice(0, 500);
export const toMetadata = (tags) => Object.fromEntries(Object.entries(tags ?? {}).map(([key, value]) => [
    toMetadataKey(key).slice(0, 40),
    sanitizeMetadataValue(value),
]));
export const fromMetadata = (metadata) => Object.fromEntries(Object.entries(metadata ?? {}).map(([key, value]) => [
    toTagKey(key),
    value,
]));
