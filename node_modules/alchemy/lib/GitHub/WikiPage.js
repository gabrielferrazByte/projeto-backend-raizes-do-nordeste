...
import { effectiveGitHubBaseUrl, gitHubBaseUrlChanged } from "./Octokit.js";
/**
 * A GitHub wiki page.
 *
 ...
 * @resource
 */
export const WikiPage = Resource("GitHub.WikiPage");
...
const pageNameFor = (title) => Effect.gen(function* () {
    if (title.trim() !== title ||
        title.length === 0 ||
        ...
        return yield* new InvalidWikiPage({
            message: "Wiki page titles must be nonempty, trimmed names without path separators or control characters.",
        });
    }
    return title.replace(/\s+/g, "-");
});
export const wikiRepository = Effect.fn(function* (props) {
    yield* pageNameFor(props.title);
    if (![props.owner, props.repository].every((part) => /^[A-Za-z0-9_.-]+$/.test(part) && part !== "." && part !== "..")) {
        return yield* new InvalidWikiPage({
...
const gitEnvironment = Effect.fn(function* (repository, directory) {
...
const git = Effect.fn(function* (directory, env, ...args) {
    const operation = args[0];
    const result = yield* exec(ChildProcess.make("git", [
        "--literal-pathspecs",
        "-c",
        "credential.helper=",
        "-c",
        ...
        ...args,
    ], { cwd: directory, env, extendEnv: false })).pipe(Effect.mapError(() => new WikiGitError({
        operation,
        reason: "command",
        message: `Unable to run git ${operation}. Ensure Git is installed and executable.`,
    })));
    if (result.exitCode !== 0) {
        const reason = /Repository not found|repository '.+' (?:not found|does not exist)|does not appear to be a git repository/i.test(result.stderr)
            ? "missing"
            : operation === "push" &&
...
}, Effect.scoped, Effect.timeout("20 seconds"));
const checkout = Effect.fn(function* (repository) {
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    ...
    yield* git(directory, env, "clone", "--quiet", "--", repository.remote, "wiki").pipe(Effect.catchTag("WikiGitError", (error) => Effect.fail(error.reason === "missing"
    ...
    const run = (...args) => git(cwd, env, ...args);
    const files = (yield* run("ls-files", "-z")).split("\0").filter(Boolean);
    return { cwd, run, files };
});
const pageFiles = (files, pageName) => Object.values(extensions)
    .flatMap((formats) => formats.map((extension) => `${pageName}.${extension}`))
    .filter((file) => files.includes(file));
const attributes = Effect.fn(function* (repository, props, wiki, file) {
    return {
        title: props.title,
...
export const readWikiPage = Effect.fn(function* (repository, props) {
...
}, Effect.scoped, (effect) => effect.pipe(Effect.catchTag("WikiRepositoryUnavailable", () => Effect.succeed(undefined))));
const retryConcurrentPush = (effect) => effect.pipe(Effect.retry({
    while: (error) => error instanceof WikiGitError && error.reason === "conflict",
    schedule: Schedule.spaced("200 millis"),
    times: 3,
}), Effect.timeout("60 seconds"));
export const syncWikiPage = Effect.fn(function* (repository, props) {
    const pageName = yield* pageNameFor(props.title);
    const wiki = yield* checkout(repository);
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
...
}, Effect.scoped, retryConcurrentPush);
export const deleteWikiPage = Effect.fn(function* (repository, props) {
    ...
    yield* wiki.run("push", "--quiet", "origin", "HEAD");
}, Effect.scoped, retryConcurrentPush, (effect) => effect.pipe(Effect.catchTag("WikiRepositoryUnavailable", () => Effect.void)));
export const WikiPageProvider = () => Provider.succeed(WikiPage, {
    stables: ["pageName"],
