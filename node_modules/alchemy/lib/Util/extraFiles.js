...
import { initialCwd } from "./Node.js";
...
export const isContextRootDest = (dest) => dest === "." || dest === "";
/**
 ...
 */
export const posixRelUnder = (root, file, path) => {
 ...
 */
export const contextRootOf = (main, extraFiles, path, resolveSource) => {
...
export const extraFileDestination = (destination) => {
    const normalized = destination.replaceAll("\\", "/").replace(/^\/+/, "");
    if (isContextRootDest(normalized))
        return CONTEXT_ROOT_DEST;
    const parts = normalized
        .split("/")
        .filter((part) => part.length > 0 && part !== "." && part !== "..");
    return parts.length === 0 ? "dist" : parts.join("/");
};
export const resolveExtraSource = (source, path) => (path.isAbsolute(source) ? source : path.resolve(initialCwd, source));
const skipCopySegment = (segment) => segment === ".git" || segment === ".alchemy";
export const hashExtraFiles = Effect.fn(function* (extraFiles) {
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    const entries = yield* Effect.all((extraFiles ?? []).map((extra) => Effect.gen(function* () {
        ...
        const hash = stat.type === "Directory"
            ? yield* hashDirectory({
                cwd: source,
                memo: {
 ...
 */
export const copyTree = Effect.fn(function* (from, to) {
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    const stat = yield* fs
        .stat(from)
    ...
    yield* Effect.all(names.flatMap((name) => {
        if (name.split(/[\\/]/).some(skipCopySegment))
            return [];
        return [
            Effect.gen(function* () {
...
export const copyExtraFiles = Effect.fn(function* (contextDir, extraFiles, options) {
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    yield* Effect.all((extraFiles ?? []).map((extra) => Effect.gen(function* () {
        const source = resolveExtraSource(extra.source, path);
        ...
        if (isContextRootDest(destName)) {
            ...
            if (stat.type === "Directory") {
                ...
                yield* Effect.all(names.map((name) => copyTree(path.join(source, name), path.join(contextDir, name))), { concurrency: ioConcurrency });
            }
            else {
...
//# sourceMappingURL=extraFiles.js.map