...
const writeBytes = (buf, offset, value, length) => {
...
const walkDirectory = (root) => Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    const files = [];
    const walk = (dir, rel) => Effect.gen(function* () {
...
const fileBlocks = (rel, size, type, mode) => {
 ...
 */
export const tarGzipDirectory = Effect.fn(function* (root, options) {
    const fs = yield* FileSystem.FileSystem;
    ...
    for (const file of files) {
        ...
        chunks.push(...fileBlocks(file.rel, content.byteLength, "0", options?.preserveMode ? file.mode : 0o644));
        chunks.push(content);
...
//# sourceMappingURL=tarGzip.js.map