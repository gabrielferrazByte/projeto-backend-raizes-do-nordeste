...
import { SERVICE_TOKEN_ENV_VAR, claimedExpiresAt, claimedIdentity, createCli, credentialWorkspaceId, credentialWorkspaceMismatchError, credentialsRequiredError, defineCommand, defineCommandFamily, defineConfigSection, defineSessionCommand, detectCI, emptyServiceTokenError, flag, loadConfig, noSessionForWorkspaceError, positional, readActiveAccessToken, realpathOr, resolveSectionOverChain, telemetryCommandGroup } from "@prisma/cli-engine";
...
import fs, { mkdir, readFile, readdir, rename, rm, rmdir, stat, unlink, writeFile } from "node:fs/promises";
		...
		const httpResult = new Promise((resolve, reject) => {
			...
			server.on("request", async (req, res) => {
				...
				try {
					...
					res.end(options.uiContext === "prisma-plugin" ? "Sign-in couldn’t be completed. Return to your ChatGPT conversation to try again." : "Sign-in could not be completed. Return to your terminal.");
					settle(() => reject(error));
					return;
				}
			});
...
var LoginState = class {
	...
	async handleCallback(url) {
		...
		if (error === "access_denied") throw new CliStructuredError("AUTH.LOGIN_DENIED", "Sign-in was not authorized.", { nextActions: [{
			kind: "run-command",
...
function renderSuccessPage(workspaceName, uiContext) {
	let body = workspaceName ? `Your terminal is now connected to your ${escapeHtml(workspaceName)} workspace. Head back to your terminal to continue.` : "Your terminal is now connected to your Prisma workspace. Head back to your terminal to continue.";
	if (uiContext === "prisma-plugin") body = "You’re connected to Prisma. Return to your ChatGPT conversation.";
	return `<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
...
<body>
  ...
  <main>
    ...
    <p>${body}</p>
    ${uiContext === "prisma-plugin" ? "" : `<section class="skills">
  ...
  ${uiContext === "prisma-plugin" ? "" : `<script>
    (() => {
...
async function readSkillsStatus(cwd, options) {
	...
	const unconfiguredDirs = agentSkillDirs(KNOWN_AGENTS.filter((agent) => !agents.includes(agent)));
	...
	return {
		...
		packages,
		skills,
		orphans: options?.orphans === false ? [] : await findOrphanedSkills(projectRoot, dirs, new Set(sources.keys()), unconfiguredDirs),
...
*/
async function findOrphanedSkills(projectRoot, dirs, provided, unconfiguredDirs = []) {
	const orphans = /* @__PURE__ */ new Map();
	const scan = [...dirs.map((dir) => ({
		dir,
		wanted: provided
	})), ...unconfiguredDirs.map((dir) => ({
		dir,
...
function projectConfigLoader(cwd) {
...
*/
async function readProjectSkillsConfig(load, configPath) {
	const loaded = await load(configPath);
	if (loaded.diagnostics.some((entry) => entry.diagnostic.severity === "error") || loaded.files.length === 0) return null;
	const resolved = resolveSectionOverChain(skillsConfigSection, loaded.files);
	if (!resolved.ok) return null;
	const section = skillsConfigSection.validate(resolved.value, resolved.provenance);
...
async function resolveAgentSetupTipCommand(ctx) {
	...
	try {
		const config = await readProjectSkillsConfig(projectConfigLoader(ctx.cwd), ctx.configPath);
...
const UNVERIFIED_CREDENTIAL_NOTICE = "Could not reach Prisma to confirm this sign-in. Showing what the local credential says.";
...
function normalizeStoredSessionUser(value) {
	...
	if (id === void 0 && email === void 0 && name === void 0) return;
	return {
		...id === void 0 ? {} : { id },
		...email === void 0 ? {} : { email },
		...name === void 0 ? {} : { name }
...
async function sessionsForDisplay(manager) {
	if (isAccountAwareCredentialManager(manager)) return manager.enrichSessions();
	return manager.sessions();
}
function sessionIdentity(session) {
	return isAccountSession(session) ? session.identity : void 0;
}
function isAccountSession(session) {
	return "identity" in session;
}
function isAccountAwareCredentialManager(manager) {
	return "enrichSessions" in manager && typeof manager.enrichSessions === "function";
}
/**
...
var FileCredentialManager = class {
	...
	*  used to be acting as. */
	...
	async sessions() {
		return storedSessions(await readCredentialState(this.#filePath));
	}
	async enrichSessions() {
		if (this.#fetchSessionMetadata === void 0) return this.sessions();
		const state = await readCredentialState(this.#filePath);
		const now = Date.now();
		const candidates = state.sessions.filter((session) => lacksFetchableMetadata(session, now));
		if (candidates.length === 0) return storedSessions(state);
		const fetched = await Promise.all(candidates.map(async (session) => ({
			workspaceId: session.workspaceId,
			token: session.token,
			metadata: await this.#lookUpSessionMetadata(session)
		})));
		if (fetched.every((result) => result.metadata === void 0)) return this.sessions();
		const byWorkspaceId = new Map(fetched.map((result) => [result.workspaceId, result]));
		return this.#mutate((current) => {
			let changed = false;
			const sessions = current.sessions.map((session) => {
				const fetchedSession = byWorkspaceId.get(session.workspaceId);
				if (fetchedSession === void 0 || fetchedSession.token !== session.token) return session;
	...
	async createSession(credential, workspaceId) {
		...
		const { workspaceName: name, user } = await this.#lookUpSessionMetadata(credential) ?? {};
		return this.#mutate((state) => {
			const record = state.sessions.find((session) => session.workspaceId === workspaceId);
			if (record === void 0) throw credentialsRequiredError("session-ended");
			if (record.token !== credential.token || name === void 0 && user === void 0) return { result: toSession(record) };
			const enriched = {
				...record,
				...name === void 0 ? {} : { name },
				...user === void 0 ? {} : { user }
			...
			return {
				state: {
					...state,
					sessions: state.sessions.map((session) => session.workspaceId === workspaceId ? enriched : session)
				},
				result: toSession(enriched)
			};
		});
	}
	...
	*  milliseconds, so anything still here is one. */
	...
	async #lookUpSessionMetadata(credential) {
		if (this.#fetchSessionMetadata === void 0) return void 0;
		try {
			const metadata = await this.#fetchSessionMetadata(credential);
			const workspaceName = metadata?.workspaceName?.trim() || void 0;
			const user = normalizeStoredSessionUser(metadata?.user);
			if (workspaceName === void 0 && user === void 0) return void 0;
			return {
				workspaceName,
				user
			};
...
function lacksFetchableMetadata(session, now) {
	if (session.expiresAt !== void 0 && Date.parse(session.expiresAt) <= now) return false;
	if (session.name === void 0) return true;
	return session.user === void 0 && claimedIdentity(session.token)?.userId !== void 0;
}
function storedSessions(state) {
	return {
...
function storedIdentity(record) {
	const user = record.user;
	return user === void 0 ? claimedIdentity(record.token) : {
		userId: user.id,
		email: user.email,
		name: user.name
	};
...
function sessionChoiceLabel(session) {
	const user = sessionUserLabel(session);
	return user === void 0 ? sessionLabel(session) : `${sessionLabel(session)} — ${user}`;
}
...
function ambiguousSessionRefError(ref, matches) {
	return new CliStructuredError("AUTH.WORKSPACE_AMBIGUOUS", `More than one workspace session is named '${ref}'.`, {
		why: `Matching sessions: ${matches.map((match) => {
			const user = sessionUserLabel(match);
			return user === void 0 ? match.workspaceId : `${match.workspaceId} (${user})`;
		}).join(", ")}.`,
		meta: {
...
const authLoginCommand = defineCommand({
	...
	args: { flags: { uiContext: flag.enum({
		values: ["prisma-plugin"],
		brief: "Show browser completion guidance for the Prisma plugin in ChatGPT (default: terminal guidance)"
	}) } },
	help: {
		summary: "Log in to your Prisma platform account",
		description: "Opens a browser sign-in and stores a session for one workspace, the account-level container that holds your Projects. Run it again to add a session for another workspace; 'auth workspace use' switches between stored sessions. In CI or other non-interactive environments, skip login and set PRISMA_SERVICE_TOKEN instead.",
		examples: ["auth login"]
	},
...
const authLogoutCommand = defineCommand({
	...
	help: {
		...
		description: "Ends every workspace session stored on this machine. To end just one and keep the others, use 'auth workspace logout' instead. A credential supplied via PRISMA_SERVICE_TOKEN is not stored here and stays in force.",
...
async function fetchedIdentity(api, signal) {
	...
	try {
		...
		if (data === void 0) return { kind: "inconclusive" };
		...
		if (CliStructuredError.is(cause)) {
			if (cause.code === "CLI.CREDENTIALS_REQUIRED") return { kind: "signed-out" };
			if (cause.code === "AUTH.SERVICE_TOKEN_REJECTED") throw cause;
		}
...
const authWhoamiCommand = defineCommand({
	help: {
		...
		description: "Shows which identity the CLI is acting as and which workspace its commands will target. Run it to check you are in the right workspace before creating or deleting resources, or to see whether a PRISMA_SERVICE_TOKEN credential is overriding your stored sessions.",
		examples: ["auth whoami", "auth whoami --json"]
	},
	handler: async (_args, ctx) => {
		const active = await ctx.activeCredential();
		const lookup = active === null ? void 0 : await fetchedIdentity(ctx.api, ctx.signal);
		const credential = lookup?.kind === "signed-out" ? null : active;
		const identity = credential === null ? null : mergedIdentity(credential.identity, lookup?.kind === "confirmed" ? lookup.identity : void 0);
		const result = {
...
function listPresentations$9(result) {
	...
	return {
		...
		stdout: () => result.sessions.map((session) => [
			sessionLabel(session),
			session.workspaceId,
			session.workspaceId === result.selectedWorkspaceId ? "current" : ""
		].join("  ").trimEnd()),
...
const authWorkspaceListCommand = defineCommand({
	...
	help: {
		...
		description: "Each 'auth login' stores one session per workspace. This lists the sessions on this machine and marks the current one, which every workspace-scoped command targets unless a PRISMA_SERVICE_TOKEN credential is set; that credential overrides stored sessions.",
...
const authWorkspaceLogoutCommand = defineCommand({
	...
	help: {
		...
		description: "Removes the stored session for one workspace and leaves the others in place. To clear every session on this machine, use 'auth logout'.",
...
async function sessionsForRef(manager, ref) {
	const stored = await manager.sessions();
	if (resolveSessionRef(stored.sessions, ref).kind !== "no-match") return stored;
...
async function promptForSession(stored, select) {
	...
	const workspaceId = await select("Select a workspace", stored.sessions.map((session) => ({
		...
		label: `${sessionChoiceLabel(session)} (${session.workspaceId})${session.workspaceId === stored.selectedWorkspaceId ? " current" : ""}`
	})));
...
const branchListCommand = defineCommand({
	help: {
		...
		description: "A Branch maps to a Git branch of the connected repository. Each one is an isolated environment with its own services, databases, buckets, and environment variables: the production branch serves live traffic, every other branch is a preview.",
		examples: ["branch list", "branch list --project my-app"]
	},
	args: { flags: { project: flag.string({
		brief: "Project id or name (default: the project this directory is linked to)",
		placeholder: "id-or-name"
	}) } },
...
const bucketCreateCommand = defineCommand({
	...
	help: {
		...
		description: "Creates blob storage in a Branch of the project. A bucket holds no credentials of its own: mint them with 'bucket key create', which prints them once.",
		examples: [
			"bucket create",
...
const bucketDeleteCommand = defineCommand({
	...
	help: {
		...
		description: "Deletion is permanent: the stored objects are destroyed and every access key stops working. The exact bucket id is the consent token; pass it with --confirm to run non-interactively.",
...
const bucketKeyCreateCommand = defineCommand({
	args: {
		...
		flags: {
			role: flag.enum({
				brief: "Access role; read_write unless set, use read for consumers that only download",
				values: ["read", "read_write"]
			}),
			name: flag.string({
				brief: "Key display name; use it to record which consumer holds the key (auto-generated if omitted)",
				placeholder: "name"
			})
		}
	},
	help: {
		...
		description: "Mints S3-compatible credentials for one bucket, shown exactly once; store them now, nothing prints them again. Give each consumer its own key so one can be revoked without breaking the others, and grant read-only consumers --role read.",
		examples: [
...
const bucketKeyDeleteCommand = defineCommand({
	...
	help: {
		...
		description: "The key's credentials stop working immediately; anything still using them loses access to the bucket. The bucket and its other keys are untouched.",
...
const bucketKeyListCommand = defineCommand({
	...
	help: {
		...
		description: "Shows each key's id, name, and role. Key secrets print once, at 'bucket key create'; listing never reveals them.",
...
const bucketListCommand = defineCommand({
	...
	help: {
		...
		description: "A bucket is blob storage for files and uploads. Buckets are branch-bound: each belongs to one Branch, the isolated environment for one Git branch.",
		examples: [
			"bucket list",
...
const feedbackCommand = defineCommand({
	help: {
		...
		description: "Anonymous unless --email is passed. Every submission includes the CLI version, node version, and OS platform/arch, and nothing else.",
...
const WORKFLOW_DOCS_URL = "https://github.com/prisma/cloud-deploy-action#readme";
...
const WORKFLOW_DETAIL = "Deploys run from the repository's GitHub Actions workflow (prisma/cloud-deploy-action), which must exist on each pushed branch; the platform does not build on push.";
const WORKFLOW_NEXT_STEP = "If the repository has no .github/workflows/prisma-deploy.yml yet, add one running prisma/cloud-deploy-action@v1 with `id-token: write`; a push of any branch that lacks the file deploys nothing.";
...
const gitDisconnectCommand = defineCommand({
	...
	help: {
		...
		description: "Stops deploy-on-push by removing the repository connection. Nothing already deployed changes: existing branches, services, and databases stay as they are.",
...
* that must stay.
*/
async function removeEmptyHarnessDirs(projectRoot, dir) {
	for (let current = dir; current !== "." && current !== ""; current = path.dirname(current)) try {
...
const skillsSyncCommand = defineCommand({
	help: {
		summary: "Copy the AI-agent instruction files (skills) from installed Prisma packages into this project",
		description: "Agent skills are instruction files that teach AI coding agents (Claude Code, Cursor, and others) how to use the installed Prisma packages. They ship inside the packages themselves, so they always describe the version in use. Sync copies them into the skill directories of the agents that skills.agents in prisma.config.ts names (every known agent when unset), and removes copies whose package is gone or whose agent is no longer named there — agents: [] removes them all. It does nothing, and exits 0, when everything is already current.",
...
*  like with like. */
function governedByAncestorConfig(files, cwd) {
...
const ANCESTOR_SKIPPED_POSTINSTALL = {
	report: {
		outcome: "skipped",
	...
	lines: [summary("info", "Skipped the postinstall hook and the prisma dev dependency: another prisma.config.ts already governs this directory, so both belong at the repository root. Pass --postinstall to add them here anyway.")],
	diagnostics: []
};
const ANCESTOR_SKIPPED_SKILLS = {
	report: {
		outcome: "skipped",
		reason: "governing-config",
		sync: null
	},
	lines: [summary("info", "Skipped the skills sync: another prisma.config.ts already governs this directory, so the skills belong at the repository root. Pass --skills with your agents to sync them here anyway.")],
	diagnostics: []
};
...
async function postinstallStep(flag, deferToAncestor, cwd) {
	if (flag === false) return SKIPPED_POSTINSTALL;
	if (deferToAncestor && flag === void 0) return ANCESTOR_SKIPPED_POSTINSTALL;
...
const initCommand = defineCommand({
	help: {
		summary: "Prepare this repository for Prisma: config file, dev dependency, AI-agent skills",
	...
	handler: async (args, ctx) => {
		...
		const postinstall = await postinstallStep(args.flags.postinstall, deferToAncestor, ctx.cwd);
		const config = await scaffoldConfigStep(ctx.cwd, skillsFlag.kind === "skip" ? [] : skillsFlag.agents, ctx.config.agentsConfigured);
		let skills;
		if (skillsFlag.kind === "skip") skills = SKIPPED_SKILLS;
		else if (deferToAncestor && args.flags.skills === void 0) skills = ANCESTOR_SKIPPED_SKILLS;
		else skills = await syncSkillsStep(ctx.cwd, skillsFlag.agents, ctx.config.check);
		const result = {
...
const postgresBackupListCommand = defineCommand({
	...
	help: {
		...
		description: "Backups are created automatically by the platform. Use the listed backup ids with 'postgres backup restore'.",
...
const postgresBackupRestoreCommand = defineCommand({
	args: {
		...
		flags: {
			...
			sourceDatabase: flag.string({
				brief: "Database the backup belongs to; use when restoring another database's backup into the target",
				placeholder: "database"
			}),
			project: projectFlag$1,
			branch: branchFlag$1
	...
	help: {
		...
		description: "Replaces the target database's current state with the backup's contents; anything written since that backup is lost, so the exact target database id is the consent token. To restore another database's backup into this one, name where the backup came from with --source-database.",
...
const postgresConnectionCreateCommand = defineCommand({
	args: {
		...
		flags: {
			name: flag.string({
				brief: "Connection name; use it to record which consumer holds the credential",
				placeholder: "name"
			}),
			project: projectFlag$1,
			branch: branchFlag$1
	...
	help: {
		summary: "Create an additional connection URL for a database; printed exactly once",
		description: "Mints an additional credential for an existing database, shown exactly once. Give each consumer (an app, CI, a teammate) its own named connection, so its access can later be rotated or revoked without touching the others.",
...
const postgresConnectionDeleteCommand = defineCommand({
	...
	help: {
		...
		description: "Revokes the credential: anything still using its URL loses access to the database. The database itself and its other connections are untouched. The exact connection id is the consent token.",
...
const postgresConnectionListCommand = defineCommand({
	...
	help: {
		...
		description: "Each connection is an independent credential for the same database, so one consumer's access can be rotated or revoked without breaking the others. Listing shows metadata only; connection URLs print once, at create or rotate, and never again.",
...
const postgresConnectionRotateCommand = defineCommand({
	...
	help: {
		...
		description: "Replaces the connection's credentials: the old URL stops working and the new one prints exactly once. Use it after a suspected leak or on a rotation schedule. The exact connection id is the consent token.",
...
const postgresCreateCommand = defineCommand({
	args: {
		...
		flags: {
			region: flag.string({
				brief: "Prisma Postgres region id; set it when the data must live near a location",
				placeholder: "region"
			}),
			project: projectFlag$1,
			branch: branchFlag$1
	...
	help: {
		...
		description: "Creates a database in a Branch of the project and prints its connection URL exactly once; store it now, nothing prints it again. Mint further URLs for other consumers with 'postgres connection create'.",
...
const postgresDeleteCommand = defineCommand({
	...
	help: {
		...
		description: "Deletion is permanent and destroys the data. The exact database id is the consent token; pass it with --confirm to run non-interactively.",
...
const postgresListCommand = defineCommand({
	...
	help: {
		...
		description: "Databases are branch-bound: each belongs to one Branch, the isolated environment for one Git branch. Without --branch, this lists databases across every branch of the project.",
		examples: [
			"postgres list",
...
const postgresShowCommand = defineCommand({
	...
	help: {
		...
		description: "Shows a database's name, branch, region, and status. Connection URLs are secrets printed once, by 'postgres create' or 'postgres connection rotate'; this command never reveals them.",
...
const projectCreateCommand = defineCommand({
	args: {
		...
		flags: { region: flag.string({
			brief: "Prisma Compute region for the project's infrastructure; set it when data or users must stay near a location",
			placeholder: "region"
		}) }
	},
	help: {
		summary: "Create a Project and link this directory: commands run here target it",
...
const projectDeleteCommand = defineCommand({
	...
	help: {
		...
		description: "Deletion is permanent: it destroys the project's databases and stops its deployed services. Because of that, the command asks for the exact project id as a consent token; pass it with --confirm to run non-interactively.",
...
const roleFlag = flag.enum({
	brief: "Project-wide scope: production, or preview (shared by every preview branch)",
...
const branchFlag = flag.string({
	brief: "Scope to one preview branch's override; use for values only that branch needs",
	placeholder: "git-name"
});
const fileFlag = flag.string({
	brief: "Read KEY=VALUE assignments from a dotenv file; use to import many variables at once",
	placeholder: "path"
});
...
const projectEnvAddCommand = defineCommand({
	...
	help: {
		summary: "Create an environment variable in one scope: production, preview, or one branch",
		description: "Variables reach a service's environment when it deploys. Writes always name their scope so production is never targeted by accident: --role production, --role preview (shared by every preview branch), or --branch for one branch's override. The value comes from KEY=VALUE, from the current shell environment when only KEY is given, or from a dotenv file with --file.",
		examples: [
...
const projectEnvDeleteCommand = defineCommand({
	...
	help: {
		summary: "Delete an environment variable from one scope: production, preview, or one branch",
		description: "Removes the variable from the named scope only; the same key in other scopes is untouched. Services lose the value on their next deploy.",
		examples: [
...
const projectEnvListCommand = defineCommand({
	args: { flags: {
		role: roleFlag,
		branch: flag.string({
			brief: "Show what one preview branch resolves: the preview scope plus its overrides",
			placeholder: "git-name"
		}),
		project: projectFlag
	} },
	help: {
		summary: "List environment variables in a scope: names and ids, never values",
		description: "Values are write-only through the CLI: listing shows names, ids, and status, never values. Without a scope flag it shows an overview of every scope; --role narrows to the production or preview scope, and --branch shows what one preview branch resolves.",
		examples: [
			"project env list",
...
const projectEnvUpdateCommand = defineCommand({
	...
	help: {
		...
		description: "Overwrites one variable's value in one scope; the variable must already exist there (use 'project env add' to create it). With --file, overwrites every variable the dotenv file lists. New values reach services on their next deploy.",
		examples: [
...
const projectListCommand = defineCommand({
	help: {
		...
		description: "A Project groups one product or codebase. It is the child of a workspace and the parent of Branches: isolated environments that hold your services, databases, and buckets.",
...
const projectRenameCommand = defineCommand({
	...
	help: {
		...
		description: "Changes the display name of the linked project, or of the one named by --project. The project id stays the same, so existing links and automation keep working.",
...
const projectShowCommand = defineCommand({
	...
	help: {
		...
		description: "Reports which Project this directory is linked to, or that it is not linked, with next steps. Pass an id or name to inspect any project without changing the local link.",
...
const projectTransferCommand = defineCommand({
	args: {
		...
		flags: {
			toWorkspace: flag.string({
				brief: "Receiving workspace, when it is one of your own stored sessions",
				placeholder: "id-or-name"
			}),
			recipientToken: flag.string({
				brief: "Access token for the receiving workspace, when you are not a member of it",
				placeholder: "token"
			})
		}
	},
	help: {
		...
		description: "Moves a project, with everything in it, out of the current workspace. Name the receiving workspace with --to-workspace when you are logged in to it too, or pass --recipient-token when someone else owns it. The exact project id is the consent token.",
...
const serviceCreateCommand = defineCommand({
	help: {
		...
		description: "Registers a service on a Branch ahead of its first deploy. Deploying through a connected repository or 'deploy' creates services automatically, so reach for this only when a service must exist before anything has been deployed to it.",
	...
	args: {
		...
		flags: {
			...
			region: flag.string({
				brief: "Prisma Compute region id; set it when the service must run near a location",
				placeholder: "region"
			}),
			branch: flag.string({
...
const serviceDeleteCommand = defineCommand({
	help: {
		...
		description: "Removes the service from the targeted Branch only: its versions stop serving and its URL goes away. The same service on other branches is untouched. Pass the confirmation token with --confirm to run non-interactively.",
...
const serviceDomainAddCommand = defineCommand({
	help: {
		...
		description: "Points a hostname you own at the service instead of its generated URL. After registering, create the DNS record the platform asks for, then 'service domain wait' blocks until verification and TLS provisioning finish.",
...
const serviceDomainDeleteCommand = defineCommand({
	help: {
		...
		description: "The hostname stops routing to the service immediately; the service itself and its generated URL keep working. Pass the hostname with --confirm to run non-interactively.",
...
const serviceDomainRetryCommand = defineCommand({
	help: {
		...
		description: "Run it after fixing what made the domain fail, typically a missing or wrong DNS record reported by 'service domain show'.",
...
const serviceDomainShowCommand = defineCommand({
	help: {
		...
		description: "Reports where the domain is in its lifecycle: DNS verification, TLS certificate issuance, active, or failed with the fix to apply. Check here first when a domain is not serving.",
...
const serviceDomainWaitCommand = defineCommand({
	help: {
		...
		description: "Blocks while DNS verification and TLS provisioning run, then exits with the outcome; useful in scripts and CI after 'service domain add'.",
...
const serviceListCommand = defineCommand({
	help: {
		...
		description: "A service is one HTTP application (a frontend or a backend) deployed on a Branch of the project. Each deploy produces a service version; at most one version serves traffic at a time.",
...
const serviceLogsCommand = defineSessionCommand({
	help: {
		...
		description: "Reads the live version's logs by default; point --version-id at any other version, such as a failed build being debugged. --follow keeps streaming new lines, and --tail or --from-start choose where reading starts.",
		examples: [
	...
	args: {
		...
		flags: {
			...
			versionId: flag.string({
				brief: "Service version id to read; use for a failed or stopped version (default: the live version)",
				placeholder: "id"
			}),
			tail: flag.number({
...
const serviceShowCommand = defineCommand({
	help: {
		...
		description: "Shows the version serving traffic and the service's live URL, when the service has a live version; a service that has never deployed has none yet. Inspect other versions with 'service version list' and 'service version show'.",
...
const serviceVersionDeleteCommand = defineCommand({
	help: {
		...
		description: "Permanently removes the version and its built artifact, so it can no longer be started or rolled back to. Pass the exact version id with --confirm to run non-interactively.",
...
const serviceVersionListCommand = defineCommand({
	help: {
		...
		description: "Every deploy produces an immutable service version; at most one serves traffic at a time. Use the listed ids with 'service version' promote, rollback, start, stop, and delete.",
...
const serviceVersionPromoteCommand = defineCommand({
	help: {
		...
		description: "The normal release path: take a version that proved itself on a preview branch, rebuild its source with production environment variables, and switch production traffic to the result. To undo a release, use 'service version rollback', which needs no rebuild.",
...
const serviceVersionRollbackCommand = defineCommand({
	help: {
		...
		description: "Returns production traffic to an already-built version without rebuilding, which makes it the fastest way out of a bad release. Defaults to the version before the live one; name an exact target with --to.",
		examples: [
	...
	args: {
		flags: {
			...
			to: flag.string({
				brief: "Version id to roll back to; use when the last good version is further back (default: the version before the live one)",
				placeholder: "version"
			})
		},
		positionals: { service: positional.optionalString({
...
const serviceVersionShowCommand = defineCommand({
	help: {
		...
		description: "Version ids are globally unique, so the id alone is the complete target: no --project or --branch scope is needed.",
	...
	commands: {
		...
		list: defineCommand({
			help: {
				...
				description: "Shows each skill, the package and version it came from, and whether the synced copies are current. Run 'skills sync' to bring stale copies up to date.",
...
const cliGroups = {
	...
	project: {
		...
		workflow: [
			...
			{
				run: "git connect",
				brief: "Connect GitHub; branches carrying the deploy workflow deploy on push"
			},
			{
	...
	"project env": {
		brief: "Manage a project's environment variables. Add, update, list, and delete values per scope",
		description: "Variables live in scopes: production, preview (shared by every preview branch), or a single branch's override. Values reach services when they deploy, and are write-only afterwards: list shows metadata, never values."
	},
	postgres: {
		brief: "Manage Prisma Postgres databases. CRUD, usage metrics, backups, and connection credentials",
		description: "Databases are branch-bound: each belongs to a Branch, the isolated environment for one Git branch of a project, so commands take --branch to target one. Address a database by its id (db_...) or name. Connection URLs are secrets that print exactly once, at create or rotate; nothing shows them again. 'backup' restores platform-taken backups, and 'connection' manages per-consumer credentials.",
		workflow: [
			{
	...
	"postgres backup": {
		brief: "Inspect and restore database backups. The platform takes them automatically",
		description: "The platform takes backups automatically; there is no backup-create command. Restore replaces a database's current state with a backup's contents after exact id confirmation."
	},
	"postgres connection": {
		brief: "Manage database connection credentials. Create, rotate, and revoke per-consumer connection URLs",
		description: "A connection is one independent credential (a connection URL) for one database. Give each consumer (an app, CI, a teammate) its own, so one can be rotated or revoked without breaking the others. URLs print once, at create or rotate; list shows metadata only."
	},
	bucket: {
		brief: "Manage S3-compatible object-store buckets for a project. CRUD operations and access keys",
		description: "A bucket is blob storage for files and uploads, bound to one Branch of a project and reachable through the standard S3 API. Access goes through S3-compatible keys minted per consumer with 'bucket key create', which prints credentials (endpoint, key id, secret) exactly once.",
		workflow: [{
	...
	"bucket key": {
		brief: "Manage a bucket's access keys. Create, list, and revoke per-consumer S3 credentials",
		description: "A key is one consumer's S3-compatible credentials for one bucket, with role read or read_write. Secrets print once at create; delete revokes access immediately."
	},
	branch: {
		brief: "View Platform branches: the isolated environment behind each Git branch",
		description: "A Branch maps to a Git branch of the connected repository. Each is an isolated environment with its own services, databases, buckets, and environment variables: the production branch serves live traffic, every other branch is a preview."
	},
	git: {
		brief: "Manage the GitHub connection behind deploy-on-push. Connect or disconnect a repository",
		description: "Connecting a GitHub repository turns on deploy-on-push: the repository's prisma/cloud-deploy-action workflow authenticates through the connection and deploys every pushed branch to a matching Platform Branch. The platform does not build on push; the workflow does. Disconnecting stops push deploys without touching anything already deployed."
	},
	service: {
		brief: "Manage deployed services. Logs, versions, promote and rollback releases, custom domains",
		description: "A service is one HTTP application (a frontend or a backend) deployed on a Branch. Every deploy produces an immutable service version; at most one serves traffic at a time. The 'version' subcommands move which one that is: promote releases a preview build into production, rollback returns production to a previous version without rebuilding. 'logs' reads and streams output, and 'domain' attaches hostnames you own.",
		workflow: [
			{
	...
	"service domain": {
		brief: "Manage custom domains for a service. Register hostnames, drive DNS and TLS verification, inspect status",
		description: "Custom domains point hostnames you own at a service's production branch. After add, create the DNS record the platform reports; wait and retry drive DNS verification and TLS provisioning to done.",
		workflow: [
			{
	...
	"service version": {
		brief: "Manage a service's deploy versions. List, inspect, promote, roll back, start, stop, delete",
		description: "Every deploy produces an immutable version; at most one serves traffic at a time on each branch. List and show inspect them. Promote and rollback choose which version serves traffic; start, stop, and delete drive one version's lifecycle."
	},
	"auth workspace": {
		brief: "Manage stored workspace sessions. List them, switch the current one, end one",
		description: "One session is stored per workspace you log in to. List them, switch the current one, or end one without touching the others."
	},
	contract: {
		brief: "Author your data contract: the PSL source of your data model. Emit, infer, format",
		description: "A contract is the declarative description of your application's data model, authored in PSL (Prisma Schema Language). Migrations are planned from it, and live databases are verified and signed against it. Emit generates its artifacts, infer derives a contract from an existing database, and format normalizes the source."
	},
	db: {
		brief: "Run contract operations against a live database. Verify, sign, update, migrate",
		description: "These commands run against a live database, addressed with --db <url>. Verify checks the database against the contract (the PSL description of your data model), sign marks it as matching, and update and migrate advance its schema."
	},
	migration: {
		brief: "Manage on-disk migrations derived from contract changes. Plan, inspect, check, track history",
		description: "A migration is an on-disk package describing one schema change, derived from edits to your contract (the PSL description of your data model). Plan writes one; the rest inspect, check integrity, and track what has run where. Apply them with 'db migrate'."
	},
	"migration ref": {
		brief: "Manage refs: named pointers to contracts. Set, list, delete",
		description: "A ref is a named pointer to a contract, letting commands target a contract by a stable name. Set, list, and delete refs here."
	},
	orm: { brief: "Initialize a Prisma ORM project" },
	skills: {
		brief: "Manage Prisma skills for AI coding agents. Sync and list the instruction files",
		description: "Agent skills are instruction files that teach AI coding agents (Claude Code, Cursor, and others) how to use the installed Prisma packages. They ship inside the packages; sync copies them into the directories the agent harnesses read."
	},
	...telemetry.groups
};
...
function buildCli() {
	return createCli({
		...
		help: {
			...
			description: "Deploy your app with isolated infrastructure for every branch: a Project groups one product, and each of its Branches maps to a Git branch with its own services, databases, and buckets. The production branch serves live traffic; every other branch is a preview.",
			workflow: [
				{
					run: "auth login",
					brief: "Sign in to your Prisma workspace"
				},
				{
					run: "project create my-app",
					brief: "Create a Project, link this directory"
...
//#endregion
//#region ../cli/src/auth/session-metadata.ts
const METADATA_LOOKUP_TIMEOUT_MS = 3e3;
function fetchSessionMetadata(apiBaseUrl) {
	return async (credential) => {
		const { data } = await createManagementApiClient({
			baseUrl: apiBaseUrl,
			token: credential.token
		}).GET("/v1/me", { signal: AbortSignal.timeout(METADATA_LOOKUP_TIMEOUT_MS) });
		if (!data) return void 0;
		const { user, workspace } = data.data;
		return {
			workspaceName: workspace?.name ?? void 0,
			user: user ? {
				id: user.id,
				email: user.email,
...
function makeSpawnChild(diagnostics, options) {
	...
	return (request) => {
		...
		const child = spawn$1(request.command, [...request.args], {
			cwd: request.cwd,
			env: request.env,
			stdio: structured ? [
				"inherit",
...
function memoizedConfigLoader(proc) {
...
async function maybeWriteSkillsStaleNotice(runtime) {
	...
	try {
		...
		const config = await readProjectSkillsConfig(runtime.loadConfig, configPathFromArgv(runtime.argv));
		...
		if (isFormat(flagTokens(runtime.argv), "markdown")) {
			...
			if (notice !== null) runtime.stdout.write(notice);
			return;
		}
...
function staleNoticeSentence(status, dirs) {
	...
	return `Prisma agent skills are out of date (installed ${outdated.library} ${outdated.version}, synced ${synced ?? "none"})`;
}
function renderStaleNotice(status, dirs) {
	const sentence = staleNoticeSentence(status, dirs);
	return sentence === null ? null : `${sentence}. Run: ${getCliName()} skills sync\n`;
}
...
function renderStaleNoticeMarkdown(status, dirs) {
	const sentence = staleNoticeSentence(status, dirs);
...
function isFormat(tokens, format) {
	return tokens.some((token, index) => token === `--format=${format}` || token === "--format" && tokens[index + 1] === format);
}
