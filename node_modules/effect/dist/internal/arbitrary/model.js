...
export function sampleFromValidatedShrink(value, shrink, validate) {
  ...
  return makeSample(value, Effect.suspend(() => {
    ...
    return Effect.flatMapEager(candidates, candidate => Effect.mapEager(toEffect(validate(candidate)), validated => Option.isSome(validated) ? sampleFromValidatedShrink(validated.value, shrink, validate) : discarded));
  }));
}
/** @internal */
...
/** @internal */
export function productSample(children, make, rebuild = children => productSample(children, make)) {
  const pulls = [];
  for (let index = 0; index < children.length; index++) {
    const shrinks = children[index].shrinks;
    if (shrinks !== undefined) {
      pulls.push(Effect.map(shrinks, attempt => mapAttempt(attempt, sample => rebuild(replaceAt(children, index, sample)))));
    }
  }
