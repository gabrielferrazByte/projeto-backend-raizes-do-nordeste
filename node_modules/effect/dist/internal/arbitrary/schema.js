...
function arraySample(children, shape, shrinks = true) {
  if (!shrinks) return Model.makeSample(children.map(child => child.value));
  const product = Model.productSample(children, children => children.map(child => child.value), children => arraySample(children, shape));
  ...
  if (shape.repeatCount > 0 && children.length - 1 >= shape.minimum) {
    ...
    structural.push(() => arraySample(children.slice(0, index).concat(children.slice(index + 1)), {
      ...shape,
      repeatCount: shape.repeatCount - 1
    }));
  } else if (shape.optionalCount > 0 && shape.repeatCount === 0 && shape.tailCount === 0 && children.length - 1 >= shape.minimum) {
    structural.push(() => arraySample(children.slice(0, -1), {
      ...shape,
  ...
  return Model.makeSample(product.value, product.shrinks === undefined ? structuralPull : Model.concatPulls([structuralPull, product.shrinks]));
}
function normalizePropertyKeySample(sample) {
  const filtered = Model.filterSample(sample, value => typeof value === "string" || typeof value === "number" || typeof value === "symbol");
  return filtered === undefined ? undefined : Model.mapSample(filtered, value => typeof value === "symbol" ? value : globalThis.String(value));
}
function objectSample(entries, minimum, nullPrototype, shrinks = true) {
  const make = entries => {
  ...
  const childPulls = entries.flatMap((entry, index) => entry.sample.shrinks === undefined ? [] : [Effect.map(entry.sample.shrinks, attempt => Model.mapAttempt(attempt, sample => objectSample(replaceAt(entries, index, {
    ...entry,
    sample
  }), minimum, nullPrototype)))]);
  ...
  const keyPulls = entries.flatMap((entry, index) => {
    ...
    const filtered = Model.filterSample(entry.keySample, key => !entries.some((other, otherIndex) => otherIndex !== index && other.key === key));
    if (filtered?.shrinks === undefined) return [];
    return [Effect.map(filtered.shrinks, attempt => Model.mapAttempt(attempt, keySample => objectSample(replaceAt(entries, index, {
      ...entry,
      key: keySample.value,
      keySample
    }), minimum, nullPrototype)))];
  });
  const structural = entries.length <= minimum ? [] : entries.flatMap((entry, index) => entry.removable ? [() => objectSample(entries.slice(0, index).concat(entries.slice(index + 1)), minimum, nullPrototype)] : []);
  const descendantPulls = [...childPulls, ...keyPulls];
  const pulls = structural.length === 0 ? descendantPulls : [Effect.map(Model.pullFromArray(structural), make => make()), ...descendantPulls];
...
function bigIntSample(value, minimum, maximum, context) {
  ...
  let candidates;
  if (context === undefined) {
    const target = minimum !== undefined && minimum > BigInt(0) ? minimum : maximum !== undefined && maximum < BigInt(0) ? maximum : BigInt(0);
    candidates = shrinkBigInt(value, target, true);
  } else if (value > BigInt(0) && value === context + BigInt(1) && (minimum === undefined || value > minimum) || value < BigInt(0) && value === context - BigInt(1) && (maximum === undefined || value < maximum)) {
    candidates = [{
      value: context,
      context: undefined
    }];
  ...
  return Model.makeSample(value, candidates.length === 0 ? undefined : Effect.map(Model.pullFromArray(candidates), candidate => bigIntSample(candidate.value, minimum, maximum, candidate.context)));
}
/** @internal */
export function compile(schema) {
      ...
      case "BigInt":
        {
          ...
          return Model.makeCompiled([], () => 0, state => {
            ...
            const center = minimum !== undefined && minimum > BigInt(0) ? minimum : maximum !== undefined && maximum < BigInt(0) ? maximum : BigInt(0);
            ...
            return state.shrinks ? bigIntSample(value, minimum, maximum) : Model.makeSample(value);
          });
        }
      case "Symbol":
        {
  ...
  const compileObjects = (ast, path, constraint) => {
    ...
    return Model.makeCompiled(dependencies, () => {
      ...
      const minimumIndexCost = indexes.length === 0 ? infinity : Math.min(...indexes.map(index => index.parameter.minCost + index.value.minCost));
      ...
      if (namedCost + (minimumIndexes === 0 ? 0 : minimumIndexes * minimumIndexCost) > state.budget.remaining) {
        selectedOptional = (shuffledOptional ?? Model.shuffle(state, optional)).sort((a, b) => a.compiled.minCost - b.compiled.minCost).slice(0, fallbackOptionalCount);
  ...
  const compileDeclaration = (ast, path, constraint) => {
    ...
    const decode = value => {
      const transformed = link.transformation._tag === "Transformation" ? link.transformation.decode.run(Option.some(value), SchemaAST.defaultParseOptions) : link.transformation.decode(Effect.succeed(Option.some(value)), SchemaAST.defaultParseOptions);
