...
const mappingSeparator = input => {
  ...
  for (let index = 0; index < input.length; index++) {
    const character = input[index];
    if (quote === "\"") {
      if (escaped) escaped = false;else if (character === "\\") escaped = true;else if (character === quote) quote = undefined;
    } else if (quote === "'") {
      if (character === quote && input[index + 1] === quote) index++;else if (character === quote) quote = undefined;
    } else if (character === "'" || character === "\"") {
      quote = character;
    } else if (character === "[" || character === "{") {
      depth++;
...
class FlowParser {
  ...
  readQuoted(quote) {
    ...
    while (this.index < this.input.length) {
      ...
      if (quote === "\"" && escaped) escaped = false;else if (quote === "\"" && character === "\\") escaped = true;else if (quote === "'" && character === quote && this.input[this.index] === quote) this.index++;else if (character === quote) return this.input.slice(start, this.index);
    }
