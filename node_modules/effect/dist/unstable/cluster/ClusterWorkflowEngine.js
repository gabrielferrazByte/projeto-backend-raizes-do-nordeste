  ...
  const engine = WorkflowEngine.makeUnsafe({
    register: (workflow, execute) => Effect.suspend(() => sharding.registerEntity(ensureEntity(workflow), Effect.gen(function* () {
      ...
      return {
        ...
        resume: () => resumeGate.withPermitsIfAvailable(1)(currentRun ? waitForRunReply(workflow, currentRun) : Effect.void).pipe(
        // Release the gate before reset can start another parent run, so
        // later child completions can wake that replay.
        Effect.flatMap(waited => Option.isSome(waited) ? resume(workflow, executionId) : Effect.void), ensureSuccess, Rpc.wrap({
          fork: true,
          uninterruptible: true
        }))
      };
