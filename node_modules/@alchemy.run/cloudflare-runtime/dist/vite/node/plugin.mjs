...
//#endregion
//#region src/core/globals/ProxyHeaders.shared.ts
const HEADER_ORIGINAL_URL = "Alchemy-Runtime-Original-URL";
const HEADER_PROXY_SHARED_SECRET = "Alchemy-Runtime-Proxy-Shared-Secret";
//#endregion
...
function proxyRequestHeaders(request, target, proxySharedSecret) {
	const original = new URL(target);
	const protocol = firstHeaderValue(request.headers["x-forwarded-proto"]);
	original.protocol = protocol === "https" || protocol === "http" ? `${protocol}:` : request.socket.encrypted ? "https:" : "http:";
	original.port = "";
...
*/
function handleWebSocket(httpServer, address, proxySharedSecret) {
...
function dev(options) {
	...
	const plugin = {
		...
		async configureServer(server) {
			...
			const { createDefaultContext, startServer } = await import("./dev-server-z7SqQHnQ.mjs");
			...
			const bindWebSocket = () => {
				removeUpgradeListener?.();
				removeUpgradeListener = server.httpServer ? handleWebSocket(server.httpServer, address, proxySharedSecret) : void 0;
			};
			/**
...
function preview(options) {
	return {
		...
		async configurePreviewServer(server) {
			...
			const { startPreviewServer } = await import("./preview-server-D3yf11tl.mjs");
			...
			const removeUpgradeListener = server.httpServer ? handleWebSocket(server.httpServer, address, handle.proxySharedSecret) : void 0;
