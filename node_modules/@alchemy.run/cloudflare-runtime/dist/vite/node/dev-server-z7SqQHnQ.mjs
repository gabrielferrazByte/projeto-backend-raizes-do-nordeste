...
const startServer = async (options, entryEnvironment, server, context, exportTypes) => {
	...
	return {
		address: await serve(options, entryEnvironment, server, exportTypes, proxySharedSecret).pipe(Effect.provide(ViteAssetsLive(server).pipe(Layer.provideMerge(Layer.succeedContext(context)))), Scope.provide(scope), Effect.runPromise),
		proxySharedSecret,
...
const serve = Effect.fn(function* (options, entryEnvironment, server, exportTypes, proxySharedSecret) {
...
//# sourceMappingURL=dev-server-z7SqQHnQ.mjs.map