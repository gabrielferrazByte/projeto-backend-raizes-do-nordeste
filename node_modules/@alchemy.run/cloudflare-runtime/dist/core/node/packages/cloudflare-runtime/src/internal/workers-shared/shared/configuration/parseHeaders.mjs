...
function parseHeaders(input, { maxRules = 100, maxLineLength = MAX_LINE_LENGTH } = {}) {
	...
	for (let i = 0; i < lines.length; i++) {
		...
		if (LINE_IS_PROBABLY_A_PATH.test(line)) {
			...
			if (rule) {
				if (isValidRule(rule)) rules.push({
					path: rule.path,
